...
exports.RandomGen7Teams = exports.ZeroAttackHPIVs = void 0;
const teams_1 = require("../gen8/teams");
const dex_1 = require("../../../sim/dex");
exports.ZeroAttackHPIVs = {
    grass: { hp: 30, spa: 30 },
    fire: { spa: 30, spe: 30 },
    ice: { def: 30 },
...
class RandomGen7Teams extends teams_1.RandomGen8Teams {
    constructor(format, prng) {
        ...
        this.moveEnforcementCheckers = {
            ...
            Flying: (movePool, moves, abilities, types, counter, species) => (!counter.get('Flying') && !['aerodactyl', 'aerodactylmega', 'mantine'].includes(species.id) &&
            ...
            Ice: (movePool, moves, abilities, types, counter) => (!counter.get('Ice') || (moves.has('icebeam') && movePool.includes('freezedry')) ||
            ...
            Psychic: (movePool, moves, abilities, types, counter) => (!counter.get('Psychic') && (types.has('Fighting') || types.has('Fairy') || movePool.includes('psychicfangs') || movePool.includes('calmmind'))),
    ...
    newQueryMoves(moves, species, preferredType, abilities) {
        ...
        const counter = new teams_1.MoveCounter();
    ...
    addMove(move, moves, types, abilities, teamDetails, species, isLead, movePool, preferredType, role) {
        ...
        this.cullMovePool(types, moves, abilities, counter, movePool, teamDetails, species, isLead, preferredType, role);
        return counter;
    }
    ...
    randomMoveset(types, abilities, teamDetails, species, isLead, movePool, preferredType, role) {
        ...
        if (movePool.length <= this.maxMoveCount) {
            ...
            while (movePool.length) {
                ...
                counter = this.addMove(moveid, moves, types, abilities, teamDetails, species, isLead, movePool, preferredType, role);
            }
            return moves;
        }
        const runEnforcementChecker = (checkerName) => {
            if (!this.moveEnforcementCheckers[checkerName])
                return false;
            return this.moveEnforcementCheckers[checkerName](movePool, moves, abilities, new Set(types), counter, species, teamDetails);
        };
        // Add required move (e.g. Relic Song for Meloetta-P)
        if (species.requiredMove) {
            const move = this.dex.moves.get(species.requiredMove).id;
            counter = this.addMove(move, moves, types, abilities, teamDetails, species, isLead, movePool, preferredType, role);
        }
        // Add other moves you really want to have, e.g. STAB, recovery, setup.
        // Enforce Facade if Guts is a possible ability
        if (movePool.includes('facade') && abilities.includes('Guts')) {
            counter = this.addMove('facade', moves, types, abilities, teamDetails, species, isLead, movePool, preferredType, role);
        }
        ...
        if (movePool.includes('thunderwave') && abilities.includes('Prankster')) {
            counter = this.addMove('thunderwave', moves, types, abilities, teamDetails, species, isLead, movePool, preferredType, role);
        }
        // Enforce Shadow Sneak on Kecleon
        if (movePool.includes('shadowsneak') && species.id === 'kecleon') {
            counter = this.addMove('shadowsneak', moves, types, abilities, teamDetails, species, isLead, movePool, preferredType, role);
        }
        ...
        if (movePool.includes('destinybond') && species.id === 'banettemega') {
            counter = this.addMove('destinybond', moves, types, abilities, teamDetails, species, isLead, movePool, preferredType, role);
        }
        // Enforce hazard removal on Bulky Support if the team doesn't already have it
        if (role === 'Bulky Support' && !teamDetails.defog && !teamDetails.rapidSpin) {
            if (movePool.includes('rapidspin')) {
                counter = this.addMove('rapidspin', moves, types, abilities, teamDetails, species, isLead, movePool, preferredType, role);
            }
            if (movePool.includes('defog')) {
                counter = this.addMove('defog', moves, types, abilities, teamDetails, species, isLead, movePool, preferredType, role);
            }
        }
        ...
        if (!counter.get('stab')) {
            ...
            if (stabMoves.length) {
                ...
                if (movePool.includes('uturn') && types.includes('Bug')) {
                    counter = this.addMove('uturn', moves, types, abilities, teamDetails, species, isLead, movePool, preferredType, role);
                }
            }
        }
        // Enforce recovery
        ...
        if (role.includes('Setup') || role === 'Z-Move user') {
            ...
            if (setupMoves.length) {
                ...
                if (movePool.includes('flamecharge')) {
                    counter = this.addMove('flamecharge', moves, types, abilities, teamDetails, species, isLead, movePool, preferredType, role);
                }
            }
        }
        ...
        while (moves.size < this.maxMoveCount && movePool.length) {
            ...
            for (const pair of MOVE_PAIRS) {
                if (moveid === pair[0] && movePool.includes(pair[1])) {
                    counter = this.addMove(pair[1], moves, types, abilities, teamDetails, species, isLead, movePool, preferredType, role);
                }
                if (moveid === pair[1] && movePool.includes(pair[0])) {
                    counter = this.addMove(pair[0], moves, types, abilities, teamDetails, species, isLead, movePool, preferredType, role);
                }
            }
        }
        return moves;
    ...
    getAbility(types, moves, abilities, counter, movePool, teamDetails, species, preferredType, role) {
        ...
        for (const ability of abilities) {
            if (!this.shouldCullAbility(ability, types, moves, abilities, counter, movePool, teamDetails, species, preferredType, role)) {
    ...
    getItem(ability, types, moves, counter, teamDetails, species, isLead, preferredType, role) {
        ...
        if (counter.get('Physical') === 4 && species.id !== 'jirachi' &&
            ...
            return (scarfReqs && (species.baseStats.atk >= 100 || ability === 'Pure Power' || ability === 'Huge Power') &&
        ...
        if (moves.has('outrage') && counter.get('setup'))
            return 'Lum Berry';
        if ((ability === 'Rough Skin') || (species.id !== 'hooh' &&
            ability === 'Regenerator' && species.baseStats.hp + species.baseStats.def >= 180 && this.randomChance(1, 2)) || (ability !== 'Regenerator' && !counter.get('setup') && counter.get('recovery') &&
        ...
        if (['kingsshield', 'protect', 'spikyshield', 'substitute'].some(m => moves.has(m)))
            return 'Leftovers';
        if ((role === 'Fast Support' || moves.has('stickyweb')) && isLead && defensiveStatTotal < 255 &&
        ...
        if (role === 'Fast Support') {
            return (counter.get('Physical') + counter.get('Special') >= 3 &&
                ...
                this.dex.getEffectiveness('Rock', species) < 2) ? 'Life Orb' : 'Leftovers';
        }
        ...
        if (['Fast Attacker', 'Setup Sweeper', 'Wallbreaker'].some(m => role === m) &&
    ...
    randomSet(species, teamDetails = {}, isLead = false) {
        ...
        for (const set of sets) {
            if (!teamDetails.zMove && set.role === 'Z-Move user')
                canZMove = true;
        }
        for (const set of sets) {
            ...
            if (canZMove && ['Setup Sweeper', 'Bulky Setup'].includes(set.role))
                continue;
        ...
        // Get moves
        const moves = this.randomMoveset(types, abilities, teamDetails, species, isLead, movePool, preferredType, role);
        const counter = this.newQueryMoves(moves, species, preferredType, abilities);
        // Get ability
        ability = this.getAbility(new Set(types), moves, baseAbilities, counter, movePool, teamDetails, species, preferredType, role);
        // Get items
        ...
        if ((!counter.get('Physical') || (counter.get('Physical') <= 1 && (moves.has('foulplay') || moves.has('rapidspin')))) &&
        ...
        if (hasHiddenPower && level < 100) {
            ...
            const HPivs = ivs.atk === 0 ? exports.ZeroAttackHPIVs[hpType] : this.dex.types.get(hpType).HPivs;
            let iv;
            for (iv in HPivs) {
                ivs[iv] = HPivs[iv];
            }
            ...
            while (baseSpeciesPool.length && pokemon.length < this.maxTeamSize) {
                ...
                if (restrict) {
                    if (!isMonotype && !this.forceMonotype) {
                        ...
                        if (!this.getPokemonCompatibility(species, pokemon))
                            continue;
                    }
        ...
        for (const curSet of setList) {
            ...
            for (const itemString of curSet.item) {
                ...
                if (itemsMax[item.id] && teamData.has[item.id] >= itemsMax[item.id])
                    continue; // reject 2+ same choice item
