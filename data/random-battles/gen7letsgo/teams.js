...
exports.RandomLetsGoTeams = void 0;
const teams_1 = require("../gen8/teams");
class RandomLetsGoTeams extends teams_1.RandomGen8Teams {
    ...
    shouldCullMove(move, types, moves, abilities, counter, movePool, teamDetails) {
        switch (move.id) {
            ...
            case 'swordsdance':
                return {
                    cull: (counter.setupType !== 'Physical' ||
    ...
    randomSet(species, teamDetails = {}) {
        ...
        do {
            ...
            for (const moveid of moves) {
                ...
                const moveIsRejectable = !move.damage && (move.category !== 'Status' || !move.flags.heal) && (move.category === 'Status' ||
                    !types.has(move.type) ||
                    move.selfSwitch ||
                ...
                if (moveIsRejectable && !cull && !isSetup && counter.get('physicalsetup') + counter.get('specialsetup') < 2 && (!counter.setupType || counter.setupType === 'Mixed' ||
                    (move.category !== counter.setupType && move.category !== 'Status') ||
                    counter.get(counter.setupType) + counter.get('Status') > 3)) {
                    if ((counter.damagingMoves.size === 0 || !counter.get('stab')) &&
