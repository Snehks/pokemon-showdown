...
const dex_1 = require("../../../sim/dex");
const lib_1 = require("../../../lib");
const prng_1 = require("../../../sim/prng");
const tags_1 = require("./../../tags");
const teams_1 = require("../../../sim/teams");
class MoveCounter extends lib_1.Utils.Multiset {
    constructor() {
        super();
...
class RandomTeams {
    constructor(format, prng) {
        ...
        this.noStab = NO_STAB;
        const ruleTable = dex_1.Dex.formats.getRuleTable(format);
        ...
        this.moveEnforcementCheckers = {
            Bug: (movePool, moves, abilities, types, counter) => (movePool.includes('megahorn') || movePool.includes('xscissor') ||
            ...
            Grass: (movePool, moves, abilities, types, counter, species) => (!counter.get('Grass') && (movePool.includes('leafstorm') || species.baseStats.atk >= 100 ||
            ...
            Steel: (movePool, moves, abilities, types, counter, species, teamDetails, isLead, isDoubles) => (!counter.get('Steel') &&
                (isDoubles || species.baseStats.atk >= 90 || movePool.includes('gigatonhammer') || movePool.includes('makeitrain'))),
            Water: (movePool, moves, abilities, types, counter, species, teamDetails, isLead, isDoubles) => (!counter.get('Water') && (!types.includes('Ground') || isDoubles)),
        };
    ...
    queryMoves(moves, species, teraType, abilities) {
        ...
        for (const moveid of moves) {
            ...
            if (SPEED_CONTROL.includes(moveid))
    ...
    shouldCullAbility(ability, types, moves, abilities, counter, teamDetails, species, isLead, isDoubles, teraType, role) {
        switch (ability) {
            ...
            case 'Iron Fist':
            case 'Skill Link':
                return !counter.get((0, dex_1.toID)(ability));
            case 'Overgrow':
    ...
    getPriorityItem(ability, types, moves, counter, teamDetails, species, isLead, isDoubles, teraType, role) {
        ...
        if (types.includes('Normal') && moves.has('doubleedge') && moves.has('fakeout'))
            return 'Silk Scarf';
        if (species.id === 'froslass' || moves.has('populationbomb') ||
        ...
        if (species.id === 'rampardos' && (role === 'Fast Attacker' || isDoubles))
            return 'Choice Scarf';
        if (species.id === 'palkia' && counter.get('Status'))
            return 'Lustrous Orb';
        ...
        if (ability === 'Anger Shell')
            return this.sample(['Expert Belt', 'Lum Berry', 'Scope Lens', 'Sitrus Berry']);
        ...
        if (species.id === 'giratina' && !isDoubles && moves.has('rest') && !moves.has('sleeptalk'))
            return 'Leftovers';
    ...
    getDoublesItem(ability, types, moves, counter, teamDetails, species, isLead, teraType, role) {
        const scarfReqs = (!counter.get('priority') && ability !== 'Speed Boost' && role !== 'Doubles Wallbreaker' &&
        ...
        if (role === 'Choice Item user') {
            if (scarfReqs || moves.has('finalgambit') || species.id === 'jirachi')
                return 'Choice Scarf';
            return (counter.get('Physical') > counter.get('Special')) ? 'Choice Band' : 'Choice Specs';
        }
        ...
        if (species.baseStats.spe <= 70 && (moves.has('ragepowder') || moves.has('followme')))
            return 'Rocky Helmet';
        if (ability === 'Intimidate' && this.dex.getEffectiveness('Rock', species) >= 1 &&
            ...
            return 'Heavy-Duty Boots';
        if ((role === 'Bulky Protect' && counter.get('setup')) ||
            ...
            return 'Leftovers';
        if (moves.has('hypervoice') && !types.includes('Normal'))
            return 'Throat Spray';
        if (role === 'Doubles Fast Attacker' && !counter.get('recoil') &&
            species.baseStats.hp + species.baseStats.def + species.baseStats.spd <= 230)
            return 'Focus Sash';
        if ((offensiveRole || (role === 'Tera Blast user' && (species.baseStats.spe >= 80 || moves.has('trickroom')))) &&
            !moves.has('fakeout') &&
            (!moves.has('uturn') || types.includes('Bug') || ability === 'Libero') &&
            ((!moves.has('icywind') && !moves.has('electroweb')) || species.id === 'ironbundle')) {
            return ((ability === 'Quark Drive' || ability === 'Protosynthesis') && !isLead && species.id !== 'ironvaliant' &&
                ['dracometeor', 'firstimpression', 'uturn', 'voltswitch'].every(m => !moves.has(m))) ? 'Booster Energy' : 'Life Orb';
        }
    ...
    getItem(ability, types, moves, counter, teamDetails, species, isLead, teraType, role) {
        ...
        if (counter.get('speedsetup') && !counter.get('physicalsetup') && role === 'Bulky Setup')
            return 'Weakness Policy';
        if (!counter.get('Status') &&
        ...
        if (this.dex.getEffectiveness('Rock', species) >= 1)
            return 'Heavy-Duty Boots';
        if ((moves.has('chillyreception') || (role === 'Fast Support' &&
            [...PIVOT_MOVES, 'defog', 'mortalspin', 'rapidspin'].some(m => moves.has(m)) &&
            !types.includes('Flying') && ability !== 'Levitate')))
            return 'Heavy-Duty Boots';
        // Low Priority
        if (moves.has('dragondance') && role === 'Bulky Setup')
            return 'Weakness Policy';
        if (ability === 'Rough Skin' || (ability === 'Regenerator' && (role === 'Bulky Support' || role === 'Bulky Attacker') &&
            (species.baseStats.hp + species.baseStats.def) >= 180 && this.randomChance(1, 2)) || (ability !== 'Regenerator' && !counter.get('setup') && counter.get('recovery') &&
        ...
        if (moves.has('protect') && ability !== 'Speed Boost')
            return 'Leftovers';
        if (role === 'Fast Support' && isLead && !counter.get('recovery') && !counter.get('recoil') &&
        ...
        if (role === 'Tera Blast user' && DEFENSIVE_TERA_BLAST_USERS.includes(species.id))
            return 'Leftovers';
        if (lifeOrbReqs && ['Fast Attacker', 'Setup Sweeper', 'Tera Blast user', 'Wallbreaker'].some(m => role === (m)))
            return 'Life Orb';
        return 'Leftovers';
    }
    ...
    getForme(species) {
        ...
        if (species.baseSpecies === 'Magearna')
            return 'Magearna' + this.sample(['', '-Original']);
    ...
    randomSet(s, teamDetails = {}, isLead = false, isDoubles = false) {
        ...
        while (evs.hp > 1) {
            ...
            else if ((moves.has('bellydrum') || moves.has('filletaway') || moves.has('shedtail')) &&
        ...
        const noAttackStatMoves = [...moves].every(m => {
            ...
            if (move.id === 'shellsidearm')
                return false;
            // Physical Tera Blast
            if (move.id === 'terablast' && (species.id === 'porygon2' || ['Contrary', 'Defiant'].includes(ability) ||
    ...
    getPokemonPool(type, pokemonToExclude = [], isMonotype = false, pokemonList) {
        const exclude = pokemonToExclude.map(p => (0, dex_1.toID)(p.species));
        const pokemonPool = {};
        ...
        while (baseSpeciesPool.length && pokemon.length < this.maxTeamSize) {
            ...
            if (pokemon.length === 0) {
                ...
                if (pokemon.length < 6) {
                    ...
                    if (Math.max(passedStat, 50) < species.baseStats[passedStatName])
                        continue;
                }
            }
            if (!species.exists)
    ...
    randomCCTeam() {
        ...
        for (let forme of randomN) {
            ...
            if (species.requiredItems && !species.requiredItems.some(req => (0, dex_1.toID)(req) === item)) {
            ...
            if (species.baseSpecies === 'Wishiwashi')
                stats = dex_1.Dex.species.get('wishiwashischool').baseStats;
            // If Terapagos, use Terastal-forme's stats
            if (species.baseSpecies === 'Terapagos')
                stats = dex_1.Dex.species.get('terapagosterastal').baseStats;
            ...
            for (const species of this.dex.species.all()) {
                ...
                if (banReason !== '') {
                    ...
                    if (banReason !== '' || this.dex.species.get(species.baseSpecies).isNonstandard !== species.isNonstandard) {
                        const nonexistentCheck = tags_1.Tags.nonexistent.genericFilter(species) && nonexistentBanReason;
    ...
    randomHCTeam() {
        ...
        for (const forme of randomN) {
            ...
            if (this.gen === 6) {
                let evpool = 510;
                do {
                    const x = this.sample(dex_1.Dex.stats.ids());
                    const y = this.random(Math.min(256 - evs[x], evpool + 1));
                    evs[x] += y;
                    evpool -= y;
                } while (evpool > 0);
                ...
                for (const x of dex_1.Dex.stats.ids()) {
    ...
    randomFactorySet(species, teamData, tier) {
        const id = (0, dex_1.toID)(species.name);
        ...
        for (const set of setList) {
            ...
            for (const itemString of set.item) {
                const itemId = (0, dex_1.toID)(itemString);
            ...
            const abilityId = (0, dex_1.toID)(this.sample(set.ability));
    ...
    randomFactoryTeam(side, depth = 0) {
        ...
        while (shuffledSpecies.length && pokemon.length < this.maxTeamSize) {
            ...
            teamData.has[(0, dex_1.toID)(set.item)] = 1;
            if (set.wantsTera) {
            ...
            for (const move of set.moves) {
                const moveId = (0, dex_1.toID)(move);
    ...
    randomBSSFactorySet(species, teamData) {
        ...
        for (const curSet of setList) {
            ...
            for (const move of curSet.moves) {
                ...
                const moveId = (0, dex_1.toID)(move[variantIndex]);
    ...
    randomDraftFactoryTeam(side) {
        ...
        const team = teams_1.Teams.unpack(matchup[this.rdfMatchupSide]);
        if (!team)
    ...
    random1v1FactoryTeam(side, depth = 0) {
        ...
        while (shuffledSpecies.length && pokemon.length < this.maxTeamSize) {
            ...
            if (teamData.has['physical'] && (atkEVs || physMoveCount >= 2 || atkBoostingMoves))
                continue;
            if (teamData.has['special'] && (spaEVs || specMoveCount >= 2 || spaBoostingMoves))
                continue;
            if (!teamData.has['physical'])
                teamData.has['physical'] = 0;
            if (!teamData.has['special'])
                teamData.has['special'] = 0;
            if (atkEVs || physMoveCount >= 2 || atkBoostingMoves)
                teamData.has['physical']++;
            if (spaEVs || specMoveCount >= 2 || spaBoostingMoves)
        ...
        if (!teamData.forceResult && pokemon.length < this.maxTeamSize)
            return this.random1v1FactoryTeam(side, ++depth);
        // Quality control we cannot afford for monotype
        if (!teamData.forceResult && !this.forceMonotype) {
            for (const type in teamData.weaknesses) {
                if (teamData.weaknesses[type] >= limitFactor)
                    return this.random1v1FactoryTeam(side, ++depth);
            }
        }
        return pokemon;
