...
const teams_1 = __importDefault(require("../gen5/teams"));
...
class RandomGen4Teams extends teams_1.default {
    constructor(format, prng) {
        ...
        this.moveEnforcementCheckers = {
            ...
            Grass: (movePool, moves, abilities, types, counter, species) => (!counter.get('Grass') &&
    ...
    randomMoveset(types, abilities, teamDetails, species, isLead, movePool, preferredType, role) {
        ...
        if (!role.includes('Setup')) {
            if (movePool.includes('batonpass') && movePool.includes('substitute')) {
                counter = this.addMove('substitute', moves, types, abilities, teamDetails, species, isLead, movePool, preferredType, role);
            }
        }
    ...
    getAbility(types, moves, abilities, counter, movePool, teamDetails, species, preferredType, role) {
        ...
        if (species.id === 'dewgong')
            return moves.has('raindance') ? 'Hydration' : 'Thick Fat';
        if (species.id === 'cloyster' && counter.get('skilllink'))
            return 'Skill Link';
