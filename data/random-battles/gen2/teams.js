...
exports.RandomGen2Teams = void 0;
const teams_1 = __importDefault(require("../gen3/teams"));
...
class RandomGen2Teams extends teams_1.default {
    constructor(format, prng) {
        ...
        this.moveEnforcementCheckers = {
            ...
            Flying: (movePool, moves, abilities, types, counter, species) => (!counter.get('Flying') && ['gligar', 'murkrow', 'xatu'].includes(species.id)),
    ...
    randomMoveset(types, abilities, teamDetails, species, isLead, movePool, preferredType, role) {
        ...
        if (movePool.includes('batonpass') && species.id === 'smeargle') {
            counter = this.addMove('batonpass', moves, types, abilities, teamDetails, species, isLead, movePool, preferredType, role);
        }
        ...
        if (['Bulky Support', 'Bulky Attacker', 'Bulky Setup'].includes(role)) {
            ...
            if (movePool.includes('rest')) {
                counter = this.addMove('rest', moves, types, abilities, teamDetails, species, isLead, movePool, preferredType, role);
            }
            if (movePool.includes('sleeptalk')) {
                counter = this.addMove('sleeptalk', moves, types, abilities, teamDetails, species, isLead, movePool, preferredType, role);
            }
        }
        // Enforce setup
        if (role.includes('Setup')) {
            ...
            if (nonSpeedSetupMoves.length) {
                ...
                if (movePool.includes('agility')) {
                    counter = this.addMove('agility', moves, types, abilities, teamDetails, species, isLead, movePool, preferredType, role);
                }
            }
        }
        // Enforce Thief
        if (role === 'Thief user') {
            if (movePool.includes('thief')) {
                counter = this.addMove('thief', moves, types, abilities, teamDetails, species, isLead, movePool, preferredType, role);
            }
        }
