...
const teams_1 = __importStar(require("../gen7/teams"));
...
class RandomGen6Teams extends teams_1.default {
    constructor(format, prng) {
        ...
        this.moveEnforcementCheckers = {
            ...
            Flying: (movePool, moves, abilities, types, counter, species) => (!counter.get('Flying') && !['aerodactyl', 'aerodactylmega', 'mantine', 'murkrow'].includes(species.id) &&
            ...
            Psychic: (movePool, moves, abilities, types, counter) => (!counter.get('Psychic') && (types.has('Fighting') || types.has('Fairy') || movePool.includes('calmmind'))),
    ...
    getAbility(types, moves, abilities, counter, movePool, teamDetails, species, preferredType, role) {
        ...
        if (!abilityAllowed.length) {
            const weatherAbilities = abilities.filter(a => ['Chlorophyll', 'Hydration', 'Sand Force', 'Sand Rush', 'Solar Power', 'Swift Swim'].includes(a));
    ...
    randomSet(species, teamDetails = {}, isLead = false) {
        ...
        if (hasHiddenPower) {
            ...
            const HPivs = ivs.atk === 0 ? teams_1.ZeroAttackHPIVs[hpType] : this.dex.types.get(hpType).HPivs;
            let iv;
            for (iv in HPivs) {
                ivs[iv] = HPivs[iv];
            }
