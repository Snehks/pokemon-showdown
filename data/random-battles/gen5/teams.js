...
class RandomGen5Teams extends teams_1.default {
    constructor(format, prng) {
        ...
        this.moveEnforcementCheckers = {
            ...
            Flying: (movePool, moves, abilities, types, counter, species) => (!counter.get('Flying') && !['aerodactyl', 'mantine', 'murkrow'].includes(species.id) &&
    ...
    randomMoveset(types, abilities, teamDetails, species, isLead, movePool, preferredType, role) {
        ...
        if (movePool.includes('stealthrock') && !teamDetails.stealthRock) {
            counter = this.addMove('stealthrock', moves, types, abilities, teamDetails, species, isLead, movePool, preferredType, role);
        }
    ...
    getPriorityItem(ability, types, moves, counter, teamDetails, species, isLead, preferredType, role) {
        ...
        if (species.id === 'exploud' && role === 'Bulky Attacker')
            return 'Choice Band';
        if (ability === 'Poison Heal' || (moves.has('facade') && species.id !== 'stoutland'))
            return 'Toxic Orb';
    ...
    getItem(ability, types, moves, counter, teamDetails, species, isLead, preferredType, role) {
        ...
        if (moves.has('pursuit') && moves.has('suckerpunch') && counter.get('Dark') &&
        ...
        if (moves.has('outrage') && counter.get('setup'))
            return 'Lum Berry';
        if ((ability === 'Rough Skin') || (species.id !== 'hooh' && role !== 'Wallbreaker' &&
        ...
        if (['protect', 'substitute'].some(m => moves.has(m)))
            return 'Leftovers';
        if (this.dex.getEffectiveness('Ground', species) >= 2 &&
            ability !== 'Levitate') {
            return 'Air Balloon';
        }
        if (role === 'Fast Support' && isLead && defensiveStatTotal < 255 && !counter.get('recovery') &&
        ...
        const expertBeltReqs = (!counter.get('Dragon') && !counter.get('Normal') && !counter.get('Poison') &&
            noExpertBeltMoves.every(m => !moves.has(m)));
        if (!counter.get('Status') && expertBeltReqs &&
    ...
    randomSet(species, teamDetails = {}, isLead = false) {
        ...
        for (const set of sets) {
            if (!teamDetails.rapidSpin && set.role === 'Spinner')
                canSpinner = true;
        }
        for (const set of sets) {
            ...
            if (canSpinner && set.role !== 'Spinner')
                continue;
        ...
        // Get ability
        ability = this.getAbility(new Set(types), moves, abilities, counter, movePool, teamDetails, species, preferredType, role);
        // Get items
