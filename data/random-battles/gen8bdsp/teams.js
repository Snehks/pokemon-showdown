...
exports.RandomBDSPTeams = void 0;
const teams_1 = require("../gen8/teams");
class RandomBDSPTeams extends teams_1.RandomGen8Teams {
    ...
    getHighPriorityItem(ability, types, moves, counter, teamDetails, species, isLead, isDoubles) {
        ...
        if (moves.has('auroraveil') || moves.has('lightscreen') && moves.has('reflect'))
            return 'Light Clay';
        const statusCuringAbility = (ability === 'Shed Skin' ||
    ...
    getMediumPriorityItem(ability, moves, counter, species, isLead, isDoubles) {
        ...
        if (counter.get('Physical') >= 4 && choiceOK)
            return 'Choice Band';
        if (((counter.get('Physical') >= 3 && moves.has('defog')) || (counter.get('Special') >= 3 && moves.has('healingwish'))) &&
        ...
        if (moves.has('raindance') || moves.has('sunnyday') ||
            ...
            return 'Life Orb';
        if (['clearsmog', 'curse', 'haze', 'healbell', 'protect', 'sleeptalk'].some(m => moves.has(m)) &&
            ability === 'Moody')
            return 'Leftovers';
    }
    ...
    shouldCullMove(move, types, moves, abilities, counter, movePool, teamDetails, species, isLead, isDoubles) {
        ...
        switch (move.id) {
            ...
            case 'leafstorm':
                ...
                return { cull: (counter.setupType === 'Physical' && leafBladePossible) ||
    ...
    shouldCullAbility(ability, types, moves, abilities, counter, movePool, teamDetails, species, preferredType, role, isDoubles) {
        ...
        switch (ability) {
            ...
            case 'Unburden':
                return (abilities.includes('Prankster') ||
            ...
            case 'Weak Armor':
                return (species.id === 'skarmory' ||
                    moves.has('shellsmash') || moves.has('rapidspin'));
        }
        return false;
    }
}
exports.RandomBDSPTeams = RandomBDSPTeams;
exports.default = RandomBDSPTeams;
