...
exports.RandomGen8Teams = exports.MoveCounter = void 0;
...
class RandomGen8Teams {
    constructor(format, prng) {
        ...
        this.moveEnforcementCheckers = {
            ...
            Normal: (movePool, moves, abilities, types, counter) => ((abilities.includes('Guts') && movePool.includes('facade')) ||
    ...
    shouldCullAbility(ability, types, moves, abilities, counter, movePool, teamDetails, species, preferredType, role, isDoubles, isNoDynamax) {
        ...
        switch (ability) {
            ...
            case 'Mold Breaker':
                return (abilities.includes('Adaptability') || abilities.includes('Scrappy') || (abilities.includes('Unburden') && !!counter.setupType) ||
            ...
            case 'Swift Swim':
                ...
                return (!moves.has('raindance') && (['Intimidate', 'Rock Head', 'Slush Rush', 'Water Absorb'].some(abil => abilities.includes(abil)) ||
    ...
    getAbility(types, moves, abilities, counter, movePool, teamDetails, species, preferredType, role, isDoubles, isNoDynamax) {
        ...
        lib_1.Utils.sortBy(abilityData, abil => -abil.rating);
        ...
        if (abilities.includes('Guts') &&
            ...
            !abilities.includes('Quick Feet') && (species.id === 'gurdurr' || species.id === 'throh' ||
        ...
        if (isDoubles) {
            ...
            if (abilities.includes('Harvest'))
                return 'Harvest';
            if (abilities.includes('Healer') && (abilities.includes('Natural Cure') ||
        ...
        for (const ability of abilityData) {
            if (ability.rating >= 1 && !this.shouldCullAbility(ability.name, types, moves, abilities, counter, movePool, teamDetails, species, '', '', isDoubles, isNoDynamax)) {
    ...
    getHighPriorityItem(ability, types, moves, counter, teamDetails, species, isLead, isDoubles) {
        ...
        const HDBBetterThanEviolite = (!isDoubles &&
    ...
    getDoublesItem(ability, types, moves, abilities, counter, teamDetails, species) {
        ...
        if (counter.get('Physical') >= 4 && ['fakeout', 'feint', 'rapidspin', 'suckerpunch'].every(m => !moves.has(m)) && (types.has('Dragon') || types.has('Fighting') || types.has('Rock') ||
            ...
            return (!counter.get('priority') && !abilities.includes('Speed Boost') &&
    ...
    getLowPriorityItem(ability, types, moves, abilities, counter, teamDetails, species, isLead, isDoubles, isNoDynamax) {
        ...
        if (moves.has('clangoroussoul') ||
            ...
            (moves.has('boomburst') && Array.from(moves).some(m => dex_1.Dex.moves.get(m).boosts?.spe)))
            return 'Throat Spray';
    ...
    randomSet(species, teamDetails = {}, isLead = false, isDoubles = false, isNoDynamax = false) {
        ...
        const data = this.randomData[species.id];
        const randMoves = (isDoubles && data.doublesMoves) ||
        ...
        do {
            ...
            for (const moveid of moves) {
                ...
                const moveIsRejectable = (!(species.id === 'genesectdouse' && move.id === 'technoblast') &&
                    ...
                    !(species.id === 'shuckle' && ['stealthrock', 'stickyweb'].includes(move.id)) && (move.category === 'Status' ||
    ...
    randomTeam() {
        ...
        while (baseSpeciesPool.length && pokemon.length < this.maxTeamSize) {
            ...
            if (pokemon.some(pkmn => pkmn.name === 'Zoroark') &&
            ...
            if (!isMonotype && !this.forceMonotype) {
                ...
                if (!this.adjustLevel && numMaxLevelPokemon >= limitFactor &&
            ...
            if (potd?.exists && (pokemon.length === 1 || this.maxTeamSize === 1))
                species = potd;
            const set = this.randomSet(species, teamDetails, pokemon.length === 0, isDoubles, this.dex.formats.getRuleTable(this.format).has('dynamaxclause'));
...
exports.RandomGen8Teams = RandomGen8Teams;
exports.default = RandomGen8Teams;
