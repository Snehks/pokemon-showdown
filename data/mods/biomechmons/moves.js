...
exports.Moves = {
        ...
        condition: {
            ...
            onStart(pokemon) {
                ...
                if (!this.dex.items.get(pokemon.item).exists) {
                    const isAbil = pokemon.m.scrambled.abilities.findIndex(e => e.inSlot === 'Item');
                    if (isAbil >= 0) {
            ...
            onEnd(pokemon) {
                ...
                if (!this.dex.items.get(pokemon.item).exists) {
                    ...
                    if (isAbil >= 0) {
                        ...
                        const findMove = pokemon.m.scrambled.moves.findIndex(e => e.inSlot === 'Item');
            ...
            onFieldStart(target, source) {
                ...
                for (const mon of this.getAllActive()) {
                    ...
                    if (!this.dex.items.get(mon.item).exists) {
                        const isAbil = mon.m.scrambled.abilities.findIndex(e => e.inSlot === 'Item');
                        if (isAbil >= 0) {
        ...
        condition: {
            ...
            onStart(pokemon) {
                ...
                if (!this.dex.abilities.get(pokemon.ability).exists) {
                    const isItem = pokemon.m.scrambled.items.findIndex(e => e.inSlot === 'Ability');
                    if (isItem >= 0) {
    ...
    skillswap: {
        ...
        onHit(target, source, move) {
            ...
            if (sourceIsBMM) {
                const isItem = source.m.scrambled.items.findIndex(e => e.inSlot === 'Ability');
                if (isItem >= 0) {
                    source.removeVolatile('item:' + this.toID(source.m.scrambled.items[isItem].thing));
                    source.m.scrambled.items.splice(isItem, 1);
                }
                else if (source.m.scrambled.moves.findIndex(e => e.inSlot === 'Ability') >= 0) {
                    const isMove = source.m.scrambled.moves.findIndex(e => e.inSlot === 'Ability');
                    source.baseMoveSlots.splice(source.baseMoveSlots.findIndex(m => this.toID(source.m.scrambled.moves[isMove].thing) === m.id), 1);
            ...
            if (targetIsBMM) {
                const isItem = target.m.scrambled.items.findIndex(e => e.inSlot === 'Ability');
                if (isItem >= 0) {
                    ...
                    target.baseMoveSlots.splice(target.baseMoveSlots.findIndex(m => this.toID(target.m.scrambled.moves[isMove].thing) === m.id), 1);
