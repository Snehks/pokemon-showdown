...
const pokemon_1 = require("../../../sim/pokemon");
exports.Scripts = {
    pokemon: {
        ...
        ignoringAbility() {
            ...
            for (const pokemon of this.battle.getAllActive()) {
                ...
                if ((pokemon.ability === 'neutralizinggas' ||
                    pokemon.m.scrambled.abilities.some(abils => this.battle.toID(abils.thing) === 'neutralizinggas')) &&
            ...
            return !!((this.battle.gen >= 5 && !this.isActive) ||
                ((this.volatiles['gastroacid'] ||
                    (neutralizinggas && (this.ability !== 'neutralizinggas' ||
                        this.m.scrambled.abilities.some(abils => this.battle.toID(abils.thing) === 'neutralizinggas')))) && !this.getAbility().flags['cantsuppress']));
        },
        setAbility(ability, source, sourceEffect, isFromFormeChange = false, isTransform = false) {
            const allThings = new Set([
                ...this.m.scrambled.abilities.map(e => e.thing),
                ...this.m.scrambled.items.map(e => e.thing),
                ...this.m.scrambled.moves.map(e => e.thing),
            ...
            if (isOldBMMAbil) {
                const isItem = this.m.scrambled.items.findIndex(e => e.inSlot === 'Ability');
                if (isItem >= 0) {
                    this.removeVolatile('item:' + this.battle.toID(this.m.scrambled.items[isItem].thing));
                    this.m.scrambled.items.splice(isItem, 1);
                }
                else if (this.m.scrambled.moves.findIndex(e => e.inSlot === 'Ability') >= 0) {
        ...
        takeItem(source) {
            ...
            if (this.battle.gen <= 4) {
                if (source.itemKnockedOff)
                    return false;
                if (this.battle.toID(this.ability) === 'multitype' || this.m.scrambled.abilities
                    .findIndex(e => this.battle.toID(e.thing) === 'multitype') >= 0) {
                    return false;
                }
                if (this.battle.toID(source.ability) === 'multitype' || source.m.scrambled.abilities
                    .findIndex(e => this.battle.toID(e.thing) === 'multitype') >= 0) {
                    return false;
                }
            }
        ...
        setItem(item, source, effect) {
            ...
            if (typeof item === 'string') {
                if (!item.length || this.battle.dex.items.get(item).exists) {
                    ...
                    let newData = this.battle.dex.abilities.get(itemString);
                    if (!newData.exists) {
            ...
            if (pokemon_1.RESTORATIVE_BERRIES.has('leppaberry')) {
        ...
        eatItem(force, source, sourceEffect) {
            ...
            if (this.battle.runEvent('UseItem', this, null, null, Dex.items.get(item.name)) &&
                ...
                if (pokemon_1.RESTORATIVE_BERRIES.has(item.id)) {
