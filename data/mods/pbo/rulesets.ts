export const Rulesets: import('../../../sim/dex-formats').ModdedFormatDataTable = {
	...
	pbomisfortune: {
		...
		onStart(target) {
			...
			// End any passive held-item effects on attach (mirrors Embargo/Klutz pattern).
	...
	pboaegis: {
		...
		onStart(target) {
			...
			(this.effectState as any).immuneOnTurn = -1;
		},
		onDamage(damage, target, source, effect) {
		...
		onDamagingHit(damage, target, source, move) {
			(this.effectState as any).immuneOnTurn = this.turn + 1;
		},
	},
	...
	pbodoomed: {
		...
		onStart(target) {
			...
			(this.effectState as any).countdown = 5;
		},
		onBasePowerPriority: 8,
		onBasePower(basePower) {
	...
	pboethereal: {
		...
		onTryBoost(boost, target, source, effect) {
			...
			if (blocked && effect && !(effect as any).secondaries) {
	...
	pbochains: {
		...
		onStart(pokemon) {
			...
			(this.effectState as any).lockedMove = null;
		},
		onAfterMove(source, target, move) {
			const state = this.effectState as any;
		...
		onLockMove(pokemon) {
			const state = this.effectState as any;
			return state.lockedMove || undefined;
		},
	...
	pbodespair: {
		...
		onStart(pokemon) {
			...
			(this.effectState as any).lastMove = null;
		},
			...
			if (state.lastMove) {
				for (const moveSlot of pokemon.moveSlots) {
					if (moveSlot.id === state.lastMove) {
						pokemon.disableMove(moveSlot.id, false, this.effect.name as any);
					}
				}
			}
		},
	...
	pbodisarray: {
		...
		onLockMove(pokemon) {
			const usable = pokemon.moveSlots
	...
	pboruin: {
		...
		onStart(pokemon) {
			...
			(this.effectState as any).turnsActive = 0;
		},
		onResidualOrder: 28,
		onResidual(pokemon) {
			const state = this.effectState as any;
			...
			for (stat in pokemon.boosts) {
				...
				if (value !== 0) {
					pokemon.boosts[stat] = -value as typeof value;
					changed = true;
				}
			}
			if (changed) {
	...
	pborupture: {
		...
		onStart(pokemon) {
			...
			(this.effectState as any).rageTriggered = false;
		},
