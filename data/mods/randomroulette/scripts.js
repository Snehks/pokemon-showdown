...
exports.Scripts = {
    start() {
        ...
        const format = Dex.formats.get(`gen${this.gen}randombattle@@@${[...this.format.ruleset, ...(this.format.customRules || [])].join(',')}`);
        this.dex = Dex.forFormat(format);
        this.ruleTable = this.dex.formats.getRuleTable(format);
        this.teamGenerator = teams_1.Teams.getGenerator(format);
        ...
        for (const side of this.sides) {
            this.teamGenerator.setSeed(prng_1.PRNG.generateSeed());
            const team = this.teamGenerator.getTeam();
            side.team = team;
            side.pokemon = [];
            for (let i = 0; i < team.length && i < 24; i++) {
                side.pokemon.push(new pokemon_1.Pokemon(team[i], side));
