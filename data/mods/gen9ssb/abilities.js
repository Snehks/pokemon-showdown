    ...
    */
    ...
    quagofruin: {
        ...
        onAnyModifyDef(def, target, source, move) {
            ...
            if (target.hasAbility('Quag of Ruin'))
                return;
            if (!move.ruinedDef?.hasAbility('Quag of Ruin'))
    ...
    clodofruin: {
        ...
        onAnyModifyAtk(atk, target, source, move) {
            if (!move)
                return;
            const abilityHolder = this.effectState.target;
            if (target.hasAbility('Clod of Ruin'))
                return;
            if (!move.ruinedAtk?.hasAbility('Clod of Ruin'))
    ...
    cascade: {
        ...
        name: "Cascade",
        onUpdate(pokemon) {
            if (pokemon.baseSpecies.baseSpecies !== 'Hatterene' || pokemon.transformed || !pokemon.hp)
                return;
            if (pokemon.species.id === 'mismagius' || pokemon.hp > pokemon.maxhp / 4)
                return;
            this.add(`c:|${(0, scripts_1.getName)('ausma')}|that's it, yall mfs are about to face the wrath of Big Stall™`);
            this.add(`c:|${(0, scripts_1.getName)('ausma')}|or i guess moreso Big Pult. pick your poison`);
            this.add('-activate', pokemon, 'ability: Cascade');
            (0, scripts_1.changeSet)(this, pokemon, random_teams_1.ssbSets['ausma-Mismagius'], true);
            pokemon.cureStatus();
    ...
    painfulexit: {
        ...
        onBeforeSwitchOut(pokemon) {
            this.add(`c:|${(0, scripts_1.getName)('Breadey')}|Just kidding!! Take this KNUCKLE SANDWICH`);
            for (const foe of pokemon.foes()) {
                if (!foe || foe.fainted || !foe.hp)
                    continue;
    ...
    hacking: {
        ...
        onStart(pokemon) {
            ...
            this.add(`c:|${(0, scripts_1.getName)(name)}|One moment, please. One does not simply go into battle blind.`);
            const side = pokemon.side.id === 'p1' ? 'p2' : 'p1';
            this.add(`message`, (`ssh sim@pokemonshowdown.com && nc -U logs/repl/sim <<< ` +
            ...
            if (!warnMoves.length) {
                this.add(`c:|${(0, scripts_1.getName)(name)}|Fascinating. None of your sets have any moves of interest.`);
                return;
            }
            const [warnMoveName, warnTarget] = this.sample(warnMoves);
            this.add('message', `${name} hacked into PS and looked at ${name === 'Cassiopeia' ? 'her' : 'their'} opponent's sets. ` +
                `${warnTarget.name}'s move ${warnMoveName} drew ${name === 'Cassiopeia' ? 'her' : 'their'} eye.`);
            this.add(`c:|${(0, scripts_1.getName)(name)}|Interesting. With that in mind, bring it!`);
        },
        flags: {},
    },
    // Chloe
    ...
    thatshacked: {
        ...
        onResidual(target, source, effect) {
            ...
            this.add(`c:|${(0, scripts_1.getName)((target.illusion || target).name)}|${this.sample(abilMessages)}`);
    ...
    masquerade: {
        ...
        onUpdate(pokemon) {
            if (['mimikyu', 'mimikyutotem'].includes(pokemon.species.id) && this.effectState.busted) {
                ...
                this.add(`c:|${(0, scripts_1.getName)('clerica')}|oop`);
            }
        },
    ...
    jetstream: {
        ...
        onAnySetWeather(target, source, weather) {
            if (this.field.isWeather('deltastream') && !STRONG_WEATHERS.includes(weather.id))
                return false;
        },
        onEnd(pokemon) {
    ...
    formchange: {
        ...
        name: "Form Change",
        onResidual(pokemon) {
            if (pokemon.baseSpecies.baseSpecies !== 'Necrozma' || pokemon.transformed || !pokemon.hp)
                return;
            let newSet = 'Dawn of Artemis';
            if (pokemon.hp > pokemon.maxhp / 2) {
                if (pokemon.species.id === 'necrozma')
                    return;
                this.add(`c:|${(0, scripts_1.getName)('Dawn of Artemis')}|Good, I'm healthy again, time to swap back.`);
            }
            else {
                if (pokemon.species.id === 'necrozmaultra')
                    return;
                this.add(`c:|${(0, scripts_1.getName)('Dawn of Artemis')}|Time for me to transform and you to witness the power of Ares now!`);
                newSet += '-Ultra';
            }
            this.add('-activate', pokemon, 'ability: Form Change');
            (0, scripts_1.changeSet)(this, pokemon, random_teams_1.ssbSets[newSet]);
        ...
        onDamage(damage, target, source, effect) {
            if (effect.id === 'recoil') {
                ...
                if (this.activeMove.id !== 'struggle') {
                    if (!trueSource.hasType(this.activeMove.type))
                        this.heal(damage);
                    return null;
                }
            }
    ...
    illiterit: {
        ...
        onTryHit(target, source, move) {
            if (target !== source && move.id.length >= 12) {
                ...
                this.add(`c:|${(0, scripts_1.getName)('in the hills')}|Gee ${source.name}, maybe I should get a dictionary so I can understand what move you just used.`);
                return null;
            }
        },
        flags: { breakable: 1 },
    ...
    deserteddunes: {
        ...
        onAnySetWeather(target, source, weather) {
            if (this.field.getWeather().id === 'deserteddunes' && !STRONG_WEATHERS.includes(weather.id))
                return false;
        },
        onEnd(pokemon) {
            if (this.field.weatherState.source !== pokemon)
    ...
    flashfreeze: {
        ...
        onSourceModifyDamage(damage, source, target, move) {
            if ((move.category === 'Special' && source.boosts['spa'] > 0) ||
            ...
            for (const mon of this.getAllActive()) {
                ...
                for (const volatile of volatilesToClear) {
                    if (mon.volatiles[volatile]) {
                        ...
                        if (volatile === 'flipped') {
                            (0, scripts_1.changeSet)(this, mon, random_teams_1.ssbSets['Clem']);
    ...
    eoncall: {
        ...
        onAfterMove(source, target, move) {
            if (move.category === 'Status' && source.species.baseSpecies === 'Latias') {
                (0, scripts_1.changeSet)(this, source, random_teams_1.ssbSets['MyPearl'], true);
            }
            else if (move.category === 'Special' && source.species.baseSpecies === 'Latios') {
                (0, scripts_1.changeSet)(this, source, random_teams_1.ssbSets['MyPearl-Latias'], true);
            }
        },
        flags: {},
    },
    ...
    ctiershitposter: {
        ...
        onDamage(damage, target, source, effect) {
            var _a;
            (_a = target.m).damageTaken ?? (_a.damageTaken = 0);
            target.m.damageTaken += damage;
            if (target.set && !target.set.shiny) {
                if (target.m.damageTaken >= 420) {
                    target.set.shiny = true;
                    if (!target.hp) {
                        return this.add(`c:|${(0, scripts_1.getName)('PartMan')}|MWAHAHA NOW YOU - oh I'm dead`);
                    }
                    this.add(`c:|${(0, scripts_1.getName)('PartMan')}|That's it. Get ready to be rapid-fire hugged.`);
                    target.clearBoosts();
    ...
    antipelau: {
        ...
        onStart(target) {
            ...
            this.actions.useMove(this.dex.getActiveMove('wish'), target, { target, sourceEffect: this.dex.abilities.get('antipelau') });
        },
        flags: {},
    },
    // Rainshaft
    rainysaura: {
        ...
        name: "Rainy's Aura",
        onStart(source) {
            if (this.suppressingAbility(source))
                return;
            if (source.species.id === 'kyogre' && source.item === 'blueorb')
                return;
            this.field.setWeather('raindance');
        ...
        onAnyBasePower(basePower, source, target, move) {
            if (target === source || move.category === 'Status' || move.type !== 'Psychic')
                return;
            if (!move.auraBooster?.hasAbility('Rainy\'s Aura'))
    ...
    aswesee: {
        ...
        onFoeAfterBoost(boost, target, source, effect) {
            if (this.randomChance(3, 4)) {
                if (effect && ['As We See', 'Mirror Herb', 'Opportunist'].includes(effect.name))
                    return;
    ...
    seenoevilhearnoevilspeaknoevil: {
        ...
        onSourceAccuracy(accuracy, target, source, move) {
            if (!source.species.id.startsWith('ogerponwellspring'))
                return;
            if (typeof accuracy !== 'number')
                return;
            return true;
        ...
        onBasePower(basePower, attacker, defender, move) {
            if (!attacker.species.id.startsWith('ogerponhearthflame'))
                return;
    ...
    wildgrowth: {
        ...
        onModifyMovePriority: -1,
        onAfterMove(source, target, move) {
            if (target.hasType('Grass') || target.hasAbility('Sap Sipper') || !move.hit || target === source)
                return null;
    ...
    treasurebag: {
        ...
        onResidual(target, source, effect) {
            if (!target.volatiles['treasurebag'])
                target.addVolatile('treasurebag');
        },
        condition: {
            ...
            onDamage(damage, pokemon, source, effect) {
                if (damage >= pokemon.hp && pokemon.m.seedActive) {
                    if (!pokemon.m.reviverSeedTriggered) {
                        ...
                        this.add(`c:|${(0, scripts_1.getName)('yeet dab xd')}|An "s"?`);
    ...
    tempochange: {
        ...
        onResidual(pokemon) {
            ...
            if (pokemon.species.name === 'Meloetta') {
                (0, scripts_1.changeSet)(this, pokemon, random_teams_1.ssbSets['Zarel-Pirouette'], true);
            }
            else {
                (0, scripts_1.changeSet)(this, pokemon, random_teams_1.ssbSets['Zarel'], true);
            }
        },
    ...
    deltastream: {
        inherit: true,
        onAnySetWeather(target, source, weather) {
            if (this.field.getWeather().id === 'deltastream' && !STRONG_WEATHERS.includes(weather.id))
                return false;
        },
    },
    desolateland: {
        inherit: true,
        onAnySetWeather(target, source, weather) {
            if (this.field.getWeather().id === 'desolateland' && !STRONG_WEATHERS.includes(weather.id))
                return false;
        },
    },
    dryskin: {
    ...
    primordialsea: {
        inherit: true,
        onAnySetWeather(target, source, weather) {
            if (this.field.getWeather().id === 'primordialsea' && !STRONG_WEATHERS.includes(weather.id))
                return false;
        },
    },
    raindish: {
    ...
    sandforce: {
        ...
        onImmunity(type, pokemon) {
            if (type === 'sandstorm' || type === 'deserteddunes')
                return false;
        },
    },
    sandrush: {
