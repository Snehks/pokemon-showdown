...
exports.Scripts = exports.PSEUDO_WEATHERS = void 0;
exports.getRoomauth = getRoomauth;
exports.getName = getName;
exports.enemyStaff = enemyStaff;
exports.changeSet = changeSet;
exports.changeMoves = changeMoves;
const lib_1 = require("../../../lib");
const dex_data_1 = require("../../../sim/dex-data");
// Similar to User.usergroups. Cannot import here due to users.ts requiring Chat
// This also acts as a cache, meaning ranks will only update when a hotpatch/restart occurs
const usergroups = {};
const usergroupData = (0, lib_1.FS)('config/usergroups.csv').readIfExistsSync().split('\n');
for (const row of usergroupData) {
    if (!(0, dex_data_1.toID)(row))
        continue;
    const cells = row.split(',');
    if (cells.length > 3)
        throw new Error(`Invalid entry when parsing usergroups.csv`);
    usergroups[(0, dex_data_1.toID)(cells[0])] = cells[1].trim() || ' ';
}
const roomauth = {};
/**
...
function getRoomauth(name, room) {
    const userid = (0, dex_data_1.toID)(name);
    const roomid = (0, dex_data_1.toID)(room);
    if (roomauth[roomid])
        return roomauth[roomid][userid] || null;
    const roomsList = JSON.parse((0, lib_1.FS)('config/chatrooms.json').readIfExistsSync() || '[]');
    const roomData = roomsList.find(r => (0, dex_data_1.toID)(r.title) === roomid);
    if (!roomData)
        return null;
...
function changeSet(context, pokemon, newSet, changeAbility = false) {
    ...
    if (oldShiny !== pokemon.set.shiny || oldGender !== pokemon.gender)
        context.add('replace', pokemon, details);
    if (changeAbility)
        pokemon.setAbility(newSet.ability, undefined, undefined, true);
    pokemon.baseMaxhp = pokemon.species.name === 'Shedinja' ? 1 : Math.floor(Math.floor(2 * pokemon.species.baseStats.hp + pokemon.set.ivs.hp + Math.floor(pokemon.set.evs.hp / 4) + 100) * pokemon.level / 100 + 10);
...
exports.Scripts = {
        ...
        switch (action.choice) {
            ...
            case 'scapegoat':
                ...
                if (percent > 66) {
                    ...
                    this.boost({ atk: 4, spa: 4, spe: 4 }, action.pokemon, action.pokemon, this.dex.moves.get('scapegoat'));
                }
                else if (percent > 33) {
    ...
    actions: {
        ...
        canTerastallize(pokemon) {
            if (pokemon.terastallized || pokemon.species.isMega || pokemon.species.isPrimal || pokemon.species.forme === "Ultra" ||
        ...
        chooseSwitch(slotText) {
            ...
            if (isNaN(slot) || slot < 0) {
                ...
                for (const [i, mon] of this.pokemon.entries()) {
                    if (slotText.toLowerCase() === mon.name.toLowerCase() || (0, dex_data_1.toID)(slotText) === mon.species.id) {
                        slot = i;
                        break;
                    }
                }
