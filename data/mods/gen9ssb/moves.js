...
const teams_1 = require("../../../sim/teams");
exports.Moves = {
    /*
    // Example
    moveid: {
    ...
    */
    ...
    iamatomic: {
        ...
        onPrepareHit(target, source, move) {
            ...
            this.add(`c:|${(0, scripts_1.getName)((source.illusion || source).name)}|I`);
            this.add(`c:|${(0, scripts_1.getName)((source.illusion || source).name)}|AM`);
            this.add(`c:|${(0, scripts_1.getName)((source.illusion || source).name)}|ATOMIC.`);
        },
        self: {
            boosts: {
                spe: -2,
    ...
    spicierextract: {
        ...
        onHit(target, source) {
            let success = false;
            if (target.addVolatile('partiallytrapped', source))
                success = true;
            if (target.addVolatile('leechseed', source))
                success = true;
            if (target.addVolatile('healblock', source))
                success = true;
            return success;
        },
        secondary: null,
    ...
    scumhunt: {
        ...
        onPrepareHit(target, source, move) {
            ...
            const moves = this.dex.moves.all().filter(m => ((![2, 4].includes(this.gen) || !source.moves.includes(m.id)) &&
    ...
    extracourse: {
        ...
        basePower: 0,
        category: "Status",
        shortDesc: "Recycles berry, +1 to 3 random stats (-acc/eva).",
        ...
        onHit(pokemon) {
            ...
            for (let i = 0; i < 3; i++) {
                ...
                for (statPlus in pokemon.boosts) {
                    if (statPlus === 'accuracy' || statPlus === 'evasion' || boostedStats.includes(statPlus))
                        continue;
                    if (pokemon.boosts[statPlus] < 6) {
                        stats.push(statPlus);
                    }
                }
                const randomStat = stats.length ? this.sample(stats) : undefined;
                if (randomStat) {
                    boost[randomStat] = 1;
    ...
    sireswitch: {
        ...
        onHit(pokemon) {
            if (pokemon.species.name === 'Quagsire') {
                ...
                (0, scripts_1.changeSet)(this, pokemon, random_teams_1.ssbSets['A Quag To The Past-Clodsire'], true);
            }
            else {
                (0, scripts_1.changeSet)(this, pokemon, random_teams_1.ssbSets['A Quag To The Past'], true);
            }
        },
        secondary: null,
        target: "self",
    ...
    funnyvamp: {
        ...
        onPrepareHit(target, source) {
            ...
            this.add(`c:|${(0, scripts_1.getName)('Arcueid')}|I've got nine lives 🐈. You knew that about cats, right, 😹? That means I haven't lost until you beat me nine times 🙀!`);
        },
        onHit(target, source, move) {
            if (source.species.name === "Deoxys-Defense") {
                (0, scripts_1.changeSet)(this, source, random_teams_1.ssbSets['Arcueid-Attack'], true);
                ...
                (0, scripts_1.changeSet)(this, source, random_teams_1.ssbSets['Arcueid'], true);
                for (let i = 0; i < 2; i++) {
                    const stats = [];
                    const boost = {};
                    let statPlus;
    ...
    automatedresponse: {
        ...
        beforeTurnCallback(source, target) {
            ...
            if (!seTypes.length)
                seTypes.push('Electric');
            if (!nveTypes.length)
                nveTypes.push('Electric');
    ...
    thinkinginprogress: {
        ...
        basePower: 0,
        category: "Status",
        shortDesc: "Cure status, +1 Def/SpD, +2 SpA.",
    ...
    sigilsstorm: {
        ...
        onModifyPriority(priority, source, target, move) {
            if (source.species.id === 'mismagius')
                return priority + 6;
        },
        ...
        beforeMoveCallback(pokemon) {
            ...
            if (pokemon.volatiles['sigilsstorm']?.lostFocus) {
                ...
                this.add(`c:|${(0, scripts_1.getName)('ausma')}|dog can you not`);
                return true;
            }
        },
        onModifyType(move, pokemon, target) {
        ...
        secondary: {
            ...
            onHit(target, source, move) {
                ...
                if (chance <= 10) {
                    ...
                    const randomStat = stats.length ? this.sample(stats) : undefined;
                    const randomStat2 = stats.length ? this.sample(stats.filter(s => s !== randomStat)) : undefined;
                    ...
                    (0, scripts_1.changeSet)(this, target, random_teams_1.ssbSets["ausma-Fennekin"], true);
                    this.add(`c:|${(0, scripts_1.getName)('ausma')}|oh shit i posted to the wrong account`);
                }
            },
        },
        target: "normal",
    ...
    preptime: {
        ...
        basePower: 0,
        category: "Status",
        shortDesc: "+2 SpA, +1 Def/SpD/Spe.",
        desc: "The user's Special Attack is boosted by 2 stages, and its Defense, Special Defense, and Speed are boosted by 1 stage.",
        name: "Prep Time",
        pp: 5,
        priority: 0,
        flags: { snatch: 1, metronome: 1 },
    ...
    yugiohreference: {
        ...
        basePower: 110,
        category: "Special",
        shortDesc: "50% chance to force the foe out.",
        desc: "Nearly always moves last. If this attack is successful, there is a 50% chance that the target is forced to switch out and be replaced with a random unfainted ally.",
    ...
    buzzerstingercounter: {
        ...
        basePower: 50,
        category: "Physical",
        shortDesc: "+3 prio if foe uses custom move. +2 Atk on KO.",
        desc: "This move will nearly always move first (+3 priority) if the target would use a custom move from Super Staff Bros Ultimate this turn. Raises the user's Attack by 2 stages if this move knocks out the target.",
        ...
        condition: {
            ...
            onTryHit(target, source, move) {
                ...
                if (success) {
                    ...
                    target.switchFlag = 'shatterandscatter';
                }
    ...
    hackcheck: {
        ...
        onHit(target, source, move) {
            if (this.randomChance(1, 5)) {
                (0, scripts_1.changeSet)(this, source, random_teams_1.ssbSets['Billo-Solgaleo'], true);
                ...
                this.add(`c:|${(0, scripts_1.getName)('Billo')}|This is a streamer mon, you're banned from the room.`);
            }
            else {
                (0, scripts_1.changeSet)(this, source, random_teams_1.ssbSets['Billo-Lunala'], true);
                this.add(`c:|${(0, scripts_1.getName)('Billo')}|Everything checks out, remember to report any suspicious mons to staff!`);
            }
        },
        target: "self",
        type: "Normal",
    ...
    testinginproduction: {
        ...
        onHit(pokemon) {
            this.add(`c:|${(0, scripts_1.getName)((pokemon.illusion || pokemon).name)}|Please don't break...`);
            let stats = [];
            const boost = {};
            let statPlus;
            for (statPlus in pokemon.boosts) {
            ...
            if (randomStat) {
                if (boost[randomStat]) {
                    boost[randomStat] = 0;
                    this.add(`c:|${(0, scripts_1.getName)((pokemon.illusion || pokemon).name)}|Well. Guess that broke. Time to roll back.`);
                    return;
                }
                else {
                    boost[randomStat] = -2;
        ...
        onAfterMove(pokemon) {
            if (this.randomChance(1, 10)) {
                this.add(`c:|${(0, scripts_1.getName)((pokemon.illusion || pokemon).name)}|Ouch! That crash is really getting on my nerves...`);
            ...
            if (this.randomChance(1, 20)) {
                ...
                this.add(`c:|${(0, scripts_1.getName)((pokemon.illusion || pokemon).name)}|Darn. A bug ${statusText} me. Guess I should have tested this first.`);
    ...
    summonmonsterviiifiendishmonstrouspiplupedecolossal: {
        ...
        onAfterMove(source, target, move) {
            this.add(`c:|${(0, scripts_1.getName)((source.illusion || source).name)}|There's no way this'll faint in one punch!`);
        },
    ...
    o: {
        ...
        onEffectiveness(typeMod, target, type) {
            if (target?.name === 'Kennedy')
                return 1;
        },
        onHit(target, source, move) {
            if (source.illusion || source.baseSpecies.baseSpecies !== 'Avalugg')
                return;
    ...
    magicalfocus: {
        ...
        onDisableMove(pokemon) {
            if (pokemon.species.id === 'necrozmaultra')
                pokemon.disableMove('magicalfocus');
        },
        self: {
    ...
    snacktime: {
        ...
        basePower: 0,
        category: "Status",
        shortDesc: "Charges.Turn 2: +2 Atk/Def, heal 33% for 3 turns.",
    ...
    teaparty: {
        ...
        onHit(pokemon) {
            ...
            if (this.randomChance(7, 8)) { // get boiled
                ...
                this.add(`c:|${(0, scripts_1.getName)((pokemon.illusion || pokemon).name)}|Just tea, thank you`);
            }
            else { // get beefed
                ...
                this.add(`c:|${(0, scripts_1.getName)((pokemon.illusion || pokemon).name)}|BOVRIL TIME`);
            }
    ...
    solidarity: {
        ...
        onHit(target, source) {
            if (target.hasType('Grass') || target.isProtected())
                return null;
        ...
        self: {
            onHit(target, source) {
                ...
                if (source.hp <= source.maxhp / 6 || source.maxhp === 1) { // Shedinja clause
                    ...
                    this.directDamage(source.maxhp / 6);
                }
            },
        },
        secondary: null,
        ...
        onHit(target, source, move) {
            ...
            for (i in source.boosts) {
                if (source.boosts[i] < 0) {
                    ...
                    this.add('-setboost', source, i, source.boosts[i], '[silent]');
                }
            }
    ...
    augurofebrietas: {
        ...
        onAfterHit(target, source, move) {
            this.add(`c:|${(0, scripts_1.getName)((source.illusion || source).name)}|as you once did for the vacuous Rom,`);
        },
        selfSwitch: true,
    ...
    scapegoat: {
        ...
        onHit(target, source) {
            this.add(`c:|${(0, scripts_1.getName)((source.illusion || source).name)}|Ok I have a stupid idea, just hear me out`);
    ...
    hydrostatics: {
        ...
        onPrepareHit(target, source) {
            if (!source.terastallized)
                this.add('-anim', source, 'Charge Beam', source);
            else
                this.add('-anim', source, 'Water Pulse', target);
        },
        onEffectiveness(typeMod, target, type, move) {
            ...
            else if (type === 'Water' && move.type === 'Water')
                return 1;
        },
        secondary: {
            chance: 100,
            onHit(target, source, move) {
                ...
                if (!source.terastallized) {
                    ...
                    if (this.randomChance(50, 100)) {
                        if (target.isActive)
                            target.trySetStatus('par', source, this.effect);
                    }
                }
                else {
    ...
    bibbidibobbidirands: {
        ...
        onHit(target, source) {
            ...
            let depth = 0;
            while (!team.length) {
                team = teams_1.Teams.generate(randFormat, { name: target.side.name });
                if (depth >= 50)
                    break; // Congrats you won the lottery!
                team = team.filter(p => {
            ...
            (0, scripts_1.changeSet)(this, target, team[0], true);
            this.add(`c:|${(0, scripts_1.getName)((source.illusion || source).name)}|${msg}`);
        },
        isZ: "irpatuziniumz",
        secondary: null,
        target: "normal",
    ...
    anfieldatmosphere: {
        ...
        condition: {
            ...
            onSetStatus(status, target, source, effect) {
                if (effect.id === 'anfieldatmosphere')
                    return;
    ...
    breakingchange: {
        ...
        onHit(target, source) {
            if (target.getAbility().flags['cantsuppress'])
                return;
            if (!target.addVolatile('gastroacid'))
                return;
            this.add(`c:|${(0, scripts_1.getName)((source.illusion || source).name)}|Sorry i tried to fix smth but accidentally broke your ability :( will fix it next week`);
        },
        ignoreAbility: true,
        secondary: null,
        target: "normal",
    ...
    plagiarism: {
        ...
        onHit(target, source, m) {
            let sigMoveName = random_teams_1.ssbSets[(target.illusion || target).name]?.signatureMove;
            if (!sigMoveName)
                sigMoveName = target.moveSlots[target.moveSlots.length - 1].id;
            ...
            if (plagiarismIndex < 0)
                return false;
            this.add(`c:|${(0, scripts_1.getName)((source.illusion || source).name)}|yoink`);
    ...
    bananabreakfast: {
        ...
        basePower: 0,
        category: "Status",
        shortDesc: "2 random stats +2; Lock-On/Laser Focus/Charge.",
        desc: "Boosts 2 random stats of the user by 2 stage each, except Accuracy and Evasion. These stats can be the same. Applies one of Lock-On, Laser Focus, or Charge to the user at random.",
        ...
        onHit(target, source) {
            ...
            const randomStat = stats.length ? this.sample(stats) : undefined;
            const randomStat2 = stats.length ? this.sample(stats) : undefined;
            if (randomStat)
                boost[randomStat] = 2;
            if (randomStat2 && randomStat === randomStat2)
                boost[randomStat] = 4;
            else if (randomStat2)
                boost[randomStat2] = 2;
    ...
    eonassault: {
        ...
        basePower: 45,
        category: "Special",
        shortDesc: "Hits twice. 40% -1 Sp. Atk, 40% -1 Sp. Def.",
        desc: "Hits 2 times. Each hit has a 40% chance to lower Special Attack by 1 stage, and a 40% chance to lower Special Defense by 1 stage.",
        name: "Eon Assault",
        gen: 9,
        pp: 15,
        priority: 0,
    ...
    qualitycontrolzoomies: {
        ...
        onHit(target, source, move) {
            ...
            this.add(`c:|${(0, scripts_1.getName)('Neko')}|${message}`);
        },
    ...
    nyaa: {
        ...
        condition: {
            onSwitchIn(target) {
                ...
                if (!target.fainted) {
                    this.add(`c:|${(0, scripts_1.getName)((source.illusion || source).name)}|~nyaa ${target.name}`);
                    this.add(`c:|${(0, scripts_1.getName)('Jeopard-E')}|**It is now ${target.name}'s turn to ask a question.**`);
    ...
    '3': {
        ...
        basePower: 70,
        category: "Special",
        shortDesc: "40% infatuates. 10% backfire chance.",
        desc: "This move has a 40% chance to infatuate the target, regardless of gender, but this move has a 10% chance to be used by the target at the user instead.",
        name: ":3",
        gen: 9,
        pp: 5,
        priority: 0,
    ...
    alting: {
        ...
        onPrepareHit(target, source) {
            this.add('-anim', source, 'Memento', target);
            this.add(`l|${(0, scripts_1.getName)((source.illusion || source).name).split('|')[1]}`);
            this.add(`j|FakePart`);
        },
        onModifyMove(move, source, target) {
            move.type = "???";
    ...
    "1000gears": {
        ...
        category: "Status",
        name: "1000 Gears",
        shortDesc: "Heals 100% HP,cures status,+1 Def/SpD/Spe,+10 levels.",
        desc: "Z-Move requiring Pearyum Z. Heals the user for 100% of its maximum HP, cures its non-volatile status effects, boosts its Defense and Special Defense by 1 stage, and raises its level by 10.",
        pp: 1,
        priority: 0,
        flags: {},
        onTryMove() {
    ...
    asongoficeandfire: {
        ...
        onHit(target, source, move) {
            if (source.species.name === 'Volcarona') {
                (0, scripts_1.changeSet)(this, source, random_teams_1.ssbSets['Pissog-Frosmoth'], true);
            }
            else if (source.species.name === 'Frosmoth') {
                (0, scripts_1.changeSet)(this, source, random_teams_1.ssbSets['Pissog'], true);
            }
        },
        target: "normal",
        type: "Fire",
    ...
    roulette: {
        ...
        category: "Status",
        name: "Roulette",
        shortDesc: "Use 2 random moves, and get a random moveset.",
        ...
        onHit(target, source) {
            ...
            const moves = this.dex.moves.all().filter(move => (!source.moves.includes(move.id) &&
    ...
    pleek: {
        ...
        category: "Status",
        name: "Pleek...",
        shortDesc: "+8 Attack + inflict Perish Song on the user.",
        desc: "Raises the user's Attack by 8 stages. Inflicts the Perish Song effect on the user, causing it to faint in three turns at the end of the turn.",
        pp: 10,
        priority: 0,
        ...
        onHit(target, source, move) {
            this.add(`c:|${(0, scripts_1.getName)((source.illusion || source).name)}|Pleek...`);
            this.boost({ atk: 8 }, source, source, move);
            source.addVolatile('perishsong');
            this.add('-start', source, 'perish3', '[silent]');
    ...
    luckpulse: {
        ...
        onTryHit(target, source, move) {
            ...
            this.add(`c:|${(0, scripts_1.getName)((source.illusion || source).name)}|The kS stands for ${this.sample(messages)}`);
        },
        secondary: {
            chance: 40,
            onHit(target, source, move) {
        ...
        condition: {
            ...
            onResidual(pokemon, source) {
                ...
                if (!pokemon || pokemon.fainted || pokemon.hp <= 0) {
                    this.add(`c:|${(0, scripts_1.getName)((target.illusion || target).name)}|Tripping off the beat kinda, dripping off the meat grinder`);
                }
                if (!target || target.fainted || target.hp <= 0) {
    ...
    worriednoises: {
        ...
        category: "Special",
        name: "*Worried Noises*",
        shortDesc: "+2 SpA. Type varies based on user's primary type.",
    ...
    magictrick: {
        ...
        onHit(target, source, move) {
            ...
            if (this.field.clearWeather())
                success = true;
            for (const pseudoWeather of scripts_1.PSEUDO_WEATHERS) {
    ...
    confiscate: {
        ...
        onTryHit(target, source) {
            this.add(`c:|${(0, scripts_1.getName)((source.illusion || source).name)}|Contraband detected, confiscating.`);
    ...
    midnightbird: {
        ...
        category: "Special",
        name: "Midnight Bird",
        shortDesc: "100% chance to raise the user's Sp. Atk by 2.",
        desc: "Has a 100% chance to raise the user's Special Attack by 2 stages.",
        pp: 10,
        priority: 0,
        flags: { protect: 1, mirror: 1 },
        onTryMove() {
    ...
    phantomweapon: {
        ...
        onPrepareHit(target, source, move) {
            ...
            if (!source.item || source.ignoringItem())
                return;
        ...
        condition: {
            ...
            onStart(target, source, effect) {
                ...
                if (!['costar', 'imposter', 'psychup', 'transform'].includes(effect?.id)) {
                    this.add('-start', target, 'move: Rising Sword');
                }
            },
    ...
    sandeat: {
        ...
        onHit(pokemon) {
            ...
            const replacementIndex = Math.max(pokemon.moves.indexOf('dynamicpunch'), pokemon.moves.indexOf('pyroball'), pokemon.moves.indexOf('aquatail'), pokemon.moves.indexOf('tripleaxel'), pokemon.moves.indexOf('stoneedge'));
            ...
            this.add(`c:|${(0, scripts_1.getName)((pokemon.illusion || pokemon).name)}|omg look HE EAT`);
        },
        onPrepareHit(pokemon, source) {
            ...
            this.add(`c:|${(0, scripts_1.getName)((pokemon.illusion || pokemon).name)}|he do be searching for rocks tho`);
            return !!this.queue.willAct() && this.runEvent('StallMove', pokemon);
        },
        secondary: null,
        target: "self",
    ...
    dillydally: {
        ...
        basePower: 30,
        category: "Physical",
        shortDesc: "3 hits, 50% +1 stat/hit. Type=User 2nd type.",
        desc: "This move hits 3 times. For each successful hit, the user boosts a random stat, except Accuracy and Evasion, by 1 stage. The typing of this move is equal to the user's secondary type; it will instead use the user's primary type if the user lacks a secondary type.",
        name: "Dilly Dally",
        pp: 20,
        priority: 0,
        multihit: 3,
        ...
        onModifyType(move, pokemon) {
            ...
            if (type === "Bird" || type === undefined)
                type = "???";
            if (type === "Stellar")
                type = pokemon.getTypes()[pokemon.getTypes(false, true).length - 1];
            move.type = type;
        },
        secondary: {
            chance: 50,
    ...
    yourcripplinginterest: {
        ...
        onHit(target, source, move) {
            let success = false;
            if (!target.volatiles['substitute'] || move.infiltrates)
                success = !!target.trySetStatus('tox', source);
            const removeTarget = [
                'reflect', 'lightscreen', 'auroraveil', 'safeguard', 'mist', 'spikes', 'toxicspikes', 'stealthrock', 'stickyweb', 'gmaxsteelsurge',
            ];
            const removeAll = [
    ...
    buildingcharacter: {
        ...
        onHit(pokemon, source) {
            if (pokemon?.terastallized) {
                this.add(`c:|${(0, scripts_1.getName)((source.illusion || source).name)}|lol never do that ever again thanks`);
    ...
    virtualavatar: {
        ...
        onHit(target, source) {
            (0, scripts_1.changeSet)(this, target, random_teams_1.ssbSets['Vistar-Idol'], true);
            this.add(`c:|${(0, scripts_1.getName)((source.illusion || source).name)}|Finally, I'm making my debut`);
            if (source.volatiles['substitute'])
                return;
    ...
    perfectmimic: {
        ...
        condition: {
            ...
            onSourceAfterMove(target, source) {
                if (target === this.effectState.target || source !== this.effectState.target)
                    return;
                if (!target.hp || !this.effectState.move)
                    return;
    ...
    scorchingtruth: {
        ...
        basePower: 80,
        category: "Special",
        desc: "Damage is increased by 75% if this move is not very effective against the target.",
        shortDesc: "Deals 1.75x damage with resisted hits.",
    ...
    whiteout: {
        ...
        onHit(target, source) {
            ...
            this.add(`c:|${(0, scripts_1.getName)((source.illusion || source).name)}|A blank canvas.`);
        },
        secondary: null,
        target: "normal",
        type: "Ice",
    ...
    solarsummon: {
        ...
        basePower: 0,
        category: "Status",
        shortDesc: "Sunny Day, Substitute, +1 Sp. Def.",
        desc: "Raises the user's Special Defense by 1 stage. Sets sun, powering up Fire-type moves and weakening Water-type moves for 5 turns. The user takes 1/4 of its maximum HP, rounded down, and puts it into a substitute to take its place in battle. If one part of this move is already in effect, the other part will still be attempted.",
        name: "Solar Summon",
        gen: 9,
        pp: 5,
        priority: 0,
    ...
    dig: {
        ...
        condition: {
            ...
            onImmunity(type, pokemon) {
                if (type === 'sandstorm' || type === 'deserteddunes' || type === 'hail')
                    return false;
            },
    ...
    ingrain: {
        ...
        condition: {
            ...
            onDragOut(pokemon, source, move) {
                if (source && this.queue.willMove(source)?.moveid === 'protectoroftheskies')
                    return;
    ...
    rest: {
        inherit: true,
        onTry(source) {
            if (source.status === 'slp' || source.hasAbility(['comatose', 'mensiscage']))
                return false;
    ...
    stickyweb: {
        ...
        condition: {
            ...
            onSwitchIn(pokemon) {
                if (!pokemon.isGrounded() || pokemon.hasItem('heavydutyboots') || pokemon.hasAbility('eternalgenerator'))
                    return;
    ...
    supermetronome: {
        ...
        onHit(target, source, effect) {
            ...
            for (const move of this.dex.moves.all()) {
                if (move.realMove || move.id.includes('metronome'))
                    continue;
