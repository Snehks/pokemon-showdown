...
class DexFormats {
  ...
  load() {
    ...
    for (const [i, format] of Formats.entries()) {
      ...
      if (format.teraPreviewDefault === void 0)
  ...
  getRuleTable(format, depth = 1, repeals) {
    ...
    for (let ruleSpec of ruleSpecs) {
      ...
      if (ruleSpec.startsWith("!") && !ruleSpec.startsWith("!!")) {
        repeals || (repeals = /* @__PURE__ */ new Map());
    ...
    for (let ruleSpec of ruleSpecs) {
      ...
      if (subRuleTable.onChooseTeam) {
        if (ruleTable.onChooseTeam) {
          throw new Error(
            `"${format.name}" has conflicting team selection rules from "${ruleTable.onChooseTeam[1]}" and "${subRuleTable.onChooseTeam[1]}"`
          );
        }
