...
|error|${type} ${message}`);
  ...
  chooseTeam(data) {
    ...
    let positions = data ? data.split(data.includes(",") ? "," : "").map((datum) => parseInt(datum) - 1) : [...this.pokemon.keys()];
      ...
      switch (choiceType) {
        ...
        case "useitem": {
          ...
          if (targetRef.includes(":")) {
            ...
            const side = this.battle.getSide(sideId);
            targetPokemon = side.pokemon[parseInt(slotStr)];
          } else {
            const side = this.battle.getSide(targetRef.slice(0, 2));
