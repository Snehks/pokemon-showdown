...
class Species extends import_dex_data.BasicEffect {
  constructor(data) {
    ...
    this.isCosmeticForme = data.isCosmeticForme || void 0;
    this.tags = data.tags || [];
    this.unreleasedHidden = data.unreleasedHidden || false;
    this.maleOnlyHidden = !!data.maleOnlyHidden;
    this.maxHP = data.maxHP || void 0;
    this.isMega = this.forme.includes("Mega") || void 0;
    this.isPrimal = this.forme === "Primal" || void 0;
...
class Learnset {
  constructor(data, species) {
    ...
    const eventData = import_utils.Utils.deepClone(this.eventData);
    let update = false;
    if (eventData) {
      ...
      if (this.dex.data.FormatsData.hasOwnProperty(id)) {
        ...
        if (this.dex.data.Pokedex?.[id]?.isCosmeticForme) {
          ...
          species = new Species({
            ...
            // Forme always exists on cosmetic forme entries
            baseForme: "",
            otherFormes: null,
            cosmeticFormes: null
          });
    ...
    if (id && this.dex.data.Pokedex.hasOwnProperty(id)) {
      ...
      if (!species.tier && !species.doublesTier && !species.natDexTier && species.baseSpecies !== species.name) {
        if (species.baseSpecies === "Mimikyu") {
          ...
          species.doublesTier = this.dex.data.FormatsData[(0, import_dex_data.toID)(species.baseSpecies)].doublesTier || species.tier;
          species.natDexTier = this.dex.data.FormatsData[(0, import_dex_data.toID)(species.baseSpecies)].natDexTier || species.tier;
          ...
          species.doublesTier = this.dex.data.FormatsData[(0, import_dex_data.toID)(species.battleOnly)]?.doublesTier || species.tier;
          species.natDexTier = this.dex.data.FormatsData[(0, import_dex_data.toID)(species.battleOnly)]?.natDexTier || species.tier;
        } else {
