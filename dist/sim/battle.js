...
class Battle {
  ...
  singleEvent(eventid, effect, state, target, source, sourceEffect, relayVar, customCallback) {
    ...
    if (eventid === "SwitchIn" && effect.effectType === "Ability" && effect.flags["breakable"] && this.suppressingAbility(target)) {
  ...
  resolvePriority(h, callbackName) {
    ...
    if (handler.effectHolder && handler.effectHolder.getStat) {
      ...
      if (handler.effect.effectType === "Ability" && handler.effect.name === "Magic Bounce" && callbackName === "onAllyTryHitSide") {
  ...
  findEventHandlers(target, eventName, source) {
    ...
    const shouldBubbleDown = target instanceof import_side.Side;
  ...
  skillSwap(source, target) {
    ...
    source.abilityState = this.initEffectState({ id: (0, import_dex.toID)(source.ability), target: source });
    target.abilityState = this.initEffectState({ id: (0, import_dex.toID)(target.ability), target });
  ...
  runPickTeam() {
    ...
    if (this.ruleTable.pickedTeamSize) {
      ...
      for (const pokemon of this.getAllPokemon()) {
        const details = pokemon.details.replace(", shiny", "").replace(/(Zacian|Zamazenta)(?!-Crowned)/g, "$1-*").replace(/(Xerneas)(-[a-zA-Z?-]+)?/g, "$1-*");
  ...
  runAction(action) {
    ...
    switch (action.choice) {
      ...
      case "useitem": {
        const scripts = this.dex.data.Scripts.bagItems;
    ...
    for (const side of this.sides) {
      const team = side.pokemon.map((pokemon) => {
        ...
        if ((0, import_dex.toID)(set.species) === "zacian" && (0, import_dex.toID)(set.item) === "rustedsword" || (0, import_dex.toID)(set.species) === "zamazenta" && (0, import_dex.toID)(set.item) === "rustedshield") {
          ...
          const ironHeadIndex = set.moves.map(import_dex.toID).indexOf("ironhead");
          if (ironHeadIndex >= 0) {
