...
var Friends = __toESM(require("./friends"));
...
var import_chat_db = require("./chat-db");
...
class MessageContext {
  ...
  splitFormat(target, atLeastOneTarget, allowRules) {
    ...
    if (targets.length > 1) {
      const { dex: dex2, format: format2, isMatch } = this.extractFormat(targets[targets.length - 1].trim(), allowRules);
      if (isMatch) {
        targets.pop();
        return { dex: dex2, format: format2, targets };
      }
...
class CommandContext extends MessageContext {
  ...
  parse(msg, options = {}) {
    ...
    if (this.user.statusType === "idle" && this.message !== "/cmd rooms" && !["unaway", "unafk", "back"].includes(this.cmd)) {
      ...
      if (this.handler) {
        if (this.handler.disabled) {
          throw new Chat.ErrorMessage(
            `The command /${this.fullCmd.trim()} is temporarily unavailable due to technical difficulties. Please try again in a few hours.`
          );
        }
...
|error|`));
  ...
  checkBroadcast(overrideCooldown, suppressMessage) {
    ...
    if (this.user.locked && !(this.room?.roomid.startsWith("help-") || this.pmTarget?.can("lock"))) {
      throw new Chat.ErrorMessage([
        `You cannot broadcast this command's information while locked.`,
  ...
  checkCanPM(targetUser, user) {
    ...
    return Users.globalAuth.atLeast(user, setting) || friends.has(user.id);
  }
      ...
      for (const tag of tags) {
        ...
        if (tagName === "img") {
          ...
          if (!/width ?= ?(?:[0-9]+|"[0-9]+")/i.test(tagContent) || !/height ?= ?(?:[0-9]+|"[0-9]+")/i.test(tagContent)) {
            throw new Chat.ErrorMessage([
              `This image is missing a width/height attribute: <${tagContent}>`,
          ...
          if (srcMatch) {
            ...
            throw new Chat.ErrorMessage([
              `This image has a broken src attribute: <${tagContent}>`,
...
const Chat = new class {
  constructor() {
    ...
    this.destroyHandlers = [
      Artemis.destroy,
      Friends.destroy,
      () => void import_chat_db.pluginDatabase.destroy(),
     ...
     */
    this.database = import_chat_db.pluginDatabase;
  ...
  parseCommand(message, recursing = false) {
    ...
    let [cmd, target] = import_lib.Utils.splitFirst(message.slice(1), " ");
    cmd = cmd.toLowerCase();
  ...
  getDataMoveHTML(move, isChampions = false) {
...
Chat.normalize = import_lib.Utils.normalize;
...
function start(processCount) {
  if (Config.usesqlite) {
    import_chat_db.pluginDatabase.spawn(processCount["chatdb"] ?? 1);
...
setTimeout(() => Chat.loadPlugins(), 5e3);
