...
async function runActions(user, room, message, response) {
  ...
  if (recommended.length) {
    ...
    if (result !== false) {
      ...
      delete roomRecord?.users[user.id];
...
const chatfilter = function(message, user, room) {
  ...
  if (muteEntry) {
    if (Date.now() > muteEntry) {
      ...
      this.sendReply(
        `|c|~|/raw <div class="message-error">Your behavior in this battle has been automatically identified as breaking <a href="https://${Config.routes.root}/rules">Pokemon Showdown's global rules.</a> Repeated instances of misbehavior may incur harsher punishment.</div>`
      );
      return false;
    }
  }
  ...
  abusemonitor: {
    ...
    async userclear(target, room, user) {
      ...
      if (user.lastCommand !== `am userclear ${targetId}`) {
        ...
        throw new Chat.ErrorMessage([
          `Are you sure you want to clear abuse monitor database records for ${targetId}?`,
    ...
    reviews() {
      ...
      for (const userid in reviews) {
        ...
        for (const review of curReviews) {
          ...
          buf += `Battle: <a href="//${Config.routes.client}/${(0, import_helptickets.getBattleLinks)(review.room)[0]}">${review.room}</a><br />`;
