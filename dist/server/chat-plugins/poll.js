...
class Poll extends Rooms.MinorActivity {
  ...
  submit(user) {
    ...
    if (this.maxVotes && this.totalVotes >= this.maxVotes) {
      this.end(this.room);
      return this.room.add(`|c|~|/log The poll hit the max vote cap of ${this.maxVotes}, and has ended.`).update();
    }
    this.update();
    this.save();
  }
  ...
  generateVotes(user) {
    const iconText = this.isQuiz ? `<i class="fa fa-question"></i> ${this.room.tr`Quiz`}` : `<i class="fa fa-bar-chart"></i> ${this.room.tr`Poll`}`;
    ...
    if (this.multiPoll) {
      ...
      for (const [num, answer] of this.answers) {
        ...
        output += `<div style="margin-top: 5px"><button style="text-align: left; border: none; background: none; color: inherit;" value="/poll ${selected ? "de" : ""}select ${num}" name="send" title="${selected ? "Deselect" : "Select"} ${num}. ${import_lib.Utils.escapeHTML(answer.name)}">${selected ? "<strong>" : ""}${selected ? chosen : empty} ${num}. `;
      ...
      for (const [num, answer] of this.answers) {
        output += `<div style="margin-top: 5px"><button class="button" style="text-align: left" value="/poll vote ${num}" name="send" title="${this.room.tr`Vote for ${num}`}. ${import_lib.Utils.escapeHTML(answer.name)}">${num}.`;
  ...
  static generateResults(options, room, ended = false, choice = null) {
    const iconText = options.isQuiz ? `<i class="fa fa-question"></i> ${room.tr`Quiz`}` : `<i class="fa fa-bar-chart"></i> ${room.tr`Poll`}`;
    ...
    for (const [num, answer] of answers) {
      ...
      const answerMarkup = options.isQuiz ? `<span style="color:${answer.correct ? "green" : "red"};">${answer.correct ? "" : "<s>"}${this.getAnswerMarkup(answer, options.supportHTML)}${answer.correct ? "" : "</s>"}</span>` : this.getAnswerMarkup(answer, options.supportHTML);
...
const commands = {
  ...
  pollhelp() {
    this.sendReply(
      `|html|<details class="readmore"><summary>/poll allows rooms to run their own polls (limit 1 at a time).<br />Polls can be used as quiz questions, by putting <code>+</code> before correct answers.<br /><code>/poll create [question], [option1], [option2], [...]</code> - Creates a poll. Requires: % @ # ~</summary><code>/poll createmulti [question], [option1], [option2], [...]</code> - Creates a poll, allowing for multiple answers to be selected. Requires: % @ # ~<br /><code>/poll htmlcreate(multi) [question], [option1], [option2], [...]</code> - Creates a poll, with HTML allowed in the question and options. Requires: # ~<br /><code>/poll vote [number]</code> - Votes for option [number].<br /><code>/poll timer [minutes]</code> - Sets the poll to automatically end after [minutes]. Requires: % @ # ~.<br /><code>/poll results</code> - Shows the results of the poll without voting. NOTE: you can't go back and vote after using this.<br /><code>/poll display</code> - Displays the poll.<br /><code>/poll end</code> - Ends a poll and displays the results. Requires: % @ # ~.<br /><code>/poll queue [question], [option1], [option2], [...]</code> - Add a poll in queue. Requires: % @ # ~<br /><code>/poll deletequeue [number]</code> - Deletes poll at the corresponding queue slot (1 = next, 2 = the one after that, etc).<br /><code>/poll clearqueue</code> - Deletes the queue of polls. Requires: % @ # ~.<br /><code>/poll viewqueue</code> - View the queue of polls in the room. Requires: % @ # ~<br /><code>/poll maxvotes [number]</code> - Set the max poll votes to the given [number]. Requires: % @ # ~<br /></details>`
    );
  }
};
const pages = {
  pollqueue(args, user) {
    ...
    for (const [i, poll] of queue.entries()) {
      const number = i + 1;
      const button = `<strong>${this.tr`#${number} in queue`} </strong><button class="button" name="send" value="/msgroom ${room.roomid},/poll deletequeue ${i + 1}">(${this.tr`delete`})</button>`;
      buf += `<hr />`;
  ...
  for (const room of Rooms.rooms.values()) {
    ...
    if (room.settings.minorActivity) {
      ...
      if (typeof room.settings.minorActivity.activityNumber !== "number") {
        room.settings.minorActivity.activityNumber = room.settings.minorActivity.pollNumber || // @ts-expect-error old format
