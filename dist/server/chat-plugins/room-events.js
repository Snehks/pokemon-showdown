...
const commands = {
  ...
  roomevents: {
    ...
    sortby(target, room, user) {
      ...
      const sortable = Object.values(room.settings.events).filter((event) => "eventName" in event);
      ...
      switch (columnName) {
        ...
        case "eventdate":
          sortable.sort(
            (a, b) => toID(a.date) < toID(b.date) ? -multiplier : toID(b.date) < toID(a.date) ? multiplier : 0
          );
          break;
        case "desc":
        case "description":
        case "eventdescription":
          sortable.sort(
            (a, b) => toID(a.desc) < toID(b.desc) ? -multiplier : toID(b.desc) < toID(a.desc) ? multiplier : 0
          );
          break;
        case "eventname":
        case "name":
          sortable.sort(
            (a, b) => toID(a.eventName) < toID(b.eventName) ? -multiplier : toID(b.eventName) < toID(a.eventName) ? multiplier : 0
          );
          break;
        default:
