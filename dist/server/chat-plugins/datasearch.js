...
const supportedDexsearchRules = Object.assign(/* @__PURE__ */ Object.create(null), {
...
const commands = {
  ...
  async dexsearch(target, room, user, connection, cmd2, message) {
    ...
    if (target.length > 300)
      throw new import_chat.Chat.ErrorMessage("Dexsearch queries may not be longer than 300 characters.");
    ...
    if (cmd2 === "nds" || defaultFormat.format && Dex.formats.getRuleTable(defaultFormat.format).has("natdexmod")) {
      target += ", natdex";
    }
  ...
  dexsearchhelp() {
    this.sendReplyBox(
      `<details class="readmore"><summary><code>/dexsearch [parameter], [parameter], [parameter], ...</code>: searches for Pok\xE9mon that fulfill the selected criteria.<br/>Search categories are: type, tier, color, moves, ability, gen, resists, weak, recovery, zrecovery, priority, stat, weight, height, egg group, pivot and restricted.<br/>Valid colors are: green, red, blue, white, brown, yellow, purple, pink, gray and black.<br/>Valid tiers are: AG/Uber/OU/UUBL/UU/RUBL/RU/NUBL/NU/PUBL/PU/ZUBL/ZU/NFE/LC/CAP/CAP NFE/CAP LC.<br/>Valid doubles tiers are: DUber/DOU/DBL/DUU/DNU.</summary>Types can be searched for by either having the type precede <code>type</code> or just using the type itself as a parameter; e.g., both <code>fire type</code> and <code>fire</code> show all Fire types; however, using <code>psychic</code> as a parameter will show all Pok\xE9mon that learn the move Psychic and not Psychic types.<br/><code>resists</code> followed by a type or move will show Pok\xE9mon that resist that typing or move (e.g. <code>resists normal</code>).<br/><code>weak</code> followed by a type or move will show Pok\xE9mon that are weak to that typing or move (e.g. <code>weak fire</code>).<br/><code>asc</code> or <code>desc</code> following a stat will show the Pok\xE9mon in ascending or descending order of that stat respectively (e.g. <code>speed asc</code>). You can use <code>tier</code> and <code>dtier</code> to sort by singles and doubles tiers, respectively.<br/>Inequality ranges use the characters <code>>=</code> for <code>\u2265</code> and <code><=</code> for <code>\u2264</code>; e.g., <code>hp <= 95</code> searches all Pok\xE9mon with HP less than or equal to 95; <code>tier <= uu</code> searches all Pok\xE9mon in singles tiers lower than UU.<br/>Parameters can be excluded through the use of <code>!</code>; e.g., <code>!water type</code> excludes all Water types.<br/>The parameter <code>mega</code> can be added to search for Mega Evolutions only, the parameter <code>gmax</code> can be added to search for Pok\xE9mon capable of Gigantamaxing only, and the parameter <code>Fully Evolved</code> (or <code>FE</code>) can be added to search for fully-evolved Pok\xE9mon.<br/><code>Alola</code>, <code>Galar</code>, <code>Therian</code>, <code>Totem</code>, or <code>Primal</code> can be used as parameters to search for those formes.<br/>Parameters separated with <code>|</code> will be searched as alternatives for each other; e.g., <code>trick | switcheroo</code> searches for all Pok\xE9mon that learn either Trick or Switcheroo.<br/>You can search for info in a specific generation by appending the generation to ds or by using the <code>maxgen</code> keyword; e.g. <code>/ds1 normal</code> or <code>/ds normal, maxgen1</code> searches for all Pok\xE9mon that were Normal type in Generation I.<br/>You can search for info in a specific mod by using <code>mod=[mod name]</code>; e.g. <code>/nds mod=gen9ssb, wonder guard</code>. All valid mod names are: <code>${dexesHelpMods}</code><br/>You can search for info in a specific rule defined metagame by using <code>rule=[rule name]</code>; e.g. <code>/nds rule=alphabetcupmovelegality, v-create</code>. All supported rule names are: <code>${dexsearchHelpRules}</code><br/>By default, <code>/dexsearch</code> will search only Pok\xE9mon obtainable in the current generation. Add the parameter <code>unreleased</code> to include unreleased Pok\xE9mon. Add the parameter <code>natdex</code> (or use the command <code>/nds</code>) to include all past Pok\xE9mon.<br/>Searching for a Pok\xE9mon with both egg group and type parameters can be differentiated by adding the suffix <code>group</code> onto the egg group parameter; e.g., searching for <code>grass, grass group</code> will show all Grass types in the Grass egg group.<br/>The parameter <code>monotype</code> will only show Pok\xE9mon that are single-typed.<br/>The order of the parameters does not matter.<br/>`
    );
  },
  rollmove: "randommove",
  randmove: "randommove",
  ...
  learnhelp: [
    ...
    `/learnlc displays how the Pok\xE9mon can learn the given moves at level 5, if it can at all.`,
...
function getRule(target) {
  ...
  return { splitTarget: arr, usedRules: ruleTerms.map(
    (x) => x.toLowerCase().replace(/[^a-z0-9=]+/g, "").split("rule=")[1]
  ), count };
}
function prepareDexsearchValidator(usedMod, rules, nationalSearch) {
  ...
  for (const rule of rules) {
    if (!ruleTable.has(toID(rule.name)))
      additionalRules.push(toID(rule.name));
  }
  if (nationalSearch && !ruleTable.has("natdexmod"))
    additionalRules.push("natdexmod");
...
function runDexsearch(target, cmd2, message, isTest) {
  ...
  const singlesTiersValues = Object.assign(/* @__PURE__ */ Object.create(null), {
    AG: 14,
    Uber: 13,
    OU: 12,
    CAP: 12,
  ...
  const doublesTiersValues = Object.assign(/* @__PURE__ */ Object.create(null), {
    DUber: 4,
    DOU: 3,
    DBL: 2,
    DUU: 1,
  ...
  const validParameter = (cat, param, isNotSearch, input) => {
    ...
    for (const group of searches) {
      ...
      if (g === void 0)
        continue;
      if (tierTraits.includes(cat) && tierInequalitySearch)
        continue;
      if (cat === "stats") {
        const inequality = param.split(",");
        const result = validStatInequality(
          group["stats"],
          inequality[0],
          inequality[1],
          inequality[2],
        ...
        if (uniqueTraits.includes(cat)) {
          for (const currentParam in g) {
            if (g[currentParam] !== isNotSearch && !isNotSearch)
              return `A Pok&eacute;mon cannot have multiple ${cat}.`;
          }
        }
        continue;
      }
  ...
  const validStatInequality = (g, statKey, direction, compareTo, value, input) => {
    const gValue = g[statKey]?.[direction]?.[compareTo];
    const swapValue = g[compareTo]?.[direction]?.[statKey];
    if (direction === "equal") {
      ...
      const inclusiveGreater = gEquality && gEquality === greater;
      const inclusiveLess = gEquality && gEquality === less;
      const gInclusiveIneq = greater && inclusiveGreater || less && inclusiveLess;
      if (gEquality && !(input.search(/([><]{1}=)/) >= 0 || gInclusiveIneq)) {
        ...
        if (greater && (inclusiveGreater && value < +greater || !inclusiveGreater && value <= +greater) || less && (inclusiveLess && value > +less || !inclusiveLess && value >= +less)) {
      ...
      const inverseValue = g[statKey]?.[inverseDirection]?.[compareTo];
      const inverseSwapValue = g[compareTo]?.[inverseDirection]?.[statKey];
      const gEquality = g[statKey]?.["equal"]?.[compareTo] || g[compareTo]?.["equal"]?.[statKey];
      ...
      if (gValue || inverseSwapValue) {
        ...
        if (direction === "greater" && (checkEquality && result > 0 || !checkEquality && result >= 0) || direction === "less" && (checkEquality && result < 0 || !checkEquality && result <= 0)) {
          return `The search '${input}' creates an invalid range.`;
        }
      } else if (compareTo !== "numeric" && (!checkEquality && (swapValue || inverseValue) || !input.includes("=") && gEquality)) {
        return `The search '${input}' creates an invalid range.`;
      }
    }
    return false;
  ...
  for (const andGroup of splitTarget) {
    ...
    for (const parameter of parameters) {
      ...
      const tierInequality = [];
      if (target.startsWith("tier")) {
        if (isNotSearch)
          return { error: "You cannot use the negation symbol '!' with inequality tier searches." };
      ...
      if (["restricted legendary", "restrictedlegendary", "restricted"].includes(target)) {
        if (restrictedSearch === isNotSearch)
          return { error: "A search cannot include and exclude 'restricted legendary'." };
        if (parameters.length > 1)
          return { error: "The parameter 'restricted legendary' cannot have alternative parameters." };
      ...
      if (inequality >= 0) {
        ...
        if (!isNaN(parseFloat(targetParts[0]))) {
          ...
          if (inequalityString.startsWith(">"))
            directions.push("greater");
          if (statKey in allStatAliases)
            statKey = allStatAliases[statKey];
          if (compareType in allStatAliases)
            compareType = allStatAliases[compareType];
        ...
        if (statKey in allStatAliases)
          statKey = allStatAliases[statKey];
        if (!allStats.includes(statKey))
          return { error: `'${target}' contained an invalid stat.` };
        if (typeof value === "number" && value <= 0)
          return { error: `Specify a positive value for numeric comparison.` };
        if (!orGroup.stats[statKey])
          orGroup.stats[statKey] = /* @__PURE__ */ Object.create(null);
        for (const direction of directions) {
          if (!orGroup.stats[statKey][direction])
            orGroup.stats[statKey][direction] = /* @__PURE__ */ Object.create(null);
  ...
  if (showAll && searches.length === 0 && singleTypeSearch === null && megaSearch === null && gmaxSearch === null && fullyEvolvedSearch === null && restrictedSearch === null && sort === null) {
    return {
  ...
  for (const species of mod.species.all()) {
    ...
    for (const rule of rules) {
      if (!ruleResult)
        break;
      if (!supportedDexsearchRules["banlist"].includes(toID(rule.name)))
        continue;
      if (!validator)
        validator = prepareDexsearchValidator(usedMod, rules, nationalSearch);
      ruleResult = !rule.onValidateSet?.call(
        validator,
        { name: species.name, species: species.id },
        validator.format,
        {},
        {}
      );
    }
    if (species.gen <= mod.gen && (nationalSearch && species.natDexTier !== "Illegal" || (species.tier !== "Unreleased" || unreleasedSearch) && species.tier !== "Illegal") && (!species.tier.startsWith("CAP") || capSearch) && megaSearchResult && gmaxSearchResult && fullyEvolvedSearchResult && restrictedSearchResult && ruleResult) {
      let newSpecies = species;
      for (const rule of rules) {
        newSpecies = rule?.onModifySpecies?.call(
          { dex: mod, clampIntRange: import_lib.Utils.clampIntRange, toID },
          newSpecies
        ) || newSpecies;
      }
  ...
  for (const alts of searches) {
    ...
    for (const mon in dex) {
      let retrieveStat2 = function(species, stat2) {
        let monStat = 0;
        if (stat2 === "bst") {
          monStat = species.bst;
          ...
          monStat = species.baseStats[stat2];
        }
        return monStat;
      };
      var retrieveStat = retrieveStat2;
      let matched = false;
      ...
      for (const stat2 in alts.stats) {
        const monStat = retrieveStat2(dex[mon], stat2);
        for (const direction in alts.stats[stat2]) {
          for (const comparisonStat in alts.stats[stat2][direction]) {
            const checkStat = alts.stats[stat2][direction][comparisonStat];
            if (!checkStat)
              continue;
            const compareTo = typeof checkStat === "number" ? checkStat : retrieveStat2(dex[mon], comparisonStat);
            if (direction === "less" && monStat < compareTo || direction === "greater" && monStat > compareTo || direction === "equal" && monStat === compareTo) {
              matched = true;
              break;
            }
          }
      ...
      if (validator) {
        for (const move of altsMoves) {
          ...
          for (const rule of rules) {
            ...
            if (matchRule === !isNotSearch)
              break;
          }
          const matchNormally = !validator.checkCanLearn(move, dex[mon], pokemonSource) === !isNotSearch;
          if (!isNotSearch && (matchNormally || numMoveValidationRules > 0 && matchRule) || isNotSearch && matchNormally && (numMoveValidationRules === 0 || matchRule)) {
            matched = true;
            break;
          }
  ...
  for (const mon of Object.values(dex).sort()) {
    if (singleTypeSearch !== null && mon.types.length === 1 !== singleTypeSearch)
      continue;
    const isRegionalForm = (["Alola", "Galar", "Hisui"].includes(mon.forme) || mon.forme.startsWith("Paldea")) && mon.baseSpecies !== "Pikachu";
    const maskForm = mon.baseSpecies === "Ogerpon" && !mon.forme.endsWith("Tera");
    const allowGmax = gmaxSearch || tierSearch;
    if (!isRegionalForm && !maskForm && mon.baseSpecies && results.includes(mod.species.get(mon.baseSpecies)) && getSortValue(mon) === getSortValue(mod.species.get(mon.baseSpecies)))
      continue;
    const teraFormeChangesFrom = mon.forme.endsWith("Tera") ? !Array.isArray(mon.battleOnly) ? mon.battleOnly : null : null;
    if (teraFormeChangesFrom && results.includes(mod.species.get(teraFormeChangesFrom)) && getSortValue(mon) === getSortValue(mod.species.get(teraFormeChangesFrom)))
      continue;
    if (mon.isNonstandard === "Gigantamax" && !allowGmax)
      continue;
    results.push(mon);
  }
  ...
  let resultsStr = message === "" ? message : `<span class="gray">${import_lib.Utils.escapeHTML(message)}:</span><br/>`;
  if (results.length > 1) {
    let mapPokemonResults2 = function(inputArr) {
      return inputArr.map(
    ...
    var mapPokemonResults = mapPokemonResults2;
    results.sort();
    if (sort) {
      const direction = sort.slice(-1);
      import_lib.Utils.sortBy(results, (species) => getSortValue(species) * (direction === "+" ? 1 : -1));
    }
    if (results.length > MAX_RANDOM_RESULTS) {
      resultsStr = formatCollapsible(message, showAll, results, mapPokemonResults2);
    } else {
      resultsStr += mapPokemonResults2(results);
    }
  ...
  if (isTest)
    return { results: results.map((species) => species.name), reply: resultsStr };
  return { reply: resultsStr };
}
function formatCollapsible(message, showall, results, mapfunc) {
  ...
  return `<details class="details"${expand ? " open" : ""}><summary><span class="gray">${import_lib.Utils.escapeHTML(header)}</span><br/><span class="details-preview">${resultsSummary}</span></summary>${resultsHidden}</details>`;
}
function runMovesearch(target, cmd2, message, isTest) {
  const searches = [];
    ...
    for (const parameter of parameters) {
      ...
      if (allVolatileStatus.includes(target)) {
        ...
        for (const trappingType of ["partiallytrapped", "trapped"]) {
          if (!orGroup.volatileStatus[trappingType])
            orGroup.volatileStatus[trappingType] = !isNotSearch;
        }
        continue;
      }
  ...
  for (const alts of searches) {
    ...
    for (const moveid in dex) {
      ...
      for (const searchStatus in alts.volatileStatus) {
        let canStatus = !!(move.secondary && move.secondary.volatileStatus === searchStatus || move.secondaries?.some((entry) => entry.volatileStatus === searchStatus) || move.volatileStatus === searchStatus || "trapped" === searchStatus && (move.onHit && /\b(?:target|pokemon)\.addVolatile\("trapped",/.test(move.onHit.toString()) || move.secondary?.onHit && /\b(?:target|pokemon)\.addVolatile\("trapped",/.test(move.secondary.onHit.toString()) || move.self?.onHit && /\b(?:target|pokemon)\.addVolatile\("trapped",/.test(move.self.onHit.toString())));
  ...
  if (targetMons.length) {
    ...
    resultsStr += message === "" ? message : `<span class="gray">${import_lib.Utils.escapeHTML(message)}:</span><br />`;
  }
  if (randomOutput && randomOutput < results.length) {
    results = import_lib.Utils.shuffle(results).slice(0, randomOutput);
  }
  if (results.length > 1) {
    let mapMoveResults2 = function(inputArray) {
    ...
    var mapMoveResults = mapMoveResults2;
    results.sort();
    if (sort) {
    ...
    if (results.length > MAX_RANDOM_RESULTS) {
      resultsStr = formatCollapsible(message, showAll, results, mapMoveResults2);
    } else {
      resultsStr += mapMoveResults2(results);
    }
  } else if (results.length === 1) {
    return { dt: `${results[0]}${usedMod ? `,${usedMod}` : ""}` };
  } else {
...
function runItemsearch(target, cmd2, message) {
  let showAll = false;
  let maxGen = 0;
  let gen = 0;
  let randomOutput = 0;
  ...
  function mapItemResults(inputArr) {
    return inputArr.map(
...
function runAbilitysearch(target, cmd2, message) {
  let showAll = false;
  let maxGen = 0;
  let gen = 0;
  let randomOutput = 0;
  ...
  function mapAbilityResults(inputArr) {
    return inputArr.map(
...
function runLearn(target, cmd2, formatid) {
  ...
  if (!problems.length) {
    const sourceNames = {
      ...
      "8V": "Pok&eacute;mon Home transfer from LGPE",
      E: "",
      S: "event",
      D: "dream world",
      X: "traded-back ",
...
function runRandtype(target, cmd2, message) {
  const icon = {};
...
const PM = new import_lib.ProcessManager.QueryProcessManager("dexsearch", module, (query) => {
  try {
...
const testables = {
  runAbilitysearch: (target, cmd2, message) => runAbilitysearch(target, cmd2, message),
  runDexsearch: (target, cmd2, message) => runDexsearch(target, cmd2, message, true),
  runMovesearch: (target, cmd2, message) => runMovesearch(target, cmd2, message, true)
};
