...
var laddertours_exports = {};
__export(laddertours_exports, {
  LadderTracker: () => LadderTracker,
  commands: () => commands,
  handlers: () => handlers,
  pages: () => pages,
  trackers: () => trackers
});
module.exports = __toCommonJS(laddertours_exports);
var import_lib = require("../../lib");
var import_username_prefixes = require("./username-prefixes");
const MINUTE = 6e4;
...
class LadderTracker {
  ...
  async getLeaderboard(display) {
    const url = `https://pokemonshowdown.com/ladder/${this.format}.json?prefix=${this.prefix}`;
    const leaderboard = [];
    let response;
    try {
      response = await (0, import_lib.Net)(url).get().then(JSON.parse);
    } catch (e) {
  ...
  static save() {
    (0, import_lib.FS)("config/chat-plugins/laddertrackers.json").writeUpdate(() => {
      const out = {};
  ...
  styleLeaderboard(leaderboard, final) {
    const diffs = this.leaderboard.last && !final ? this.getDiffs(this.leaderboard.last, leaderboard) : /* @__PURE__ */ new Map();
    let buf = "<center>";
    if (final) {
      buf += `<h1 style="margin-bottom: 0.2em">Final Leaderboard - ${this.prefix}</h1><div style="margin-bottom: 1em"><small><em>${final}</em></small></div>`;
    }
    buf += `<div class="ladder" style="max-height: 250px; overflow-y: auto"><table><tr><th></th><th>Name</th><th><abbr title="Elo rating">Elo</abbr></th><th><abbr title="user's percentage chance of winning a random battle (aka GLIXARE)">GXE</abbr></th><th><abbr title="Glicko-1 rating system: rating\xB1deviation (provisional if deviation>100)">Glicko-1</abbr></th></tr>`;
    for (const [i, p] of leaderboard.entries()) {
      ...
      let rank = `${i + 1}`;
      if (diff) {
        const symbol = diff[2] < diff[3] ? '<span style="color: #F00">\u25BC</span>' : '<span style="color: #008000">\u25B2</span>';
        rank = `${symbol}${rank}`;
      }
      buf += `<tr><td style="text-align: right"><a href='${link}' class="subtle">${rank}</a></td>` + import_lib.Utils.html`<td><username class="username">${p.name}</username></td>` + `<td><strong>${p.elo}</strong></td><td>${p.gxe.toFixed(1)}%</td><td>${p.glicko} \xB1 ${p.glickodev}</td></tr>`;
    }
    buf += "</table></div></center>";
    return buf;
  }
  getDiffs(last, current, num) {
  ...
  togglePrefix(oldPrefix) {
    if (!this.room.settings.isPrivate) {
      ...
      try {
        import_username_prefixes.prefixManager.addPrefix(this.prefix, "privacy");
      } catch {
      }
    }
  }
  ...
  stop() {
    if (this.started) {
      ...
      this.started = void 0;
      this.leaderboard.current = void 0;
      this.leaderboard.last = void 0;
    }
  }
}
const trackers = {};
try {
  const data = JSON.parse((0, import_lib.FS)("config/chat-plugins/laddertrackers.json").readIfExistsSync() || "{}");
...
const commands = {
  ...
  laddertrack: {
    ...
    ""(target, room, user, connection, cmd) {
      ...
      if (rawCutoff && (!cutoff || cutoff < 1e3)) {
      ...
      trackers[room.roomid] = new LadderTracker(room, config);
...
//# sourceMappingURL=laddertours.js.map
