...
class Auction extends Rooms.SimpleRoomGame {
  ...
  generatePriceList() {
    ...
    for (const team of this.teams.values()) {
      ...
      if (this.ended)
        smogonExport += `[SPOILER="${team.name}"]${smogonTable2}[/SPOILER]`;
    }
    let table = `<table>`;
  ...
  sendBidInfo() {
    ...
    if (this.type === "auction") {
      ...
      buf += import_lib.Utils.html`Top bidder: <b>${this.highestBidder.name}</b> `;
      buf += `Managers: ${this.highestBidder.getManagers().map((m) => `<username class="username">${import_lib.Utils.escapeHTML(m)}</username>`).join(" ")}<br/>`;
    }
