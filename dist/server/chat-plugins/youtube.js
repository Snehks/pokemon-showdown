...
__export(youtube_exports, {
  ...
  destroy: () => destroy,
  pages: () => pages,
  searchDataCache: () => searchDataCache,
...
function loadData() {
  const raw = JSON.parse((0, import_lib.FS)(STORAGE_PATH).readIfExistsSync() || "{}");
  if (!(raw.channels && raw.categories)) {
    const data = {};
    data.channels = raw;
    data.categories = [];
    (0, import_lib.FS)(STORAGE_PATH).writeUpdate(() => JSON.stringify(data));
    return data;
  }
  return raw;
}
...
class YoutubeInterface {
  ...
  async getChannelData(link, username) {
    ...
    const raw = await (0, import_lib.Net)(`${ROOT}channels`).get({
  ...
  randChannel(cat) {
    ...
    if (cat) {
      ...
      channels = channels.filter((id2) => {
        const channel = this.data.channels[id2];
        return channel.category && toID(channel.category) === cat;
      });
    }
    const id = import_lib.Utils.shuffle(channels)[0];
...
const Twitch = new class {
  ...
  async getChannel(channel) {
    ...
    let res;
    try {
      res = await (0, import_lib.Net)(`https://api.twitch.tv/helix/search/channels`).get({
        headers: {
...
const _GroupWatch = class extends Rooms.SimpleRoomGame {
  ...
  hints() {
    ...
    if (this.started && this.info.groupwatchType === "youtube") {
      ...
      hints.push(`Video is currently at ${Chat.toDurationString(diff)} (${Math.floor(diff / 1e3)} seconds)`);
    }
    return hints;
  }
  getStatsDisplay() {
  ...
  getVideoDisplay() {
    if (this.info.groupwatchType === "twitch") {
      let buf2 = `<p style="background: #6441a5; padding: 5px;border-radius:8px;color:white;font-weight:bold;text-align:center;">`;
      buf2 += `<twitch src="${this.info.url}" width="600" height="330" />`;
      return buf2;
    }
    ...
    if (this.started) {
      const diff = Date.now() - this.started;
      addendum = `&start=${Math.floor(diff / 1e3)}`;
    }
  ...
  display() {
    return import_lib.Utils.html`<center><div class="pad"><strong>${this.room.title} Groupwatch - ${this.title}</strong><br /><br />` + `<p>${this.started ? this.getVideoDisplay() : ""}</p><hr /><p>${this.started ? this.getStatsDisplay() : "<i>Waiting to start the video...</i>"}</p><p>${this.hints().join("<br />")}</p>`;
  }
  update() {
...
const commands = {
  ...
  youtube: {
    ...
    setcategory(target, room, user) {
      ...
      this.checkCan("mute", null, room);
      target = target.trim();
      const [category, id] = import_lib.Utils.splitFirst(target, ",").map((item) => item.trim());
  ...
  groupwatch: {
    async create(target, room, user) {
      ...
      room.add(
        `|uhtml|${game.id}|<button class="button" name="send" value="/j view-groupwatch-${game.id}">Join the ongoing group watch!</button>`
      );
