...
var thing_of_the_day_exports = {};
__export(thing_of_the_day_exports, {
  commands: () => commands,
  destroy: () => destroy,
  handlers: () => handlers,
  otdCommands: () => otdCommands,
  otdData: () => otdData,
  otds: () => otds,
  pages: () => pages,
  prenoms: () => prenoms,
...
module.exports = __toCommonJS(thing_of_the_day_exports);
...
const prenoms = JSON.parse((0, import_lib.FS)(PRENOMS_FILE).readIfExistsSync() || "{}");
const otdData = JSON.parse((0, import_lib.FS)(DATA_FILE).readIfExistsSync() || "{}");
const otds = /* @__PURE__ */ new Map();
...
class OtdHandler {
  ...
  generateWinnerList(context) {
    ...
    for (let i = 0; i < this.keys.length; i++) {
      if (i === 0 || ["song", "event", "link", "tagline", "sport", "country"].includes(this.keys[i]) && !(this.keys[i] === "link" && this.keys.includes("song"))) {
...
const commands = {
  otd: {
    create(target, room, user) {
      ...
      const count = [...otds.values()].filter((otd2) => otd2.room.roomid === room.roomid).length;
      if (count > 3) {
