...
var smogtours_exports = {};
__export(smogtours_exports, {
  commands: () => commands,
  pages: () => pages,
  renderPageChooser: () => renderPageChooser,
  start: () => start,
  tours: () => tours
});
module.exports = __toCommonJS(smogtours_exports);
...
function saveTours() {
  (0, import_lib.FS)("config/chat-plugins/smogtours.json").writeUpdate(() => JSON.stringify(tours));
}
...
const pages = {
  tournaments: {
    all(query, user) {
      ...
      buf += Object.keys(tours).map((catID) => `<li><a class="blocklink" target="replace" href="/view-tournaments-section-${catID}"><i class="fa fa-play"></i> <strong>${tours[catID].title}</strong></a></li>`).join(" ");
      buf += `</ul>`;
      buf += `<hr />`;
    ...
    view(query, user) {
      ...
      this.title += `${tour.title}`.replace(/&quot;/g, '"').replace(/&gt;/g, ">").replace(/&lt;/g, "<").replace(/&amp;/g, "&");
