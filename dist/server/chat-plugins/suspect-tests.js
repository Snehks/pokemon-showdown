...
__export(suspect_tests_exports, {
  commands: () => commands,
  saveSuspectTests: () => saveSuspectTests,
...
let suspectTests = JSON.parse((0, import_fs.FS)(SUSPECTS_FILE).readIfExistsSync() || "{}");
...
const commands = {
  ...
  suspects: {
    ""(target, room, user) {
      ...
      for (const test of Object.values(suspects)) {
        buffer += "<br />";
        buffer += `${import_lib.Utils.escapeHTML(test.tier)}: <a href="${test.url}">${import_lib.Utils.escapeHTML(test.suspect)}</a> (${test.date})`;
      }
    ...
    async verify(target, room, user) {
      ...
      this.sendReply(
        out.result ? `You have successfully verified for the ${formatid} suspect test.` : `You could not verify for the ${formatid} suspect test, as you do not meet the requirements.`
      );
    },
    help() {
      return this.parse("/help suspects");
  ...
  suspectshelp() {
    this.sendReplyBox(
      `Commands to manage suspect tests:<br /><code>/suspects</code>: displays currently running suspect tests.<br /><code>/suspects add [tier], [suspect], [date], [...reqs]</code>: adds a suspect test. Date in the format MM/DD. Reqs in the format [key]=[value], where valid keys are 'coil', 'elo', and 'gxe', delimited by commas. At least one is required. <br />(note that if you are using COIL, you must set a B value independently with <code>/suspects setcoil</code>). Requires: ~<br /><code>/suspects remove [tier]</code>: deletes a suspect test. Requires: ~<br /><code>/suspects whitelist [username]</code>: allows [username] to add suspect tests. Requires: ~<br /><code>/suspects unwhitelist [username]</code>: disallows [username] from adding suspect tests. Requires: ~<br /><code>/suspects setbvalue OR /suspects sbv [formatid], [B value]</code>: Activate COIL ranking for the given [formatid] with the given [B value].Requires: suspect whitelist ~`
    );
  }
};
