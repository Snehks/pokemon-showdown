...
const TeamsHandler = new class {
  constructor() {
    this.readyPromise = teamsDB ? (async () => {
      try {
        ...
        await teamsDB.query((0, import_database.SQL)((0, import_lib.FS)(`databases/schemas/teams.sql`).readSync()));
      }
    })() : null;
  }
  destroy() {
  ...
  async search(search, user, count = 10, includePrivate = false) {
    ...
    if (search.format) {
      where.push(where.length ? import_database.SQL` AND ` : import_database.SQL`WHERE `);
      where.push(import_database.SQL`format = ${toID(search.format)}`);
    }
    if (search.owner) {
      where.push(where.length ? import_database.SQL` AND ` : import_database.SQL`WHERE `);
      where.push(import_database.SQL`ownerid = ${toID(search.owner)}`);
    }
    if (search.gen) {
      where.push(where.length ? import_database.SQL` AND ` : import_database.SQL`WHERE `);
      where.push(import_database.SQL`format LIKE ${`gen${search.gen}%`}`);
    }
    if (!includePrivate) {
      where.push(where.length ? import_database.SQL` AND ` : import_database.SQL`WHERE `);
      where.push(import_database.SQL`private IS NULL`);
    }
...
- Your Unowns spell out a banned word: ${unownWord.toUpperCase()}`
  ...
  updateViews(teamid) {
    return teamsTable.updateOne(import_database.SQL`views = views + 1`)`WHERE teamid = ${teamid}`;
  }
  list(userid, count, publicOnly = false) {
    const publicOnlyQuery = publicOnly ? import_database.SQL`AND private IS NULL ` : import_database.SQL``;
...
const commands = {
  teams: {
    ...
    async save(target, room, user, connection, cmd) {
      ...
      const targets = import_lib.Utils.splitFirst(target, ",", isEdit ? 4 : 3).map((x) => x.trim());
      const rawTeamID = isEdit ? targets.shift() : void 0;
      const [teamName, formatid, isPrivate, rawTeam] = targets;
      const teamID = isEdit ? Number(rawTeamID) : void 0;
...
const pages = {
  ...
  teams: {
    ...
    async browse(query, user, connection) {
      ...
      switch (sorter) {
        case "views":
          order = import_database.SQL` ORDER BY views DESC `;
          name = "most viewed";
          break;
        case "latest":
          order = import_database.SQL` ORDER BY date DESC`;
          break;
        default:
...
function start() {
  if (Config.usepostgres && Config.usepostgresteams) {
    teamsDB = new import_database.PGDatabase(Config.usepostgres);
