...
class Tournament extends Rooms.RoomGame {
  ...
  addUser(user, output) {
    ...
    if ((this.room.settings.tournaments?.autoconfirmedOnly || this.autoconfirmedOnly) && !user.autoconfirmed && !user.trusted) {
  ...
  replaceUser(user, replacementUser, output) {
    ...
    const challengePlayer = player.pendingChallenge && (player.pendingChallenge.from || player.pendingChallenge.to);
    if (challengePlayer) {
...
function createTournament(room, formatId, generator, playerCap, isRated, generatorMod, name, output) {
  ...
  const tour = room.game = new Tournament(
    room,
    format,
    createTournamentGenerator(generator, generatorMod),
    playerCap,
    isRated,
    name
  );
...
const commands = {
  ...
  tournament: {
    ""(target, room, user) {
      ...
      const started = import_lib.Utils.sortBy(roomTours.filter((tour) => tour.isTournamentStarted), (tour) => tour.room.roomid);
      const signups = import_lib.Utils.sortBy(roomTours.filter((tour) => !tour.isTournamentStarted), (tour) => tour.room.roomid);
      function renderLink(tour) {
        const name = Dex.formats.get(tour.name).exists ? Dex.formats.get(tour.name).name : tour.name;
        const icon = tour.generator.name === "Round Robin" ? '<i class="fa fa-th"></i>' : tour.generator.name === "Single Elimination" ? '<i class="fa fa-share-alt"></i>' : '<i class="fa fa-share-alt"></i><i class="fa fa-share-alt"></i>';
        const plural = tour.players.length !== 1 ? "s" : "";
        return `<li><a href="/${tour.room.roomid}" class="blocklink">&laquo;<strong>${tour.room.roomid}</strong>&raquo;<small style="float:right">(${tour.players.length} player${plural})</small><br />${icon} <small>${import_lib.Utils.escapeHTML(name)} ${tour.generator.name}</small></a></li>`;
      }
      let buf = ``;
      if (signups.length) {
    ...
    formats(target, room, user) {
      ...
      for (const format of Dex.formats.all()) {
        ...
        if (format.section !== section) {
          ...
          buf += import_lib.Utils.html`<br /><strong>${section}:</strong><br />&bull; ${name}`;
        } else {
          buf += import_lib.Utils.html`<br />&bull; ${name}`;
        }
      }
