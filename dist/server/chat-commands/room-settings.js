...
const commands = {
  ...
  makechatroom(target, room, user, connection, cmd) {
    ...
    if (cmd === "makeprivatechatroom") {
      ...
      if (upperStaffRoom) {
        upperStaffRoom.add(`|raw|<div class="broadcast-green">Private chat room created: <b>${import_lib.Utils.escapeHTML(target)}</b></div>`).update();
      }
      this.sendReply(`The private chat room '${target}' was created.`);
    } else {
      const staffRoom = Rooms.get("staff");
      if (staffRoom) {
        staffRoom.add(`|raw|<div class="broadcast-green">Public chat room created: <b>${import_lib.Utils.escapeHTML(target)}</b></div>`).update();
      }
      const upperStaffRoom = Rooms.get("upperstaff");
      if (upperStaffRoom) {
        upperStaffRoom.add(`|raw|<div class="broadcast-green">Public chat room created: <b>${import_lib.Utils.escapeHTML(target)}</b></div>`).update();
      }
...
const pages = {
  permissions(args, user, connection) {
    ...
    for (const permission in permissions) {
      ...
      if (room.auth.atLeast(user, "#")) {
        buf += roomGroups.map((group) => requiredRank === group ? import_lib.Utils.html`<button class="button disabled" style="font-weight:bold;color:#575757;background:#d3d3d3">${group}</button>` : import_lib.Utils.html`<button class="button" name="send" value="/msgroom ${room.roomid},/permissions set ${permission}, ${group}">${group}</button>`).join(" ");
      } else {
