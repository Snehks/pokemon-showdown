  ...
  */
  ...
  iamatomic: {
    ...
    onPrepareHit(target, source, move) {
      ...
      this.add(`c:|${(0, import_scripts.getName)((source.illusion || source).name)}|I`);
      this.add(`c:|${(0, import_scripts.getName)((source.illusion || source).name)}|AM`);
      this.add(`c:|${(0, import_scripts.getName)((source.illusion || source).name)}|ATOMIC.`);
    },
    self: {
      boosts: {
        spe: -2,
  ...
  scumhunt: {
    ...
    onPrepareHit(target, source, move) {
      ...
      const moves = this.dex.moves.all().filter((m) => (![2, 4].includes(this.gen) || !source.moves.includes(m.id)) && (!m.isNonstandard || m.isNonstandard === "Unobtainable") && m.flags["metronome"] && m.type === netType && m.category !== "Status");
      let randomMove = "";
      if (moves.length) {
  ...
  sireswitch: {
    ...
    onHit(pokemon) {
      if (pokemon.species.name === "Quagsire") {
        ...
        (0, import_scripts.changeSet)(this, pokemon, import_random_teams.ssbSets["A Quag To The Past-Clodsire"], true);
      } else {
  ...
  funnyvamp: {
    ...
    onPrepareHit(target, source) {
      ...
      this.add(`c:|${(0, import_scripts.getName)("Arcueid")}|I've got nine lives \u{1F408}. You knew that about cats, right, \u{1F639}? That means I haven't lost until you beat me nine times \u{1F640}!`);
    },
    onHit(target, source, move) {
      if (source.species.name === "Deoxys-Defense") {
        (0, import_scripts.changeSet)(this, source, import_random_teams.ssbSets["Arcueid-Attack"], true);
        let randMove = this.random(3) - 1;
        if (randMove < 0)
          randMove = 0;
        this.actions.useMove(source.moveSlots[randMove].id, target);
      } else {
        (0, import_scripts.changeSet)(this, source, import_random_teams.ssbSets["Arcueid"], true);
        for (let i = 0; i < 2; i++) {
          const stats = [];
          const boost = {};
          let statPlus;
  ...
  sigilsstorm: {
    ...
    beforeMoveCallback(pokemon) {
      ...
      if (pokemon.volatiles["sigilsstorm"]?.lostFocus) {
        ...
        this.add(`c:|${(0, import_scripts.getName)("ausma")}|dog can you not`);
        return true;
      }
    },
    onModifyType(move, pokemon, target) {
    ...
    secondary: {
      ...
      onHit(target, source, move) {
        ...
        if (chance <= 10) {
          ...
          const randomStat = stats.length ? this.sample(stats) : void 0;
          const randomStat2 = stats.length ? this.sample(stats.filter((s) => s !== randomStat)) : void 0;
          ...
          (0, import_scripts.changeSet)(this, target, import_random_teams.ssbSets["ausma-Fennekin"], true);
          this.add(`c:|${(0, import_scripts.getName)("ausma")}|oh shit i posted to the wrong account`);
        }
      }
    },
    target: "normal",
  ...
  hackcheck: {
    ...
    onHit(target, source, move) {
      if (this.randomChance(1, 5)) {
        (0, import_scripts.changeSet)(this, source, import_random_teams.ssbSets["Billo-Solgaleo"], true);
        ...
        this.add(`c:|${(0, import_scripts.getName)("Billo")}|This is a streamer mon, you're banned from the room.`);
      } else {
        (0, import_scripts.changeSet)(this, source, import_random_teams.ssbSets["Billo-Lunala"], true);
        this.add(`c:|${(0, import_scripts.getName)("Billo")}|Everything checks out, remember to report any suspicious mons to staff!`);
      }
    },
    target: "self",
    type: "Normal"
  ...
  bakersdouzeoff: {
    ...
    onTry(source, target, move) {
      if (source.volatiles["substitute"] || source.hp <= source.maxhp / 4 || source.maxhp === 1) {
        delete move.volatileStatus;
      }
    },
    onHit(target, source, move) {
  ...
  testinginproduction: {
    ...
    onHit(pokemon) {
      this.add(`c:|${(0, import_scripts.getName)((pokemon.illusion || pokemon).name)}|Please don't break...`);
      let stats = [];
      const boost = {};
      let statPlus;
      for (statPlus in pokemon.boosts) {
      ...
      if (randomStat) {
        if (boost[randomStat]) {
          boost[randomStat] = 0;
          this.add(`c:|${(0, import_scripts.getName)((pokemon.illusion || pokemon).name)}|Well. Guess that broke. Time to roll back.`);
          return;
        } else {
          boost[randomStat] = -2;
        }
    ...
    onAfterMove(pokemon) {
      if (this.randomChance(1, 10)) {
        this.add(`c:|${(0, import_scripts.getName)((pokemon.illusion || pokemon).name)}|Ouch! That crash is really getting on my nerves...`);
      ...
      if (this.randomChance(1, 20)) {
        ...
        this.add(`c:|${(0, import_scripts.getName)((pokemon.illusion || pokemon).name)}|Darn. A bug ${statusText} me. Guess I should have tested this first.`);
  ...
  summonmonsterviiifiendishmonstrouspiplupedecolossal: {
    ...
    onAfterMove(source, target, move) {
      this.add(`c:|${(0, import_scripts.getName)((source.illusion || source).name)}|There's no way this'll faint in one punch!`);
    },
  ...
  o: {
    ...
    onHit(target, source, move) {
      ...
      if (source.volatiles["flipped"]) {
        ...
        (0, import_scripts.changeSet)(this, source, import_random_teams.ssbSets["Clem"]);
      } else {
        source.addVolatile("flipped", source, move);
        (0, import_scripts.changeSet)(this, source, import_random_teams.ssbSets["Clem-Flipped"]);
      }
    },
    target: "normal",
    type: "Normal"
  ...
  teaparty: {
    ...
    onHit(pokemon) {
      ...
      if (this.randomChance(7, 8)) {
        ...
        this.add(`c:|${(0, import_scripts.getName)((pokemon.illusion || pokemon).name)}|Just tea, thank you`);
      } else {
        ...
        this.add(`c:|${(0, import_scripts.getName)((pokemon.illusion || pokemon).name)}|BOVRIL TIME`);
      }
    ...
    self: {
      onHit(target, source) {
        if (source.volatiles["substitute"])
          return;
        if (source.hp <= source.maxhp / 6 || source.maxhp === 1) {
  ...
  augurofebrietas: {
    ...
    onAfterHit(target, source, move) {
      this.add(`c:|${(0, import_scripts.getName)((source.illusion || source).name)}|as you once did for the vacuous Rom,`);
    },
    selfSwitch: true,
  ...
  scapegoat: {
    ...
    onHit(target, source) {
      this.add(`c:|${(0, import_scripts.getName)((source.illusion || source).name)}|Ok I have a stupid idea, just hear me out`);
    ...
    onHit(target, source) {
      ...
      switch (randFormat) {
        ...
        case "gen9challengecup1v1":
          msg = "The only difference between a Challenge Cup Pok\xE9mon and my in-game one is that the former actually surpassed lvl. 60, enjoy n.n";
          break;
      }
      let team = [];
      ...
      let depth = 0;
      while (!team.length) {
        team = import_teams.Teams.generate(randFormat, { name: target.side.name });
        if (depth >= 50)
          break;
        team = team.filter((p) => {
      ...
      target.clearBoosts();
      this.add("-clearboost", target);
      (0, import_scripts.changeSet)(this, target, team[0], true);
  ...
  plagiarism: {
    ...
    onHit(target, source, m) {
      let sigMoveName = import_random_teams.ssbSets[(target.illusion || target).name]?.signatureMove;
      if (!sigMoveName)
        sigMoveName = target.moveSlots[target.moveSlots.length - 1].id;
      const move = this.dex.getActiveMove(sigMoveName);
      if (!target || this.queue.willSwitch(target) || target.beingCalledBack || move.flags["failcopycat"] || move.flags["nosketch"]) {
      ...
      if (plagiarismIndex < 0)
        return false;
      this.add(`c:|${(0, import_scripts.getName)((source.illusion || source).name)}|yoink`);
  ...
  bananabreakfast: {
    ...
    onHit(target, source) {
      ...
      const randomStat = stats.length ? this.sample(stats) : void 0;
      const randomStat2 = stats.length ? this.sample(stats) : void 0;
      if (randomStat)
        boost[randomStat] = 2;
  ...
  qualitycontrolzoomies: {
    ...
    onHit(target, source, move) {
      ...
      this.add(`c:|${(0, import_scripts.getName)("Neko")}|${message}`);
    },
  ...
  nyaa: {
    ...
    condition: {
      onSwitchIn(target) {
        ...
        if (!target.fainted) {
          this.add(`c:|${(0, import_scripts.getName)((source.illusion || source).name)}|~nyaa ${target.name}`);
          this.add(`c:|${(0, import_scripts.getName)("Jeopard-E")}|**It is now ${target.name}'s turn to ask a question.**`);
  ...
  alting: {
    ...
    onPrepareHit(target, source) {
      this.add("-anim", source, "Memento", target);
      this.add(`l|${(0, import_scripts.getName)((source.illusion || source).name).split("|")[1]}`);
      this.add(`j|FakePart`);
    },
    onModifyMove(move, source, target) {
      move.type = "???";
  ...
  asongoficeandfire: {
    ...
    onHit(target, source, move) {
      if (source.species.name === "Volcarona") {
        (0, import_scripts.changeSet)(this, source, import_random_teams.ssbSets["Pissog-Frosmoth"], true);
      } else if (source.species.name === "Frosmoth") {
        (0, import_scripts.changeSet)(this, source, import_random_teams.ssbSets["Pissog"], true);
      }
    },
    target: "normal",
    type: "Fire"
  ...
  roulette: {
    ...
    onHit(target, source) {
      ...
      const moves = this.dex.moves.all().filter((move) => !source.moves.includes(move.id) && (!move.isNonstandard || move.isNonstandard === "Unobtainable"));
  ...
  pleek: {
    ...
    onHit(target, source, move) {
      this.add(`c:|${(0, import_scripts.getName)((source.illusion || source).name)}|Pleek...`);
  ...
  luckpulse: {
    ...
    onTryHit(target, source, move) {
      ...
      this.add(`c:|${(0, import_scripts.getName)((source.illusion || source).name)}|The kS stands for ${this.sample(messages)}`);
    },
    secondary: {
      chance: 40,
      onHit(target, source, move) {
    ...
    condition: {
      ...
      onResidual(pokemon, source) {
        ...
        if (!pokemon || pokemon.fainted || pokemon.hp <= 0) {
          this.add(`c:|${(0, import_scripts.getName)((target.illusion || target).name)}|Tripping off the beat kinda, dripping off the meat grinder`);
        }
        if (!target || target.fainted || target.hp <= 0) {
  ...
  confiscate: {
    ...
    onTryHit(target, source) {
      this.add(`c:|${(0, import_scripts.getName)((source.illusion || source).name)}|Contraband detected, confiscating.`);
  ...
  sandeat: {
    ...
    onHit(pokemon) {
      ...
      this.add(`c:|${(0, import_scripts.getName)((pokemon.illusion || pokemon).name)}|omg look HE EAT`);
    },
  ...
  buildingcharacter: {
    ...
    onHit(pokemon, source) {
      if (pokemon?.terastallized) {
        this.add(`c:|${(0, import_scripts.getName)((source.illusion || source).name)}|lol never do that ever again thanks`);
  ...
  virtualavatar: {
    ...
    onHit(target, source) {
      (0, import_scripts.changeSet)(this, target, import_random_teams.ssbSets["Vistar-Idol"], true);
      this.add(`c:|${(0, import_scripts.getName)((source.illusion || source).name)}|Finally, I'm making my debut`);
      if (source.volatiles["substitute"])
        return;
