...
const Scripts = {
  ...
  init() {
    ...
    for (const id in this.data.Moves) {
      if (this.data.Moves[id].flags["charge"] && id !== "skydrop") {
        (_a = this.modData("Moves", id)).condition || (_a.condition = {});
        if ("onEnd" in this.modData("Moves", id).condition)
          throw new Error(`onEnd needs manual override for move ${id}`);
  ...
  pokemon: {
    ...
    queryLinkMove(move, ignoreDisabled) {
      ...
      if (!linkedMoves.length)
        return { linkIndex: -1, linkedMoves };
