...
const Abilities = {
  ...
  biogenesis: {
    ...
    onSwitchIn(pokemon) {
      ...
      const attackingMoves = pokemon.baseMoveSlots.map((slot) => this.dex.moves.get(slot.id)).filter((move) => move.category !== "Status");
      const types = attackingMoves.length ? [...new Set(attackingMoves.slice(0, 2).map((move) => move.type))] : pokemon.types;
