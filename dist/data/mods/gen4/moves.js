...
const Moves = {
  ...
  metronome: {
    ...
    onHit(pokemon) {
      const moves = this.dex.moves.all().filter((move) => move.flags["metronome"] && !pokemon.moves.includes(move.id) && (!move.isNonstandard || move.isNonstandard === "Unobtainable") && !(this.field.pseudoWeather["gravity"] && move.flags["gravity"]) && !(pokemon.volatiles["healblock"] && move.flags["heal"]));
      let randomMove = "";
      if (moves.length) {
  ...
  pursuit: {
    inherit: true,
    beforeTurnCallback(pokemon) {
      if (["frz", "slp"].includes(pokemon.status) || pokemon.hasAbility("truant") && pokemon.volatiles["truant"])
        return;
      for (const target of pokemon.foes()) {
