...
class RandomAFDTeams extends import_teams.default {
  constructor() {
    super(...arguments);
  ...
  getPriorityItem(ability, types, moves, counter, teamDetails, species, isLead, isDoubles, teraType, role) {
    ...
    if (moves.has("courtchange") || !isDoubles && (species.id === "luvdisc" || species.id === "terapagos" && !moves.has("rest"))) return "Hoots";
  ...
  randomAFDSet(s, teamDetails = {}, isLead = false, isDoubles = false) {
  ...
  randomTeam() {
    ...
    const zaMegaList = Object.keys(this.randomAFDSets).filter((x) => this.dex.species.get(x).isMega && this.dex.species.get(x).gen === 9);
    ...
    while (baseSpeciesPool.length && pokemon.length < this.maxTeamSize) {
      ...
      const weakToScald = this.dex.getEffectiveness("Water", species) > 0 || this.dex.getEffectiveness("Water", species) > -2 && types.includes("Steel");
...
var random_teams_default = RandomAFDTeams;
