...
const Items = {
  ...
  chilanberry: {
    inherit: true,
    onSourceModifyDamage(damage, source, target, move) {
      if (move.type === target.teraType && (!target.volatiles["substitute"] || move.flags["bypasssub"] || move.infiltrates && this.gen >= 6)) {
        if (target.eatItem()) {
          this.debug("-50% reduction");
          this.add("-enditem", target, this.effect, "[weaken]");
          return this.chainModify(0.5);
  ...
  souldew: {
    inherit: true,
    onBasePower(basePower, user, target, move) {
      if (move && (user.baseSpecies.num === 380 || user.baseSpecies.num === 381) && move.type === user.teraType) {
        return this.chainModify([4915, 4096]);
      }
    }
  },
