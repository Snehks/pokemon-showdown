...
const Formats = [
  ...
  },
  {
    ...
    checkCanLearn(move, species, setSources, set) {
      ...
      const natDex = TeamValidator.get(`gen${this.gen}nationaldexag`).checkCanLearn(move, species, this.allSources(species), set);
    ...
    onValidateSet(set) {
      ...
      if (requireObtainable) {
        if (species.natDexTier === "Unreleased") {
          ...
          if (this.ruleTable.has(`+pokemon:${species.id}`) || this.ruleTable.has(`+basepokemon:${basePokemon}`) || this.ruleTable.has("+pokemontag:unobtainable")) {
            return;
          }
          return [`${set.name || set.species} does not exist in the National Dex.`];
        }
  ...
  },
  {
    ...
    onSwitchIn(pokemon) {
      if ((pokemon.illusion || pokemon).getTypes(true, true).join("/") !== this.dex.forGen(9).species.get((pokemon.illusion || pokemon).species.name).types.join("/") && !pokemon.terastallized) {
  ...
  },
  {
    ...
    onValidateTeam(team) {
      ...
      for (const set of team) {
        ...
        if (!(item.forcedForme && !item.zMove) && !item.megaStone && !item.isPrimalOrb && !item.name.startsWith("Rusted")) continue;
  ...
  },
  {
    ...
    validateSet(set, teamHas) {
      ...
      if (!effectFunctions.some((f) => f.get(set.ability).exists) && !(set.item && effectFunctions.some((f) => f.get(set.item).exists)) && !set.moves.every((move) => effectFunctions.some((f) => f.get(move).exists))) {
      ...
      if (allThings.some((y) => effectFunctions.some((x) => x.get(y).isNonstandard && !this.ruleTable.has(`+tag:${this.toID(x.get(y).isNonstandard)}`)))) {
        return this.validateSet(set, teamHas);
      }
      ...
      if (abilities.some((abil) => !Object.values(species.abilities).map(this.toID).includes(abil.id)) && this.ruleTable.has("obtainableabilities")) {
      ...
      for (const curMove of moves) {
        ...
        if (requiredMove && moves.map((m) => m.id).includes(curMove.id) && curMove.id !== this.toID(requiredMove)) {
    ...
    onBeforeSwitchIn(pokemon) {
      ...
      if (pokemon.hasItem("abilityshield") || pokemon.m.scrambled.items.some((e) => this.toID(e.thing) === "abilityshield")) {
        ngas = false;
      }
      ...
      if (ngas) {
        if (pokemon.m.scrambled.moves.findIndex((e) => e.inSlot === "Ability") >= 0) {
      ...
      if (this.field.getPseudoWeather("magicroom")) {
        if (pokemon.m.scrambled.moves.findIndex((e) => e.inSlot === "Item") >= 0) {
  ...
  },
  {
    ...
    validateSet(set, teamHas) {
      ...
      const crossNonstandard = !this.ruleTable.has("natdexmod") && crossSpecies.isNonstandard === "Past" || crossSpecies.isNonstandard === "Future";
  ...
  },
  {
    ...
    onValidateSet(set, format, setHas, teamHas) {
      ...
      if (species.name !== species.baseSpecies && species.baseSpecies === "Arceus" && this.dex.items.get(set.item).onPlate !== species.types[0]) {
  ...
  },
  {
    ...
    validateSet(set, teamHas) {
      ...
      const species = this.dex.species.get(set.species);
      if (!species.exists || species.num < 1) return [`The Pok\xE9mon "${set.species}" does not exist.`];
      if (species.isNonstandard && !this.ruleTable.has(`+tag:${this.toID(species.isNonstandard)}`)) {
      ...
      if (!pokemonWithAbility) return [`${ability.name} is not available on a legal Pok\xE9mon.`];
      this.format.debug = true;
      if (!teamHas.abilitySources) teamHas.abilitySources = /* @__PURE__ */ Object.create(null);
  ...
  },
  {
    ...
    validateSet(set, teamHas) {
      ...
      if (set.moves?.length) {
        ...
        for (const moveid of originalMoves) {
          ...
          if (pokemove.isNonstandard && !(this.ruleTable.has(`+tag:${this.toID(pokemove.isNonstandard)}`) || this.ruleTable.has(`+pokemon:${pokemove.id}`) || this.ruleTable.has(`+basepokemon:${this.toID(pokemove.baseSpecies)}`))) {
    ...
    onModifyMove(move, pokemon, target) {
      ...
      if (species.exists) {
        ...
        move.category = species.baseStats["spa"] > species.baseStats["atk"] ? "Special" : species.baseStats["spa"] < species.baseStats["atk"] ? "Physical" : pokemon.getStat("atk", false, true) > pokemon.getStat("spa", false, true) ? "Physical" : "Special";
    ...
    actions: {
      switchIn(pokemon, pos, sourceEffect, isDrag) {
        ...
        if (unfaintedActive) {
          ...
          if (sourceEffect && typeof sourceEffect.selfSwitch) {
            if (typeof sourceEffect.selfSwitch === "string") {
  ...
  },
  {
    ...
    pokemon: {
      getTypes(excludeAdded, preterastallized) {
        ...
        if (addTeraType) return Array.from(/* @__PURE__ */ new Set([...types, addTeraType]));
        return types;
      },
  ...
  },
  {
    ...
    validateSet(set, teamHas) {
      ...
      if (!teamHas.trademarks) teamHas.trademarks = new this.dex.Multiset();
  ...
  },
  {
    ...
    desc: `Randomized teams of 24 Pok&eacute;mon with sets that are generated to be competitively viable. Merges a chosen team of 6 into a single Pok\xE9mon depending on the order chosen at team preview: It gains the typing of the first, item of the second, ability of the third, stats of the fourth, the first two moves of the fifth, and the last two moves of the sixth.`,
    mod: "gen9",
    searchShow: false,
    bestOfDefault: true,
    team: "random",
    ruleset: ["Obtainable", "Chimera 1v1 Rule", "Species Clause", "HP Percentage Mod", "Cancel Mod", "Sleep Clause Mod", "Max Team Size = 24", "Adjust Level = 100", "Best of = 5"]
  },
  {
  ...
  },
  {
    ...
    onSwitchIn(pokemon) {
      ...
      if (this.dex.species.get(name).exists || this.dex.moves.get(name).exists || this.dex.abilities.get(name).exists || name === "blitz") {
        name = `${name}user`;
      }
  ...
  },
  {
    ...
    onBegin() {
      for (const pokemon of this.getAllPokemon()) {
        if (!pokemon.set.teraCaptain) pokemon.canTerastallize = null;
      }
    ...
    onTeamPreview() {
      this.add("clearpoke");
      for (const pokemon of this.getAllPokemon()) {
        const details = pokemon.details.replace(", shiny", "").replace(/(Greninja|Gourgeist|Pumpkaboo|Xerneas|Zacian|Zamazenta|Dudunsparce)(-[a-zA-Z?-]+)?/g, "$1-*");
      ...
      for (const side of this.sides) {
        ...
        for (const pokemon of side.pokemon) {
          if (!pokemon.set.teraCaptain) continue;
  ...
  },
  {
    ...
    onValidateTeam(team) {
      ...
      if (restrictedSpecies.length > 4) {
        problems.push(`You can only use up to four restricted Pok\xE9mon; you have: ${restrictedSpecies.join(", ")}`);
      }
      if (nonRestrictedSpecies.length < 2) {
        problems.push(`You must use at least two non-restricted Pok\xE9mon; you have: ${nonRestrictedSpecies.length ? nonRestrictedSpecies[0] : "None"}.`);
      }
      return problems;
    },
    onChooseTeam(positions, pokemon, autoChoose) {
      ...
      if (restrictedCount > 2) {
        return `You can only bring up to two restricted Pok\xE9mon to the battle.`;
      }
    }
  },
  {
