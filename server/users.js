...
exports.Users = exports.User = exports.Connection = void 0;
 ...
 * User groups
*********************************************************/
const globalAuth = new user_groups_1.GlobalAuth();
function isUsernameKnown(name) {
    const userid = toID(name);
    if (exports.Users.get(userid))
        return true;
...
class User extends Chat.MessageContext {
    constructor(connection) {
        ...
        this.id = '';
        this.tempGroup = user_groups_1.Auth.defaultSymbol();
        this.language = null;
    ...
    can(permission, target = null, room = null, cmd, cmdToken) {
        return user_groups_1.Auth.hasPermission(this, permission, target, room, cmd, cmdToken);
    }
    /**
    ...
    hasConsoleAccess(connection) {
        if (this.hasSysopAccess())
            return true;
        if (!this.can('console'))
            return false; // normal permission check
    ...
    async validateToken(token, name, userid, connection) {
        ...
        const [tokenData, tokenSig] = lib_1.Utils.splitFirst(token, ';');
        ...
        if (!this.trusted && userType === '1') { // userType '1' means unregistered
            ...
            if (elapsed < NAMECHANGE_THROTTLE && !Config.nothrottle) {
                if (this.newNames >= NAMES_PER_THROTTLE) {
                    this.send(`|nametaken|${name}|You must wait ${Chat.toDurationString(NAMECHANGE_THROTTLE - elapsed)} more
    ...
    handleRename(name, userid, newlyRegistered, userType) {
        ...
        if (conflictUser) {
            ...
            if (!registered && !conflictUser.registered && conflictUser.latestIp === this.latestIp &&
        ...
        this.isPublicBot = exports.Users.isPublicBot(userid);
    ...
    updateGroup(registered, isMerge) {
        ...
        exports.Users.Avatars?.handleLogin(this);
    ...
    chat(message, room, connection) {
        ...
        if (this.chatQueueTimeout) {
            if (!this.chatQueue)
                this.chatQueue = []; // this should never happen
            if (this.chatQueue.length >= THROTTLE_BUFFER_LIMIT - 1) {
                connection.sendTo(room, `|raw|<strong class="message-throttle-notice">Your message was not sent because you've been typing too quickly.</strong>`);
                return false;
            }
            else {
    ...
    startChatQueue(delay = null) {
        ...
        this.chatQueueTimeout = setTimeout(() => this.processChatQueue(), delay);
    }
    clearChatQueue() {
        this.chatQueue = null;
        if (this.chatQueueTimeout) {
...
function pruneInactive(threshold) {
    ...
    for (const user of users.values()) {
        if (user.statusType === 'online') {
            ...
            const bypass = !user.can('bypassall') && (user.can('bypassafktimer') ||
...
function socketReceive(worker, workerid, socketid, message) {
    ...
    if (!lines[lines.length - 1])
        lines.pop();
    const maxLineCount = (user.can('bypassall') ? THROTTLE_MULTILINE_WARN_ADMIN :
...
exports.Users = {
    ...
    isTrusted,
    isPublicBot,
    PLAYER_SYMBOL: user_groups_1.PLAYER_SYMBOL,
    HOST_SYMBOL: user_groups_1.HOST_SYMBOL,
    connections,
    User,
    Connection,
    socketDisconnect,
