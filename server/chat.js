...
exports.Chat = exports.CommandContext = exports.PageContext = exports.MessageContext = exports.Interruption = exports.ErrorMessage = void 0;
const ConfigLoader = __importStar(require("./config-loader"));
const Friends = __importStar(require("./friends"));
const lib_1 = require("../lib");
const Artemis = __importStar(require("./artemis"));
const sim_1 = require("../sim");
const private_messages_1 = require("./private-messages");
const pathModule = __importStar(require("path"));
const JSX = __importStar(require("./chat-jsx"));
...
const chat_formatter_1 = require("./chat-formatter");
let probe = null;
try {
...
const PM = (0, lib_1.SQL)('chat-db', module, {
...
class MessageContext {
    ...
    extractFormat(formatOrMod, allowRules) {
        if (!formatOrMod) {
            return { dex: sim_1.Dex.includeData(), format: null, isMatch: false };
        }
        const format = sim_1.Dex.formats.get(formatOrMod);
        if (format.effectType === 'Format' || allowRules && format.effectType === 'Rule') {
            return { dex: sim_1.Dex.forFormat(format), format, isMatch: true };
        }
        if (toID(formatOrMod) in sim_1.Dex.dexes) {
            return { dex: sim_1.Dex.mod(toID(formatOrMod)), format: null, isMatch: true };
        }
...
class PageContext extends MessageContext {
    ...
    async resolve(pageid) {
        ...
        if (!this.connection.openPages)
            this.connection.openPages = new Set();
        ...
        try {
            ...
            this.setHTML(`<div class="pad"><div class="broadcast-red">` +
        ...
        if (res === Rooms.RETRY_AFTER_LOGIN) {
            this.setHTML(`Please log in before accessing this page (don't worry, it will load automatically once you do so).`);
        }
        return res;
    }
}
...
class CommandContext extends MessageContext {
    constructor(options) {
        super(options.user, options.room && options.room.settings.language ?
        ...
        try {
            if (this.handler) {
                if (this.handler.disabled) {
                    throw new exports.Chat.ErrorMessage(`The command /${this.fullCmd.trim()} is temporarily unavailable due to technical difficulties. ` +
    ...
    sendChatMessage(message) {
        if (this.pmTarget) {
            ...
            if (blockInvites && /^<<.*>>$/.test(message.trim())) {
                if (!this.user.can('lock') && blockInvites === true ||
                    !Users.globalAuth.atLeast(this.user, blockInvites)) {
                    exports.Chat.maybeNotifyBlocked(`invite`, this.pmTarget, this.user);
                    throw new exports.Chat.ErrorMessage(`${this.pmTarget.name} is blocking room invites.`);
                }
            }
            exports.Chat.PrivateMessages.send(message, this.user, this.pmTarget);
        }
        else if (this.room) {
    ...
    run(handler) {
        if (typeof handler === 'string')
            handler = exports.Chat.commands[handler];
        if (!handler.broadcastable && this.cmdToken === '!') {
            throw new exports.Chat.ErrorMessage([`The command "${this.fullCmd}" can't be broadcast.`,
    ...
    checkFormat(room, user, message) {
        if (!room)
            return true;
        if (!room.settings.filterStretching && !room.settings.filterCaps &&
        ...
        if (room.settings.filterLinks) {
            ...
            if (bannedLinks.length) {
                throw new exports.Chat.ErrorMessage(`You have linked to ${bannedLinks.length > 1 ? 'unrecognized external websites' : 'an unrecognized external website'} ` +
    ...
    checkBanwords(room, message) {
        ...
        if (room.banwordRegex !== true && room.banwordRegex.test(message)) {
            throw new exports.Chat.ErrorMessage(`Your message contained a word banned by this room.`);
        }
    ...
    pmTransform(originalMessage, sender, receiver) {
        ...
        return originalMessage.split('\n').map(message => {
            if (message.startsWith('||')) {
                ...
                const [uhtmlid, html] = lib_1.Utils.splitFirst(message.slice(7), '|');
                return prefix + `/uhtml ${uhtmlid},${html}`;
            }
            else if (message.startsWith(`|uhtmlchange|`)) {
                const [uhtmlid, html] = lib_1.Utils.splitFirst(message.slice(13), '|');
    ...
    errorReply(message) {
        if (this.bypassRoomCheck) { // if they're not in the room, we still want a good error message for them
            return this.popupReply(`|html|<strong class="message-error">${message.replace(/\n/ig, '<br />')}</strong>`);
        }
    ...
    canUseConsole() {
        if (!this.user.hasConsoleAccess(this.connection)) {
            throw new exports.Chat.ErrorMessage(`${(this.cmdToken + this.fullCmd).trim()} - Requires console access, please set up \`Config.consoleips\`.`);
        }
        return true;
    }
    shouldBroadcast() {
    ...
    checkBroadcast(overrideCooldown, suppressMessage) {
        ...
        if (this.user.locked && !(this.room?.roomid.startsWith('help-') || this.pmTarget?.can('lock'))) {
            throw new exports.Chat.ErrorMessage([`You cannot broadcast this command's information while locked.`,
        ...
        if (this.room && !this.user.can('show', null, this.room, this.cmd, this.cmdToken)) {
            ...
            throw new exports.Chat.ErrorMessage([`You need to be ${atLeast} to broadcast this command's information.`,
                `To see it for yourself, use: /${this.message.slice(1)}`]);
        }
        if (!this.room && !this.pmTarget) {
            throw new exports.Chat.ErrorMessage([`Broadcasting a command with "!" in a PM or chatroom will show it that user or room.`,
                `To see it for yourself, use: /${this.message.slice(1)}`]);
        }
        // broadcast cooldown
        ...
        if (cooldownMessage && this.room && this.room.lastBroadcast === cooldownMessage &&
    ...
    checkChat(message = null, room = null, targetUser = null) {
        ...
        if (!user.can('bypassall')) {
            ...
            if (room) {
                if (lockType && !room.settings.isHelp) {
                    ...
                    if (user.locked === '#hostfilter') {
                        throw new exports.Chat.ErrorMessage(this.tr `You are locked due to your proxy / VPN and can't talk in chat.`);
                    }
                    else {
                ...
                if (!room.persist && !room.roomid.startsWith('help-') && !(user.registered || user.autoconfirmed)) {
                    this.sendReply(this.tr `|html|<div class="message-error">You must be registered to chat in temporary rooms (like battles).</div>` +
                        this.tr `You may register in the <button name="openOptions"><i class="fa fa-cog"></i> Options</button> menu.`);
                    throw new exports.Chat.Interruption();
                }
                ...
                if (room.settings.modchat && !room.auth.atLeast(user, room.settings.modchat)) {
                    if (room.settings.modchat === 'autoconfirmed') {
                        this.errorReply(this.tr `Moderated chat is set. To speak in this room, your account must be autoconfirmed, which means being registered for at least one week and winning at least one rated game (any game started through the 'Battle!' button).`);
                        if (!user.registered) {
                    ...
                    if (room.settings.modchat === 'trusted') {
                        throw new exports.Chat.ErrorMessage(this.tr `Because moderated chat is set, your account must be staff in a public room or have a global rank to speak in this room.`);
                    }
                    const groupName = Config.groups[room.settings.modchat] && Config.groups[room.settings.modchat].name ||
                        room.settings.modchat;
                    throw new exports.Chat.ErrorMessage(this.tr `Because moderated chat is set, you must be of rank ${groupName} or higher to speak in this room.`);
                }
            ...
            if (targetUser) {
                ...
                if (!(user.registered || user.autoconfirmed)) {
                    this.sendReply(this.tr `|html|<div class="message-error">You must be registered to send private messages.</div>` +
                        this.tr `You may register in the <button name="openOptions"><i class="fa fa-cog"></i> Options</button> menu.`);
                    throw new exports.Chat.Interruption();
                }
                if (targetUser.id !== user.id && !(targetUser.registered || targetUser.autoconfirmed)) {
                    throw new exports.Chat.ErrorMessage(this.tr `That user is unregistered and cannot be PMed.`);
                }
                if (lockType && !targetUser.can('lock')) {
                    this.sendReply(`|html|<a href="view-help-request--appeal" class="button">${this.tr `Get help with this`}</a>`);
                    if (user.locked === '#hostfilter') {
                        throw new exports.Chat.ErrorMessage(this.tr `You are locked due to your proxy / VPN and can only private message members of the global moderation team.`);
                    }
                    else {
        ...
        if (room?.settings.highTraffic &&
            ...
            throw new exports.Chat.ErrorMessage(this.tr `Due to this room being a high traffic room, your message must contain at least two letters.`);
        }
        if (room) {
    ...
    checkCanPM(targetUser, user) {
        ...
        if (setting === true && !user.can('lock'))
            return false; // this is to appease TS
    ...
    checkPMHTML(targetUser) {
        ...
        if (targetUser.settings.blockPMs &&
            (targetUser.settings.blockPMs === true ||
                ...
                !Users.globalAuth.atLeast(this.user, targetUser.settings.blockPMs)) &&
            !this.user.can('lock')) {
    ...
    checkBannedLinks(message) {
        ...
        return (message.match(exports.Chat.linkRegex) || []).filter(link => {
    ...
    checkEmbedURI(uri) {
        ...
        if (uri.startsWith('data:')) {
            ...
            throw new exports.Chat.ErrorMessage("Image URLs must begin with 'https://' or 'data:'; 'http://' cannot be used.");
        }
    }
    /**
            ...
            for (const tag of tags) {
                ...
                if (tagName === 'img') {
                    if (!this.room || (this.room.settings.isPersonal && !this.user.can('lock'))) {
                        throw new exports.Chat.ErrorMessage(`This tag is not allowed: <${tagContent}>. Images are not allowed outside of chatrooms.`);
                    }
                    if (!/width ?= ?(?:[0-9]+|"[0-9]+")/i.test(tagContent) || !/height ?= ?(?:[0-9]+|"[0-9]+")/i.test(tagContent)) {
                        ...
                        throw new exports.Chat.ErrorMessage([`This image is missing a width/height attribute: <${tagContent}>`,
                    ...
                    if (srcMatch) {
                        ...
                        throw new exports.Chat.ErrorMessage([`This image has a broken src attribute: <${tagContent}>`,
                    ...
                    if ((!this.room || this.room.settings.isPersonal || this.room.settings.isPrivate === true) && !this.user.can('lock')) {
                        ...
                        if (buttonName === 'send' && buttonValue && msgCommandRegex.test(buttonValue)) {
                            ...
                            if (auth.get(toID(pmTarget)) !== '*' && toID(pmTarget) !== this.user.id) {
                                throw new exports.Chat.ErrorMessage([`This button is not allowed: <${tagContent}>`,
    ...
    checkRecursion() {
        if (this.recursionDepth > 5) {
            throw new exports.Chat.ErrorMessage(`/${this.cmd} - Too much command recursion has occurred.`);
        }
    }
    requireRoom(id) {
        if (!this.room) {
    ...
    requireGame(constructor, subGame = false) {
        ...
        if (subGame) {
            if (!room.subGame) {
                throw new exports.Chat.ErrorMessage(`This command requires a sub-game of ${constructor.name} (this room has no sub-game).`);
            }
            const game = room.getGame(constructor, subGame);
            // must be a different game
            if (!game) {
                throw new exports.Chat.ErrorMessage(`This command requires a sub-game of ${constructor.name} (this sub-game is ${room.subGame.title}).`);
            }
            return game;
        }
        if (!room.game) {
    ...
    commandDoesNotExist() {
        ...
        throw new exports.Chat.ErrorMessage(`The command "${(this.cmdToken + this.fullCmd).trim()}" does not exist. To send a message starting with "${this.cmdToken}${this.fullCmd}", type "${this.cmdToken}${this.cmdToken}${this.fullCmd}".`);
    }
    refreshPage(pageid) {
...
exports.Chat = new class {
    constructor() {
        ...
        this.Friends = new Friends.FriendsDatabase();
        this.FriendsPM = Friends.PM;
        this.PrivateMessages = private_messages_1.PrivateMessages;
        this.multiLinePattern = new PatternTester();
        this.destroyHandlers = [Artemis.destroy, Friends.destroy];
        this.crqHandlers = {};
        ...
        this.packageData = {};
        this.formatText = chat_formatter_1.formatText;
        this.linkRegex = chat_formatter_1.linkRegex;
        this.stripFormatting = chat_formatter_1.stripFormatting;
        this.filterWords = {};
        this.monitors = {};
    ...
    filter(message, context) {
        ...
        for (const curFilter of exports.Chat.filters) {
            const output = curFilter.call(context, message, context.user, context.room, context.connection, context.pmTarget, originalMessage);
            if (output === false)
                return null;
    ...
    namefilter(name, user) {
        if (!Config.disablebasicnamefilter) {
            ...
            if (/[^\uAC00-\uD7A3\u1100-\u11F9\u3131-\u318E][\u110B\u114C\u11BC\u11EE\u11F0\u3147\u3180]+[^\uAC00-\uD7A3\u1100-\u11F9\u3131-\u318E]/.test(name)) {
            ...
            // \u00ae\u00a9 (R) (C)
            if (nameSymbols.length > 4 ||
                /[^a-z0-9][a-z0-9][^a-z0-9]/.test(name.toLowerCase() + ' ') || /[\u00ae\u00a9].*[a-zA-Z0-9]/.test(name)) {
                name = name.replace(
                // eslint-disable-next-line no-misleading-character-class
                /[\u00A1-\u00BF\u00D7\u00F7\u02B9-\u0362\u2012-\u2027\u2030-\u205E\u2050-\u205F\u2190-\u23FA\u2500-\u2BD1\u2E80-\u32FF\u3400-\u9FFF\uF900-\uFAFF\uFE00-\uFE6F]+/g, '').replace(/[^A-Za-z0-9]{2,}/g, ' ').trim();
            }
        }
    ...
    async loadTranslations() {
        const directories = await (0, lib_1.FS)(TRANSLATION_DIRECTORY).readdir();
        ...
        for (const dirname of directories) {
            ...
            if (/[^a-z0-9]/.test(dirname))
                continue;
            const dir = (0, lib_1.FS)(`${TRANSLATION_DIRECTORY}/${dirname}`);
            // For some reason, toID() isn't available as a global when this executes.
            const languageID = sim_1.Dex.toID(dirname);
    ...
    tr(language, strings = '', ...keys) {
        ...
        if (exports.Chat.translationsLoaded && !exports.Chat.translations.has(language)) {
    ...
    async prepareDatabase() {
        ...
        if (process.send)
            return; // We don't need a database in a subprocess that requires Chat.
        if (!Config.usesqlite)
            return;
        // check if we have the db_info table, which will always be present unless the schema needs to be initialized
        const { hasDBInfo } = await this.database.get(`SELECT count(*) AS hasDBInfo FROM sqlite_master WHERE type = 'table' AND name = 'db_info'`);
        if (!hasDBInfo)
            await this.database.runFile('./databases/schemas/chat-plugins.sql');
        const result = await this.database.get(`SELECT value as curVersion FROM db_info WHERE key = 'version'`);
        ...
        for (const migrationFile of (await (0, lib_1.FS)(migrationsFolder).readdir())) {
        ...
        lib_1.Utils.sortBy(migrationsToRun, ({ version }) => version);
        for (const { file } of migrationsToRun) {
            await this.database.runFile(pathModule.resolve(migrationsFolder, file));
        }
        exports.Chat.destroyHandlers.push(() => void exports.Chat.database?.destroy(), () => exports.Chat.PrivateMessages.destroy());
    }
    /**
     * Command parser
     *
    ...
    logSlowMessage(startTime, context) {
        ...
        if (context.cmd === 'search' || context.cmd === 'savereplay')
            return;
        const logMessage = (`[slow command] ${timeUsed}ms - ${context.user.name} (${context.connection.ip}): ` +
    ...
    loadPluginFile(file) {
        if (!file.endsWith('.js'))
            return;
        this.loadPlugin(require((0, lib_1.FS)(file).path), this.getPluginName(file));
    }
    loadPluginDirectory(dir, depth = 0) {
        for (const file of (0, lib_1.FS)(dir).readdirSync()) {
            const path = pathModule.join(dir, file);
            if ((0, lib_1.FS)(path).isDirectorySync()) {
                depth++;
    ...
    annotateCommands(commandTable, namespace = '', pluginName) {
    ...
    loadPlugin(plugin, name) {
        ...
        plugin = { ...plugin };
        if (plugin.commands) {
            Object.assign(exports.Chat.commands, this.annotateCommands(plugin.commands, '', name));
        }
        if (plugin.pages) {
        ...
        if (plugin.chatfilter)
            exports.Chat.filters.push(plugin.chatfilter);
        if (plugin.namefilter)
            exports.Chat.namefilters.push(plugin.namefilter);
        if (plugin.hostfilter)
            exports.Chat.hostfilters.push(plugin.hostfilter);
        if (plugin.loginfilter)
            exports.Chat.loginfilters.push(plugin.loginfilter);
        if (plugin.punishmentfilter)
            exports.Chat.punishmentfilters.push(plugin.punishmentfilter);
        if (plugin.nicknamefilter)
            exports.Chat.nicknamefilters.push(plugin.nicknamefilter);
        if (plugin.statusfilter)
            exports.Chat.statusfilters.push(plugin.statusfilter);
        if (plugin.onRenameRoom) {
            if (!exports.Chat.handlers['onRenameRoom'])
                exports.Chat.handlers['onRenameRoom'] = [];
            exports.Chat.handlers['onRenameRoom'].push(plugin.onRenameRoom);
        }
        if (plugin.onRoomClose) {
            if (!exports.Chat.handlers['onRoomClose'])
                exports.Chat.handlers['onRoomClose'] = [];
            exports.Chat.handlers['onRoomClose'].push(plugin.onRoomClose);
        }
        if (plugin.handlers) {
            for (const handlerName in plugin.handlers) {
                if (!exports.Chat.handlers[handlerName])
                    exports.Chat.handlers[handlerName] = [];
                exports.Chat.handlers[handlerName].push(plugin.handlers[handlerName]);
            }
        }
        if (plugin.start) {
    ...
    loadPlugins(oldPlugins) {
        if (exports.Chat.commands)
            return;
        if (oldPlugins)
            exports.Chat.oldPlugins = oldPlugins;
        void (0, lib_1.FS)('package.json').readIfExists().then(data => {
            if (data)
                exports.Chat.packageData = JSON.parse(data);
        });
        ...
        lib_1.Utils.sortBy(exports.Chat.filters, filter => -(filter.priority || 0));
    }
    destroy() {
    ...
    handleRoomRename(oldID, newID, room) {
        exports.Chat.runHandlers('onRenameRoom', oldID, newID, room);
    }
    handleRoomClose(roomid, user, connection) {
        exports.Chat.runHandlers('onRoomClose', roomid, user, connection, roomid.startsWith('view-'));
    }
    /**
    ...
    parseCommand(message, recursing = false) {
        ...
        if (cmdToken === BROADCAST_TOKEN && /[^A-Za-z0-9]/.test(message.charAt(1)))
            return null;
        let [cmd, target] = lib_1.Utils.splitFirst(message.slice(1), ' ');
        cmd = cmd.toLowerCase();
        if (cmd.endsWith(','))
            cmd = cmd.slice(0, -1);
        ...
        do {
            ...
            if (commandHandler && typeof commandHandler === 'object') {
                [cmd, target] = lib_1.Utils.splitFirst(target, ' ');
                cmd = cmd.toLowerCase();
                prevCmdName = fullCmd;
                fullCmd += ' ' + cmd;
    ...
    allCommands(table = exports.Chat.commands) {
        ...
        if (table !== exports.Chat.commands)
            return results;
    ...
    validateRegex(word) {
        ...
        try {
            ...
            throw new exports.Chat.ErrorMessage(e.message.startsWith('Invalid regular expression: ') ?
                e.message :
    ...
    getReadmoreBlock(str, isCode, cutoff = 3) {
        ...
        for (const [i, param] of params.entries()) {
            ...
            if (param.length > cutoff * 160 && i < cutoff)
                cutoff = i;
            output.push(lib_1.Utils[isCode ? 'escapeHTMLForceWrap' : 'escapeHTML'](param));
        }
    ...
    getDataPokemonHTML(species, gen = 8, tier = '') {
        ...
        if (gen >= 3) {
            ...
            if (species.abilities['1'] && (gen >= 4 || sim_1.Dex.abilities.get(species.abilities['1']).gen === 3)) {
    ...
    parseArguments(str, delim = ',', opts = { useIDs: true }) {
        ...
        for (const part of str.split(delim)) {
            let [key, val] = lib_1.Utils.splitFirst(part, opts.paramDelim || (opts.paramDelim = "=")).map(f => f.trim());
            if (opts.useIDs)
                key = toID(key);
            if (!toID(key) || (!opts.allowEmpty && !toID(val))) {
                throw new exports.Chat.ErrorMessage(`Invalid option ${part}. Must be in [key]${opts.paramDelim}[value] format.`);
            }
            if (!result[key])
                result[key] = [];
            result[key].push(val);
    ...
    maybeNotifyBlocked(blocked, targetUser, user) {
        ...
        if (blocked === 'pm') {
            if (!targetUser.notified.blockPMs) {
                targetUser.send(`${prefix}The user '${lib_1.Utils.escapeHTML(user.name)}' attempted to PM you but was blocked. To enable PMs, use /unblockpms ${options}`);
                targetUser.notified.blockPMs = true;
            }
        }
        else if (blocked === 'challenge') {
            if (!targetUser.notified.blockChallenges) {
                targetUser.send(`${prefix}The user '${lib_1.Utils.escapeHTML(user.name)}' attempted to challenge you to a battle but was blocked. To enable challenges, use /unblockchallenges ${options}`);
                targetUser.notified.blockChallenges = true;
            }
        }
        else if (blocked === 'invite') {
            if (!targetUser.notified.blockInvites) {
                targetUser.send(`${prefix}The user '${lib_1.Utils.escapeHTML(user.name)}' attempted to invite you to a room but was blocked. To enable invites, use /unblockinvites.`);
    ...
    registerMonitor(id, entry) {
        if (!exports.Chat.filterWords[id])
            exports.Chat.filterWords[id] = [];
...
exports.Chat.escapeHTML = lib_1.Utils.escapeHTML;
exports.Chat.splitFirst = lib_1.Utils.splitFirst;
exports.Chat.sendPM = exports.Chat.PrivateMessages.send.bind(exports.Chat.PrivateMessages);
...
function start(processCount) {
    if (Config.usesqlite) {
        ...
        exports.Chat.databaseReadyPromise = exports.Chat.prepareDatabase();
    }
    exports.Chat.PrivateMessages.start(processCount);
