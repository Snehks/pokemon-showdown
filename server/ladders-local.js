...
class LadderStore {
    ...
    async load() {
        ...
        try {
            const data = await (0, lib_1.FS)('config/ladders/' + this.formatid + '.tsv').readIfExists();
            const ladder = [];
    ...
    async save() {
        ...
        const stream = (0, lib_1.FS)(`config/ladders/${this.formatid}.tsv`).createWriteStream();
    ...
    async updateRating(p1name, p2name, p1score, room) {
        ...
        try {
            ...
            if (!reasons.startsWith('-'))
                reasons = '+' + reasons;
            room.addRaw(lib_1.Utils.html `${p1name}'s rating: ${Math.round(p1elo)} &rarr; <strong>${Math.round(p1newElo)}</strong><br />(${reasons})`);
            reasons = `${Math.round(p2newElo) - Math.round(p2elo)} for ${p2score > 0.9 ? 'winning' : (p2score < 0.1 ? 'losing' : 'tying')}`;
            if (!reasons.startsWith('-'))
                reasons = '+' + reasons;
            room.addRaw(lib_1.Utils.html `${p2name}'s rating: ${Math.round(p2elo)} &rarr; <strong>${Math.round(p2newElo)}</strong><br />(${reasons})`);
            room.update();
        }
        catch (e) {
            if (!room.battle)
...
exports.LadderStore = LadderStore;
