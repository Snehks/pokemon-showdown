...
exports.pages = exports.commands = void 0;
const path = __importStar(require("path"));
const child_process = __importStar(require("child_process"));
const lib_1 = require("../../lib");
function hasDevAuth(user) {
...
exports.commands = {
    potd(target, room, user) {
        ...
        if (!species.exists)
            throw new Chat.ErrorMessage(`Pokemon "${target}" not found.`);
    ...
    addhtmlbox(target, room, user, connection, cmd) {
        ...
        target = Chat.collapseLineBreaksHTML(target);
        if (user.tempGroup !== '*') {
            target += lib_1.Utils.html `<div class="gray" style="float:right;font-size:8pt">[${user.name}]</div><div style="clear:both"></div>`;
        }
        return `/raw <div class="infobox">${target}</div>`;
    },
    addhtmlboxhelp: [
    ...
    addrankhtmlbox(target, room, user, connection, cmd) {
        ...
        if (user.tempGroup !== '*') {
            html += lib_1.Utils.html `<div class="gray" style="float:right;font-size:8pt">[${user.name}]</div><div style="clear:both"></div>`;
        }
        room.sendRankedUsers(`|html|<div class="infobox">${html}</div>`, rank);
    },
    addrankhtmlboxhelp: [
    ...
    addrankuhtml(target, room, user, connection, cmd) {
        ...
        room.sendRankedUsers(html, rank);
    },
    addrankuhtmlhelp: [
    ...
    async setnamecolor(target, room, user, connection, cmd) {
        ...
        if (cmd.startsWith('d')) {
            ...
            throw new Chat.ErrorMessage(`Specify a source username to take the color from. Name must be <19 characters.`);
        }
        ...
        if (source) {
            return this.sendReply(`|html|<username>${userid}</username>'s namecolor was ` +
    ...
    highlighthtmlpage(target, room, user) {
        ...
        let [pageid, title, highlight] = lib_1.Utils.splitFirst(rest, ',', 2);
        pageid = `${user.id}-${toID(pageid)}`;
        if (!pageid || !target)
    ...
    sendprivatehtmlbox(target, room, user, connection, cmd) {
        ...
        if (!html)
            return this.parse('/help sendprivatehtmlbox');
        html = `${lib_1.Utils.html `<div class="gray" style="font-size:8pt">[Private from ${user.name}]</div>`}${Chat.collapseLineBreaksHTML(html)}`;
        if (plainHtml)
    ...
    async hotpatch(target, room, user, connection, cmd) {
        ...
        target = toID(target);
        try {
            lib_1.Utils.clearRequireCache({ exclude: ['/lib/process-manager'] });
            if (target === 'all') {
                if (lock['all']) {
                ...
                if (lock['processmanager']) {
                    throw new Chat.ErrorMessage(`Hot-patching formats has been disabled by ${lock['processmanager'].by} ` +
                        `(${lock['processmanager'].reason})`);
                }
                this.sendReply('Hotpatching processmanager prototypes...');
                // keep references
                const cache = { ...require.cache };
                lib_1.Utils.clearRequireCache();
                const newPM = require('../../lib/process-manager');
                require.cache = cache;
                const protos = [
                    [lib_1.ProcessManager.QueryProcessManager, newPM.QueryProcessManager],
                    [lib_1.ProcessManager.StreamProcessManager, newPM.StreamProcessManager],
                    [lib_1.ProcessManager.ProcessManager, newPM.ProcessManager],
                    [lib_1.ProcessManager.RawProcessManager, newPM.RawProcessManager],
                    [lib_1.ProcessManager.QueryProcessWrapper, newPM.QueryProcessWrapper],
                    [lib_1.ProcessManager.StreamProcessWrapper, newPM.StreamProcessWrapper],
                    [lib_1.ProcessManager.RawProcessManager, newPM.RawProcessWrapper],
                ...
                this.sendReply(`DONE`);
                this.sendReply(`Updated ${Chat.count(counts.updated, 'methods')}` +
                ...
                (0, lib_1.FS)('config/custom.css').unwatch();
            ...
            Rooms.global.notifyRooms(['development', 'staff'], `|c|${user.getIdentity()}|/log ${user.name} used /hotpatch ${target} - but something failed while trying to hot-patch.`);
            throw new Chat.ErrorMessage([`Something failed while trying to hot-patch ${target}:`, e.stack]);
        }
        Rooms.global.notifyRooms(['development', 'staff'], `|c|${user.getIdentity()}|/log ${user.name} used /hotpatch ${target}`);
    },
    hotpatchhelp: [
    ...
    nohotpatch(target, room, user, connection, cmd) {
        ...
        if (!reason || !target.includes(separator))
            return this.parse('/help nohotpatch');
        const lock = Monitor.hotpatchLock;
        const validDisable = [
        ...
        if (enable) {
            if (!lock[hotpatch])
                throw new Chat.ErrorMessage(`Hot-patching ${hotpatch} is not disabled.`);
        ...
        Rooms.global.notifyRooms(['development', 'staff', 'upperstaff'], `|c|${user.getIdentity()}|/log ${user.name} has ${enable ? 'enabled' : 'disabled'} hot-patching ${hotpatch}. Reason: ${reason}`);
    },
    nohotpatchhelp: [
    ...
    async processes(target, room, user) {
        ...
        for (const manager of lib_1.ProcessManager.processManagers) {
    ...
    async savelearnsets(target, room, user, connection) {
        ...
        await (0, lib_1.FS)('data/learnsets.js').write(`'use strict';\n\nexports.Learnsets = {\n` +
            Object.entries(Dex.data.Learnsets).map(([id, entry]) => (`\t${id}: {learnset: {\n` +
                lib_1.Utils.sortBy(Object.entries(Dex.species.getLearnsetData(id).learnset), ([moveid]) => moveid).map(([moveid, sources]) => (`\t\t${moveid}: ["` + sources.join(`", "`) + `"],\n`)).join('') +
                `\t}},\n`)).join('') +
            `};\n`);
    ...
    disableladder(target, room, user) {
        ...
        const innerHTML = (`<b>Due to technical difficulties, the ladder has been temporarily disabled.</b><br />` +
    ...
    enableladder(target, room, user) {
        ...
        const innerHTML = (`<b>The ladder is now back.</b><br />` +
    ...
    autolockdownkill(target, room, user) {
        this.checkCan('lockdown');
        if (Config.autolockdown === undefined)
    ...
    async kill(target, room, user) {
        ...
        if (!noSave) {
            ...
            for (const u of Users.users.values()) {
                u.send(`|pm|~|${u.getIdentity()}|/raw <div class="broadcast-red"><b>The server is restarting soon.</b><br />` +
    ...
    loadbanlist(target, room, user, connection) {
        ...
        Punishments.loadBanlist().then(() => connection.sendTo(room, "ipbans.txt has been reloaded."), error => connection.sendTo(room, `Something went wrong while loading ipbans.txt: ${error}`));
    },
    loadbanlisthelp: [
    ...
    async updateserver(target, room, user, connection) {
        ...
        if (target === 'private') {
            ...
            success = success && await updateserver(this, lib_1.FS.ROOT_PATH);
    ...
    async updateloginserver(target, room, user) {
        ...
        if (err) {
            Rooms.global.notifyRooms(['staff', 'development'], `|c|${user.getIdentity()}|/log ${user.name} used /updateloginserver - but something failed while updating.`);
            throw new Chat.ErrorMessage([err.message, err.stack || '']);
        }
        if (!result)
            throw new Chat.ErrorMessage('No result received.');
        ...
        Rooms.global.notifyRooms(['staff', 'development'], `|c|${user.getIdentity()}|/log ${message}`);
    },
    ...
    async updateclient(target, room, user) {
        ...
        if (err) {
            Rooms.global.notifyRooms(['staff', 'development'], `|c|${user.getIdentity()}|/log ${user.name} used /updateclient - but something failed while updating.`);
            throw new Chat.ErrorMessage([err.message, err.stack || '']);
        }
        if (!result)
            throw new Chat.ErrorMessage('No result received.');
    ...
    async eval(target, room, user, connection) {
        ...
        const generateHTML = (direction, contents) => (`<table border="0" cellspacing="0" cellpadding="0"><tr><td valign="top">` +
            lib_1.Utils.escapeHTML(direction).repeat(2) +
        ...
        try {
            ...
            if (result?.then) {
                ...
                this.update();
                result = `Promise -> ${lib_1.Utils.visualize(await result)}`;
    ...
    async evalsql(target, room) {
        ...
        this.runBroadcast(true);
        if (!Config.usesqlite)
            throw new Chat.ErrorMessage(`SQLite is disabled.`);
        const logRoom = Rooms.get('upperstaff') || Rooms.get('staff');
        if (!target)
            throw new Chat.ErrorMessage(`Specify a database to access and a query.`);
        const [db, query] = lib_1.Utils.splitFirst(target, ',').map(item => item.trim());
        if (!(0, lib_1.FS)('./databases').readdirSync().includes(`${db}.db`)) {
        ...
        this.sendReply(`|html|<table border="0" cellspacing="0" cellpadding="0"><tr><td valign="top">SQLite&gt; [${db}.db] &nbsp;</td>` +
            `<td>${Chat.getReadmoreCodeBlock(query)}</td></tr><table>`);
        logRoom?.roomlog(`SQLite> ${target}`);
        const database = (0, lib_1.SQL)('evalsql', module, {
        ...
        function formatResult(result) {
            if (!Array.isArray(result)) {
                return (`<table border="0" cellspacing="0" cellpadding="0"><tr><td valign="top">` +
            ...
            buffer += `</th></tr><tr>`;
            buffer += result.map(item => (`<td>${Object.values(item).map(val => lib_1.Utils.escapeHTML(val)).join('</td><td>')}</td>`)).join('</tr><tr>');
            buffer += `</tr></table></div>`;
            return buffer;
        }
    ...
    editbattle(target, room, user) {
        ...
        switch (cmd) {
            ...
            case 'h':
                ...
                void battle.stream.write(`>eval let p=pokemon('${player}', '${pokemon}');p.sethp(${parseInt(value)});` +
            ...
            case 's':
                ...
                void battle.stream.write(`>eval let pl=player('${player}');let p=pokemon(pl,'${pokemon}');p.setStatus('${value}');if (!p.isActive){battle.add('','please ignore the above');battle.add('-status',pl.active[0],pl.active[0].status,'[silent]');}`);
                break;
            case 'pp':
                if (targets.length !== 4) {
                    this.errorReply("Incorrect command use");
                ...
                void battle.stream.write(`>eval pokemon('${player}','${pokemon}').getMoveData('${move}').pp = ${parseInt(value)};`);
                break;
            case 'boost':
            case 'b':
                if (targets.length !== 4) {
                ...
                void battle.stream.write(`>eval let p=pokemon('${player}','${pokemon}');battle.boost({${stat}:${parseInt(value)}},p)`);
                break;
            case 'volatile':
            case 'v':
                if (targets.length !== 3) {
                ...
                [player, pokemon, value] = targets.map(toID);
                void battle.stream.write(`>eval pokemon('${player}','${pokemon}').addVolatile('${value}')`);
                break;
            case 'sidecondition':
            case 'sc':
                if (targets.length !== 2) {
...
exports.pages = {
    bot(args, user, connection) {
        ...
        bot.sendTo(room ? room.roomid : 'lobby', `|pm|${user.getIdentity()}|${bot.getIdentity()}||requestpage|${user.name}|${pageid}`);
    },
};
