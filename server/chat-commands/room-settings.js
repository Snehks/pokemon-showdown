...
exports.pages = exports.roomSettings = exports.commands = exports.RoomSections = exports.sections = void 0;
/**
...
exports.commands = {
    ...
    roomsettings(target, room, user, connection) {
        ...
        let output = lib_1.Utils.html `<div class="infobox">Room Settings for ${room.title}<br />`;
        for (const handler of Chat.roomSettings) {
            ...
            for (const option of setting.options) {
                // disabled button
                if (option[1] === true) {
                    output += lib_1.Utils.html `<button class="button disabled" style="font-weight:bold;color:#575757;` +
                    ...
                    if (!setting.permission)
                        continue;
                    output += lib_1.Utils.html `<button class="button" name="send" value="/roomsetting ${option[1]}">${option[0]}</button> `;
                }
            }
            output += `<br />`;
        }
    ...
    modchat(target, room, user) {
        ...
        if (room.settings.modchat && room.settings.modchat.length <= 1 &&
        ...
        if (room.battle?.forcedSettings.modchat && !user.can('rangeban')) {
            throw new Chat.ErrorMessage(`This battle is required to have modchat on due to one of the players having a username that starts with ` +
        ...
        if (!room.settings.modchat) {
            ...
            const modchatSetting = lib_1.Utils.escapeHTML(room.settings.modchat);
    ...
    automodchat(target, room, user) {
        ...
        if (this.meansNo(toID(target))) {
            ...
            if (room.modchatTimer)
                clearTimeout(room.modchatTimer); // fallback just in case (should never happen)
            this.privateModAction(`${user.name} turned off automatic modchat.`);
            return this.modlog(`AUTOMODCHAT`, null, 'OFF');
        }
        let [rawTime, rank] = lib_1.Utils.splitFirst(target, ',').map(i => i.trim());
        if (!rawTime) {
        ...
        const validGroups = [...Config.groupsranking, 'trusted', 'autoconfirmed'];
    ...
    modjoin(target, room, user) {
        ...
        if (room.battle) {
            ...
            if (room.battle.forcedSettings.privacy) {
                throw new Chat.ErrorMessage(`This battle is required to be public due to a player having a name prefixed by '${room.battle.forcedSettings.privacy}'.`);
            }
        ...
        if (target === 'sync' && !room.settings.modchat) {
            const lowestGroup = Config.groupsranking.find(group => {
                ...
                return (groupInfo.symbol !== Users.Auth.defaultSymbol() &&
    ...
    permissions: {
        ...
        set(target, room, user) {
            ...
            if (Users.Auth.ROOM_PERMISSIONS.includes(perm) &&
    ...
    linkfilter(target, room, user) {
        room = this.requireRoom();
        if (!target) {
            return this.sendReply(`This room's link filter is currently: ${room.settings.filterEmojis ? "ON" : "OFF"}`);
        }
        this.checkChat();
        this.checkCan('editroom', null, room);
        if (this.meansYes(target)) {
    ...
    banword: {
        ...
        add(target, room, user, connection, cmd) {
            ...
            if (!regex) {
                words = words.map(word => {
                    ...
                    return lib_1.Utils.escapeRegex(word);
                });
            }
        ...
        delete(target, room, user) {
            ...
            if (!room.settings.banwords.length)
                room.settings.banwords = undefined;
            this.sendReply(room.settings.banwords ?
    ...
    showapprovals(target, room, user) {
        ...
        if (this.meansNo(target)) {
            ...
            if (!room.settings.permissions || room.settings.permissions['/show'] === '@') {
                this.privateModAction(`Note: Due to this room's settings, Drivers aren't allowed to use /show directly, ` +
    ...
    makechatroom(target, room, user, connection, cmd) {
        ...
        if (cmd === 'makeprivatechatroom') {
            ...
            if (upperStaffRoom) {
                upperStaffRoom.add(`|raw|<div class="broadcast-green">Private chat room created: <b>${lib_1.Utils.escapeHTML(target)}</b></div>`).update();
            }
            ...
            if (staffRoom) {
                staffRoom.add(`|raw|<div class="broadcast-green">Public chat room created: <b>${lib_1.Utils.escapeHTML(target)}</b></div>`).update();
            }
            const upperStaffRoom = Rooms.get('upperstaff');
            if (upperStaffRoom) {
                upperStaffRoom.add(`|raw|<div class="broadcast-green">Public chat room created: <b>${lib_1.Utils.escapeHTML(target)}</b></div>`).update();
            }
    ...
    makegroupchat(target, room, user, connection, cmd) {
        ...
        const titleMsg = lib_1.Utils.html `Welcome to ${parent ? room.title : user.name}'s` +
            lib_1.Utils.html `${!/^[0-9]+$/.test(title) ? ` ${title}` : ''}${parent ? ' subroom' : ''} groupchat!`;
    ...
    deregisterchatroom(target, room, user) {
        ...
        if (Rooms.global.deregisterChatRoom(id)) {
            ...
            target = lib_1.Utils.escapeHTML(target);
            if (isPrivate) {
                if (upperStaffRoom) {
    ...
    deleteroom(target, room, user, connection, cmd) {
        ...
        if (room.persist) {
            if (room.settings.isPrivate) {
                const upperStaffRoom = Rooms.get('upperstaff');
                if (upperStaffRoom) {
                    upperStaffRoom.add(lib_1.Utils.html `|raw|<div class="broadcast-red">Private chat room ` +
                        `deleted by ${user.id}: <b>${title}</b></div>`).update();
                }
            }
            else {
                const staffRoom = Rooms.get('staff');
                if (staffRoom) {
                    staffRoom.add(lib_1.Utils.html `|raw|<div class="broadcast-red">Public chat room deleted: <b>${title}</b></div>`).update();
                }
                const upperStaffRoom = Rooms.get('upperstaff');
                if (upperStaffRoom) {
                    upperStaffRoom.add(lib_1.Utils.html `|raw|<div class="broadcast-red">Public chat ` +
                        `room deleted by ${user.id}: <b>${title}</b></div>`).update();
                }
            }
        }
    ...
    renameroom(target, room, user, connection, cmd) {
        ...
        if (!isGroupchat) {
            Rooms.global.notifyRooms(room.settings.isPrivate === true ? ['upperstaff'] : ['upperstaff', 'staff'], lib_1.Utils.html `|raw|<div class="broadcast-green">${privacy} chat room <b>${oldTitle}</b> renamed to <b>${target}</b></div>`);
        }
        room.add(lib_1.Utils.html `|raw|<div class="broadcast-green">The room has been renamed to <b>${target}</b></div>`).update();
    },
    ...
    subrooms(target, room, user, connection, cmd) {
        ...
        const subRoomText = subRooms.map(subRoom => lib_1.Utils.html `<a href="/${subRoom.roomid}">${subRoom.title}</a><br/><small>${subRoom.settings.desc}</small>`);
        return this.sendReplyBox(`<p style="font-weight:bold;">${lib_1.Utils.escapeHTML(room.title)}'s subroom${Chat.plural(subRooms)}:</p><ul><li>${subRoomText.join('</li><br/><li>')}</li></ul></strong>`);
    },
    subroomhelp: [
    ...
    roomdesc(target, room, user) {
        ...
        if (!target) {
            ...
            this.sendReplyBox(lib_1.Utils.html `The room description is: ${room.settings.desc}`);
            return;
        }
        this.checkCan('makeroom');
        if (target.length > 80) {
    ...
    roomintro(target, room, user, connection, cmd) {
        ...
        if (!target) {
            ...
            if (!this.broadcasting && user.can('declare', null, room, 'roomintro') && cmd !== 'topic') {
                const code = lib_1.Utils.escapeHTML(room.settings.introMessage).replace(/\n/g, '<br />');
    ...
    staffintro(target, room, user, connection, cmd) {
        ...
        if (!target) {
            ...
            if (user.can('ban', null, room, 'staffintro') && cmd !== 'stafftopic') {
                const code = lib_1.Utils.escapeHTML(room.settings.staffMessage).replace(/\n/g, '<br />');
    ...
    removeroomalias(target, room, user) {
        ...
        if (aliasIndex >= 0) {
            ...
            if (!room.settings.aliases.length)
                room.settings.aliases = undefined;
    ...
    roomtierdisplay(target, room, user, connection, cmd) {
        ...
        if (!target) {
            if (!this.runBroadcast())
                return;
            return this.sendReplyBox(`This room is currently displaying ${room.settings.dataCommandTierDisplay} as the tier when using /data.`);
        }
        this.checkCan('declare', null, room);
    ...
    roomsection(target, room, user) {
        room = this.requireRoom();
        const sectionNames = exports.RoomSections.sectionNames;
        if (!target) {
            if (!this.runBroadcast())
                return;
            this.sendReplyBox(lib_1.Utils.html `This room is ${room.settings.section ? `in the ${sectionNames[room.settings.section]} section` : `not in a section`}.`);
            return;
        }
    ...
    roomsectionhelp: [
        ...
        `Valid sections: ${exports.sections.join(', ')}`,
    ],
...
exports.roomSettings = [
    ...
    room => ({
        label: "Language",
        permission: 'editroom',
        options: [...Chat.languages].map(([id, name]) => [name, id === (room.settings.language || 'english') || `roomlanguage ${id || 'off'}`]),
    }),
    room => ({
    ...
    room => ({
        ...
        options: ['off', 5, 10, 20, 30, 60].map(time => [`${time}`, time === (room.settings.slowchat || 'off') || `slowchat ${time || 'off'}`]),
    }),
    room => ({
...
exports.pages = {
    permissions(args, user, connection) {
        ...
        for (const permission in permissions) {
            ...
            if (room.auth.atLeast(user, '#')) {
                buf += roomGroups.map(group => (requiredRank === group ?
                    lib_1.Utils.html `<button class="button disabled" style="font-weight:bold;color:#575757;background:#d3d3d3">${group}</button>` :
                    lib_1.Utils.html `<button class="button" name="send" value="/msgroom ${room.roomid},/permissions set ${permission}, ${group}">${group}</button>`)).join(' ');
            }
            else {
                buf += lib_1.Utils.html `<button class="button disabled" style="font-weight:bold;color:#575757;background:#d3d3d3">${requiredRank}</button>`;
            }
            buf += `</td>`;
        }
        if (!atLeastOne) {
