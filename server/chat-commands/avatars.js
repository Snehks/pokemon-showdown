...
exports.commands = exports.Avatars = void 0;
...
try {
    const configAvatars = JSON.parse((0, lib_1.FS)(AVATARS_FILE).readSync());
    ...
    (0, lib_1.FS)(AVATARS_FILE).writeSync(JSON.stringify(customAvatars));
}
...
function saveCustomAvatars(instant) {
    (0, lib_1.FS)(AVATARS_FILE).writeUpdate(() => JSON.stringify(customAvatars), { throttle: instant ? null : 60000 });
}
exports.Avatars = new class {
    constructor() {
    ...
    exists(avatar) {
        if (avatar.includes('.')) {
            return (0, lib_1.FS)(`config/avatars/${avatar}`).isFile();
        }
        if (!avatar.startsWith('#')) {
            return OFFICIAL_AVATARS.has(avatar);
        }
        return (0, lib_1.Net)(exports.Avatars.src(avatar)).get().then(() => true).catch(() => false);
    }
    convert(avatar) {
    ...
    img(avatar, noAlt) {
        ...
        if (!src)
            return Chat.h("strong", null,
                Chat.h("code", null, avatar));
        return Chat.h("img", { src: src, alt: noAlt ? '' : avatar, width: "80", height: "80", class: "pixelated", style: { verticalAlign: 'middle' } });
    }
    getDefault(userid) {
    ...
    tryNotify(user) {
        ...
        if (entry?.notNotified) {
            user.send(`|pm|~|${user.getIdentity()}|/raw ` +
                Chat.html `${Chat.h(Chat.Fragment, null,
                    Chat.h("p", null, "You have a new custom avatar!"),
                    Chat.h("p", null, entry.allowed.map(avatar => avatar && [exports.Avatars.img(avatar), ' '])),
                    "Use ",
                    Chat.h("button", { class: "button", name: "send", value: "/avatars" },
                        Chat.h("code", null, "/avatars")),
...
function listUsers(users) {
    return users.flatMap((userid, i) => [i ? ', ' : null, Chat.h("username", { class: "username" }, userid)]);
}
...
exports.commands = {
    avatar(target, room, user) {
        ...
        if (!silent || this.broadcasting) {
            ...
            this.sendReply(Chat.html `|raw|${exports.Avatars.img(avatar)}`);
    ...
    avatars(target, room, user) {
        ...
        if (targetUserids) {
            ...
            for (const id of targetUserids) {
                ...
                if (allowed) {
                    out.push(Chat.h("p", null,
                        "Custom avatars from account ",
                        Chat.h("strong", null, id),
                        ":"), allowed.filter(Boolean).map(avatar => (Chat.h("p", null,
                        hasButton ? (Chat.h("button", { name: "send", value: `/avatar ${avatar}`, class: "button" }, exports.Avatars.img(avatar))) : (exports.Avatars.img(avatar)),
                        " ",
                        Chat.h("code", null,
                            "/avatar ",
                            avatar.replace('#', ''))))));
            ...
            if (!out.length && target) {
                out.push(Chat.h("p", null,
                    "User ",
                    Chat.h("strong", null, toID(target)),
                    " doesn't have any custom avatars."));
            }
        }
        if (!out.length) {
            out.push(Chat.h("p", null, "Custom avatars require you to be a contributor/staff or win a tournament prize."));
        }
        this.sendReplyBox(Chat.h(Chat.Fragment, null,
            !target && [Chat.h("p", null,
                    "You can ",
                    Chat.h("button", { name: "avatars", class: "button" }, "change your avatar"),
                    " by clicking on it in the ",
                    Chat.h("button", { name: "openOptions", class: "button", "aria-label": "Options" },
                        Chat.h("i", { class: "fa fa-cog" })),
                    " menu in the upper ",
                    "right."), Chat.h("p", null,
                    "Avatars from generations other than 4-5 are hidden. You can find them in this ",
                    Chat.h("a", { href: "https://play.pokemonshowdown.com/sprites/trainers/" },
                        Chat.h("strong", null, "full list of avatars")),
                    ". ",
                    "You can use them by typing ",
                    Chat.h("code", null,
                        "/avatar ",
                        Chat.h("i", null, "[avatar's name]")),
                    " into any chat. For example, ",
                    Chat.h("code", null, "/avatar erika-gen2"),
                    ".")],
            out));
    },
    avatarshelp: [
    ...
    async defaultavatar(target, room, user) {
        ...
        this.sendReplyBox(Chat.h("div", null,
            exports.Avatars.img(avatar),
            Chat.h("br", null),
            "Added to ",
            Chat.h("username", { class: "username" }, inputUsername)));
    },
    ...
    removeavatar(target, room, user) {
        ...
        if (avatar) {
            ...
            this.sendReplyBox(Chat.h("div", null,
                allowed.map(curAvatar => [exports.Avatars.img(curAvatar), ' ']),
                Chat.h("br", null),
                "Removed from ",
                Chat.h("username", { class: "username" }, inputUsername)));
        }
    ...
    async avatarusers(target, room, user) {
        ...
        this.sendReplyBox(Chat.h(Chat.Fragment, null,
            Chat.h("p", null, exports.Avatars.img(target, true)),
            Chat.h("p", null,
                Chat.h("code", null, target),
                Chat.h("br", null),
                users ? listUsers(users) : Chat.h("p", null, "No users currently allowed to use this avatar"))));
    },
    ...
    async masspavatar(target, room, user) {
        ...
        this.sendReplyBox(Chat.h("div", null,
            userids.map(userid => exports.Avatars.img('#' + userid)),
            Chat.h("br", null),
            "Added ",
            userids.length,
            " avatars"));
    ...
    async massxmasavatar(target, room, user) {
        ...
        this.sendReplyBox(Chat.h("div", null,
            userids.map(userid => exports.Avatars.img(`#${userid}xmas`)),
            Chat.h("br", null),
            "Added ",
            userids.length,
            " avatars"));
    ...
    async massgavatar(target, room, user) {
        ...
        for (const avatar in toUpdate) {
            ...
            out.push(Chat.h("p", null, exports.Avatars.img(avatar, true)));
            out.push(Chat.h("div", null,
                Chat.h("code", null, avatar)));
            if (added.length)
                out.push(Chat.h("div", null,
                    oldUsers.size ? 'Added' : 'New',
                    ": ",
                    listUsers(added)));
            if (removed.length)
                out.push(Chat.h("div", null,
            ...
            if (!added.length && !removed.length)
                out.push(Chat.h("div", null, "No change"));
        }
        this.sendReplyBox(Chat.h(Chat.Fragment, null, out));
        exports.Avatars.save(true);
    },
};
Users.Avatars = exports.Avatars;
Chat.multiLinePattern.register('/massgavatar', '/masspavatar', '/massxmasavatar');
