...
exports.pages = exports.handlers = exports.commands = exports.formatsDataCache = void 0;
exports.getCommonBattles = getCommonBattles;
exports.findFormats = findFormats;
exports.getFormatResources = getFormatResources;
...
const youtube_1 = require("../chat-plugins/youtube");
...
function getCommonBattles(userID1, user1, userID2, user2, connection) {
    ...
    for (const curRoom of Rooms.rooms.values()) {
        if (!curRoom.battle)
            continue;
        if ((user1?.inRooms.has(curRoom.roomid) || curRoom.auth.get(userID1) === Users.PLAYER_SYMBOL) &&
...
function findFormats(targetId, isOMSearch = false) {
    ...
    for (const format of formatList) {
        ...
        if (!targetId || format[targetId + 'Show'] || sectionId === targetId ||
...
exports.formatsDataCache = new Map();
async function getFormatResources(format) {
    const cached = exports.formatsDataCache.get(format);
    if (cached !== undefined)
        return cached;
    try {
        const raw = await (0, lib_1.Net)(`https://www.smogon.com/dex/api/formats/by-ps-name/${format}`).get();
        const data = JSON.parse(raw);
        exports.formatsDataCache.set(format, data);
        return data;
    }
    catch {
        // some sort of json error or request can't be made
        // so something on smogon's end. freeze the request, punt
        exports.formatsDataCache.set(format, null);
        return null;
    }
}
// clear every 15 minutes to ensure it's only minimally stale
const resourceRefreshInterval = setInterval(() => exports.formatsDataCache.clear(), 15 * 60 * 1000);
function destroy() {
...
exports.commands = {
    ...
    whois(target, room, user, connection, cmd) {
        ...
        let buf = lib_1.Utils.html `<strong class="username"><small style="display:none">${targetUser.tempGroup}</small>${targetUser.name}</strong> `;
        ...
        if (!targetUser.connected)
            buf += ` <em style="color:gray">(offline)</em>`;
        const roomauth = room?.auth.getDirect(targetUser.id);
        if (roomauth && Config.groups[roomauth]?.name) {
            buf += lib_1.Utils.html `<br />${Config.groups[roomauth].name} (${roomauth})`;
        }
        if (Config.groups[targetUser.tempGroup]?.name) {
            buf += lib_1.Utils.html `<br />Global ${Config.groups[targetUser.tempGroup].name} (${targetUser.tempGroup})`;
        }
        if (Users.globalAuth.sectionLeaders.has(targetUser.id)) {
            buf += lib_1.Utils.html `<br />Section Leader (${room_settings_1.RoomSections.sectionNames[Users.globalAuth.sectionLeaders.get(targetUser.id)]})`;
        }
        ...
        if (canViewAlts) {
            let prevNames = targetUser.previousIDs.map(userid => {
                ...
                if (!punishments || !user.can('alts'))
                    return userid;
                return punishments.map(punishment => (`${userid}${punishment ? ` (${Punishments.punishmentTypes.get(punishment.type)?.desc || `punished`}` +
                    `${punishment.id !== targetUser.id ? ` as ${punishment.id}` : ``})` : ``}`)).join(' | ');
            }).join(", ");
            if (prevNames)
                buf += lib_1.Utils.html `<br />Previous names: ${prevNames}`;
            for (const targetAlt of targetUser.getAltUsers(true)) {
                ...
                const punishMsg = !user.can('alts') ? '' : punishments.map(punishment => (` (${Punishments.punishmentTypes.get(punishment.type)?.desc || 'punished'}` +
                    `${punishment.id !== targetAlt.id ? ` as ${punishment.id}` : ''})`)).join(' | ');
                buf += lib_1.Utils.html `<br />Alt: <span class="username">${targetAlt.name}</span>${punishMsg}`;
                ...
                prevNames = targetAlt.previousIDs.map(userid => {
                    ...
                    if (!p || !user.can('alts'))
                        return userid;
                    return p.map(cur => `${userid} (${Punishments.punishmentTypes.get(cur.type)?.desc || 'punished'}` + `${cur.id !== targetAlt.id ? ` as ${cur.id}` : ``})`).join(' | ');
                }).join(", ");
                if (prevNames)
                    buf += `<br />Previous names: ${prevNames}`;
            }
        }
            ...
            if (targetUser.namelocked) {
                ...
                if (punishment) {
                    ...
                    if (punishment.reason)
                        buf += lib_1.Utils.html ` (reason: ${punishment.reason})`;
                }
            }
            ...
            if (user.can('lock')) {
                ...
                if (battlebanned) {
                    ...
                    if (battlebanned.reason)
                        buf += lib_1.Utils.html ` (reason: ${battlebanned.reason})`;
                }
                ...
                if (groupchatbanned) {
                    ...
                    if (groupchatbanned.reason)
                        buf += lib_1.Utils.html ` (reason: ${groupchatbanned.reason})`;
                }
                ...
                if (ticketbanned) {
                    ...
                    if (ticketbanned.reason)
                        buf += lib_1.Utils.html ` (reason: ${ticketbanned.reason})`;
                }
            }
        ...
        if (user === targetUser ? user.can('ipself') : user.can('ip', targetUser)) {
            ...
            if (user.tempGroup !== ' ' && targetUser.latestHost) {
                buf += lib_1.Utils.html `<br />Host: ${targetUser.latestHost} [${targetUser.latestHostType}]`;
            }
        }
        ...
        if (gameRooms.length) {
            buf += `<br />Recent games: ${gameRooms.map(id => {
                ...
                return lib_1.Utils.html `<a href="/${id}">${shortId}</a>`;
            }).join(' | ')}`;
        }
        if (canViewPunishments) {
    ...
    offlinewhois(target, room, user) {
        ...
        if (!userid)
            throw new Chat.ErrorMessage("Please enter a valid username.");
        const targetUser = Users.get(userid);
        let buf = lib_1.Utils.html `<strong class="username">${target}</strong>`;
        if (!targetUser?.connected)
            buf += ` <em style="color:gray">(offline)</em>`;
        ...
        if (Users.globalAuth.sectionLeaders.has(userid)) {
            buf += `<br />Section Leader (${room_settings_1.RoomSections.sectionNames[Users.globalAuth.sectionLeaders.get(userid)]})`;
        }
        buf += `<br /><br />`;
        ...
        if (idPunishments) {
            for (const p of idPunishments) {
                ...
                if (expiresIn)
                    buf += expiresIn;
                if (reason)
                    buf += lib_1.Utils.html ` (reason: ${reason})`;
                buf += '<br />';
                atLeastOne = true;
            }
        }
    ...
    sharedbattles(target, room) {
        ...
        this.sendReplyBox(lib_1.Utils.html `Common battles between ${targetUsername1} and ${targetUsername2}:<br />` + battles.map(id => {
            const shortId = id.startsWith('battle-') ? id.slice(7) : id;
            return lib_1.Utils.html `<a href="/${id}">${shortId}</a>`;
        }).join(' | '));
    },
    ...
    async host(target, room, user, connection, cmd) {
        ...
        if (!net.isIPv4(target))
            throw new Chat.ErrorMessage('You must pass a valid IPv4 IP to /host.');
    ...
    checkchallenges(target, room, user) {
        ...
        if (!chall) {
            return this.sendReplyBox(lib_1.Utils.html `${user1.name} and ${user2.name} are not challenging each other.`);
        }
        const [from, to] = user1.id === chall.from ? [user1, user2] : [user2, user1];
        this.sendReplyBox(lib_1.Utils.html `${from.name} is challenging ${to.name} in ${Dex.formats.get(chall.format).name}.`);
    },
            ...
            switch (newTarget.searchType) {
                ...
                case 'pokemon':
                    ...
                    if (format?.onModifySpecies) {
                        pokemon = format.onModifySpecies.call({ dex, clampIntRange: lib_1.Utils.clampIntRange, toID }, pokemon) || pokemon;
                    }
                    ...
                    if (showDetails) {
                        ...
                        if (dex.gen === 1)
                            details["Crit Rate"] = `${((pokemon.baseStats.spe * 100) / 512).toFixed(2)}%`;
    ...
    detailshelp() {
        this.sendReplyBox(`<code>/details [Pok\u00e9mon/item/move/ability/nature]</code>: get additional details on this Pok\u00e9mon/item/move/ability/nature.<br />` +
    ...
    weakness(target, room, user) {
        ...
        if (species.name !== "" && !species.exists && type1.name !== "" && !type1.exists) {
            ...
            if (typeSearchResults && typeSearchResults[0].name !== "") {
                ...
                return this.sendReplyBox(lib_1.Utils.html `${originalSearch} isn't a recognized type or Pokemon${Dex.gen > dex.gen ? ` in Gen ${dex.gen}` : ""}.`);
            }
        }
    ...
    effectiveness(target, room, user) {
        ...
        if (targets.length !== 2)
            throw new Chat.ErrorMessage("Attacker and defender must be separated with a comma.");
    ...
    coverage(target, room, user) {
        ...
        if (sources.length > 4)
            throw new Chat.ErrorMessage("Specify a maximum of 4 moves or types.");
    ...
    statcalc(target, room, user) {
        if (!target)
            return this.parse("/help statcalc");
        if (!this.runBroadcast())
            return;
        ...
        for (const arg of targets) {
            ...
            if (!lvlSet) {
                if (lowercase === 'lc') {
                    ...
                    lvlSet = true;
                    if (isNaN(level)) {
                        return this.sendReplyBox('Invalid value for level: ' + lib_1.Utils.escapeHTML(arg));
                    }
            ...
            if (!ivSet) {
                if (lowercase.endsWith('iv') || lowercase.endsWith('ivs')) {
                    ...
                    ivSet = true;
                    if (isNaN(iv)) {
                        return this.sendReplyBox('Invalid value for IVs: ' + lib_1.Utils.escapeHTML(arg));
                    }
                    continue;
                }
            }
            if (!evSet) {
                if (lowercase === 'invested' || lowercase === 'max') {
                    ...
                    evSet = true;
                    if (isNaN(ev)) {
                        return this.sendReplyBox('Invalid value for EVs: ' + lib_1.Utils.escapeHTML(arg));
                    }
                    if (ev > 255 || ev < 0) {
            ...
            if (!modSet) {
                ...
                if (isNaN(modifier)) {
                    return this.sendReplyBox('Invalid value for modifier: ' + lib_1.Utils.escapeHTML(String(modifier)));
                }
                if (modifier > 6) {
            ...
            if (!realSet) {
                if (lowercase.endsWith('real')) {
                    ...
                    realSet = true;
                    if (isNaN(realStat)) {
                        return this.sendReplyBox('Invalid value for target real stat: ' + lib_1.Utils.escapeHTML(arg));
                    }
                    if (realStat < 0) {
        ...
        if (realSet) {
            if (!baseSet) {
                if (calcHP) {
                    ...
                    baseStat = Math.ceil((100 * Math.ceil(realStat) - nature * (level * (Math.floor(ev / 4) + iv) + 500)) /
                        (2 * level * nature));
                }
                if (baseStat < 0) {
    ...
    uptime(target, room, user) {
        ...
        if (uptime > 24 * 60 * 60) {
            ...
            if (uptimeHours)
                uptimeText += `, ${uptimeHours} ${uptimeHours === 1 ? "hour" : "hours"}`;
        }
        else {
    ...
    groups(target, room, user) {
        ...
        this.sendReplyBox((showRoom ? roomRanks.map(str => this.tr(str)).join('<br />') : ``) +
    ...
    punishments(target, room, user) {
        ...
        this.sendReplyBox((showRoom ? roomPunishments.map(str => this.tr(str)).join('<br />') : ``) +
            (showRoom && showGlobal ? `<br /><br />` : ``) +
    ...
    opensource(target, room, user) {
        if (!this.runBroadcast())
            return;
        this.sendReplyBox(`Pok&eacute;mon Showdown is open source:<br />` +
    ...
    bugs(target, room, user) {
        ...
        if (room?.battle) {
            ...
            this.sendReplyBox(`Have a replay showcasing a bug on Pok&eacute;mon Showdown?<br />` +
    ...
    intro(target, room, user) {
        if (!this.runBroadcast())
            return;
        this.sendReplyBox(`New to competitive Pok&eacute;mon?<br />` +
    ...
    smogintro(target, room, user) {
        if (!this.runBroadcast())
            return;
        this.sendReplyBox(`Welcome to Smogon's official simulator! The <a href="https://www.smogon.com/forums/forums/intro_hub">Information & Resources forum</a> can help you get integrated into the community.<br />` +
    ...
    calc(target, room, user, connection, cmd) {
        ...
        if (RANDOMS_CALC_COMMANDS.includes(cmd) ||
            ...
            return this.sendReplyBox(`Random Battles damage calculator. (Courtesy of dhelmise &amp; jetou)<br />` +
        ...
        if (BATTLESPOT_CALC_COMMANDS.includes(cmd) || (isBattleSpotBattle && !DEFAULT_CALC_COMMANDS.includes(cmd))) {
            return this.sendReplyBox(`Battle Spot damage calculator. (Courtesy of cant say &amp; LegoFigure11)<br />` +
                `- <a href="https://cantsay.github.io/">Battle Spot Damage Calculator</a>`);
        }
        this.sendReplyBox(`Pok&eacute;mon Showdown! damage calculator. (Courtesy of Honko, Austin, dhelmise, &amp; jetou)<br />` +
    ...
    cap(target, room, user) {
        if (!this.runBroadcast())
            return;
        this.sendReplyBox(`An introduction to the Create-A-Pok&eacute;mon project:<br />` +
    ...
    async formathelp(target, room, user, connection, cmd) {
        if (!target && this.runBroadcast()) {
            return this.sendReplyBox(`- <a href="https://www.smogon.com/forums/forums/smogon-metagames.725/">Smogon Metagames</a><br />` +
        ...
        if (!totalMatches)
            return this.errorReply("No matched formats found.");
        const format = totalMatches === 1 ? Dex.formats.get(Object.values(sections)[0].formats[0]) : null;
        if (!this.runBroadcast(`!formathelp ${format ? format.id : target}`))
            return;
        if (format) {
            const rules = [];
            let rulesetHtml = '';
            if (['Format', 'Rule', 'ValidatorRule'].includes(format.effectType)) {
                if (format.ruleset?.length) {
                    rules.push(`<b>Ruleset</b> - ${lib_1.Utils.escapeHTML(format.ruleset.join(", "))}`);
                }
                if (format.banlist?.length) {
                    rules.push(`<b>Bans</b> - ${lib_1.Utils.escapeHTML(format.banlist.join(", "))}`);
                }
                if (format.unbanlist?.length) {
                    rules.push(`<b>Unbans</b> - ${lib_1.Utils.escapeHTML(format.unbanlist.join(", "))}`);
                }
                if (format.restricted?.length) {
                    rules.push(`<b>Restricted</b> - ${lib_1.Utils.escapeHTML(format.restricted.join(", "))}`);
                }
                if (rules.length > 0) {
        ...
        for (const sectionId in sections) {
            buf.push(lib_1.Utils.html `<th style="border:1px solid gray" colspan="2">${sections[sectionId].name}</th>`);
            for (const section of sections[sectionId].formats) {
                const subformat = Dex.formats.get(section);
                const nameHTML = lib_1.Utils.escapeHTML(subformat.name);
    ...
    roomhelp(target, room, user) {
        ...
        if (!this.runBroadcast(false, '!htmlbox'))
            return;
        const strings = [
            [
        ...
        this.sendReplyBox(strings.map(par => par.map(string => this.tr(string)).join('<br />')).join('<br /><br />'));
    },
    ...
    rules(target, room, user, connection, cmd) {
        if (!target) {
            if (!this.runBroadcast())
                return;
            this.sendReplyBox(`${room ? this.tr `Please follow the rules:` + '<br />' : ``}` +
                `${room?.settings.rulesLink ? lib_1.Utils.html `- <a href="${room.settings.rulesLink}">${this.tr `${room.title} room rules`}</a><br />` : ``}` +
        ...
        if (possibleRoom) {
            ...
            return this.sendReplyBox(`${possibleRoom.title}'s rules:<br />` +
                `${rulesLink ? lib_1.Utils.html `- <a href="${rulesLink}">${this.tr `${possibleRoom.title} room rules`}</a><br />` : `None set.`}`);
        }
        ...
        if (target === 'delete' || target === 'remove') {
            if (!room.settings.rulesLink)
                throw new Chat.ErrorMessage(`This room does not have rules set to remove.`);
    ...
    smogdex(target, room, user) {
        if (!target)
            return this.parse('/help smogdex');
        if (!this.runBroadcast())
            return;
        const targets = target.split(',');
        ...
        if (pokemon.exists) {
            ...
            if (['ou', 'uu'].includes(formatId) && generation === 'sm' &&
                ...
                this.sendReplyBox(lib_1.Utils.html `<a href="https://www.smogon.com/dex/${generation}/pokemon/${id}/${formatId}/?lang=${supportedLanguages[room.settings.language]}">${generation.toUpperCase()} ${formatName} ${pokemon.name} analysis</a>, brought to you by <a href="https://www.smogon.com">Smogon University</a>`);
            }
            else if (['ou', 'uu'].includes(formatId) && generation === 'sm') {
                this.sendReplyBox(lib_1.Utils.html `<a href="https://www.smogon.com/dex/${generation}/pokemon/${id}/${formatId}">${generation.toUpperCase()} ${formatName} ${pokemon.name} analysis</a>, brought to you by <a href="https://www.smogon.com">Smogon University</a><br />` +
                ...
                this.sendReplyBox(lib_1.Utils.html `<a href="https://www.smogon.com/dex/${generation}/pokemon/${id}${(formatId ? '/' + formatId : '')}">${generation.toUpperCase()} ${formatName} ${pokemon.name} analysis</a>, brought to you by <a href="https://www.smogon.com">Smogon University</a>`);
            }
        }
        // Item
        ...
        if (format.id) {
            ...
            if (formatName) {
                atLeastOne = true;
                this.sendReplyBox(lib_1.Utils.html `<a href="https://www.smogon.com/dex/${generation}/formats/${formatId}">${generation.toUpperCase()} ${formatName} format analysis</a>, brought to you by <a href="https://www.smogon.com">Smogon University</a>`);
            }
        }
        if (!atLeastOne) {
    ...
    bulbapedia(target, broadcast, user) {
        if (!target)
            return this.parse('/help bulbapedia');
        if (!this.runBroadcast())
            return;
        ...
        if (pokemon.exists) {
            ...
            if (pokemon.id.startsWith('flabebe'))
                baseSpecies = 'Flabébé';
            const link = `${baseLink}${encodeURIComponent(baseSpecies)}_(Pokémon)`;
            this.sendReplyBox(lib_1.Utils.html `<a href="${link}">${pokemon.name} in-game information</a>, provided by Bulbapedia`);
        }
        // Item
        if (item.exists) {
            atLeastOne = true;
            ...
            if (Dex.moves.get(item.name).exists)
                link += '_(item)';
            this.sendReplyBox(lib_1.Utils.html `<a href="${link}">${item.name} item description</a>, provided by Bulbapedia`);
        }
        // Ability
        if (ability.exists) {
            atLeastOne = true;
    ...
    dice(target, room, user) {
        if (!target || /[^\d\sdHL+-]/i.test(target))
            return this.parse('/help dice');
        if (!this.runBroadcast(true))
            return;
        ...
        let offsetFragment = "";
        if (offset)
            offsetFragment += `${offset > 0 ? ` + ${offset}` : offset}`;
        if (diceQuantity === 1)
            return this.sendReplyBox(`Rolling (1 to ${diceFaces})${offsetFragment}: ${rollSum}`);
        const outlierFragment = removeOutlier ? ` except ${removeOutlier > 0 ? "highest" : "lowest"}` : ``;
        const rollsFragment = trackRolls ? ": " + rolls.join(", ") : "";
        return this.sendReplyBox(`${diceQuantity} rolls (1 to ${diceFaces})${rollsFragment}<br />` +
    ...
    pickrandom(target, room, user) {
        if (!target?.includes(','))
            return this.parse('/help pick');
        if (!this.runBroadcast(true))
            return false;
        if (this.broadcasting) {
            [, target] = lib_1.Utils.splitFirst(this.message, ' ');
        }
        const options = target.split(',');
        const pickedOption = options[Math.floor(Math.random() * options.length)].trim();
        return this.sendReplyBox(lib_1.Utils.html `<em>We randomly picked:</em> ${pickedOption}`);
    },
    ...
    shuffle(target, room, user) {
        if (!target?.includes(','))
            return this.parse('/help shuffle');
        const args = target.split(',');
        if (!this.runBroadcast(true))
            return false;
        const results = lib_1.Utils.shuffle(args.map(arg => arg.trim()));
        return this.sendReplyBox(lib_1.Utils.html `<em>Shuffled:</em><br> ${results.join(', ')}`);
    },
    shufflehelp: [
    ...
    async requestshow(target, room, user) {
        ...
        if (user.can('showmedia', null, room, 'show'))
            throw new Chat.ErrorMessage(`Use !show instead.`);
        if (room.pendingApprovals?.has(user.id))
            throw new Chat.ErrorMessage('You have a request pending already.');
        if (!toID(target))
        ...
        room.sendMods(lib_1.Utils.html `|uhtml|request-${user.id}|<div class="infobox">${user.name} wants to show <a href="${link}">${link}</a><br>` +
            (comment ? lib_1.Utils.html `Comment: ${comment}<br>` : '') +
    ...
    async approveshow(target, room, user) {
        ...
        const userid = toID(target);
        if (!userid)
            return this.parse(`/help approveshow`);
        const request = room.pendingApprovals?.get(userid);
        if (!request)
            throw new Chat.ErrorMessage(`${userid} has no pending request.`);
        if (userid === user.id) {
        ...
        if (request.dimensions) { // image
            ...
            buf = lib_1.Utils.html `<img src="${request.link}" width="${width}" height="${height}" />`;
            if (resized)
                buf += lib_1.Utils.html `<br /><a href="${request.link}" target="_blank">full-size image</a>`;
        }
        else {
            buf = await youtube_1.YouTube.generateVideoDisplay(request.link, false);
            if (!buf)
                throw new Chat.ErrorMessage('Could not get YouTube video');
        }
        buf += lib_1.Utils.html `<br /><div class="infobox"><small>(Requested by ${request.name})</small>`;
        if (request.comment) {
            buf += lib_1.Utils.html `<br />${request.comment}</small></div>`;
        }
        else {
            buf += `</small></div>`;
        }
    ...
    denyshow(target, room, user) {
        ...
        target = toID(target);
        if (!target)
            return this.parse(`/help denyshow`);
        const entry = room.pendingApprovals?.get(target);
        if (!entry)
            throw new Chat.ErrorMessage(`${target} has no pending request.`);
    ...
    async show(target, room, user, connection) {
        ...
        const [link, comment] = lib_1.Utils.splitFirst(target, ',').map(f => f.trim());
        this.checkBroadcast();
        if (this.broadcastMessage) {
            if (room) {
        ...
        this.runBroadcast();
        let buf;
        if (youtube_1.YouTube.linkRegex.test(link)) {
            buf = await youtube_1.YouTube.generateVideoDisplay(link, false);
            this.message = this.message.replace(/&ab_channel=(.*)(&|)/ig, '').replace(/https:\/\/www\./ig, '');
        }
        else if (youtube_1.Twitch.linkRegex.test(link)) {
            const channelId = youtube_1.Twitch.linkRegex.exec(link)?.[2]?.trim();
            if (!channelId)
                throw new Chat.ErrorMessage(`Specify a Twitch channel.`);
            buf = lib_1.Utils.html `Watching <b><a class="subtle" href="https://twitch.tv/${toID(channelId)}">${channelId}</a></b>...<br />`;
            buf += lib_1.Utils.html `<twitch src="${link}" />`;
        }
        else {
            if (Chat.linkRegex.test(link)) {
                if (/^https?:\/\/(.*)\.(mp4|mov)\b(\?|$)/i.test(link)) { // video
                    ...
                    buf = lib_1.Utils.html `<video src="${link}" controls="" width="300px" height="300px"></video>`;
                }
                else if (/^https?:\/\/(.*)\.(mp3|wav)\b(\?|$)/i.test(link)) { // audio
                    buf = lib_1.Utils.html `<audio src="${link}" controls=""></audio>`;
                }
            }
            if (link.includes('data:image/png;base64')) {
            ...
            if (!buf) { // fall back on image
                try {
                    ...
                    buf = lib_1.Utils.html `<img src="${link}" width="${width}" height="${height}" />`;
                    if (resized)
                        buf += lib_1.Utils.html `<br /><a href="${link}" target="_blank">full-size image</a>`;
                }
                catch {
        ...
        if (comment) {
            ...
            buf += lib_1.Utils.html `<br />(${comment})</div>`;
        }
        this.sendReplyBox(buf);
    },
    showhelp: [
    ...
    async registertime(target, room, user, connection) {
        ...
        if (!user.autoconfirmed)
            throw new Chat.ErrorMessage(`Only autoconfirmed users can use this command.`);
        target = toID(target);
        if (!target)
            target = user.id;
        let rawResult;
        try {
            rawResult = await (0, lib_1.Net)(`https://${Config.routes.root}/users/${target}.json`).get();
        }
        catch (e) {
            if (e.message.includes('Not found'))
                throw new Chat.ErrorMessage(`User '${target}' is unregistered.`);
        ...
        const regTimeAgo = duration > 24 * 60 * 60 * 1000 ?
            ...
            'less than a day';
        this.sendReplyBox(lib_1.Utils.html `The user '${target}' registered ${regTimeAgo} ago, on the date ${date.toDateString()}.`);
    },
    registertimehelp: [`/registertime OR /regtime [user] - Find out when [user] registered.`],
    pi(target, room, user) {
        if (!this.runBroadcast())
            return false;
        return this.sendReplyBox('Did you mean: 1. 3.1415926535897932384626... (Decimal)<br />' +
    ...
    code(target, room, user, connection) {
        ...
        if (!target)
            return this.parse('/help code');
        if (target.length >= 8192)
            throw new Chat.ErrorMessage("Your code must be under 8192 characters long!");
        ...
        if (this.room?.settings.isPersonal !== false && this.shouldBroadcast()) {
            ...
            if (!target)
                throw new Chat.ErrorMessage(`Invalid code.`);
        }
    ...
    makecustomchallenge(target, room, user) {
        ...
        if (targetUserID) {
            ...
            this.connection.send(`>view-buildformat-${format.id}\n|selectorhtml|#output|` +
    ...
    adminhelp(target, room, user) {
        ...
        const canExecute = (perm) => !(
        // gotta make sure no lower group has it
        Object.values(Config.groups).slice(1).some(f => f[perm]));
        cmds = cmds.filter(f => f.requiredPermission && canExecute(f.requiredPermission) && f.fullCmd !== this.handler?.fullCmd);
        cmds = lib_1.Utils.sortBy(cmds, f => f.fullCmd);
        ...
        namespaces = new Map(lib_1.Utils.sortBy([...namespaces], ([, cmdList]) => cmdList.length === 1));
    ...
    randtopic(target, room, user) {
        ...
        this.runBroadcast();
        this.sendReply(lib_1.Utils.html `|html|<div class="broadcast-blue">${lib_1.Utils.randomElement(room.settings.topics)}</div>`);
    },
    randtopichelp: [
...
exports.handlers = {
    onRename(user, oldID, newID) {
        if (oldID === newID || !Config.usesqlite || [oldID, newID].some(f => f.startsWith('guest')))
            return;
        void Chat.database.run(`REPLACE INTO alts_log (to_id, from_id, ip) VALUES (?, ?, ?)`, [newID, oldID, user.latestIp]);
    },
};
exports.pages = {
    async altslog(query, user) {
        ...
        const results = await Chat.database.all('SELECT to_id, from_id, ip FROM alts_log WHERE (to_id = ? OR from_id = ?) LIMIT ?', [target, target, num]);
        let buf = `<div class="pad"><h2>Alternate accounts for ${target}</h2>`;
        buf += `${results.length} found.<hr />`;
        const ipTable = {};
        const userids = new Set();
        const useridToIp = new Map();
    ...
    topics(query, user) {
        ...
        for (const [i, topic] of topics.entries()) {
            buf += lib_1.Utils.html `<li>${i + 1}: "${topic}"</li>`;
        }
        buf += `</ul></div>`;
        return buf;
    },
    battlerules(query, user) {
        ...
        rulesets.push(`</table></div>`, `<h3>Value rules</h3>`, `<ul><li>Value rules are formatted like [Name] = [value], e.g. "Force Monotype = Water" or "Min Team Size = 4"</li>`, `<li>To remove a value rule, use <code>![rule name]</code>.</li>`, `<li>To override another value rule, use <code>!! [Name] = [new value]</code>. For example, overriding the Min Source Gen on [Gen 8] VGC 2021 from 8 to 3 would look like <code>!! Min Source Gen = 3</code>.</li></ul>`, `<div class="ladder"><table><tr><th>Rule Name</th><th>Description</th></tr>`);
        for (const rule of rules) {
            if (!rule.hasValue)
                continue;
            const desc = rule.desc ? rule.desc : "No description.";
    ...
    punishments(query, user) {
        ...
        this.checkCan('mute', null, room);
        // Ascending order
        const sortedPunishments = lib_1.Utils.sortBy([...Punishments.getPunishments(room.roomid)], ([id, entry]) => (entry.expireTime));
        const sP = new Map();
    ...
    globalpunishments(query, user) {
        ...
        this.checkCan('lock');
        // Ascending order
        const sortedPunishments = lib_1.Utils.sortBy([...Punishments.getPunishments()], ([id, entry]) => (entry.expireTime));
        const sP = new Map();
    ...
    approvals(args) {
        ...
        if (!room.pendingApprovals)
            room.pendingApprovals = new Map();
        if (room.pendingApprovals.size < 1)
...
process.nextTick(() => {
    Dex.includeData();
    Chat.multiLinePattern.register('/htmlbox', '/quote', '/addquote', '!htmlbox', '/addhtmlbox', '/addrankhtmlbox', '/adduhtml', '/changeuhtml', '/addrankuhtmlbox', '/changerankuhtmlbox', '/addrankuhtml', '/addhtmlfaq', '/sendhtmlpage');
});
