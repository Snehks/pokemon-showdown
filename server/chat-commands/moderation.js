...
exports.runCrisisDemote = runCrisisDemote;
/**
 * Moderation commands
...
function runCrisisDemote(userid) {
    ...
    if (section) {
        from.push(`Section Leader (${room_settings_1.RoomSections.sectionNames[section] || section})`);
...
exports.commands = {
    roomowner(target, room, user) {
        ...
        if (rest)
            throw new Chat.ErrorMessage(`This command does not support specifying a reason.`);
        ...
        if (room.auth.getDirect(userid) === '#')
            throw new Chat.ErrorMessage(`${targetUsername} is already a room owner.`);
        room.auth.set(userid, '#');
    ...
    roompromote(target, room, user, connection, cmd) {
        ...
        for (const toPromote of users) {
            ...
            else if (nextSymbol in Config.groups && oldSymbol in Config.groups &&
    ...
    authority(target, room, user, connection) {
        ...
        const buffer = lib_1.Utils.sortBy(Object.entries(rankLists), ([symbol]) => -Users.Auth.getGroup(symbol).rank).map(([symbol, names]) => (`${(Config.groups[symbol] ? `**${Config.groups[symbol].name}s** (${symbol})` : symbol)}:\n` +
            lib_1.Utils.sortBy(names, name => toID(name)).join(", ")));
        if (!showAll)
    ...
    roomauth(target, room, user, connection, cmd) {
        ...
        const buffer = lib_1.Utils.sortBy(Object.entries(rankLists), ([symbol]) => -Users.Auth.getGroup(symbol).rank).map(([symbol, names]) => {
            ...
            return `${group}:\n` +
                lib_1.Utils.sortBy(names).map(userid => {
    ...
    userauth(target, room, user, connection) {
        ...
        if (sectionLeader) {
            buffer.push(`Section leader: ${room_settings_1.RoomSections.sectionNames[sectionLeader]}`);
        }
    ...
    sectionleaders(target, room, user, connection) {
        ...
        for (sectionid in sections) {
            if (!sections[sectionid].size)
                continue;
            buffer.push(`**${room_settings_1.RoomSections.sectionNames[sectionid]}**:\n` + lib_1.Utils.sortBy([...sections[sectionid]]).join(', '));
        }
        if (!buffer.length)
    ...
    async autojoin(target, room, user, connection) {
        ...
        const autojoins = [];
        const promises = targets.map(roomid => user.tryJoinRoom(roomid, connection).then(ret => {
        ...
        if (!targetRoom) {
            if (target.startsWith('view-')) {
                ...
                if (target.startsWith('view-bot-')) {
                    ...
                    if (bot)
                        bot.sendTo(null, `|pm|${user.getIdentity()}|${botId}||closepage|${user.name}|${pageId}`);
                }
    ...
    warn(target, room, user) {
        ...
        const globalWarn = (!room || ['staff', 'adminlog'].includes(room.roomid) ||
        ...
        if (!targetUser?.connected) {
            ...
            this.addGlobalModAction(`${targetID} was warned by ${user.name} while offline.${publicReason ? ` (${publicReason})` : ``}`);
        ...
        if (targetUser.can('makeroom'))
            throw new Chat.ErrorMessage("You are not allowed to warn upper staff members.");
        const now = Date.now();
    ...
    redir(target, room, user, connection) {
        ...
        if (targetRoom.roomid === "global")
            throw new Chat.ErrorMessage(`Users cannot be redirected to the global room.`);
    ...
    mute(target, room, user, connection, cmd) {
        ...
        if (targetUser.can('makeroom'))
            throw new Chat.ErrorMessage("You are not allowed to mute upper staff members.");
    ...
    unmute(target, room, user) {
        ...
        const successfullyUnmuted = room.unmute(targetUser?.id || toID(targetUsername), `Your mute in '${room.title}' has been lifted.`);
    ...
    ban(target, room, user, connection, cmd) {
        ...
        if (targetUser.can('makeroom'))
            throw new Chat.ErrorMessage("You are not allowed to ban upper staff members.");
        ...
        if (targetUser.trusted) {
            if (!force) {
                return this.sendReply(`${name} is a trusted user. If you are sure you would like to ban them, use /force${week ? 'week' : 'room'}ban.`);
            }
        }
        else if (force) {
        ...
        if (targetUser.id in room.users || user.can('lock')) {
            targetUser.popup(`|modal||html|<p>${lib_1.Utils.escapeHTML(user.name)} has banned you from the room ${room.roomid} ` +
                `${(room.subRooms ? ` and its subrooms` : ``)}${week ? ' for 1 week' : ' for 2 days'}.` +
                `</p>${(publicReason ? `<p>Reason: ${lib_1.Utils.escapeHTML(publicReason)}</p>` : ``)}` +
        ...
        this.addModAction(`${name} was banned${week ? ' for 1 week' : ' for 2 days'} from ${room.title}` +
    ...
    async lock(target, room, user, connection, cmd) {
        ...
        if (!targetUser && !Punishments.search(userid).length && !force) {
            throw new Chat.ErrorMessage(`User '${targetUsername}' not found. Use \`\`/force${month ? 'month' : (week ? 'week' : '')}lock\`\` if you need to to lock them anyway.`);
        }
        if (reason.length > MAX_REASON_LENGTH) {
            throw new Chat.ErrorMessage(`The reason is too long. It cannot exceed ${MAX_REASON_LENGTH} characters.`);
        }
        ...
        this.globalModlog((force ? `FORCE` : ``) + (week ? "WEEKLOCK" : (month ? "MONTHLOCK" : "LOCK")), targetUser || userid, privateReason);
    ...
    unlockname(target, room, user) {
        ...
        if (!punishment)
            throw new Chat.ErrorMessage("This name isn't locked.");
    ...
    unlockip(target, room, user) {
        ...
        for (const curUser of Users.findUsers([], [target])) {
            if ((range ? curUser.locked === '#rangelock' : !curUser.locked?.startsWith('#')) &&
    ...
    async globalban(target, room, user, connection, cmd) {
        ...
        const { privateReason, publicReason } = this.parseSpoiler(reason);
        targetUser?.popup(`|modal|${user.name} has globally banned you.${(publicReason ? `\n\nReason: ${publicReason}` : ``)} ` +
    ...
    deroomvoiceall(target, room, user) {
        ...
        if (!room.auth.size)
            throw new Chat.ErrorMessage("Room does not have roomauth.");
        if (!target) {
    ...
    async yearticketban(target, room, user) {
        ...
        this.addGlobalModAction(`${user.name} banned ${descriptor}${target} from opening tickets for a year` +
            `${reason ? ` (${reason})` : ""}`);
        this.globalModlog('YEARTICKETBAN', isIP ? null : target, reason, isIP ? target : undefined);
    },
    yearticketbanhelp: [
    ...
    banip(target, room, user, connection, cmd) {
        ...
        if (!reason)
            throw new Chat.ErrorMessage("/banip requires a ban reason");
        this.checkCan('rangeban');
        ...
        this.addGlobalModAction(`${user.name} ${duration}-banned the ${ipDesc}: ${reason}`);
        this.globalModlog(`${year ? "YEAR" : ""}RANGEBAN`, null, `${ip.endsWith('*') ? ip : `[${ip}]`}: ${reason}`);
    },
    baniphelp: [
    ...
    yearlockname(target, room, user) {
        this.checkCan('rangeban');
        const [targetUsername, rest] = lib_1.Utils.splitFirst(target, ',').map(k => k.trim());
    ...
    lockip(target, room, user, connection, cmd) {
        ...
        if (!reason)
            throw new Chat.ErrorMessage("/lockip requires a lock reason");
        this.checkCan('rangeban');
        ...
        this.globalModlog(`${year ? 'YEAR' : 'RANGE'}${type}`, null, `${ip.endsWith('*') ? ip : `[${ip}]`}: ${reason}`, ip.endsWith('*') ? `${ip.slice(0, -2)}` : ip);
    },
    lockiphelp: [
    ...
    modnote(target, room, user, connection) {
        ...
        if (['staff', 'upperstaff'].includes(room.roomid) ||
    ...
    promote(target, room, user, connection, cmd) {
        ...
        if (!cmd.startsWith('global')) {
            ...
            if (Config.groups[nextGroup].globalonly)
                throw new Chat.ErrorMessage(`Did you mean "/global${groupid}"?`);
    ...
    trustuser(target, room, user, connection, cmd) {
        ...
        if (untrust) {
            ...
            if (!Users.globalAuth.has(userid))
                throw new Chat.ErrorMessage(`User '${name}' is not trusted.`);
            if (targetUser) {
            ...
            if (!targetUser && !force)
                throw new Chat.ErrorMessage(`User '${name}' is offline. Use /force${cmd} if you're sure.`);
            if (currentGroup) {
                if (Users.globalAuth.has(userid)) {
                    if (currentGroup === Users.Auth.defaultSymbol())
                        throw new Chat.ErrorMessage(`User '${name}' is already trusted.`);
    ...
    sectionleader(target, room, user, connection, cmd) {
        ...
        if (Users.globalAuth.sectionLeaders.has(targetUser?.id || userid) && !demoting) {
            throw new Chat.ErrorMessage(`${name} is already a Section Leader of ${room_settings_1.RoomSections.sectionNames[section]}.`);
        }
        ...
        if (!demoting) {
            ...
            this.addGlobalModAction(`${name} was appointed Section Leader of ${room_settings_1.RoomSections.sectionNames[section]} by ${user.name}.`);
            this.globalModlog(`SECTION LEADER`, userid, section);
            targetUser?.popup(`You were appointed Section Leader of ${room_settings_1.RoomSections.sectionNames[section]} by ${user.name}.`);
        }
        else {
            const group = Users.globalAuth.get(userid);
            Users.globalAuth.deleteSection(userid);
            this.privateGlobalModAction(`${name} was demoted from Section Leader of ${room_settings_1.RoomSections.sectionNames[section]} by ${user.name}.`);
            if (group === ' ')
            ...
            targetUser?.popup(`You were demoted from Section Leader of ${room_settings_1.RoomSections.sectionNames[section]} by ${user.name}.`);
        }
        if (targetUser) {
            targetUser.updateIdentity();
            Rooms.global.checkAutojoin(targetUser);
    ...
    sectionleaderhelp: [
        ...
        `Valid sections: ${room_settings_1.RoomSections.sections.join(', ')}`,
    ...
    declare(target, room, user) {
        ...
        if (target.length > 2000)
            throw new Chat.ErrorMessage("Declares should not exceed 2000 characters.");
        for (const id in room.users) {
            room.users[id].sendTo(room, `|notify|${room.title} announcement!|${target}`);
        }
        this.add(lib_1.Utils.html `|raw|<div class="broadcast-blue"><b>${target}</b></div>`);
    ...
    htmldeclare(target, room, user) {
        ...
        for (const u in room.users) {
            Users.get(u)?.sendTo(room, `|notify|${room.title} announcement!|${lib_1.Utils.stripHTML(target)}`);
        }
    ...
    notifyrank(target, room, user, connection, cmd) {
        ...
        if (!(rank in Config.groups))
            throw new Chat.ErrorMessage(`Group '${rank}' does not exist.`);
        ...
        if (cmd === 'notifyoffrank') {
            ...
            if (notification.length > 300)
                throw new Chat.ErrorMessage(`Notifications should not exceed 300 characters.`);
    ...
    forcerename(target, room, user) {
        ...
        Rooms.global.notifyRooms(['staff'], `|html|${roomMessage}` + lib_1.Utils.html `<span class="username">${targetUser?.name || targetID}</span> ${rankMessage} ${forceRenameMessage}`);
    ...
    noforcerename: {
        add(target, room, user) {
            const [targetUsername, rest] = lib_1.Utils.splitFirst(target, ',').map(f => f.trim());
            const targetId = toID(targetUsername);
            if (!targetId)
    ...
    forceclearstatus(target, room, user) {
        ...
        if (!targetUser.userMessage)
            throw new Chat.ErrorMessage(this.tr `${targetUser.name} does not have a status set.`);
    ...
    async namelock(target, room, user, connection, cmd) {
        ...
        const userid = toID(targetUsername);
        if (!targetUser && !force) {
            throw new Chat.ErrorMessage(`User '${targetUsername}' not found. Use \`\`/force${week ? 'week' : ''}namelock\`\` if you need to namelock them anyway.`);
        }
    ...
    hidetext(target, room, user, connection, cmd) {
        ...
        if (/^[0-9]+\s*(,|$)/.test(reason)) {
            if (hasLineCount) {
                let lineCountString;
                [lineCountString, reason] = lib_1.Utils.splitFirst(reason, ',').map(p => p.trim());
    ...
    blacklist(target, room, user, connection, cmd) {
        ...
        this.checkChat();
        if (toID(target) === 'show')
            throw new Chat.ErrorMessage(`You're looking for /showbl`);
        ...
        if (targetUser.trusted) {
            if (!force) {
                return this.sendReply(`${targetUser.name} is a trusted user. If you are sure you would like to blacklist them use /forceblacklist.`);
            }
        }
        else if (force) {
        ...
        if (targetUser.id in room.users || user.can('lock')) {
            targetUser.popup(`|modal||html|<p>${lib_1.Utils.escapeHTML(user.name)} has blacklisted you from the room ${room.roomid}${(room.subRooms ? ` and its subrooms` : '')}. Reason: ${lib_1.Utils.escapeHTML(reason)}</p>` +
        ...
        this.privateModAction(`${name} was blacklisted from ${room.title} by ${user.name}${expireTime ? ' for ten years' : ''}.` +
            `${reason ? ` (${reason})` : ''}`);
    ...
    async groupchatban(target, room, user, connection, cmd) {
        ...
        const createdGroupchats = await Punishments.groupchatBan(targetUser, (isMonth ? Date.now() + 30 * DAY : null), null, reason);
        ...
        for (const roomid of createdGroupchats) {
            ...
            if (!targetRoom)
                continue;
            const participants = targetRoom.warnParticipants?.(`This groupchat (${targetRoom.title}) has been deleted due to inappropriate conduct by its creator, ${targetUser.name}.` +
    ...
    blacklistname(target, room, user) {
        ...
        for (const userid of targets) {
            if (!userid)
                throw new Chat.ErrorMessage(`User '${userid}' is not a valid userid.`);
    ...
    unblacklistall(target, room, user) {
        ...
        if (!unblacklisted)
            throw new Chat.ErrorMessage("No users are currently blacklisted in this room to unblacklist.");
    ...
    showblacklist(target, room, user, connection, cmd) {
        ...
        if (!room)
            throw new Chat.ErrorMessage(`The room "${target}" was not found.`);
        this.checkCan('mute', null, room);
        const SOON_EXPIRING_TIME = 3 * 30 * 24 * 60 * 60 * 1000; // 3 months
        if (!room.persist)
            throw new Chat.ErrorMessage("This room does not support blacklists.");
        ...
        let buf = lib_1.Utils.html `Blacklist for ${room.title}${soonExpiring ? ` (expiring within 3 months)` : ''}:<br />`;
