...
exports.loginfilter = exports.pages = exports.commands = exports.crqHandlers = void 0;
...
exports.crqHandlers = {
    ...
    cmdsearch(target, user, trustable) {
        ...
        for (const command of Chat.allCommands()) {
            ...
            if (!!req &&
                !(command.hasRoomPermissions ? !!this.room && user.can(req, null, this.room) : user.can(req))) {
                continue;
            }
            const cmds = [
                command.fullCmd,
...
exports.commands = {
    async version(target, room, user) {
        ...
        let gitVersion;
        try {
            gitVersion = (await lib_1.ProcessManager.exec(['git', 'rev-parse', '--short', 'HEAD'])).stdout.trim();
        }
        catch { }
    ...
    userlist(target, room, user) {
        ...
        for (const id in room.users) {
            ...
            if (!curUser?.named)
                continue;
            userList.push(lib_1.Utils.escapeHTML(curUser.getIdentity(room)));
        }
    ...
    async linksmogon(target, room, user) {
        ...
        if (!user.registered) {
            throw new Chat.ErrorMessage("You must be registered in order to use this command. If you just registered, please refresh and try again.");
        }
    ...
    async msgroom(target, room, user, connection) {
        const [targetId, message] = lib_1.Utils.splitFirst(target, ',').map(i => i.trim());
        if (!targetId || !message) {
    ...
    msg(target, room, user, connection) {
        ...
        if (targetUsername === '~') {
            ...
            if (Chat.PrivateMessages.offlineIsEnabled) {
                ...
                user.lastCommand = 'pm';
                throw new Chat.ErrorMessage(this.tr `User ${targetUsername} is offline. Send the message again to confirm. If you are using /msg, use /offlinemsg instead.`);
            }
    ...
    async offlinemsg(target, room, user) {
        ...
        let [username, message] = lib_1.Utils.splitFirst(target, ',').map(i => i.trim());
    ...
    async rank(target, room, user) {
        ...
        buffer += lib_1.Utils.html `<div>User: <strong>${target}</strong></div>`;
    ...
    language(target, room, user) {
        ...
        return this.sendReply(`|html|` + this.tr `Pokémon Showdown will now be displayed in ${language} (except in language rooms).`);
    },
    languagehelp: [
    ...
    allowexportinputlog(target, room, user) {
        ...
        if (Object.keys(battle.playerTable).length === battle.allowExtraction[targetUser.id].size) {
            ...
            const inputLog = battle.inputLog.map(lib_1.Utils.escapeHTML).join(`<br />`);
        ...
        if (user.can('forcewin') || Dex.formats.get(battle.format).team) {
            ...
            for (const id in battle.playerTable) {
                ...
                if (playerUser.id === user.id) {
                    ...
                    playerUser.sendTo(room, lib_1.Utils.html `|html|${user.name} wants to extract the battle input log. <button name="send" value="/allowexportinputlog ${user.id}">Share your team and choices with "${user.name}"</button>`);
                }
            }
    ...
    importinputlog(target, room, user, connection) {
        ...
        if (!battleRoom)
            return; // createBattle will inform the user if creating the battle failed
    ...
    async showset(target, room, user, connection, cmd) {
        ...
        let resultString = lib_1.Utils.escapeHTML(Teams.export(team, { hideStats }));
        if (showAll) {
    ...
    acceptopenteamsheets(target, room, user, connection, cmd) {
        ...
        player.sendRoom(lib_1.Utils.html `|uhtmlchange|otsrequest|`);
    ...
    rejectopenteamsheets(target, room, user) {
        ...
        for (const otherPlayer of battle.players) {
            otherPlayer.sendRoom(lib_1.Utils.html `|uhtmlchange|otsrequest|`);
        }
        ...
        if (!battle.players.some(curPlayer => curPlayer.wantsTie)) {
            ...
            for (const otherPlayer of battle.players) {
                if (otherPlayer !== player) {
                    otherPlayer.sendRoom(lib_1.Utils.html `|uhtml|offertie|<button class="button" name="send" value="/accepttie"><strong>${this.tr `Accept tie`}</strong></button> <button class="button" name="send" value="/rejecttie">${this.tr `Reject`}</button>`);
                }
                else {
            ...
            player.sendRoom(lib_1.Utils.html `|uhtmlchange|offertie|`);
    ...
    rejecttie(target, room, user) {
        ...
        for (const otherPlayer of battle.players) {
            otherPlayer.sendRoom(lib_1.Utils.html `|uhtmlchange|offertie|`);
        }
    ...
    invitebattle(target, room, user, connection) {
        ...
        if (!targetUser.inRooms.has(room.roomid) || !player.hasTeam) {
            ...
            Ladders.challenges.add(new Ladders.BattleInvite(user.id, targetUser.id, ready, {
    ...
    forcewin(target, room, user) {
        ...
        if (!room.battle &&
            !(room.game && typeof room.game.tie === 'function' && typeof room.game.win === 'function')) {
    ...
    async search(target, room, user, connection) {
        if (target) {
            ...
            if (!user.registered && Config.forceregisterelo && await ladder.getRating(user.id) >= Config.forceregisterelo) {
                user.send(lib_1.Utils.html `|popup||html|${this.tr `Since you have reached ${Config.forceregisterelo} ELO in ${target}, you must register your account to continue playing that format on ladder.`}<p style="text-align: center"><button name="register" value="${user.id}"><b>${this.tr `Register`}</b></button></p>`);
                return false;
            }
    ...
    async accept(target, room, user, connection) {
        ...
        if (!gameRoom)
            return false;
        this.sendChatMessage(lib_1.Utils.html `/nonotify ${user.name} accepted the challenge, starting &laquo;<a href="/${gameRoom.roomid}">${gameRoom.roomid}</a>&raquo;`);
        return true;
    },
    ...
    trn(target, room, user, connection) {
        if (target === user.name)
            return false;
        const [name, registeredString, token] = lib_1.Utils.splitFirst(target, ',', 2);
...
process.nextTick(() => {
    ...
    Chat.multiLinePattern.register('>>>? ', '/(?:room|staff)intro ', '/(?:staff)?topic ', '/(?:add|widen)datacenters ', '/bash ', '!code ', '/code ', '/modnote ', '/mn ', '/eval', '!eval', '/evalbattle', '/evalsql', '>>sql', '/importinputlog ');
});
