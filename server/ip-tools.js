...
exports.IPTools = new class {
    ...
    ipToNumber(ip) {
        ...
        for (const part of parts) {
            num *= 256;
            const partAsInt = lib_1.Utils.parseExactInt(part);
    ...
    getCidrRange(cidr) {
        ...
        if (index <= 0) {
            const ip = exports.IPTools.ipToNumber(cidr);
            if (ip === null)
                return null;
            return { minIP: ip, maxIP: ip };
        }
        let low = exports.IPTools.ipToNumber(cidr.slice(0, index));
        const bits = lib_1.Utils.parseExactInt(cidr.slice(index + 1));
    ...
    stringToRange(range) {
        ...
        if (range.endsWith('*')) {
            ...
            if (parts.length > 3)
                return null;
            const [a, b, c] = parts;
            const minIP = exports.IPTools.ipToNumber(`${a || '0'}.${b || '0'}.${c || '0'}.0`);
            const maxIP = exports.IPTools.ipToNumber(`${a || '255'}.${b || '255'}.${c || '255'}.255`);
            if (minIP === null || maxIP === null)
                return null;
            return { minIP, maxIP };
        }
        ...
        if (index <= 0) {
            ...
            const ip = exports.IPTools.ipToNumber(range);
            if (ip === null)
                return null;
    ...
    async loadHostsAndRanges() {
        const data = await (0, lib_1.FS)(HOSTS_FILE).readIfExists() + await (0, lib_1.FS)(PROXIES_FILE).readIfExists();
        ...
        for (const row of rows) {
            ...
            switch (type) {
                ...
                case 'RANGE':
                    if (!host)
                        continue;
                    const minIP = exports.IPTools.ipToNumber(hostOrLowIP);
                    if (minIP === null) {
                        Monitor.error(`Bad IP address in host or proxy file: '${hostOrLowIP}'`);
                        continue;
                    }
                    const maxIP = exports.IPTools.ipToNumber(highIP);
                    if (maxIP === null) {
                        Monitor.error(`Bad IP address in host or proxy file: '${highIP}'`);
                        continue;
                    }
                    const range = { host: exports.IPTools.urlToHost(host), maxIP, minIP };
    ...
    saveHostsAndRanges() {
        ...
        for (const range of exports.IPTools.ranges) {
            const data = `RANGE,${exports.IPTools.rangeToString(range, ',')}${range.host ? `,${range.host}` : ``}\n`;
    ...
    checkRangeConflicts(insertion, sortedRanges, widen) {
        if (insertion.maxIP < insertion.minIP) {
            throw new Error(`Invalid data for address range ${exports.IPTools.rangeToString(insertion)} (${insertion.host})`);
        }
        let iMin = 0;
        ...
        if (iMin < sortedRanges.length) {
            ...
            if (insertion.minIP === next.minIP && insertion.maxIP === next.maxIP) {
                throw new Error(`The address range ${exports.IPTools.rangeToString(insertion)} (${insertion.host}) already exists`);
            }
            if (insertion.minIP <= next.minIP && insertion.maxIP >= next.maxIP) {
                if (widen) {
                ...
                throw new Error(`Too wide: ${exports.IPTools.rangeToString(insertion)} (${insertion.host})\n` +
                    `Intersects with: ${exports.IPTools.rangeToString(next)} (${next.host})`);
            }
            if (insertion.maxIP >= next.minIP) {
                throw new Error(`Could not insert: ${exports.IPTools.rangeToString(insertion)} ${insertion.host}\n` +
                    `Intersects with: ${exports.IPTools.rangeToString(next)} (${next.host})`);
            }
        }
        if (iMin > 0) {
            ...
            if (insertion.minIP >= prev.minIP && insertion.maxIP <= prev.maxIP) {
                throw new Error(`Too narrow: ${exports.IPTools.rangeToString(insertion)} (${insertion.host})\n` +
                    `Intersects with: ${exports.IPTools.rangeToString(prev)} (${prev.host})`);
            }
    ...
    sortRanges() {
        lib_1.Utils.sortBy(exports.IPTools.ranges, range => range.minIP);
    }
    ...
    async updateTorRanges() {
        try {
            const raw = await (0, lib_1.Net)('https://check.torproject.org/torbulkexitlist').get();
