...
const ladders_challenges_1 = require("./ladders-challenges");
/**
 * Keys are formatids
 */
...
class Ladder extends LadderStore {
    async prepBattle(connection, challengeType, team = null, isRated = false) {
        ...
        return new ladders_challenges_1.BattleReady(userid, this.formatid, settings, rating, challengeType);
    }
    ...
    async makeChallenge(connection, targetUser) {
        ...
        if (currentChallenges && currentChallenges.length >= 3 && !user.autoconfirmed) {
            connection.popup(`This user already has 3 pending challenges.\n` +
        ...
        const existingChall = exports.Ladders.challenges.search(user.id, targetUser.id);
        if (existingChall) {
            if (existingChall.from === targetUser.id &&
                existingChall.to === user.id &&
                existingChall.format === this.formatid &&
                existingChall.ready) {
                if (exports.Ladders.challenges.remove(existingChall)) {
                    exports.Ladders.match([existingChall.ready, ready]);
                    return true;
                }
            }
            else {
                connection.popup(`There's already a challenge (${existingChall.format}) between you and ${targetUser.name}!`);
                exports.Ladders.challenges.update(user.id, targetUser.id);
                return false;
            }
        }
        exports.Ladders.challenges.add(new ladders_challenges_1.BattleChallenge(user.id, targetUser.id, ready));
        exports.Ladders.challenges.send(user.id, targetUser.id, `/log ${user.name} wants to battle!`);
    ...
    static async acceptChallenge(connection, chall) {
        const ladder = (0, exports.Ladders)(chall.format);
        const ready = await ladder.prepBattle(connection, 'challenge');
        if (!ready)
            return;
        if (exports.Ladders.challenges.remove(chall)) {
            return exports.Ladders.match([chall.ready, ready]);
        }
    }
    cancelSearch(user) {
    ...
    matchmakingOK(matches) {
        ...
        if (searchRange > 300)
            searchRange = 300 + (searchRange - 300) / 10; // +1 every 3 sec after 300
        if (searchRange > 600)
            searchRange = 600;
    ...
    static periodicMatch() {
        ...
        for (const [formatid, formatTable] of exports.Ladders.searches) {
            ...
            const matchmaker = (0, exports.Ladders)(formatid);
            let longest = null;
            for (const search of formatTable.searches.values()) {
                if (!longest) {
...
const periodicMatchInterval = setInterval(() => Ladder.periodicMatch(), PERIODIC_MATCH_INTERVAL);
exports.Ladders = Object.assign(getLadder, {
    BattleReady: ladders_challenges_1.BattleReady,
    LadderStore,
    Ladder,
    BattleChallenge: ladders_challenges_1.BattleChallenge,
    GameChallenge: ladders_challenges_1.GameChallenge,
    BattleInvite: ladders_challenges_1.BattleInvite,
    ...
    match: Ladder.match,
    searches,
    challenges: ladders_challenges_1.challenges,
