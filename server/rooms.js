...
exports.Rooms = exports.GameRoom = exports.ChatRoom = exports.GlobalRoomState = exports.BasicRoom = void 0;
...
const room_settings_1 = require("./chat-commands/room-settings");
const room_battle_1 = require("./room-battle");
const room_battle_bestof_1 = require("./room-battle-bestof");
const room_game_1 = require("./room-game");
const room_minor_activity_1 = require("./room-minor-activity");
const roomlogs_1 = require("./roomlogs");
const user_groups_1 = require("./user-groups");
const modlog_1 = require("./modlog");
...
class BasicRoom {
    constructor(roomid, title, options = {}) {
        ...
        this.auth = new user_groups_1.RoomAuth(this);
        ...
        if (!options.auth)
            options.auth = {};
        this.log = roomlogs_1.Roomlogs.create(this, options);
        ...
        if (options.parentid) {
            this.setParent(exports.Rooms.get(options.parentid) || null);
        }
        this.subRooms = null;
        this.active = false;
        this.muteTimer = null;
    ...
    mute(user, setTime) {
        ...
        if (!setTime)
            setTime = 7 * 60000; // default time: 7 minutes
        if (setTime > 90 * 60000)
            setTime = 90 * 60000; // limit 90 minutes
    ...
    reportJoin(type, entry, user) {
        ...
        if (this.batchJoins) {
            ...
            if (!this.reportJoinsInterval) {
                this.reportJoinsInterval = setTimeout(() => this.update(), this.batchJoins);
            }
        }
        else {
            this.send(entry);
    ...
    getSubRooms(includeSecret = false) {
        if (!this.subRooms)
            return [];
        return [...this.subRooms.values()].filter(room => includeSecret ? true : !room.settings.isPrivate && !room.settings.isPersonal);
    }
    validateTitle(newTitle, newID, oldID) {
        ...
        if (newID !== oldID && exports.Rooms.search(newTitle))
    ...
    validateSection(section) {
        ...
        if (!room_settings_1.RoomSections.sections.includes(target)) {
            throw new Chat.ErrorMessage(`"${target}" is not a valid room section. Valid categories include: ${room_settings_1.RoomSections.sections.join(', ')}`);
        }
        return target;
    }
    setSection(section) {
        ...
        if (section) {
            ...
            if (oldSection === section) {
                throw new Chat.ErrorMessage(`${this.title}'s room section is already set to "${room_settings_1.RoomSections.sectionNames[oldSection]}".`);
            }
    ...
    runAutoModchat() {
        ...
        if (!staff.length) {
            ...
            if (!time || time < 5) {
                throw new Error(`Invalid time setting for automodchat (${lib_1.Utils.visualize(this.settings.autoModchat)})`);
            }
            ...
            this.modchatTimer = setTimeout(() => {
                ...
                `|raw|<div class="broadcast-blue"><strong>This room has had no active staff for ${time} minutes,` +
                    ` and has had modchat set to ${rank}.</strong></div>`).update();
                this.modlog({
...
class GlobalRoomState {
    constructor() {
        ...
        this.settingsList = [];
        try {
            this.settingsList = require((0, lib_1.FS)('config/chatrooms.json').path);
        ...
        if (Config.logladderip) {
            ...
            this.ladderIpLog = new lib_1.Streams.WriteStream({ write() { return undefined; } });
        }
        this.reportUserStatsInterval = setInterval(() => this.reportUserStats(), REPORT_USER_STATS_INTERVAL);
        // init users
        this.maxUsers = 0;
        this.maxUsersDate = 0;
        this.lockdown = false;
    ...
    start(processCount) {
        ...
        (0, room_battle_1.start)(processCount);
    }
    ...
    deserializeBattleRoom(battle) {
        ...
        const room = exports.Rooms.createBattle({
            format: formatid,
            inputLog,
            roomid,
            title,
    ...
    writeChatRoomData() {
        (0, lib_1.FS)('config/chatrooms.json').writeUpdate(() => (JSON.stringify(this.settingsList)
            .replace(/\{"title":/g, '\n{"title":')
            .replace(/\]$/, '\n]')), { throttle: 5000 });
    }
    writeNumRooms() {
        ...
        Monitor.logPath('lastbattle.txt').writeUpdate(() => `${this.lastWrittenBattle}`);
    }
    reportUserStats() {
    ...
    get configRankList() {
        ...
        for (const rank in Config.groups) {
            ...
            rankList.push({
                ...
                type: groupType
            }); // send the first character in the rank, incase they put a string several characters long
        }
        const typeOrder = ['punishment', 'normal', 'staff', 'leadership'];
        lib_1.Utils.sortBy(rankList, rank => -typeOrder.indexOf(rank.type));
    ...
    getRooms(user) {
        const roomsData = {
            chat: [],
            sectionTitles: Object.values(room_settings_1.RoomSections.sectionNames),
        ...
        for (const room of this.chatRooms) {
            ...
            if (room.parent)
                continue;
            if (room.settings.modjoin ||
                (room.settings.isPrivate && !['hidden', 'voice'].includes(room.settings.isPrivate)) ||
            ...
            const roomData = {
                ...
                section: room.settings.section ?
                    (room_settings_1.RoomSections.sectionNames[room.settings.section] || room.settings.section) : undefined,
    ...
    onCreateBattleRoom(players, room, options) {
        ...
        if (Config.reportbattles) {
            ...
            for (const roomid of Config.reportbattles) {
                const reportRoom = exports.Rooms.get(roomid);
                if (reportRoom) {
    ...
    deregisterChatRoom(id) {
        ...
        if (!room.persist)
            return false; // room isn't registered
    ...
    startLockdown(err = null, slow = false) {
        ...
        const stack = (err ? lib_1.Utils.escapeHTML(err.stack).split(`\n`).slice(0, 2).join(`<br />`) : ``);
    ...
    automaticKillRequest() {
        ...
        if (Config.autolockdown === undefined)
            Config.autolockdown = true; // on by default
        if (Config.autolockdown && exports.Rooms.global.lockdown === true && exports.Rooms.global.battleCount === 0) {
            ...
            setTimeout(() => {
                if (Config.autolockdown && exports.Rooms.global.lockdown === true) {
                    ...
                    this.notifyRooms(notifyPlaces, `|html|<div class="broadcast-red"><b>Automatic server lockdown kill canceled.</b><br /><br />In the last final seconds, the automatic lockdown was manually disabled.</div>`);
                }
            }, 10 * 1000);
        }
    }
    ...
    reportCrash(err, crasher = "The server") {
        ...
        const [stackFirst, stackRest] = lib_1.Utils.splitFirst(lib_1.Utils.escapeHTML(stack), `<br />`);
        ...
        if (devRoom) {
            ...
            exports.Rooms.lobby?.add(crashMessage).update();
            exports.Rooms.get('staff')?.add(crashMessage).update();
        }
...
class GameRoom extends BasicRoom {
    ...
    async uploadReplay(user, connection, options) {
        ...
        const hidden = options === 'auto' ? 10 :
            options === 'forpunishment' || this.unlistReplay ? 2 :
                isPrivate ? 1 :
                    0;
        if (isPrivate && hidden === 10) {
            password = (battle.password || (battle.password = replays_1.Replays.generatePassword()));
        }
        ...
        if (replays_1.Replays.db) {
            ...
            try {
                ...
                connection?.popup(`|html|<p>Your replay has been uploaded! It's available at:</p><p> ` +
...
function getRoom(roomid) {
    if (typeof roomid === 'string') {
        ...
        if ((roomid.startsWith('battle-') || roomid.startsWith('game-bestof')) && roomid.endsWith('pw')) {
            const room = exports.Rooms.rooms.get(roomid.slice(0, roomid.lastIndexOf('-')));
            if (room)
                return room;
        }
        return exports.Rooms.rooms.get(roomid);
    }
    return roomid;
}
exports.Rooms = {
    Modlog: modlog_1.mainModlog,
    /**
    ...
    createGameRoom(roomid, title, options) {
        if (exports.Rooms.rooms.has(roomid))
    ...
    createBattle(options) {
        ...
        if (exports.Rooms.global.lockdown === 'pre' && isBestOf && !options.isBestOfSubBattle) {
            for (const user of players) {
        ...
        if (exports.Rooms.global.lockdown === true && !options.isBestOfSubBattle) {
            for (const user of players) {
        ...
        if (format.gameType === 'multi') {
            ...
            roomid || (roomid = `game-bestof${isBestOf}-${format.id}-${++exports.Rooms.global.lastBattle}`);
        }
        ...
        roomid || (roomid = exports.Rooms.global.prepBattleRoom(options.format));
        ...
        if (options.isBestOfSubBattle || !isBestOf) {
            game = new room_battle_1.RoomBattle(room, options);
        }
        else {
            game = new room_battle_bestof_1.BestOfGame(room, options);
        }
        room.game = game;
    ...
    GameRoom,
    ChatRoom: BasicRoom,
    RoomGame: room_game_1.RoomGame,
    SimpleRoomGame: room_game_1.SimpleRoomGame,
    RoomGamePlayer: room_game_1.RoomGamePlayer,
    MinorActivity: room_minor_activity_1.MinorActivity,
    RETRY_AFTER_LOGIN,
    Roomlogs: roomlogs_1.Roomlogs,
    RoomBattle: room_battle_1.RoomBattle,
    BestOfGame: room_battle_bestof_1.BestOfGame,
    RoomBattlePlayer: room_battle_1.RoomBattlePlayer,
    RoomBattleTimer: room_battle_1.RoomBattleTimer,
    PM: room_battle_1.PM,
    Replays: replays_1.Replays,
};
