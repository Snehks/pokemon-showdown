...
exports.BestOfGame = exports.BestOfPlayer = void 0;
const lib_1 = require("../lib");
const room_game_1 = require("./room-game");
const BEST_OF_IN_BETWEEN_TIME = 40;
class BestOfPlayer extends room_game_1.RoomGamePlayer {
...
exports.BestOfPlayer = BestOfPlayer;
class BestOfGame extends room_game_1.RoomGame {
    ...
    nextGame() {
        ...
        battleRoom.add(lib_1.Utils.html `|html|<table width="100%"><tr><td align="left">${p1.name}</td><td align="right">${p2.name}</tr>` +
        ...
        this.room.add(lib_1.Utils.html `|uhtml|game${gameNum}|<a href="/${battleRoom.roomid}">${battleRoom.title}</a>`);
        this.updateDisplay();
    ...
    updateDisplay() {
        ...
        let buf = lib_1.Utils.html `<br /><strong>${p1name} and ${p2name}'s Best-of-${this.bestOf} progress:</strong><br />`;
        buf += '<table>';
        for (const p of this.players) {
            buf += lib_1.Utils.html `<tr><td>${p.name}: </td><td>`;
        ...
        for (const i of [0, null, 1]) {
            ...
            buf += lib_1.Utils.html `<td><center><strong>${this.players[i].name}</strong></center></td>`;
        }
        buf += `</tr><tr>`;
        for (const i of [0, null, 1]) {
            if (i === null) {
        ...
        buf = this.games.map(({ room, winner }, index) => {
            ...
            if (winner)
                progress = lib_1.Utils.html `won by ${winner.name}`;
            if (winner === null)
                progress = `tied`;
            return lib_1.Utils.html `<p>Game ${index + 1}: <a href="/${room}"><strong>${progress}</strong></a></p>`;
        }).join('');
        if (this.winner) {
            buf += lib_1.Utils.html `<p>${this.winner.name} won!</p>`;
        }
    ...
    onBattleWin(room, winnerid) {
        if (this.ended)
            return; // can happen if the bo3 is destroyed fsr
        ...
        if (winner) {
            ...
            this.room.add(lib_1.Utils.html `|html|${winner.name} won game ${this.games.length}!`).update();
        }
        else {
            this.ties++;
