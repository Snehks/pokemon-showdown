...
class LoginServerInstance {
    ...
    async instantRequest(action, data = null) {
        if (this.openRequests > 5) {
            return Promise.resolve([null, new RangeError("Request overflow")]);
        }
        this.openRequests++;
        try {
            const request = (0, lib_1.Net)(this.uri);
    ...
    request(action, data = null) {
        if (this.disabled) {
            return Promise.resolve([null, new Error(`Login server connection disabled.`)]);
        }
    ...
    async makeRequests() {
        ...
        try {
            const request = (0, lib_1.Net)(`${this.uri}action.php`);
...
exports.LoginServer = Object.assign(new LoginServerInstance(), {
    TimeoutError,
    ladderupdateServer: new LoginServerInstance(),
    prepreplayServer: new LoginServerInstance(),
});
(0, lib_1.FS)('./config/custom.css').onModify(() => {
