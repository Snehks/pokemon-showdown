...
exports.mainModlog = exports.Modlog = exports.MODLOG_DB_PATH = void 0;
exports.start = start;
...
exports.MODLOG_DB_PATH = Config.nofswriting ? ':memory:' : (0, lib_1.FS)(`databases/modlog.db`).path;
...
const PM = (0, lib_1.SQL)('modlog', module, {
...
class Modlog {
    ...
    async setupDatabase() {
        ...
        this.modlogInsertionQuery = await this.database.prepare(`INSERT INTO modlog (timestamp, roomid, visual_roomid, action, userid, autoconfirmed_userid, ip, action_taker_userid, is_global, note)` +
        ...
        this.globalPunishmentsSearchQuery = await this.database.prepare(`SELECT * FROM modlog WHERE is_global = 1 ` +
            `AND (userid = ? OR autoconfirmed_userid = ? OR EXISTS(SELECT * FROM alts WHERE alts.modlog_id = modlog.modlog_id AND userid = ?)) ` +
            `AND timestamp > ? ` +
            `AND action IN (${lib_1.Utils.formatSQLArray(GLOBAL_PUNISHMENTS, [])})`);
    ...
    prepareSQLSearch(rooms, maxLines, onlyPunishments, search) {
        ...
        if (rooms !== 'all') {
            const args = [];
            let roomChecker = `roomid IN (${lib_1.Utils.formatSQLArray(rooms, args)})`;
        ...
        if (onlyPunishments) {
            const args = [];
            ands.push({ query: `action IN (${lib_1.Utils.formatSQLArray(PUNISHMENTS, args)})`, args });
        }
