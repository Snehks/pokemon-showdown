...
class LadderStore {
    ...
    async updateRating(p1name, p2name, p1score, room) {
        ...
        if (!p1Reasons.startsWith('-'))
            p1Reasons = '+' + p1Reasons;
        room.addRaw(lib_1.Utils.html `${p1name}'s rating: ${p1OldElo} &rarr; <strong>${p1NewElo}</strong><br />(${p1Reasons})`);
        ...
        if (!p2Reasons.startsWith('-'))
            p2Reasons = '+' + p2Reasons;
        room.addRaw(lib_1.Utils.html `${p2name}'s rating: ${p2OldElo} &rarr; <strong>${p2NewElo}</strong><br />(${p2Reasons})`);
