...
exports.PM = exports.RoomBattleStream = exports.RoomBattle = exports.RoomBattleTimer = exports.RoomBattlePlayer = void 0;
exports.start = start;
const child_process_1 = require("child_process");
const lib_1 = require("../lib");
const battle_stream_1 = require("../sim/battle-stream");
...
class RoomBattlePlayer extends room_game_1.RoomGamePlayer {
...
class RoomBattleTimer {
    ...
    nextRequest(player) {
        ...
        if (this.battle.ended || !this.timerRequesters.size)
            return;
        // if there's only 1 player left
        if (this.battle.players.filter(p => p.secondsLeft > 0).length <= 1)
            return;
    ...
    nextTick() {
        ...
        if (this.battle.players.every(p => p.request.isWait === 'cantUndo'))
            return;
        const room = this.battle.room;
        for (const player of this.battle.players) {
            if (player.request.isWait)
...
class RoomBattle extends room_game_1.RoomGame {
        ...
        if (options.inputLog) {
            ...
            for (const player of this.players) {
                ...
                if (nameIndex1 < 0 || nameIndex2 < 0)
                    break; // shouldn't happen. incomplete inputlog?
    ...
    async listen() {
        ...
        try {
            for await (const next of this.stream) {
                if (!this.room)
                    return; // room deleted in the middle of simulation
    ...
    async updateLadder(p1score, winnerid) {
        ...
        const [score, p1rating, p2rating] = await Ladders(this.ladder).updateRating(p1, p2, p1score, this.room);
    ...
    checkForcedUserSettings(user) {
        ...
        if (this.players.some(p => p.getUser()?.battleSettings.special) ||
    ...
    start() {
        ...
        if (suspectTest) {
            ...
            this.room.add(`|html|<div class="broadcast-blue"><strong>${format.name} is currently suspecting ${suspectTest.suspect}! ` +
                `For information on how to participate check out the <a href="${suspectTest.url}">suspect thread</a>.</strong></div>`).update();
        }
    ...
    sendInviteForm(connection) {
        ...
        connection.sendTo(this.room, `|uhtmlchange|invites|<div class="broadcast broadcast-blue"><strong>This battle needs more players to start</strong><br /><br />${playerForms.join(``)}</div>`);
    }
    destroy() {
        if (!this.ended) {
            this.setEnded();
...
class RoomBattleStream extends battle_stream_1.BattleStream {
    constructor() {
 ...
 * Process manager
 *********************************************************/
exports.PM = new lib_1.ProcessManager.StreamProcessManager('sim', module, () => new RoomBattleStream(), message => {
    if (message.startsWith(`SLOW\n`)) {
        Monitor.slow(message.slice(5));
    }
});
function start(processCount) {
    exports.PM.spawn(processCount['simulator'] ?? 1);
}
if (!exports.PM.isParentProcess) {
    ...
    try {
        const head = (0, child_process_1.execSync)('git rev-parse HEAD', {
            stdio: ['ignore', 'pipe', 'ignore'],
        });
        const merge = (0, child_process_1.execSync)('git merge-base origin/master HEAD', {
            stdio: ['ignore', 'pipe', 'ignore'],
        });
