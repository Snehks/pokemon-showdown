...
exports.Punishments = new class {
    constructor() {
         ...
         */
        this.sharedRanges = new Map();
        /**
         * sharedIpBlacklist is an ip:note Map
         */
        this.sharedIpBlacklist = new Map();
        /**
    ...
    async loadPunishments() {
        const data = await (0, lib_1.FS)(PUNISHMENT_FILE).readIfExists();
        if (!data)
            return;
        for (const row of data.split("\n")) {
            ...
            for (const key of keys) {
                if (!key.trim())
                    continue; // ignore empty ips / userids
    ...
    async loadRoomPunishments() {
        const data = await (0, lib_1.FS)(ROOM_PUNISHMENT_FILE).readIfExists();
        if (!data)
            return;
        for (const row of data.split("\n")) {
            if (!row || row === '\r')
    ...
    savePunishments() {
        (0, lib_1.FS)(PUNISHMENT_FILE).writeUpdate(() => {
    ...
    saveRoomPunishments() {
        (0, lib_1.FS)(ROOM_PUNISHMENT_FILE).writeUpdate(() => {
            const saveTable = [];
    ...
    appendPunishment(entry, id, filename, allowNonUserIDs) {
        ...
        return (0, lib_1.FS)(filename).append(buf);
    }
    ...
    async loadBanlist() {
        const data = await (0, lib_1.FS)('config/ipbans.txt').readIfExists();
        if (!data)
            return;
        const rangebans = [];
        for (const row of data.split("\n")) {
     ...
     */
    async loadSharedIps() {
        const data = await (0, lib_1.FS)(SHAREDIPS_FILE).readIfExists();
        if (!data)
            return;
        ...
        for (const row of data.replace('\r', '').split("\n")) {
            ...
            if (IPTools.ipRegex.test(note)) {
                ...
                exports.Punishments.sharedIps.set(note, ip);
                needsSave = true;
                continue;
            }
            if (!IPTools.ipRegex.test(ip)) {
                ...
                if (pattern) {
                    exports.Punishments.sharedRanges.set(pattern, note);
                }
                else {
    ...
    appendSharedIp(ip, note) {
        ...
        return (0, lib_1.FS)(SHAREDIPS_FILE).append(buf);
    }
    saveSharedIps() {
        let buf = 'IP\tType\tNote\r\n';
        for (const [ip, note] of exports.Punishments.sharedIps) {
            buf += `${ip}\tSHARED\t${note}\r\n`;
        }
        for (const [range, note] of exports.Punishments.sharedRanges) {
            buf += `${IPTools.rangeToString(range)}\tSHARED\t${note}\r\n`;
        }
        return (0, lib_1.FS)(SHAREDIPS_FILE).write(buf);
    }
    /**
     * sharedips.tsv is in the format:
     * IP, type (in this case always SHARED), note
    ...
    async loadSharedIpBlacklist() {
        const data = await (0, lib_1.FS)(SHAREDIPS_BLACKLIST_FILE).readIfExists();
        if (!data)
            return;
        for (const row of data.replace('\r', '').split("\n")) {
            if (!row)
    ...
    appendSharedIpBlacklist(ip, reason) {
        const buf = `${ip}\t${reason}\r\n`;
        return (0, lib_1.FS)(SHAREDIPS_BLACKLIST_FILE).append(buf);
    }
    saveSharedIpBlacklist() {
        ...
        return (0, lib_1.FS)(SHAREDIPS_BLACKLIST_FILE).write(buf);
    }
    async loadWhitelistedNames() {
        const data = await (0, lib_1.FS)(WHITELISTED_NAMES_FILE).readIfExists();
        if (!data)
            return;
    ...
    appendWhitelistedName(name, whitelister) {
        return (0, lib_1.FS)(WHITELISTED_NAMES_FILE).append(`${toID(name)}\t${toID(whitelister)}\r\n`);
    }
    saveNameWhitelist() {
        ...
        return (0, lib_1.FS)(WHITELISTED_NAMES_FILE).write(buf);
    }
    /*********************************************************
     * Adding and removing
     *********************************************************/
    ...
    unlock(name) {
        ...
        if (['LOCK', 'YEARLOCK'].some(type => exports.Punishments.unpunish(name, type))) {
            if (!success.length)
                success.push(name);
        }
        if (!success.length)
    ...
    checkPunishmentTime(user, punishment) {
        ...
        if (timeLeft <= 1) {
            if (user.locked === id)
                exports.Punishments.unlock(user.id);
            return;
        }
    ...
    isBattleBanned(user) {
        ...
        for (const ip of user.ips) {
            ...
            if (punishment) {
                if (exports.Punishments.isSharedIp(ip) && user.autoconfirmed)
                    return;
                return punishment;
            }
        }
    ...
    isGroupchatBanned(user) {
        ...
        if (targetUser && !targetUser.trusted) {
            for (const ip of targetUser.ips) {
                ...
                if (punishment) {
                    if (exports.Punishments.isSharedIp(ip) && targetUser.autoconfirmed)
                        return;
                    return punishment;
                }
            }
    ...
    isTicketBanned(user) {
        ...
        if (ips.some(ip => exports.Punishments.isSharedIp(ip)))
            return false;
        for (const ip of ips) {
    ...
    monitorGroupchatJoin(room, newUser) {
        if (exports.Punishments.lastGroupchatMonitorTime[room.roomid] > (Date.now() - GROUPCHAT_MONITOR_INTERVAL))
            return;
    ...
    addSharedIp(ip, note) {
        ...
        for (const user of Users.users.values()) {
            const sharedIp = user.ips.some(curIP => (isRange ? IPTools.checkPattern([pattern], IPTools.ipToNumber(curIP)) : curIP === ip));
    ...
    removeSharedIp(ip) {
        ...
        if (pattern && pattern.minIP !== pattern.maxIP) {
            ...
            exports.Punishments.sharedRanges = new Map([...exports.Punishments.sharedRanges].filter(([range]) => !isMatch(range)));
        }
        else {
    ...
    hasPunishType(name, types, ip) {
        if (typeof types === 'string')
            types = [types];
        const byName = exports.Punishments.userids.get(name)?.some(p => types.includes(p.type));
        if (!ip)
            return byName;
        return byName || exports.Punishments.ipSearch(ip)?.some(p => types.includes(p.type));
    }
    hasRoomPunishType(room, name, types) {
        ...
        if (typeof room.roomid === 'string')
            room = room.roomid;
        return exports.Punishments.roomUserids.nestedGet(room, name)?.some(p => types.includes(p.type));
    }
    byWeight(punishments, room = false) {
        if (!punishments)
            return [];
        return lib_1.Utils.sortBy(punishments, p => -(room ? this.sortedRoomTypes : this.sortedTypes).indexOf(p.type));
    }
    ipSearch(ip, type) {
    ...
    checkName(user, userid, registered) {
        ...
        if (user.namelocked) {
            let punishment = exports.Punishments.userids.get(user.namelocked)?.[0];
            if (!punishment)
                punishment = { type: 'NAMELOCK', id: user.namelocked, expireTime: 0, reason: '' };
            punishments.push(punishment);
        }
        if (user.locked) {
            let punishment = exports.Punishments.userids.get(user.locked)?.[0];
            if (!punishment)
        ...
        for (const punishment of punishments) {
            ...
            if ((id === 'LOCK' || id === 'NAMELOCK') && punishUserid !== userid && exports.Punishments.isSharedIp(user.latestIp)) {
                if (!user.autoconfirmed) {
                    user.semilocked = `#sharedip ${user.locked}`;
                }
                user.locked = null;
            ...
            if (id === 'BAN') {
                ...
                user.popup(`Your username (${user.name}) is banned${bannedUnder}. Your ban will expire in a few days.${reason}` +
    ...
    checkIp(user, connection) {
        ...
        let punishments = exports.Punishments.ipSearch(ip);
        if (!punishments && exports.Punishments.checkRangeBanned(ip)) {
            punishments = [{ type: 'LOCK', id: '#ipban', expireTime: Infinity, reason: '' }];
        }
        if (punishments) {
            const isSharedIP = exports.Punishments.isSharedIp(ip);
            ...
            for (const punishment of punishments) {
                if (isSharedIP) {
                    ...
                    if (['BAN', 'LOCK', 'NAMELOCK'].includes(punishment.type)) {
                        ...
                        const info = exports.Punishments.punishmentTypes.get(punishment.type);
                        info?.onActivate?.call(this, user, punishment, null, punishment.id === user.id);
                    }
                }
            }
            if (!sharedAndHasPunishment)
                exports.Punishments.checkPunishmentTime(user, exports.Punishments.byWeight(punishments)[0]);
        }
    ...
    checkIpBanned(connection) {
        ...
        if (IPTools.torProxyIps.has(ip)) {
            connection.popup(`Due to persistent spam and abuse, we do not allow Tor connections. Please use another browser.`);
            return '#tor';
        }
        if (exports.Punishments.isSharedIp(ip))
            return false;
        let banned = false;
        ...
        if (!banned)
            return false;
        const appealUrl = Config.banappealurl || Config.appealurl;
        connection.send(`|popup||modal|You are banned because you have the same IP (${ip}) as banned user '${banned}'. ` +
    ...
    isRoomBanned(user, roomid) {
        ...
        for (const id of user.previousIDs) {
            punishments = exports.Punishments.roomUserids.nestedGet(roomid, id);
            for (const p of punishments || []) {
        ...
        if (room.parent)
            return exports.Punishments.isRoomBanned(user, room.parent.roomid);
    }
    isGlobalBanned(user) {
        if (!user)
            throw new Error(`Trying to check if a non-existent user is global banned.`);
        const punishment = exports.Punishments.userids.getByType(user.id, "BAN") || exports.Punishments.userids.getByType(user.id, "FORCEBAN");
        if (punishment)
            return punishment;
    }
    ...
    getRoomPunishments(user, options = {}) {
        ...
        for (const curRoom of Rooms.global.chatRooms) {
            if (!curRoom || curRoom.settings.isPrivate === true ||
    ...
    getPunishments(roomid, ignoreMutes) {
        ...
        if (roomid && (!exports.Punishments.roomIps.has(roomid) || !exports.Punishments.roomUserids.has(roomid)))
        ...
        (roomid ? exports.Punishments.roomUserids.get(roomid) : exports.Punishments.userids).each((punishment, userid) => {
            ...
            if (userid !== id)
                entry.userids.push(userid); // guaranteed as per above check
        });
    ...
    async monitorRoomPunishments(user) {
        ...
        if (!minPunishments)
            return;
        let punishments = exports.Punishments.getRoomPunishments(user, { checkIps: true, publicOnly: true });
        punishments = punishments.filter(([room, punishment]) => (exports.Punishments.roomPunishmentTypes.get(punishment.type)?.activatePunishMonitor));
        if (punishments.length >= minPunishments) {
            ...
            const punishmentText = punishments.map(([room, punishment]) => {
                ...
                let punishDesc = exports.Punishments.roomPunishmentTypes.get(punishType)?.desc;
                if (!punishDesc)
                    punishDesc = `punished`;
    ...
    renameRoom(oldID, newID) {
        for (const table of [exports.Punishments.roomUserids, exports.Punishments.roomIps]) {
