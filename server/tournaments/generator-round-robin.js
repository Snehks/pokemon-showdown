...
class RoundRobin {
    ...
    getPendingBracketData(players) {
        return {
            ...
            tableContents: players.map((p1, row) => players.map((p2, col) => {
    ...
    freezeBracket(players) {
        ...
        this.matches = players.map((p1, row) => players.map((p2, col) => {
            if (!this.isDoubles && col >= row)
                return null;
            if (p1 === p2)
                return null;
    ...
    getResults() {
        ...
        const sortedScores = utils_1.Utils.sortBy([...this.players], p => -p.score);
        const results = [];
