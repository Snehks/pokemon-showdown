...
exports.roomSettings = exports.pages = exports.commands = exports.Hangman = exports.hangmanData = void 0;
/*
* Hangman chat plugin
...
exports.hangmanData = {};
try {
    exports.hangmanData = JSON.parse((0, lib_1.FS)(HANGMAN_FILE).readSync());
    let save = false;
    for (const roomid in exports.hangmanData) {
        const roomData = exports.hangmanData[roomid] || {};
        const roomKeys = Object.keys(roomData);
        if (roomKeys.length && !roomData[roomKeys[0]].hints) {
            save = true;
            for (const key of roomKeys) {
                roomData[key] = { hints: roomData[key] };
            }
        }
    }
    if (save) {
        (0, lib_1.FS)(HANGMAN_FILE).writeUpdate(() => JSON.stringify(exports.hangmanData));
    }
}
catch { }
const maxMistakes = 6;
class Hangman extends Rooms.SimpleRoomGame {
    ...
    choose(user, word) {
        ...
        for (const guessid of this.guesses) {
            if (normalized === toID(guessid))
                throw new Chat.ErrorMessage(`Your guess "${word}" has already been guessed.`);
        }
    ...
    guessWord(word, guesser) {
        ...
        if (ourWord.length !== guessedWord.length)
            return false;
        for (let i = 0; i < ourWord.length; i++) {
            if (wordSoFar.charAt(i) === '_') {
                // Can't be a correct guess if it contains letters already guessed
                if (this.letterGuesses.some(guess => guess.toLowerCase().startsWith(guessedWord.charAt(i))))
                    return false;
            }
    ...
    generateWindow() {
        ...
        let escapedWord = this.wordSoFar.map(lib_1.Utils.escapeHTML);
        if (result === 1) {
            const word = this.word;
            escapedWord = escapedWord.map((letter, index) => letter === '_' ? `<font color="#7af87a">${word.charAt(index)}</font>` : letter);
        }
        const wordString = escapedWord.join('').replace(DIACRITICS_AFTER_UNDERSCORE, '_');
        if (this.hint)
            output += lib_1.Utils.html `<div>(Hint: ${this.hint})</div>`;
        output += `<p style="font-weight:bold;font-size:12pt;letter-spacing:3pt">${wordString}</p>`;
        if (this.guesses.length) {
            if (this.letterGuesses.length) {
                output += 'Letters: ' + this.letterGuesses.map(g => `<strong${g[1] === '1' ? '' : ' style="color: #DBA"'}>${g[0]}</strong>`).join(', ');
            }
            if (result === 2) {
                output += lib_1.Utils.html `<br />Winner: ${this.lastGuesser}`;
            }
            else if (this.guesses[this.guesses.length - 1].length === 1) {
                // last guess was a letter
                output += lib_1.Utils.html ` <small>&ndash; ${this.lastGuesser}</small>`;
            }
            else {
                output += lib_1.Utils.html `<br />Guessed: ${this.guesses[this.guesses.length - 1]} ` +
    ...
    static getRandom(room, tag) {
        if (!exports.hangmanData[room]) {
            exports.hangmanData[room] = {};
            this.save();
        }
        let phrases = Object.keys(exports.hangmanData[room]);
        if (!phrases.length)
            throw new Chat.ErrorMessage(`The room ${room} has no saved hangman words.`);
        if (tag) {
            tag = toID(tag);
            phrases = phrases.filter(key => exports.hangmanData[room][key].tags?.map(toID).includes(tag));
            if (!phrases.length) {
                throw new Chat.ErrorMessage(`No terms found with tag ${tag}`);
            }
        }
        const shuffled = lib_1.Utils.randomElement(phrases);
        const entry = exports.hangmanData[room][shuffled];
        return {
            question: shuffled,
            hint: lib_1.Utils.randomElement(entry.hints),
        };
    }
    static validateParams(params) {
        ...
        if (!phrase.length)
            throw new Chat.ErrorMessage("Enter a valid word");
        ...
        if (!/[a-zA-Z]/.test(phrase))
            throw new Chat.ErrorMessage("Word must contain at least one letter.");
        let hint;
        if (params.length > 1) {
...
exports.commands = {
    hangman: {
        ...
        new(target, room, user, connection) {
            ...
            if (target !== text)
                throw new Chat.ErrorMessage("You are not allowed to use filtered words in hangmans.");
            const params = text.split(',');
            this.checkCan('minigame', null, room);
            if (room.settings.hangmanDisabled)
                throw new Chat.ErrorMessage("Hangman is disabled for this room.");
            this.checkChat();
            if (room.game)
                throw new Chat.ErrorMessage(`There is already a game of ${room.game.title} in progress in this room.`);
            if (!params)
                throw new Chat.ErrorMessage("No word entered.");
        ...
        guess(target, room, user) {
            ...
            if (word !== target)
                throw new Chat.ErrorMessage(`You may not use filtered words in guesses.`);
        ...
        addrandom(target, room, user) {
            ...
            if (!exports.hangmanData[room.roomid])
                exports.hangmanData[room.roomid] = {};
            if (!target)
                return this.parse('/help hangman');
            // validation
            const args = target.split(target.includes('|') ? '|' : ',');
            const { phrase } = Hangman.validateParams(args);
            if (!exports.hangmanData[room.roomid][phrase])
                exports.hangmanData[room.roomid][phrase] = { hints: [] };
            args.shift();
            exports.hangmanData[room.roomid][phrase].hints.push(...args);
            Hangman.save();
        ...
        removerandom(target, room, user) {
            ...
            if (!hints.length) {
                ...
                exports.hangmanData[room.roomid][word].hints = exports.hangmanData[room.roomid][word].hints.filter(item => !hints.includes(toID(item)));
                if (!exports.hangmanData[room.roomid][word].hints.length) {
                    delete exports.hangmanData[room.roomid][word];
                }
        ...
        addtag(target, room, user) {
            ...
            if (!exports.hangmanData[room.roomid][term]) {
                throw new Chat.ErrorMessage(`Term ${term} not found.`);
            }
            if (!exports.hangmanData[room.roomid][term].tags)
                exports.hangmanData[room.roomid][term].tags = [];
            for (const [i, tag] of tags.entries()) {
                if (exports.hangmanData[room.roomid][term].tags.includes(tag)) {
            ...
            exports.hangmanData[room.roomid][term].tags.push(...tags);
            Hangman.save();
            this.privateModAction(`${user.name} added ${Chat.count(tags, "tags")} to the hangman term ${term}`);
            this.modlog(`HANGMAN ADDTAG`, null, `${term}: ${tags.map(lib_1.Utils.escapeHTML).join(', ')}`);
            this.refreshPage(`hangman-${room.roomid}`);
        },
        untag(target, room, user) {
            ...
            if (tags.length) {
                ...
                this.modlog(`HANGMAN UNTAG`, null, `${term}: ${tags.map(lib_1.Utils.escapeHTML).join(', ')}`);
                exports.hangmanData[room.roomid][term].tags = exports.hangmanData[room.roomid][term].tags?.filter(t => !tags.includes(t));
            }
            else {
            ...
            if (!exports.hangmanData[room.roomid][term].tags.length) {
                delete exports.hangmanData[room.roomid][term].tags;
            }
            Hangman.save();
            this.refreshPage(`hangman-${room.roomid}`);
        },
    ...
    hangman(args, user) {
        ...
        for (const t in roomTerms) {
            ...
            if (user.can('mute', null, room, 'hangman addrandom')) {
                ...
                buf += roomTerms[t].hints.map(hint => `${hint} <button class="button" name="send" value="/msgroom ${room.roomid}, /hangman rr ${t},${hint}" aria-label="Delete"><i class="fa fa-trash"></i></button>`).join(', ');
                ...
                if (roomTerms[t].tags) {
                    ...
                    buf += roomTerms[t].tags?.map(tag => `${tag} <button class="button" name="send" value="/msgroom ${room.roomid}, /hangman untag ${t},${tag}" aria-label="Delete"><i class="fa fa-trash"></i></button>`).join(', ');
