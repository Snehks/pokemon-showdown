...
exports.commands = exports.pages = exports.ScavengerHuntPlayer = exports.ScavengerHunt = void 0;
exports.sanitizeAnswer = sanitizeAnswer;
const lib_1 = require("../../lib");
const scavenger_games_1 = require("./scavenger-games");
...
const databaseContentsJSON = (0, lib_1.FS)(DATABASE_FILE).readIfExistsSync();
...
class Ladder {
    ...
    load() {
        const json = (0, lib_1.FS)(this.file).readIfExistsSync();
        if (json)
    ...
    addPoints(name, aspect, points, noUpdate) {
        ...
        if (!noUpdate)
            this.data[userid].name = name; // always keep the last used name
    ...
    write() {
        (0, lib_1.FS)(this.file).writeUpdate(() => JSON.stringify(this.data));
    }
    visualize(sortBy, userid) {
        ...
        return new Promise((resolve, reject) => {
            ...
            let lastPlacement = 1;
            const ladder = lib_1.Utils.sortBy(Object.entries(this.data).filter(([u, bit]) => sortBy in bit), ([u, bit]) => -bit[sortBy]).map(([u, chunk], i) => {
            ...
            for (const u in this.data) {
                ...
                for (const a in userData) {
                    if (/^(?:cumulative|history)-/i.test(a) || a === 'name')
                        continue; // cumulative does not need to be soft reset
    ...
    if (queue?.length) {
        buffer = queue.map((item, index) => {
            ...
            const unratedText = item.gameType === 'unrated' ?
                ...
                '';
            const hosts = lib_1.Utils.escapeHTML(Chat.toListString(item.hosts.map(h => h.name)));
            ...
            if (!broadcasting && (item.hosts.some(h => h.id === viewer.id) || viewer.id === item.staffHostId)) {
                questions = item.questions.map((q, i) => {
                    if (i % 2) {
                        q = q;
                        return lib_1.Utils.html `<span style="color: green"><em>[${q.join(' / ')}]</em></span><br />`;
                    }
                    else {
                        q = q;
                        return item.isHTML ? q : lib_1.Utils.escapeHTML(q);
                    }
                }).join(" ");
            }
            else {
...
class ScavengerHuntDatabase {
    ...
    static updateDatabaseOnDisk() {
        (0, lib_1.FS)(DATABASE_FILE).writeUpdate(() => JSON.stringify(scavengersData));
    }
    static isEmpty() {
...
class ScavengerHunt extends Rooms.RoomGame {
    ...
    loadMod(modData) {
        ...
        if (typeof modData === 'string') {
            ...
            if (!scavenger_games_1.ScavMods.twists[modId])
                return this.announce(`Invalid mod. Starting the hunt without the mod ${modId}.`);
            twist = scavenger_games_1.ScavMods.twists[modId];
        }
        else {
            twist = modData;
        }
    ...
    getCreationMessage(newHunt) {
        ...
        if (message)
            return message;
        const hosts = lib_1.Utils.escapeHTML(Chat.toListString(this.hosts.map(h => h.name)));
        const staffHost = this.hosts.some(h => h.id === this.staffHostId) ?
            `` :
            lib_1.Utils.html ` by <em>${this.staffHostName}</em>`;
    ...
    joinGame(user) {
        if (this.hosts.some(h => h.id === user.id) || user.id === this.staffHostId) {
            return user.sendTo(this.room, "You cannot join your own hunt! If you wish to view your questions, use /viewhunt instead!");
        }
    ...
    cacheUserIps(user) {
        ...
        if (!('ips' in user))
            return; // ghost user object cached from queue
    ...
    runEvent(event_id, ...args) {
        ...
        if (!events)
            return;
        lib_1.Utils.sortBy(events, event => -event.priority);
    ...
    getQuestion(question, showHints) {
        ...
        return `|raw|<div class="ladder"><table style="width:100%;table-layout:fixed;"><tr>` +
            ...
            `<td><div style="overflow:auto; max-width: 100vw; max-height: 50vh">${this.formatOutput(current.question.hint) +
                (showHints && current.question.spoilers.length ?
                    `<details><summary>Extra Hints:</summary>${current.question.spoilers.map(p => `- ${p}`).join('<br />')}</details>` :
                    ``)}</div></td>` +
            `</tr></table></div>`;
    }
    ...
    onViewHunt(user) {
        ...
        user.sendTo(this.room, `|raw|<div class="ladder"><table style="width: 100%;table-layout:fixed;">` +
            `<tr><th style="width: 10%;">#</th><th style="width: 70%;">Hint</th><th style="width: 20%;">Answer</th></tr>` +
            this.questions.slice(0, qLimit).map((q, i) => (`<tr><td>${i + 1}</td><td><div style="overflow:auto; max-width: 100vw; max-height: 30vh">${this.formatOutput(q.hint) +
                (q.spoilers.length ?
                    `<details><summary>Extra Hints:</summary>${q.spoilers.map(s => `- ${s}`).join('<br />')}</details>` :
                    ``)}</div></td><td>${i + 1 >= qLimit ?
                `` :
                lib_1.Utils.escapeHTMLForceWrap(q.answer.join(' ; '))}</td></tr>`)).join("") +
            `</table><div>`);
    }
    onComplete(player) {
        if (player.completed)
        ...
        const place = lib_1.Utils.formatOrder(this.completed.length);
        const completionMessage = this.runEvent('ConfirmCompletion', player, time, blitz, place, result);
        this.announce(completionMessage ||
            lib_1.Utils.html `<em>${result.name}</em> has finished the hunt in ${place} place! (${time}${(blitz ? " - BLITZ" : "")})`);
        player.destroy(); // remove from user.games;
    }
    onShowEndBoard(endedBy) {
        const sliceIndex = this.gameType === 'official' ? 5 : 3;
        const hosts = Chat.toListString(this.hosts.map(h => `<em>${lib_1.Utils.escapeHTML(h.name)}</em>`));
        this.announce(`The ${this.gameType ? `${this.gameType} ` : ""}scavenger hunt by ${hosts} was ended ${(endedBy ? "by " + lib_1.Utils.escapeHTML(endedBy.name) : "automatically")}.<br />` +
            `${this.completed.slice(0, sliceIndex).map((p, i) => `${lib_1.Utils.formatOrder(i + 1)} place: <em>${lib_1.Utils.escapeHTML(p.name)}</em> <span style="color: lightgreen;">[${p.time}]</span>.<br />`).join("")}` +
            `${this.completed.length > sliceIndex ? `Consolation Prize: ${this.completed.slice(sliceIndex).map(e => `<em>${lib_1.Utils.escapeHTML(e.name)}</em> <span style="color: lightgreen;">[${e.time}]</span>`).join(', ')}<br />` : ''}<br />` +
            `<details style="cursor: pointer; overflow:auto; max-height: 50vh"><summary>Solution: </summary><br />` +
            `${this.questions.map((q, i) => `${i + 1}) ${this.formatOutput(q.hint)} <span style="color: lightgreen">[<em>${lib_1.Utils.escapeHTML(q.answer.join(' / '))}</em>]</span>`).join("<br />")}` +
            `</details>`);
    }
    onEnd(reset, endedBy) {
                ...
                if (winPoints || blitzPoints) {
                    for (const [i, completed] of this.completed.entries()) {
                        if (!completed.blitz && i >= winPoints.length)
                            break; // there won't be any more need to keep going
    ...
    onChatMessage(msg) {
        ...
        const filtered = this.questions.some(q => q.answer.some(a => {
            ...
            if (lib_1.Utils.levenshtein(msgId, a, md) <= md)
                return true;
            return false;
        }));
        if (filtered)
    ...
    static parseQuestions(questionArray, force = false) {
        if (questionArray.length % 2 === 1 && !force)
            return { err: "Your final question is missing an answer" };
        if (questionArray.length < 6 && !force)
...
exports.ScavengerHunt = ScavengerHunt;
...
exports.ScavengerHuntPlayer = ScavengerHuntPlayer;
const ScavengerCommands = {
    ...
    join(target, room, user) {
        ...
        const game = room.getGame(ScavengerHunt);
        if (!game)
            throw new Chat.ErrorMessage("There is no scavenger hunt currently running.");
        this.checkChat();
        game.joinGame(user);
    },
    ...
    games: {
        ...
        start(target, room, user) {
            ...
            if (room.scavgame)
                throw new Chat.ErrorMessage('There is already a scavenger game running.');
            ...
            target = toID(target);
            const game = scavenger_games_1.ScavMods.LoadGame(room, target);
            if (!game)
                throw new Chat.ErrorMessage('Invalid game mode.');
            room.scavgame = game;
            ...
            this.checkCan('mute', null, room);
            if (!room.scavgame)
                throw new Chat.ErrorMessage(`There is no scavenger game currently running.`);
        ...
        kick(target, room, user) {
            ...
            if (targetId === 'constructor' || !targetId)
                throw new Chat.ErrorMessage("Invalid player.");
        ...
        async leaderboard(target, room, user) {
            ...
            if (!room.scavgame.leaderboard)
                throw new Chat.ErrorMessage("This scavenger game does not have a leaderboard.");
        ...
        async rank(target, room, user) {
            ...
            if (!rank) {
                ...
                this.sendReplyBox(lib_1.Utils.html `User '${rank.name}' is #${rank.rank} on the scavenger games leaderboard with ${rank.points} points.`);
            }
        },
    },
    teamscavs: {
        ...
        createteam(target, room, user) {
            ...
            if (!game || game.id !== 'teamscavs')
                throw new Chat.ErrorMessage('There is currently no game of Team Scavs going on.');
            let [teamName, leader] = target.split(',');
            teamName = teamName.trim();
            if (game.teams[teamName])
                throw new Chat.ErrorMessage(`The team ${teamName} already exists.`);
            const leaderUser = Users.get(leader);
            if (!leaderUser)
                throw new Chat.ErrorMessage('The user you specified is currently not online');
            if (game.getPlayerTeam(leaderUser))
                throw new Chat.ErrorMessage('The user is already a member of another team.');
            game.teams[teamName] = { name: teamName, answers: [], players: [leaderUser.id], question: 1, completed: false };
            game.announce(lib_1.Utils.html `A new team "${teamName}" has been created with ${leaderUser.name} as the leader.`);
        },
        ...
        removeteam(target, room, user) {
            ...
            if (!game.teams[target])
                throw new Chat.ErrorMessage(`The team ${target} does not exist.`);
            delete game.teams[target];
            game.announce(lib_1.Utils.html `The team "${target}" has been removed.`);
        },
        addplayer(target, room, user) {
            ...
            if (!userTeam)
                throw new Chat.ErrorMessage('You must be the leader of a team to add people into the team.');
            const targetUsers = target.split(',').map(id => Users.getExact(id)).filter(u => u?.connected);
            if (!targetUsers.length)
                throw new Chat.ErrorMessage('Please select a user that is currently online.');
            const errors = [];
            ...
            game.announce(lib_1.Utils.html `${Chat.toListString(targetUsers.map(u => u.name))} ${targetUsers.length > 1 ? 'have' : 'has'} been added into ${userTeam.name}.`);
        },
        editplayers(target, room, user) {
            ...
            if (!team)
                throw new Chat.ErrorMessage('Invalid team.');
        ...
        teams(target, room, user) {
            ...
            for (const teamID in game.teams) {
                const team = game.teams[teamID];
                display.push(lib_1.Utils.html `<strong>${team.name}</strong> - <strong>${team.players[0]}</strong>${team.players.length > 1 ? ', ' + team.players.slice(1).join(', ') : ''}`);
            }
        ...
        guesses(target, room, user) {
            ...
            if (!team)
                throw new Chat.ErrorMessage('You are not currently part of this Team Scavs game.');
            this.sendReplyBox(lib_1.Utils.html `<strong>Question #${team.question} guesses:</strong> ${team.answers.sort().join(', ')}`);
        },
        chat: 'note',
        note(target, room, user) {
            ...
            if (!team)
                throw new Chat.ErrorMessage('You are not currently part of this Team Scavs game.');
            if (!target)
                throw new Chat.ErrorMessage('Please include a message as the note.');
            game.teamAnnounce(user, lib_1.Utils.html `<strong> Note from ${user.name}:</strong> ${target}`);
        },
    },
    teamscavshelp: [
    ...
    create(target, room, user, connection, cmd) {
        ...
        this.checkCan('mute', null, room);
        if (room.game)
            throw new Chat.ErrorMessage(`There is already a game in this room - ${room.game.title}.`);
        ...
        if (!cmd.includes('force') && ['regular', 'unrated', 'recycled'].includes(gameType) && !mod &&
        ...
        if (gameType === 'recycled') {
            ...
            if (questions) {
                ...
                if (!ScavengerHuntDatabase.hasHunt(huntNumber))
                    throw new Chat.ErrorMessage("You specified an invalid hunt number.");
        ...
        if (res.err)
            throw new Chat.ErrorMessage(res.err);
        room.game = new ScavengerHunt({
            room,
            staffHost: user,
            hosts,
    ...
    status(target, room, user) {
        ...
        const hostersMsg = lib_1.Utils.escapeHTML(Chat.toListString(game.hosts.map(h => h.name)));
        const hostMsg = game.hosts.some(h => h.id === game.staffHostId) ?
            '' : lib_1.Utils.html ` (started by - ${game.staffHostName})`;
        const finishers = lib_1.Utils.html `${game.completed.map(u => u.name).join(', ')}`;
            ...
            for (let i = 0; i < game.questions.length; i++) {
                ...
                if (!players.length) {
                    ...
                    str += players.map(pl => pl.lastGuess > Date.now() - 1000 * 300 ?
                        lib_1.Utils.html `<strong>${pl.name}</strong>` :
                        lib_1.Utils.escapeHTML(pl.name)).join(", ");
                }
            }
            const completed = game.preCompleted ? game.preCompleted : game.completed;
            str += lib_1.Utils.html `<tr><td>Completed</td><td>${completed.length ? completed.map(pl => pl.name).join(", ") : 'None'}`;
    ...
    hint(target, room, user) {
        ...
        if (!game.onSendQuestion(user, true))
            throw new Chat.ErrorMessage("You are not currently participating in the hunt.");
    },
    ...
    inherit(target, room, user) {
        ...
        if (game.staffHostId === user.id)
            throw new Chat.ErrorMessage('You already have staff permissions for this hunt.');
    ...
    viewhunt(target, room, user) {
        ...
        if (!('onViewHunt' in game))
            throw new Chat.ErrorMessage('There is currently no hunt to be viewed.');
    ...
    edithunt(target, room, user) {
        ...
        if (!game)
            throw new Chat.ErrorMessage(`There is no scavenger hunt currently running.`);
        if ((!game.hosts.some(h => h.id === user.id) || !user.can('show', null, room)) &&
            game.staffHostId !== user.id) {
    ...
    spoiler(target, room, user) {
        ...
        if (!game.questions[question])
            throw new Chat.ErrorMessage(`Invalid question number.`);
        if (!hint)
            throw new Chat.ErrorMessage('The hint cannot be left empty.');
    ...
    removehint(target, room, user) {
        ...
        if (!game.questions[question].spoilers[hint])
            throw new Chat.ErrorMessage('Invalid hint number.');
    ...
    queue(target, room, user) {
        ...
        if (this.cmd === 'queuerecycled') {
            ...
            if (results.err)
                throw new Chat.ErrorMessage(results.err);
            if (!room.settings.scavQueue)
                room.settings.scavQueue = [];
            room.settings.scavQueue.push({
                hosts,
    ...
    next(target, room, user) {
        ...
        if (!room.settings.scavQueue[huntId])
            return false; // no need for an error reply - this is done via UI anyways
    ...
    async ladder(target, room, user) {
        ...
        this.sendReply(`|uhtml${isChange ? 'change' : ''}|scavladder|<div class="ladder" style="overflow-y: scroll; max-height: 300px;"><table style="width: 100%"><tr><th>Rank</th><th>Name</th><th>Points</th></tr>${ladder.map(entry => {
            ...
            if (isStaff && hideStaff)
                return '';
            return `<tr><td>${entry.rank}</td><td>${(isStaff ? `<em>${lib_1.Utils.escapeHTML(entry.name)}</em>` : (entry.rank <= 5 ? `<strong>${lib_1.Utils.escapeHTML(entry.name)}</strong>` : lib_1.Utils.escapeHTML(entry.name)))}</td><td>${entry.points}</td></tr>`;
        }).join('')}</table></div>` +
    ...
    async rank(target, room, user) {
        ...
        if (!rank) {
            ...
            this.sendReplyBox(lib_1.Utils.html `User '${rank.name}' is #${rank.rank} on the scavengers leaderboard with ${rank.points} points.`);
        }
    },
    /**
    ...
    setblitz(target, room, user) {
        ...
        if (!RATED_TYPES.includes(gameType))
            throw new Chat.ErrorMessage(`You cannot set blitz points for ${gameType} hunts.`);
    ...
    sethostpoints(target, room, user) {
        ...
        if (isNaN(points))
            throw new Chat.ErrorMessage(`${target} is not a valid number of points.`);
    ...
    setpoints(target, room, user) {
        ...
        if (!RATED_TYPES.includes(type))
            throw new Chat.ErrorMessage(`You cannot set win points for ${type} hunts.`);
    ...
    settwist(target, room, user) {
        ...
        if (target === 'RESET') {
            ...
            if (!scavenger_games_1.ScavMods.twists[twist] || twist === 'constructor')
                throw new Chat.ErrorMessage('Invalid twist.');
    ...
    twists(target, room, user) {
        ...
        buffer += Object.values(scavenger_games_1.ScavMods.twists).map(twist => (lib_1.Utils.html `<tr><td style="padding: 5px;">${twist.name}</td><td style="padding: 5px;">${twist.desc}</td></tr>`)).join('');
        buffer += `</table>`;
    ...
    async huntlogs(target, room, user) {
        ...
        if (!sortingFields.includes(sortMethod))
            sortMethod = 'points'; // default sort method
        const data = await HostLeaderboard.visualize(sortMethod);
        this.sendReply(`|${isUhtmlChange ? 'uhtmlchange' : 'uhtml'}|scav-huntlogs|<div class="ladder" style="overflow-y: scroll; max-height: 300px;"><table style="width: 100%"><tr><th>Rank</th><th>Name</th><th>Hunts Created</th><th>Total Hunts Created</th><th>History</th></tr>${data.map(entry => {
            const auth = room.auth.get(toID(entry.name)).trim();
            const color = auth ? 'inherit' : 'gray';
            return `<tr><td>${entry.rank}</td><td><span style="color: ${color}">${auth || '&nbsp;'}</span>${lib_1.Utils.escapeHTML(entry.name)}</td>` +
                ...
                `</tr>`;
        }).join('')}</table></div><div style="text-align: center">${sortingFields.map(f => `<button class="button${f === sortMethod ? ' disabled' : ''}" name="send" value="/scav huntlogs ${f}, 1">${f}</button>`).join(' ')}</div>`);
    },
    async playlogs(target, room, user) {
        ...
        if (!sortingFields.includes(sortMethod))
            sortMethod = 'finish'; // default sort method
        ...
        this.sendReply(`|${isUhtmlChange ? 'uhtmlchange' : 'uhtml'}|scav-playlogs|<div class="ladder" style="overflow-y: scroll; max-height: 300px;"><table style="width: 100%"><tr><th>Rank</th><th>Name</th><th>Finished Hunts</th><th>Joined Hunts</th><th>Ratio</th><th>Infractions</th></tr>${formattedData.map(entry => {
            const auth = room.auth.get(toID(entry.name)).trim();
            const color = auth ? 'inherit' : 'gray';
            return `<tr><td>${entry.rank}</td><td><span style="color: ${color}">${auth || '&nbsp;'}</span>${lib_1.Utils.escapeHTML(entry.name)}</td>` +
                ...
                `</tr>`;
        }).join('')}</table></div><div style="text-align: center">${sortingFields.map(f => `<button class="button${f === sortMethod ? ' disabled' : ''}" name="send" value="/scav playlogs ${f}, 1">${f}</button>`).join(' ')}</div>`);
    },
    uninfract: "infract",
    infract(target, room, user) {
        ...
        const targetId = toID(target);
        if (!targetId)
            throw new Chat.ErrorMessage(`Please include the name of the user to ${this.cmd}.`);
    ...
    recycledhunts(target, room, user) {
        ...
        if (cmd === 'addhunt') {
            if (!target)
                throw new Chat.ErrorMessage(`Usage: ${cmd} Hunt Text`);
            ...
            if (result.err)
                throw new Chat.ErrorMessage(result.err);
        ...
        if (!params)
            throw new Chat.ErrorMessage(usageMessages[cmd]);
