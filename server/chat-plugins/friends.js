...
exports.loginfilter = exports.handlers = exports.pages = exports.commands = exports.Friends = void 0;
...
exports.Friends = new class {
    async notifyPending(user) {
        ...
        if (pendingCount === 1) {
            ...
            let buf = utils_1.Utils.html `/uhtml sent,<button class="button" name="send" value="/friends accept ${sender}">Accept</button> | `;
            buf += utils_1.Utils.html `<button class="button" name="send" value="/friends reject ${sender}">Deny</button><br /> `;
            buf += `<small>(You can also stop this user from sending you friend requests with <code>/ignore</code>)</small>`;
            (0, friends_1.sendPM)(utils_1.Utils.html `/raw <span class="username">${senderName}</span> sent you a friend request!`, user.id);
            (0, friends_1.sendPM)(buf, user.id);
            (0, friends_1.sendPM)(`/raw <small>Note: If this request is accepted, your friend will be notified when you come online, ` +
                `and you will be notified when they do, unless you opt out of receiving them.</small>`, user.id);
        }
        else {
            (0, friends_1.sendPM)(`/nonotify You have ${pendingCount} friend requests pending!`, user.id);
            (0, friends_1.sendPM)(`/raw <button class="button" name="send" value="/j view-friends-received">View</button></div>`, user.id);
        }
    }
    async notifyConnection(user) {
        ...
        const message = `/nonotify Your friend <username class="username">${utils_1.Utils.escapeHTML(user.name)}</username> has just connected!`;
    ...
    displayFriend(userid, login) {
        ...
        const name = utils_1.Utils.escapeHTML(user ? user.name : userid);
        ...
        let buf = user ?
            ...
            utils_1.Utils.html `<i>${name}</i> <small>(${statusType})</small>`;
        buf += `<br />`;
        const curUser = Users.get(userid); // might be an alt
        if (user) {
            if (user.userMessage)
                buf += utils_1.Utils.html `Status: <i>${user.userMessage}</i><br />`;
        }
    ...
    checkCanUse(context) {
        ...
        if (!user.autoconfirmed) {
            throw new Chat.ErrorMessage(context.tr `To use the friends feature you must be autoconfirmed, which means being registered for at least one week and winning one rated game.`);
        }
...
function headerButtons(type, user) {
    ...
    const refresh = (`<button class="button" name="send" value="/j view-friends${type?.trim() ? `-${type}` : ''}" style="float: right">` +
...
exports.commands = {
    ...
    friends: {
        ...
        async add(target, room, user, connection) {
            ...
            await exports.Friends.request(user, target);
        ...
        async remove(target, room, user) {
            ...
            if (!target)
                return this.parse('/help friends');
            await exports.Friends.removeFriend(user.id, target);
        ...
        async accept(target, room, user, connection) {
            ...
            if (!target)
                return this.parse('/help friends');
            await exports.Friends.approveRequest(user.id, target);
            const targetUser = Users.get(target);
            (0, friends_1.sendPM)(`You accepted a friend request from "${target}".`, user.id);
            this.refreshPage('friends-received');
            if (targetUser) {
                (0, friends_1.sendPM)(`/text ${user.name} accepted your friend request!`, targetUser.id);
                (0, friends_1.sendPM)(`/uhtmlchange sent-${targetUser.id},`, targetUser.id);
                (0, friends_1.sendPM)(`/uhtmlchange undo-${targetUser.id},`, targetUser.id);
            }
            await Chat.Friends.updateUserCache(user);
            if (targetUser)
                await Chat.Friends.updateUserCache(targetUser);
        ...
        async reject(target, room, user, connection) {
            ...
            if (!target)
                return this.parse('/help friends');
            const res = await exports.Friends.removeRequest(user.id, target);
            if (!res.changes) {
                throw new Chat.ErrorMessage(`You do not have a friend request pending from '${target}'.`);
            }
            this.refreshPage('friends-received');
            return (0, friends_1.sendPM)(`You denied a friend request from '${target}'.`, user.id);
        },
        toggle(target, room, user, connection) {
            ...
            if (this.meansYes(target)) {
                ...
                this.sendReply(this.tr(setting ? `You are currently blocking friend requests.` : `You are not blocking friend requests.`));
            }
        ...
        async undorequest(target, room, user, connection) {
            exports.Friends.checkCanUse(this);
            target = toID(target);
            await exports.Friends.removeRequest(target, user.id);
            this.refreshPage('friends-sent');
            return (0, friends_1.sendPM)(`You removed your friend request to '${target}'.`, user.id);
        },
        ...
        viewnotifications(target, room, user, connection, cmd) {
            ...
            if (!cmd.includes('hide') || target && this.meansYes(target)) {
                ...
                if (target)
                    this.errorReply(this.tr `Unrecognized setting.`);
                this.sendReply(this.tr(setting ? `You are currently allowing friend notifications.` : `Your friend notifications are disabled.`));
            }
            this.refreshPage('friends-settings');
            user.update();
        },
        ...
        invalidatecache(target, room, user) {
            ...
            Rooms.global.notifyRooms(['staff', 'development'], `|c|${user.getIdentity()}|/log ${user.name} used /friends invalidatecache`);
    ...
    async friends(args, user) {
        ...
        switch (toID(type)) {
            ...
            case 'sent':
                ...
                buf += `<h3>You have ${Chat.count(sent.size, 'friend requests')} pending${sent.size === friends_1.MAX_REQUESTS ? ` (maximum reached)` : ''}.</h3>`;
            ...
            case 'viewuser':
                ...
                buf += await exports.Friends.visualizePublicList(target);
                break;
            case 'help':
            ...
            case 'spectate':
                ...
                for (const friend of friends) {
                    const curBattles = [...friend.inRooms]
                        ...
                        return (battle?.playerTable[friend.id] &&
                ...
                utils_1.Utils.sortBy(battles, ([, id]) => -Number(id.split('-')[2]));
                if (!battles.length) {
...
exports.handlers = {
    ...
    onBattleEnd(battle, winner, players) {
        for (const id of players) {
            ...
            if (!user)
                continue;
            exports.Friends.updateSpectatorLists(user);
        }
    },
    onDisconnect(user) {
...
const loginfilter = user => {
    ...
    void exports.Friends.notifyPending(user);
    // (quietly) notify their friends (that have opted in) that they are online
    void exports.Friends.notifyConnection(user);
    // write login time
    void Chat.Friends.writeLogin(user.id);
