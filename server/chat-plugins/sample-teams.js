...
exports.commands = exports.handlers = exports.destroy = exports.SampleTeams = exports.teamData = void 0;
const lib_1 = require("../../lib");
const SAMPLE_TEAMS = 'config/chat-plugins/sample-teams.json';
exports.teamData = (() => {
    try {
        return JSON.parse((0, lib_1.FS)(SAMPLE_TEAMS).readIfExistsSync());
    }
    catch {
        return {
            whitelist: {},
...
function save() {
    (0, lib_1.FS)(SAMPLE_TEAMS).writeUpdate(() => JSON.stringify(exports.teamData));
}
for (const formatid in exports.teamData.teams) {
    if (!exports.teamData.teams[formatid].uncategorized)
        exports.teamData.teams[formatid].uncategorized = {};
}
save();
exports.SampleTeams = new class SampleTeams {
    ...
    whitelistedRooms(formatid, names = false) {
        ...
        if (!exports.teamData.whitelist[formatid]?.length)
            return null;
        return lib_1.Utils.sortBy(exports.teamData.whitelist[formatid], x => {
            if (!names)
                return x;
    ...
    whitelistRooms(formatids, roomids) {
        for (const unsanitizedFormatid of formatids) {
            ...
            if (!exports.teamData.whitelist[formatid])
                exports.teamData.whitelist[formatid] = [];
            for (const roomid of roomids) {
    ...
    unwhitelistRoom(formatid, roomid) {
        ...
        if (!exports.teamData.whitelist[formatid].length)
            delete exports.teamData.whitelist[formatid];
        save();
    }
    ...
    addTeam(user, formatid, teamName, team, category = "uncategorized") {
        ...
        if (!exports.teamData.teams[formatid][category])
    ...
    removeTeam(user, formatid, teamid, category) {
        ...
        if (!Object.keys(exports.teamData.teams[formatid][categoryName]).length)
            delete exports.teamData.teams[formatid][categoryName];
        if (!Object.keys(exports.teamData.teams[formatid]).filter(x => x !== 'uncategorized').length)
            delete exports.teamData.teams[formatid];
        save();
        return oldTeam;
    }
    formatTeam(teamName, teamStr, broadcasting = false) {
        ...
        if (!broadcasting) {
            ...
            buf += `<br />${lib_1.Utils.escapeHTML(teamName.toUpperCase())}</strong></center>`;
            buf += Chat.getReadmoreCodeBlock(Teams.export(team).trim());
        }
        else {
            buf += `<details><summary>${team.map(x => `<psicon pokemon="${x.species}" />`).join('')} &ndash; ${lib_1.Utils.escapeHTML(teamName)}</summary>`;
...
exports.destroy = exports.SampleTeams.destroy;
exports.handlers = {
    onRenameRoom(oldID, newID) {
        for (const formatid in exports.teamData.whitelist) {
            if (!exports.SampleTeams.whitelistedRooms(formatid)?.includes(oldID))
                continue;
