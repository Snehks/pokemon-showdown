...
exports.handlers = exports.commands = exports.pages = exports.destroy = exports.spotlights = void 0;
...
exports.spotlights = {};
try {
    exports.spotlights = JSON.parse((0, lib_1.FS)(SPOTLIGHT_FILE).readIfExistsSync() || "{}");
...
if (!exports.spotlights || typeof exports.spotlights !== 'object')
    exports.spotlights = {};
function saveSpotlights() {
    (0, lib_1.FS)(SPOTLIGHT_FILE).writeUpdate(() => JSON.stringify(exports.spotlights));
}
function nextDaily() {
    for (const roomid in exports.spotlights) {
...
exports.pages = {
    spotlights(query, user, connection) {
        ...
        const sortedEntries = lib_1.Utils.sortBy(Object.entries(exports.spotlights[room.roomid]), ([key]) => {
            switch (sortType) {
                case 'time': {
                    // find most recently added/updated spotlight in that name, sort all by that
                    const sortedSpotlights = lib_1.Utils.sortBy(exports.spotlights[room.roomid][key].slice(), k => -k.time);
...
exports.commands = {
    removedaily(target, room, user) {
        ...
        if (!exports.spotlights[room.roomid][key])
        ...
        if (!rest || rest === 'current' || rest === '0' || rest === 'all') {
            ...
            if (!exports.spotlights[room.roomid][key].length || rest === 'all') {
            ...
            if (isNaN(queueNumber) || queueNumber < 1)
                throw new Chat.ErrorMessage("Invalid queue number");
            if (queueNumber >= exports.spotlights[room.roomid][key].length) {
                throw new Chat.ErrorMessage(`Queue number must be between 1 and ${exports.spotlights[room.roomid][key].length - 1}`);
            }
            exports.spotlights[room.roomid][key].splice(queueNumber, 1);
            saveSpotlights();
            this.modlog(`DAILY REMOVE`, `${key}[${queueNumber}]`);
            this.privateModAction(`${user.name} removed the ${queueNumber}th entry from the queue of the daily spotlight named '${key}'.`);
        }
    ...
    swapdaily(target, room, user) {
        ...
        if (!room.persist)
            throw new Chat.ErrorMessage("This command is unavailable in temporary rooms.");
        if (!exports.spotlights[room.roomid])
            throw new Chat.ErrorMessage("There are no dailies for this room.");
        this.checkCan('announce', null, room);
    ...
    async setdaily(target, room, user, connection, cmd) {
        ...
        if (!key)
            return this.parse('/help daily');
        if (key.length > 20)
            throw new Chat.ErrorMessage("Spotlight names can be a maximum of 20 characters long.");
        if (key === 'constructor')
            return false;
        if (!exports.spotlights[room.roomid])
            exports.spotlights[room.roomid] = {};
        const queueLength = exports.spotlights[room.roomid][key]?.length || 0;
        if (indexString && !NUMBER_REGEX.test(indexString))
            throw new Chat.ErrorMessage("The queue number must be a number.");
        ...
        if (!exports.spotlights[room.roomid][key])
            exports.spotlights[room.roomid][key] = [];
        if (cmd === 'setdaily') {
            exports.spotlights[room.roomid][key].shift();
    ...
    daily(target, room, user) {
        ...
        if (!this.broadcasting && user.can('ban', null, room, 'setdaily')) {
            const code = lib_1.Utils.escapeHTML(description).replace(/\n/g, '<br />');
    ...
    dailyhelp() {
        this.sendReply(`|html|<details class="readmore"><summary><code>/daily [name]</code>: shows the daily spotlight.<br />` +
...
exports.handlers = {
    onRenameRoom(oldID, newID) {
        if (exports.spotlights[oldID]) {
            if (!exports.spotlights[newID])
                exports.spotlights[newID] = {};
