...
exports.handlers = exports.loginfilter = exports.punishmentfilter = exports.commands = exports.pages = exports.textTickets = exports.HelpTicket = exports.settings = exports.tickets = exports.REPLAY_REGEX = exports.BATTLES_REGEX = void 0;
exports.writeTickets = writeTickets;
exports.writeSettings = writeSettings;
exports.writeStats = writeStats;
exports.notifyStaff = notifyStaff;
exports.getBattleLinks = getBattleLinks;
exports.listOpponentsFrom = listOpponentsFrom;
exports.getOpponent = getOpponent;
exports.getBattleLog = getBattleLog;
exports.start = start;
const lib_1 = require("../../lib");
const info_1 = require("../chat-commands/info");
const ConfigLoader = __importStar(require("../config-loader"));
const helptickets_auto_1 = require("./helptickets-auto");
...
exports.BATTLES_REGEX = /\bbattle-(?:[a-z0-9]+)-(?:[0-9]+)(?:-[a-z0-9]{31}pw)?/g;
exports.REPLAY_REGEX = new RegExp(`${lib_1.Utils.escapeRegex(Config.routes.replays)}/(?:[a-z0-9]-)?(?:[a-z0-9]+)-(?:[0-9]+)(?:-[a-z0-9]{31}pw)?`, "g");
...
exports.settings = (() => {
    try {
        ...
        // to the JSON as well
        return { ...defaults, ...JSON.parse((0, lib_1.FS)(SETTINGS_FILE).readSync()) };
    }
    catch {
        return { ...defaults };
    }
...
try {
    const ticketData = JSON.parse((0, lib_1.FS)(TICKET_FILE).readSync());
...
function writeTickets() {
    (0, lib_1.FS)(TICKET_FILE).writeUpdate(() => JSON.stringify(exports.tickets), { throttle: 5000 });
}
function writeSettings() {
    (0, lib_1.FS)(SETTINGS_FILE).writeUpdate(() => JSON.stringify(exports.settings));
}
...
class HelpTicket extends Rooms.SimpleRoomGame {
    ...
    onJoin(user, connection) {
        ...
        if (!this.ticket.claimed) {
            ...
            if (!this.firstClaimTime) {
                ...
                const users = Object.entries(this.room.users).filter(u => !((u[1].isStaff && u[1].id !== this.ticket.userid) || !u[1].named));
                if (!users.length)
                    this.emptyRoom = true;
            }
            ...
            exports.tickets[this.ticket.userid] = this.ticket;
            writeTickets();
    ...
    onLogMessage(message, user) {
        ...
        if ((!user.isStaff || this.ticket.userid === user.id) && !this.ticket.active) {
            ...
            switch (this.ticket.type) {
                case 'PM Harassment':
                    this.room.add(`|c|~Staff|Global staff might take more than a few minutes to handle your report. ` +
                        `If you are being disturbed by another user, you can type \`\`/ignore [username]\`\` in any chat to ignore their messages immediately`).update();
                    break;
            }
    ...
    forfeit(user) {
        ...
        if (this.playerCount - 1 > 0)
            return; // There are still users in the ticket room, dont close the ticket
    ...
    getButton() {
        ...
        const creator = (this.ticket.claimed ? lib_1.Utils.html `${this.ticket.creator}` : lib_1.Utils.html `<strong>${this.ticket.creator}</strong>`);
        ...
        return (`<a class="button ${color}" href="/help-${this.ticket.userid}"` +
    ...
    getPreview() {
        ...
        const noteBuf = Object.entries(this.ticket.notes || {})
            .map(([userid, note]) => lib_1.Utils.html `${note} (by ${userid})`)
            .join('&#10;');
    ...
    deleteTicket(staff) {
        ...
        delete exports.tickets[this.ticket.userid];
        writeTickets();
        notifyStaff();
        this.room.destroy();
    }
    ...
    destroy() {
        if (exports.tickets[this.ticket.userid] && this.ticket.open) {
    ...
    static list(sorter) {
        ...
        return lib_1.Utils.sortBy(Object.values(exports.tickets), sorter);
    }
    ...
    static async getTextLogs(search, date) {
        ...
        if (await ConfigLoader.checkRipgrepAvailability()) {
            const searchString = search.length > 1 ?
                ...
                lib_1.Utils.escapeRegex(JSON.stringify(search[1]).slice(0, -1)) :
                "";
            const args = [
            ...
            let lines;
            try {
                lines = await lib_1.ProcessManager.exec([
    ...
    static formatBattleLog(logs, info, reported) {
        ...
        for (const line of log) {
            ...
            buf += lib_1.Utils.html `${username}:</strong></span> ${message}</div>`;
        }
        if (buf)
    ...
    static async visualizeBattleLogs(rooms, reported) {
        ...
        if (existingRooms.length) {
            const chatBuffer = existingRooms
                ...
                .join('');
            if (chatBuffer) {
                return (`<div class="infobox"><details class="readmore"><summary><strong>Battle chat logs:</strong><br /></summary>` +
    ...
    static getTextButton(ticket) {
        let buf = '';
        const titleBuf = [
            ...ticket.text[0].split('\n').map(lib_1.Utils.escapeHTML),
            ...ticket.text[1].split('<br />').map(lib_1.Utils.stripHTML),
        ].slice(0, 3);
    ...
    static notifyResolved(user, ticket, userid = user.id) {
        ...
        exports.tickets[userid].resolved.seen = true;
        writeTickets();
    }
...
function notifyUnclaimedTicket(hasAssistRequest) {
    ...
    unclaimedTicketTimer[room.roomid] = null;
    timerEnds[room.roomid] = 0;
    for (const ticket of Object.values(exports.tickets)) {
        if (!ticket.open)
            continue;
        if (!ticket.active)
            continue;
        ...
        if (ticket.needsDelayWarning && !ticket.claimed && delayWarnings[ticket.type]) {
            ticketRoom.add(`|c|~Staff|${ticketRoom.tr(delayWarningPreamble)}${ticketRoom.tr(delayWarnings[ticket.type])}`).update();
    ...
    for (const i in room.users) {
        ...
        if (user.can('mute', null, room) && !user.settings.ignoreTickets) {
            user.sendTo(room, `|tempnotify|helptickets|Unclaimed help tickets!|${hasAssistRequest ? 'Public Room Staff need help' : 'There are unclaimed Help tickets'}`);
        }
    }
}
function notifyStaff() {
    ...
    const listOnlyTypes = Object.keys(exports.textTickets).filter(type => exports.textTickets[type].listOnly);
    let count = 0;
    ...
    for (const type of listOnlyTypes) {
        const matches = sortedTickets.filter(ticket => HelpTicket.getTypeId(ticket.type) === type && ticket.open && !ticket.resolved);
        if (matches.length) {
            hasUnclaimed = true;
...
function checkIp(ip) {
    for (const t in exports.tickets) {
        if (exports.tickets[t].ip === ip && exports.tickets[t].open && !Punishments.isSharedIp(ip)) {
            return exports.tickets[t];
        }
    }
    return false;
}
function getBattleLinks(text) {
    const rooms = new Set();
    const battles = text.match(exports.BATTLES_REGEX);
    const replays = text.match(exports.REPLAY_REGEX);
    if (battles) {
...
async function listOpponentsFrom(ticket) {
    const opps = new lib_1.Utils.Multiset();
    ...
    return lib_1.Utils.sortBy([...opps], ([, count]) => -count).map(opp => toID(opp[0]));
}
...
async function getBattleLog(battle, noReplay = false) {
    ...
    if (battleRoom && battleRoom.type !== 'chat' && battleRoom.battle) {
        ...
        if (Rooms.Replays.db) { // direct conn exists, use it
            ...
            // call out to API
            try {
                const raw = await (0, lib_1.Net)(`https://${Config.routes.replays}/${battle}.json`).get();
                data = JSON.parse(raw);
            }
            catch { }
        }
...
for (const room of Rooms.rooms.values()) {
    ...
    if (game.ticket && exports.tickets[game.ticket.userid])
        game.ticket = exports.tickets[game.ticket.userid];
}
...
exports.textTickets = {
    pmharassment: {
        ...
        async getReviewDisplay(ticket, staff, conn) {
            ...
            const sharedBattles = (0, info_1.getCommonBattles)(ticket.userid, null, reportUserid, null, conn);
            ...
            buf += HelpTicket.displayPunishmentList(ticket.userid, `spoiler:PMs with ${reportUserid} (as ${ticket.userid})${replayString ? `, ${replayString}` : ''}`, ticket, `Punish <strong>${ticket.userid}</strong> (reporter)`, `<h2 style="color:red">You are about to punish the reporter. Are you sure you want to do this?</h2>`);
            ...
            buf += HelpTicket.displayPunishmentList(reportUserid, `spoiler:PMs with ${ticket.userid}${replayString ? `, ${replayString}` : ''}`, ticket, `Punish <strong>${reportUserid}</strong> (reported user)`);
            if (replays.length) {
        ...
        onSubmit(ticket, text, submitter, conn) {
            ...
            (0, info_1.getCommonBattles)(targetId, Users.get(targetId), submitter.id, submitter, conn);
        },
    },
    inapname: {
        ...
        getReviewDisplay(ticket, staff, conn, state) {
            ...
            const name = lib_1.Utils.escapeHTML(Users.getExact(tar)?.name || tar);
    ...
    battleharassment: {
        ...
        async getReviewDisplay(ticket, staff, connection) {
            ...
            buf += HelpTicket.displayPunishmentList(ticket.userid, proof, ticket, `Punish <strong>${ticket.userid}</strong> (reporter)`, `<h2 style="color:red">You are about to punish the reporter. Are you sure you want to do this?</h2>`);
            ...
            if (opp) {
                ...
                buf += HelpTicket.displayPunishmentList(opp, proof, ticket, `Punish <strong>${opp}</strong> (reported user)`);
            }
    ...
    inappokemon: {
        ...
        checker(input) {
            if (exports.BATTLES_REGEX.test(input) || exports.REPLAY_REGEX.test(input))
                return true;
            return ['Please provide at least one valid battle or replay URL.'];
        },
        async getReviewDisplay(ticket, staff, conn) {
            ...
            if (opp) {
                ...
                buf += `<button class="button" name="send" value="/modlog room=global,user='${opp}'">Global Modlog</button><br />`;
                buf += HelpTicket.displayPunishmentList(opp, proof, ticket, `Punish <strong>${opp}</strong> (reported)`);
            }
            ...
            for (const link of links) {
                ...
                for (const id of Object.values(roomData.players)) {
                    ...
                    if (team) {
                        const teamNames = team.map(p => (p.name !== p.species ? lib_1.Utils.html `${p.name} (${p.species})` : p.species));
    ...
    ipappeal: {
        ...
        async getReviewDisplay(ticket, staff, conn, state) {
            ...
            for (const data of info) {
                ...
                if (ipPunishments) {
                    const str = ipPunishments.map(p => (`${Punishments.punishmentTypes.get(p.type)?.desc || p.type} as ` +
                        `<a href="https://${Config.routes.root}/users/${p.id}">${p.id}</a>` +
                        (p.reason ? lib_1.Utils.html ` (${p.reason})` : '')));
                    if (str)
    ...
    help: {
        request(query, user, connection) {
            ...
            if (ticket?.open || ipTicket) {
                ...
                if (!helpRoom && !ticket.text) {
                    // Should never happen
                    exports.tickets[ticket.userid].open = false;
                    writeTickets();
                }
                else {
                    if (helpRoom) {
            ...
            for (const [i, page] of query.entries()) {
                ...
                switch (page) {
                    ...
                    case 'reason':
                        ...
                        for (const [idx, punishment] of punishments.entries()) {
                            if (punishments.indexOf(punishment) !== idx) {
                                ...
                                buf += lib_1.Utils.html `<p>Your ${punishment.type} was for: ${punishment.reason}.</p>`;
                            }
                        }
                        break;
                    case 'startedit':
                    ...
                    default:
                        ...
                        const submitMeta = lib_1.Utils.splitFirst(meta, '-', 2).join('|'); // change the delimiter as some ticket titles include -
                        const textTicket = exports.textTickets[page.slice(7)];
                        if (textTicket) {
            ...
            buf = buf.replace(/<Button>([a-z]+)<\/Button>/g, (match, id) => (`<a class="button" href="/view-help-request${curPageLink}-${id}${meta}" target="replace">${this.tr(ticketPages[id])}</a>`));
            return buf;
        },
        async list(query, user, connection) {
            ...
            if (!type) {
                ...
                const keys = Object.keys(exports.textTickets).filter(id => exports.textTickets[id].listOnly);
                if (!keys.length) {
            ...
            let count = 0;
            for (const k in exports.tickets) {
                const ticket = exports.tickets[k];
                ...
                if (!ticket.resolved && ticket.text && typeId === type) {
                    ...
                    buf += await exports.textTickets[typeId].getReviewDisplay(ticket, user, this.connection, { list: true });
        ...
        tickets(query, user, connection) {
            ...
            for (const ticket of sortedTickets) {
                ...
                buf += `<tr><td>${icon}</td>`;
                buf += `<td>${lib_1.Utils.escapeHTML(ticket.creator)}</td>`;
                buf += `<td>${ticket.type}</td>`;
                buf += lib_1.Utils.html `<td>${ticket.claimed ? ticket.claimed : `-`}</td>`;
                buf += `<td>`;
            ...
            const ticketBans = lib_1.Utils.sortBy([...Punishments.getPunishments()].filter(([id, entry]) => entry.punishType === 'TICKETBAN'), ([id, entry]) => entry.expireTime);
            for (const [userid, entry] of ticketBans) {
                let ids = [userid];
                if (entry.userids)
                    ids = ids.concat(entry.userids);
                buf += `<tr><td>${ids.map(lib_1.Utils.escapeHTML).join(', ')}</td>`;
        ...
        async text(query, user, connection) {
            ...
            const ticket = exports.tickets[toID(userid)];
            if (!ticket) {
            ...
            const ticketInfo = exports.textTickets[HelpTicket.getTypeId(ticket.type)];
            ...
            if (ticket.notes) {
                ...
                for (const staff in ticket.notes) {
                    buf += lib_1.Utils.html `<p>${ticket.notes[staff]} (by ${staff})</p>`;
                }
            ...
            if (!ticket.resolved) {
                const typeId = HelpTicket.getTypeId(ticket.type);
                const responses = exports.settings.responses[typeId];
                ...
                buf += lib_1.Utils.html `<strong>Resolved: by ${ticket.resolved.by}</strong><br />`;
                buf += lib_1.Utils.html `<strong>Result:</strong> ${Chat.collapseLineBreaksHTML(ticket.resolved.result)}<br />`;
                if (ticket.resolved.staffReason.includes('PROOF')) { // a note was added, show it
                    buf += lib_1.Utils.html `<strong>Resolver notes:</strong> ${Chat.collapseLineBreaksHTML(ticket.resolved.staffReason)}<br />`;
                }
            }
            return buf;
        },
        async logs(query, user, connection) {
            ...
            lib_1.Utils.sortBy(logs, log => -log.resolved.time);
            for (const ticket of logs) {
                ...
                buf += await ticketInfo.getReviewDisplay(ticket, user, connection, ticket.state);
                buf += `<br />`;
                buf += `<div class="infobox">`;
                const [text, context] = ticket.text;
                buf += `<p><strong>Report text:</strong></p><hr />`;
        ...
        stats(query, user, connection) {
            ...
            const prevDate = new Date(date.getMonth() === 0 ?
                date.getFullYear() - 1 :
                date.getFullYear(), date.getMonth() === 0 ?
                11 :
                date.getMonth() - 1);
            const nextDate = new Date(date.getMonth() === 11 ?
                date.getFullYear() + 1 :
                date.getFullYear(), date.getMonth() === 11 ?
                0 :
                date.getMonth() + 1);
            ...
            if (table === 'tickets') {
                ...
                const sortedStats = lib_1.Utils.sortBy(Object.keys(typeStats), t => {
                    if (col === 'type') {
                        // Alphabetize strings
                        return t;
                    }
                ...
                const sortedStaff = lib_1.Utils.sortBy(Object.keys(staffStats), staff => {
                    if (col === 'staff') {
                        // Alphabetize strings
                        return staff;
                    }
...
exports.commands = {
    ...
    helpticket: {
        ...
        async submit(target, room, user, connection, cmd) {
            ...
            let [ticketType, reportTargetType, reportTarget, text, contextString,] = lib_1.Utils.splitFirst(target, '|', 4).map(s => s.trim());
            reportTarget = lib_1.Utils.escapeHTML(reportTarget);
            ...
            const textTicket = exports.textTickets[typeId];
            if (textTicket) {
                let pageId = '';
                ...
                exports.tickets[user.id] = ticket;
                ...
                void (0, helptickets_auto_1.runPunishments)(ticket, typeId);
            ...
            if (reportTargetType === 'user' && reportTarget) {
                switch (ticket.type) {
                    case 'PM Harassment':
                        ...
                        break;
                    case 'Inappropriate Username':
                        staffIntroButtons = lib_1.Utils.html `<button class="button" name="send" value="/forcerename ${reportTarget}">Force-rename ${reportTarget}</button> `;
                        break;
                }
                staffIntroButtons += lib_1.Utils.html `<button class="button" name="send" value="/modlog room=global, user='${reportTarget}'">Global Modlog for ${reportTarget}</button> <button class="button" name="send" value="/sharedbattles ${user.id}, ${toID(reportTarget)}">Shared battles</button> `;
            }
            if (ticket.type === 'Appeal') {
                staffIntroButtons += lib_1.Utils.html `<button class="button" name="send" value="/modlog room=global, user='${user.name}'">Global Modlog for ${user.name}</button>`;
            }
            const introMsg = lib_1.Utils.html `<h2 style="margin:0">${this.tr `Help Ticket`} - ${user.name}</h2>` +
            ...
            if (reportTargetType === 'room') {
                ...
                if (targetID !== ticket.userid) {
                    const commonBattles = (0, info_1.getCommonBattles)(targetID, Users.get(reportTarget), ticket.userid, Users.get(ticket.userid), this.connection);
                    if (!commonBattles.length) {
                        reportTargetInfo += lib_1.Utils.html `There are no common battles between '${reportTarget}' and '${ticket.creator}'.`;
                    }
                    else {
                        reportTargetInfo += lib_1.Utils.html `Showing ${commonBattles.length} common battle(s) between '${reportTarget}' and '${ticket.creator}': `;
                        reportTargetInfo += commonBattles.map(roomid => lib_1.Utils.html `<a href=/${roomid}>${roomid.replace(/^battle-/, '')}`).join(', ');
                    }
                }
            }
        ...
        async resolve(target, room, user) {
            this.checkCan('lock');
            const [ticketerName, result] = lib_1.Utils.splitFirst(target, ',').map(i => i.trim());
            const ticketId = toID(ticketerName);
            if (!ticketId || !result) {
                return this.parse(`/help helpticket`);
            }
            const ticket = exports.tickets[ticketId];
            if (!ticket)
        ...
        addnote(target, room, user) {
            ...
            const [ticketName, note] = lib_1.Utils.splitFirst(target, ',').map(i => i.trim());
        ...
        removenote(target, room, user) {
            ...
            let [ticketName, staff] = lib_1.Utils.splitFirst(target, ',').map(i => i.trim());
            ...
            const ticket = exports.tickets[targetId];
        ...
        addresponse(target, room, user) {
            this.checkCan('lock');
            const [type, name, response] = lib_1.Utils.splitFirst(target, ',', 2).map(f => f.trim());
            if (!toID(type) || !toID(name) || !toID(response)) {
                return this.parse(`/help helpticket addresponse`);
            }
            const typeId = HelpTicket.getTypeId(type);
            if (!(typeId in exports.textTickets)) {
                throw new Chat.ErrorMessage([`'${type}' is not a valid text ticket type.`,
                    `Valid types: ${Object.keys(exports.textTickets).join(', ')}.`]);
            }
            if (!exports.settings.responses[typeId]) {
                exports.settings.responses[typeId] = {};
            }
            if (exports.settings.responses[typeId][name] && !this.cmd.includes('f')) {
            ...
            exports.settings.responses[typeId][name] = response;
            writeSettings();
        ...
        removeresponse(target, room, user) {
            this.checkCan('lock');
            const [type, name] = lib_1.Utils.splitFirst(target, ',').map(f => f.trim());
            if (!toID(type) || !toID(name))
                return this.parse(`/help helpticket removeresponse`);
            const typeId = HelpTicket.getTypeId(type);
            if (!(type in exports.textTickets)) {
                throw new Chat.ErrorMessage(`'${type}' is not a valid text ticket type.`);
            }
            if (!exports.settings.responses[typeId]?.[name]) {
                throw new Chat.ErrorMessage(`'${name}' is not a response for the ${typeId} ticket type .`);
            }
            delete exports.settings.responses[typeId][name];
            if (!Object.keys(exports.settings.responses[typeId]).length) {
                delete exports.settings.responses[typeId];
            }
            writeSettings();
        ...
        listresponses(target, room, user) {
            ...
            target = toID(target);
            if (target && !(target in exports.textTickets)) {
                throw new Chat.ErrorMessage(`Invalid ticket type: ${target}.`);
            }
            buf += `${target ? `for the type ${target}:` : ""}</strong><hr />`;
            const table = target ? { [target]: exports.settings.responses[target] } : exports.settings.responses;
            if (!Object.keys(table).length) {
                buf += `<p class="message-error">None</p>`;
                return this.sendReplyBox(buf);
            }
            buf += Object.keys(table).map(type => (`<p>${ticketTitles[type]}<p>` +
                Object.keys(exports.settings.responses[type])
                    .map(name => lib_1.Utils.html `<p>- ${name}: "${exports.settings.responses[type][name]}"</p>`).join(''))).join('<hr />');
            return this.sendReplyBox(buf);
        },
        listresponseshelp: [
        ...
        close(target, room, user) {
            ...
            const ticket = exports.tickets[toID(targetUsername)];
        ...
        delete(target, room, user) {
            ...
            const ticket = exports.tickets[toID(target)];
            if (!ticket)
            ...
            if (targetRoom) {
                ...
                delete exports.tickets[ticket.userid];
                writeTickets();
                notifyStaff();
            }
        ...
        logs(target, room, user) {
            this.checkCan('lock');
            const [targetString, dateString] = lib_1.Utils.splitFirst(target, ',').map(i => i.trim());
...
const punishmentfilter = (user, punishment) => {
    ...
    if (typeof user === 'object') {
        ...
        for (const userid of ids) {
            (0, exports.punishmentfilter)(userid, punishment);
        }
    }
    else {
...
const loginfilter = user => {
    const ticket = exports.tickets[user.id];
    if (ticket?.resolved) {
...
exports.handlers = {
    onRoomClose(room, user, conn, isPage) {
        ...
        const ticket = exports.tickets[userid];
...
function start() {
    Chat.multiLinePattern.register('/ht resolve ', '/helpticket resolve ', '/requesthelp resolve ', '/helprequest resolve ', '/ht submit ', '/helpticket submit ');
}
