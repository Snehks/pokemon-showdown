...
exports.roomSettings = exports.commands = exports.UNO = void 0;
/**
 * UNO
...
class UNO extends Rooms.RoomGame {
    constructor(room, cap, suppressMessages) {
        ...
        this.deck = lib_1.Utils.shuffle(createDeck());
        this.discards = [];
        this.topCard = null;
    ...
    onConnect(user, connection) {
        if (this.state === 'signups') {
            connection.sendTo(this.room, `|uhtml|uno-${this.gameNumber}|<div class="broadcast-blue">` +
            ...
            connection.sendTo(this.room, `|uhtml|uno-${this.gameNumber}|<div class="infobox"><p>A game of UNO is currently in progress.<button class="button" name="send" value="/uno spectate">Spectate Game</button></p>` +
    ...
    eliminate(player) {
        ...
        if (removingCurrentPlayer) {
            if (this.state === 'color') {
                ...
                this.sendToRoom(`|raw|${lib_1.Utils.escapeHTML(name)} has not picked a color, the color will stay as <span style="color: ${textColors[this.topCard.changedColor]}">${this.topCard.changedColor}</span>.`);
            }
        }
        if (this.awaitUnoPlayer === player)
            this.awaitUnoPlayer = null;
        if (!this.topCard) {
        ...
        if (!this.suppressMessages || overrideSuppress) {
            ...
            for (const i in this.spectators) {
                if (i in this.playerTable)
                    continue; // don't double send to users already in the game.
    ...
    getPlayers(showCards) {
        ...
        if (!showCards) {
            return playerList.map(p => lib_1.Utils.escapeHTML(p.name)).join(', ');
        }
        let buf = `<ol style="padding-left:0;">`;
        for (const player of playerList) {
            buf += `<li${this.currentPlayer === player ? ` style="font-weight:bold;"` : ''}>`;
            buf += `${lib_1.Utils.escapeHTML(player.name)} (${player.hand.length})`;
            buf += `</li>`;
        }
        buf += `</ol>`;
        return buf;
    ...
    getNextPlayer() {
        // if none is set
        this.currentPlayer || (this.currentPlayer = this.players[Math.floor(this.playerCount * Math.random())]);
        let player = this.players[this.players.indexOf(this.currentPlayer) + this.direction];
        // wraparound
        player || (player = this.direction === 1 ? this.players[0] : this.players[this.playerCount - 1]);
        return player;
    }
    onDraw(player) {
        if (this.currentPlayer !== player || this.state !== 'play')
            return false;
        if (player.cardLock)
            return true;
        this.onCheckUno();
    ...
    onPlay(player, cardName) {
        ...
        this.sendToRoom(`|raw|${lib_1.Utils.escapeHTML(player.name)} has played a <span style="font-weight:bold;color: ${textColors[card.color]}">${card.name}</span>.`);
        // handle hand size
    ...
    onDrawCard(player, count) {
        ...
        player.sendRoom(`|raw|You have drawn the following card${Chat.plural(drawnCards)}: ` +
    ...
    drawCard(count) {
        ...
        for (let i = 0; i < count; i++) {
            if (!this.deck.length) {
                ...
                this.deck = this.discards.length ? lib_1.Utils.shuffle(this.discards) : lib_1.Utils.shuffle(createDeck());
    ...
    onUno(player, unoId) {
        ...
        if (this.unoId !== unoId || player !== this.awaitUnoPlayer)
            return false;
    ...
    onWin(player) {
        this.sendToRoom(lib_1.Utils.html `|raw|<div class="broadcast-blue">Congratulations to ${player.name} for winning the game of UNO!</div>`, true);
        this.destroy();
    }
    destroy() {
        if (this.timer)
...
class UNOPlayer extends Rooms.RoomGamePlayer {
    ...
    buildHand() {
        return lib_1.Utils.sortBy(this.hand, card => [card.color, card.value])
...
exports.commands = {
    uno: {
        ...
        disqualify(target, room, user) {
            ...
            if (!player)
                throw new Chat.ErrorMessage(`Player "${target}" not found.`);
