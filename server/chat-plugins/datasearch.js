...
exports.testables = exports.PM = exports.commands = void 0;
exports.start = start;
...
const supportedDexsearchRules = Object.assign(Object.create(null), {
...
exports.commands = {
    ...
    async dexsearch(target, room, user, connection, cmd, message) {
        ...
        if (target.length > 300)
            throw new chat_1.Chat.ErrorMessage('Dexsearch queries may not be longer than 300 characters.');
        ...
        if (!response.error && !this.runBroadcast())
            return;
        if (response.error) {
            throw new chat_1.Chat.ErrorMessage(response.error);
        }
        else if (response.reply) {
            this.sendReplyBox(response.reply);
        }
        else if (response.dt) {
            chat_1.Chat.commands.data.call(this, response.dt, room, user, connection, 'dt', this.broadcastMessage ? "" : message);
        }
    },
    dexsearchhelp() {
        this.sendReplyBox(`<details class="readmore"><summary><code>/dexsearch [parameter], [parameter], [parameter], ...</code>: searches for Pok\u00e9mon that fulfill the selected criteria.<br/>` +
    ...
    async randommove(target, room, user, connection, cmd, message) {
        ...
        for (const arg of targets) {
            ...
            if (Number.isInteger(num)) {
                if (qty)
                    throw new chat_1.Chat.ErrorMessage("Only specify the number of Pok\u00e9mon Moves once.");
                qty = num;
                if (qty < 1 || MAX_RANDOM_RESULTS < qty) {
                    throw new chat_1.Chat.ErrorMessage(`Number of random Pok\u00e9mon Moves must be between 1 and ${MAX_RANDOM_RESULTS}.`);
                }
    ...
    async randompokemon(target, room, user, connection, cmd, message) {
        ...
        for (const arg of targets) {
            ...
            if (Number.isInteger(num)) {
                if (qty)
                    throw new chat_1.Chat.ErrorMessage("Only specify the number of Pok\u00e9mon once.");
                qty = num;
                if (qty < 1 || MAX_RANDOM_RESULTS < qty) {
                    throw new chat_1.Chat.ErrorMessage(`Number of random Pok\u00e9mon must be between 1 and ${MAX_RANDOM_RESULTS}.`);
                }
                targetsBuffer.push(`random${qty}`);
            }
            else {
    ...
    async randomability(target, room, user, connection, cmd, message) {
        ...
        for (const arg of targets) {
            ...
            if (Number.isInteger(num)) {
                if (qty)
                    throw new chat_1.Chat.ErrorMessage("Only specify the number of abilities once.");
                qty = num;
                if (qty < 1 || MAX_RANDOM_RESULTS < qty) {
                    throw new chat_1.Chat.ErrorMessage(`Number of random abilities must be between 1 and ${MAX_RANDOM_RESULTS}.`);
                }
                targetsBuffer.push(`random${qty}`);
            }
            else {
    ...
    movesearchhelp() {
        this.sendReplyBox(`<code>/movesearch [parameter], [parameter], [parameter], ...</code>: searches for moves that fulfill the selected criteria.<br/><br/>` +
    ...
    itemsearchhelp() {
        this.sendReplyBox(`<code>/itemsearch [item description]</code>: finds items that match the given keywords.<br/>` +
    ...
    async randomitem(target, room, user, connection, cmd, message) {
        ...
        for (const arg of targets) {
            ...
            if (Number.isInteger(num)) {
                if (qty)
                    throw new chat_1.Chat.ErrorMessage("Only specify the number of items once.");
                qty = num;
                if (qty < 1 || MAX_RANDOM_RESULTS < qty) {
                    throw new chat_1.Chat.ErrorMessage(`Number of random items must be between 1 and ${MAX_RANDOM_RESULTS}.`);
                }
                targetsBuffer.push(`random${qty}`);
            }
            else {
    ...
    abilitysearchhelp() {
        this.sendReplyBox(`<code>/abilitysearch [ability description]</code>: finds abilities that match the given keywords.<br/>` +
            `This command accepts natural language. (tip: fewer words tend to work better)<br/>` +
    ...
    async learn(target, room, user, connection, cmd, message) {
        ...
        if (target.length > 300)
            throw new chat_1.Chat.ErrorMessage(`Query too long.`);
    ...
    async randomtype(target, room, user, connection, cmd, message) {
        ...
        for (const arg of targets) {
            ...
            if (Number.isInteger(num)) {
                if (qty)
                    throw new chat_1.Chat.ErrorMessage("Only specify the number of types once.");
                qty = num;
                if (qty < 1 || MAX_RANDOM_RESULTS < qty) {
                    throw new chat_1.Chat.ErrorMessage(`Number of random types must be between 1 and ${MAX_RANDOM_RESULTS}.`);
                }
                targetsBuffer.push(`random${qty}`);
            }
            else {
...
function getRule(target) {
    ...
    return { splitTarget: arr, usedRules: ruleTerms.map(x => x.toLowerCase().replace(/[^a-z0-9=]+/g, '').split('rule=')[1]), count };
}
function prepareDexsearchValidator(usedMod, rules, nationalSearch) {
    ...
    return team_validator_1.TeamValidator.get(`${format}${additionalRules.length ? `@@@${additionalRules.join(',')}` : ''}`);
}
function runDexsearch(target, cmd, message, isTest) {
    const searches = [];
    ...
    const allTiers = Object.assign(Object.create(null), {
    ...
    const singlesTiersValues = Object.assign(Object.create(null), {
        AG: 14, Uber: 13,
        OU: 12, CAP: 12,
        UUBL: 11, UU: 10,
        RUBL: 9, RU: 8,
    ...
    const allDoublesTiers = Object.assign(Object.create(null), {
    ...
    const doublesTiersValues = Object.assign(Object.create(null), {
        DUber: 4, DOU: 3,
        DBL: 2, DUU: 1,
        '(DUU)': 0,
        NFE: -1,
    ...
    const allEggGroups = Object.assign(Object.create(null), {
        amorphous: 'Amorphous',
        bug: 'Bug',
        ditto: 'Ditto',
        dragon: 'Dragon',
    ...
    const validParameter = (cat, param, isNotSearch, input) => {
        ...
        for (const group of searches) {
            ...
            if (cat === 'stats') {
                ...
                const result = validStatInequality(group['stats'], inequality[0], inequality[1], inequality[2], +inequality[3], input);
                if (!result)
                    continue;
                return result;
            }
        ...
        for (const parameter of parameters) {
            ...
            if (inequality >= 0) {
                ...
                if (!orGroup.stats[statKey])
                    orGroup.stats[statKey] = Object.create(null);
    ...
    if (showAll && searches.length === 0 && singleTypeSearch === null &&
    ...
    for (const species of mod.species.all()) {
        ...
        for (const rule of rules) {
            ...
            if (!validator)
                validator = prepareDexsearchValidator(usedMod, rules, nationalSearch);
            ruleResult = !rule.onValidateSet?.call(validator, { name: species.name, species: species.id }, validator.format, {}, {});
        }
        if (species.gen <= mod.gen &&
            ...
            for (const rule of rules) {
                newSpecies = rule?.onModifySpecies?.call({ dex: mod, clampIntRange: lib_1.Utils.clampIntRange, toID }, newSpecies) || newSpecies;
            }
    ...
    lib_1.Utils.sortBy(searches, search => (Object.values(search).reduce(accumulateKeyCount, 0)));
    for (const alts of searches) {
        ...
        for (const mon in dex) {
            ...
            for (const targetResist in alts.resists) {
                ...
                if (notImmune && !move.ohko && move.damage === undefined) {
                    for (const defenderType of dex[mon].types) {
                        ...
                        const moveMod = move.onEffectiveness?.call({ dex: mod }, baseMod, null, defenderType, move);
            ...
            if (matched)
                continue;
            function retrieveStat(species, stat) {
                let monStat = 0;
                if (stat === 'bst') {
                    monStat = species.bst;
                }
                    ...
                    monStat = species.baseStats[stat];
                }
                return monStat;
            }
            for (const stat in alts.stats) {
                ...
                for (const direction in alts.stats[stat]) {
                    for (const comparisonStat in alts.stats[stat][direction]) {
                        const checkStat = alts.stats[stat][direction][comparisonStat];
                        if (!checkStat)
                            continue;
            ...
            if (validator) {
                for (const move of altsMoves) {
                    ...
                    for (const rule of rules) {
                        ...
                        matchRule = !rule.checkCanLearn?.call(validator, move, dex[mon], pokemonSource, {}) === !isNotSearch;
    ...
    for (const mon of Object.values(dex).sort()) {
        ...
        if (!isRegionalForm && !maskForm && mon.baseSpecies && results.includes(mod.species.get(mon.baseSpecies)) &&
            getSortValue(mon) === getSortValue(mod.species.get(mon.baseSpecies)))
            continue;
        ...
        if (teraFormeChangesFrom && results.includes(mod.species.get(teraFormeChangesFrom)) &&
            getSortValue(mon) === getSortValue(mod.species.get(teraFormeChangesFrom)))
            continue;
    ...
    if (randomOutput && randomOutput < results.length) {
        results = lib_1.Utils.shuffle(results).slice(0, randomOutput);
    }
    let resultsStr = (message === "" ? message :
        `<span class="gray">${lib_1.Utils.escapeHTML(message)}:</span><br/>`);
    if (results.length > 1) {
        results.sort();
        if (sort) {
            const direction = sort.slice(-1);
            lib_1.Utils.sortBy(results, species => getSortValue(species) * (direction === '+' ? 1 : -1));
        }
        function mapPokemonResults(inputArr) {
            return inputArr.map(result => `<a href="//${Config.routes.dex}/pokemon/${toID(result.name)}" target="_blank" class="subtle" style="white-space:nowrap"><psicon pokemon="${result.name}" style="vertical-align:-7px;margin:-2px" />${result.name}</a>`).join(", ");
        }
...
function formatCollapsible(message, showall, results, mapfunc) {
    ...
    return `<details class="details"${expand ? ' open' : ''}><summary><span class="gray">${lib_1.Utils.escapeHTML(header)}</span><br/><span class="details-preview">${resultsSummary}</span></summary>${resultsHidden}</details>`;
}
function runMovesearch(target, cmd, message, isTest) {
    const searches = [];
    ...
    for (const arg of splitTarget) {
        ...
        for (const parameter of parameters) {
            ...
            if (allCategories.includes(target)) {
                ...
                if ((orGroup.categories[target] && isNotSearch) ||
            ...
            if (allContestTypes.includes(target)) {
                target = target.charAt(0).toUpperCase() + target.substr(1);
                if ((orGroup.contestTypes[target] && isNotSearch) ||
            ...
            if (target.startsWith('targets ')) {
                ...
                if (Object.keys(allTargets).includes(target)) {
                    ...
                    if ((orGroup.targets[moveTarget] && isNotSearch) ||
            ...
            if (species.exists) {
                if (parameters.length > 1)
                    return { error: "A Pok\u00e9mon learnset cannot have alternative parameters." };
            ...
            if (allVolatileStatus.includes(target)) {
                if ((orGroup.volatileStatus[target] && isNotSearch) ||
    ...
    for (const moveid of validMoves) {
        const move = mod.moves.get(moveid);
        if (move.gen <= mod.gen) {
            if ((!nationalSearch && move.isNonstandard && move.isNonstandard !== "Gigantamax") ||
        ...
        for (const moveid in dex) {
            ...
            if (Object.keys(alts.contestTypes).length) {
                if (alts.contestTypes[move.contestType || 'Cool'])
                    continue;
                if (Object.values(alts.contestTypes).includes(false) &&
            ...
            for (const prop in alts.property) {
                if (typeof alts.property[prop].less === "number") {
                    if (move[prop] !== true &&
                        move[prop] < alts.property[prop].less) {
                        matched = true;
                        break;
                    }
                }
                if (typeof alts.property[prop].greater === "number") {
                    if ((move[prop] === true && move.category !== "Status") ||
                        move[prop] > alts.property[prop].greater) {
                        matched = true;
                        break;
                    }
                }
            ...
            for (const searchStatus in alts.status) {
                let canStatus = !!(move.status === searchStatus ||
            ...
            for (const searchStatus in alts.volatileStatus) {
                let canStatus = !!((move.secondary && move.secondary.volatileStatus === searchStatus) ||
    ...
    if (targetMons.length) {
        ...
        resultsStr += (message === "" ? message : `<span class="gray">${lib_1.Utils.escapeHTML(message)}:</span><br />`);
    }
    if (randomOutput && randomOutput < results.length) {
        results = lib_1.Utils.shuffle(results).slice(0, randomOutput);
    }
    if (results.length > 1) {
        ...
        if (sort) {
            ...
            const direction = sort.slice(-1);
            lib_1.Utils.sortBy(results, moveName => {
        ...
        function mapMoveResults(inputArray) {
            return inputArray.map(result => `<a href="//${Config.routes.dex}/moves/${toID(result)}" target="_blank" class="subtle" style="white-space:nowrap">${result}</a>` +
                (sort ?
...
function runItemsearch(target, cmd, message) {
    let showAll = false;
    let maxGen = 0;
    let gen = 0;
    let randomOutput = 0;
        ...
        for (const item of dex.items.all()) {
            ...
            if (basePower && effect) {
                if (item.fling.basePower === basePower &&
                    (item.fling.status === effect || item.fling.volatileStatus === effect))
    ...
    function mapItemResults(inputArr) {
        return inputArr.map(result => `<a href="//${Config.routes.dex}/items/${toID(result)}" target="_blank" class="subtle" style="white-space:nowrap"><psicon item="${result}" style="vertical-align:-7px" />${result}</a>`).join(", ");
    }
    let resultsStr = (message === "" ? message : `<span class="gray">${lib_1.Utils.escapeHTML(message)}:</span><br />`);
    if (randomOutput !== 0) {
        const randomItems = [];
        if (foundItems.length === 0) {
...
function runAbilitysearch(target, cmd, message) {
    ...
    for (const ability of dex.abilities.all()) {
        ...
        if (/[1-9.]+x/.test(descWords))
            descWords += ' increases';
        descWords = descWords.replace(/super[-\s]effective/g, 'supereffective');
        const descWordsArray = chat_1.Chat.normalize(descWords).split(' ');
        for (const word of searchedWords) {
            switch (word) {
                case 'specialattack':
    ...
    function mapAbilityResults(inputArr) {
        return inputArr.map(result => `<a href="//${Config.routes.dex}/abilities/${toID(result)}" target="_blank" class="subtle" style="white-space:nowrap">${result}</a>`).join(", ");
    }
...
function runLearn(target, cmd, formatid) {
    ...
    const validator = team_validator_1.TeamValidator.get(format);
...
function runSearch(query, user) {
    if (user) {
        if (user.lastCommand.startsWith('/datasearch ')) {
            throw new chat_1.Chat.ErrorMessage(`You already have a datasearch query pending. Wait until it's complete before running another.`);
        }
        user.lastCommand = `/datasearch ${query.cmd}`;
    }
    return exports.PM.query(query).finally(() => {
        if (user) {
            user.lastCommand = '';
        }
    });
}
function runRandtype(target, cmd, message) {
    const icon = {};
    ...
    let resultsStr = (message === "" ? message : `<span class="gray">${lib_1.Utils.escapeHTML(message)}:</span><br />`);
    resultsStr += randTypes.map(type => icon[type]).join(' ');
    return { reply: resultsStr };
}
/*********************************************************
 * Process manager
 *********************************************************/
exports.PM = new lib_1.ProcessManager.QueryProcessManager('dexsearch', module, query => {
    try {
...
exports.testables = {
    runAbilitysearch: (target, cmd, message) => runAbilitysearch(target, cmd, message),
    runDexsearch: (target, cmd, message) => runDexsearch(target, cmd, message, true),
    runMovesearch: (target, cmd, message) => runMovesearch(target, cmd, message, true),
};
function start(processCount) {
    exports.PM.spawn(processCount['datasearch'] ?? 1);
}
function destroy() {
