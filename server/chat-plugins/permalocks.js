...
exports.pages = exports.commands = exports.Nominations = exports.Smogon = void 0;
...
function getIPData(ip) {
    try {
        return (0, lib_1.Net)("https://miapi.dev/api/ip/" + ip).get().then(JSON.parse);
    }
    catch {
        return null;
    }
}
exports.Smogon = new class {
    async post(threadNum, postText) {
        if (!Config.smogon)
            return null;
        try {
            const raw = await (0, lib_1.Net)(`https://www.smogon.com/forums/api/posts`).get({
                method: 'POST',
...
exports.Nominations = new class {
    ...
    load() {
        try {
            let data = JSON.parse((0, lib_1.FS)('config/chat-plugins/permas.json').readSync());
            if (Array.isArray(data)) {
                data = { noms: data, icons: {} };
                (0, lib_1.FS)('config/chat-plugins/permas.json').writeSync(JSON.stringify(data));
            }
            this.noms = data.noms;
    ...
    save() {
        (0, lib_1.FS)('config/chat-plugins/permas.json').writeUpdate(() => JSON.stringify({ noms: this.noms, icons: this.icons }));
    }
    notifyStaff() {
    ...
    async add(target, connection) {
        ...
        const [primary, rawAlts, rawIps, type, details] = lib_1.Utils.splitFirst(target, '|', 4).map(f => f.trim());
        ...
        this.noms.push({
            by: user.id,
            alts: [...altTable],
            ips: lib_1.Utils.sortBy([...ipTable], z => -(IPTools.ipToNumber(z) || Infinity)),
            info: details,
            primaryID,
            standing: type,
            date: Date.now(),
        });
        lib_1.Utils.sortBy(this.noms, nom => -nom.date);
        this.save();
        this.notifyStaff();
    ...
    displayModlog(results) {
        ...
        return results.map(result => {
            ...
            if (result.note)
                line += lib_1.Utils.html `: ${result.note}`;
...
exports.commands = {
    perma: {
        ...
        submit(target, room, user) {
            this.checkCan('lock');
            return exports.Nominations.add(target, this.connection);
        },
        list() {
            this.checkCan('lock');
            return this.parse(`/j view-permalocks-list`);
        ...
        async actmain(target, room, user) {
            ...
            const icon = exports.Nominations.icons[user.id] ? `:${exports.Nominations.icons[user.id]}: - ` : ``;
            ...
            const modlog = await exports.Nominations.fetchModlog(nom.primaryID);
            if (modlog?.results.length) {
                let rawHTML = exports.Nominations.displayModlog(modlog.results);
                rawHTML = rawHTML.replace(/<br \/>/g, '\n');
                rawHTML = lib_1.Utils.stripHTML(rawHTML);
                rawHTML = rawHTML.replace(/&#x2f;/g, '/');
                postBuf += `\n[spoiler=Modlog]${rawHTML}[/spoiler]`;
            }
            const res = await exports.Smogon.post(threadNum, postBuf);
            if (!res || res.error) {
        ...
        async standing(target) {
            this.checkCan('rangeban');
            const [name, rawStanding, reason] = lib_1.Utils.splitFirst(target, ',', 2).map(f => f.trim());
            const id = toID(name);
            if (!id || id.length > 18) {
        ...
        async ipstanding(target) {
            this.checkCan('rangeban');
            const [ip, standingName, reason] = lib_1.Utils.splitFirst(target, ',', 2).map(f => f.trim());
        ...
        resolve(target) {
            this.checkCan('rangeban');
            exports.Nominations.close(target, this);
        },
        seticon(target, room, user) {
            ...
            exports.Nominations.icons[targetId] = mon.name.toLowerCase();
            exports.Nominations.save();
            this.sendReply(`|html|Updated ${targetId === user.id ? 'your' : `${targetId}'s`} permalock post icon to ` +
                `<psicon pokemon='${mon.name.toLowerCase()}' />`);
        },
        deleteicon(target, room, user) {
            this.checkCan('rangeban');
            const targetID = toID(target);
            if (!exports.Nominations.icons[targetID]) {
                throw new Chat.ErrorMessage(`${targetID} does not have an icon set.`);
            }
            delete exports.Nominations.icons[targetID];
            exports.Nominations.save();
...
exports.pages = {
    permalocks: {
        list(query, user, conn) {
            ...
            return exports.Nominations.displayAll(user.can('rangeban'));
        },
        view(query, user) {
            this.checkCan('rangeban');
            ...
            const nom = exports.Nominations.find(id);
            if (!nom)
                throw new Chat.ErrorMessage(`No nomination found for '${id}'.`);
            this.title = `[Perma Nom] ${nom.primaryID}`;
            return exports.Nominations.displayActionPage(nom);
        },
        submit() {
            this.checkCan('lock');
            this.title = '[Perma Nom] Create';
            return exports.Nominations.displayNomPage();
        },
    },
};
function start() {
