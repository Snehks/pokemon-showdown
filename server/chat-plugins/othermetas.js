...
exports.commands = {
    ...
    mixandmega(target, room, user) {
        ...
        if (!species.exists)
            throw new Chat.ErrorMessage(`Error: Pok\u00e9mon not found.`);
        let baseSpecies;
        let megaSpecies;
        switch (stone.id) {
            case 'blueorb':
        ...
        const mixedSpecies = lib_1.Utils.deepClone(species);
        mixedSpecies.abilities = lib_1.Utils.deepClone(megaSpecies.abilities);
        ...
        for (statName in species.baseStats) { // Add the changed stats and weight
            mixedSpecies.baseStats[statName] = lib_1.Utils.clampIntRange(mixedSpecies.baseStats[statName] + deltas.baseStats[statName], 1, 255);
        ...
        this.sendReply(`|raw|<font size="1">` + Object.entries(details).map(([detail, value]) => (value === '' ? detail : `<font color="#686868">${detail}:</font> ${value}`)).join("&nbsp;|&ThickSpace;") + `</font>`);
    },
    mixandmegahelp: [
    ...
    stone(target) {
        ...
        const stones = [];
        if (!stone) {
            const formeIdRegex = new RegExp(`(?:mega[xyz]?|primal|origin|crowned|epilogue|cornerstone|wellspring|hearthflame|douse|shock|chill|burn|${dex.types.all().map(x => x.id).filter(x => x !== 'normal').join('|')})$`);
            ...
            for (const poke of species.otherFormes) {
                const formeRegex = new RegExp(`(?:-Douse|-Shock|-Chill|-Burn|-Cornerstone|-Wellspring|-Hearthflame|-Crowned|-Epilogue|-Origin|-Primal|-Mega(?:-[XYZ])?|${dex.types.names().filter(x => x !== 'Normal').map(x => '-' + x).join('|')})$`);
                ...
                if (/mega[xyz]$/.test(targetid) && toID(megaPoke.name) !== toID(dex.species.get(targetid)))
                    continue;
                if (!flag)
                    continue;
    ...
    '350cup'(target, room, user) {
        ...
        const species = lib_1.Utils.deepClone(dex.species.get(args[0]));
    ...
    tiershift(target, room, user, connection, cmd) {
        ...
        for (const statName in species.baseStats) {
            ...
            if (dex.gen === 1 && statName === 'spd')
                continue;
            species.baseStats[statName] = lib_1.Utils.clampIntRange(species.baseStats[statName] + boost, 1, 255);
    ...
    franticfusions(target, room, user, connection, cmd) {
        ...
        if (targets.length > 2)
            return this.parse('/help franticfusions');
        const species = lib_1.Utils.deepClone(dex.species.get(targets[0]));
        ...
        for (const statName in species.baseStats) {
            ...
            if (!fusion.name.length) {
                ...
                species.baseStats[statName] = lib_1.Utils.clampIntRange(species.baseStats[statName] + addition, 1, 255);
                species.bst += species.baseStats[statName];
            }
        }
        const abilities = new Set([...Object.values(species.abilities), ...Object.values(fusion.abilities)]);
    ...
    scalemons(target, room, user, connection, cmd) {
        ...
        for (const stat in species.baseStats) {
            ...
            if (dex.gen === 1 && stat === 'spd')
                continue;
            species.baseStats[stat] = lib_1.Utils.clampIntRange(species.baseStats[stat] * scale / bstNoHP, 1, 255);
    ...
    flipped(target, room, user, connection, cmd) {
        ...
        const species = lib_1.Utils.deepClone(dex.species.get(mon));
        if (!species.exists || species.gen > dex.gen) {
    ...
    natureswap(target, room, user, connection, cmd) {
        ...
        const species = lib_1.Utils.deepClone(dex.species.get(pokemon));
        if (!species.exists || species.gen > dex.gen) {
            const monName = species.gen > dex.gen ? species.name : args[0].trim();
            const additionalReason = species.gen > dex.gen ? ` in Generation ${dex.gen}` : ``;
            throw new Chat.ErrorMessage(`Error: Pok\u00e9mon '${monName}' not found${additionalReason}.`);
    ...
    crossevolve(target, user, room) {
        ...
        if (!species.exists)
            throw new Chat.ErrorMessage(`Error: Pok\u00e9mon '${pokes[0]}' not found.`);
        if (!crossSpecies.exists)
            throw new Chat.ErrorMessage(`Error: Pok\u00e9mon '${pokes[1]}' not found.`);
        ...
        const mixedSpecies = lib_1.Utils.deepClone(species);
        mixedSpecies.abilities = lib_1.Utils.deepClone(crossSpecies.abilities);
        mixedSpecies.baseStats = lib_1.Utils.deepClone(mixedSpecies.baseStats);
        mixedSpecies.bst = 0;
        let statName;
        for (statName in species.baseStats) {
            const statChange = crossSpecies.baseStats[statName] - prevo.baseStats[statName];
            mixedSpecies.baseStats[statName] = lib_1.Utils.clampIntRange(mixedSpecies.baseStats[statName] + statChange, 1, 255);
            mixedSpecies.bst += mixedSpecies.baseStats[statName];
        }
    ...
    showevo(target, room, user, connection, cmd) {
        const args = target.split(',');
        if (!toID(args[0]))
            return this.parse('/help reevohelp');
        this.runBroadcast();
        let dex = Dex;
        if (args[1] && toID(args[1]) in Dex.dexes) {
            dex = Dex.dexes[toID(args[1])];
        ...
        if (!evo.prevo) {
            const evoBaseSpecies = dex.species.get((Array.isArray(evo.battleOnly) ? evo.battleOnly[0] : evo.battleOnly) || evo.changesFrom || evo.name);
            ...
            const deltas = lib_1.Utils.deepClone(evo);
            if (!isReEvo) {
                deltas.tier = 'CE';
                ...
                if (deltas.types.length) {
                    ...
                    deltas.types = deltas.types.filter((type) => type !== undefined);
            ...
            for (i in evo.baseStats) {
                ...
                if (!isReEvo) {
                    ...
                    deltas.baseStats[i] = lib_1.Utils.clampIntRange(evoBaseSpecies.baseStats[i] + statChange, 1, 255);
                    deltas.baseStats[i] = lib_1.Utils.clampIntRange(deltas.baseStats[i] + formeChange, 1, 255);
                }
            ...
            for (i in evo.baseStats) {
                ...
                if (!isReEvo) {
                    ...
                    deltas.baseStats[i] = lib_1.Utils.clampIntRange(deltas.baseStats[i] + statChange, 1, 255);
                }
                deltas.bst += deltas.baseStats[i];
            }
            const details = {
    ...
    pokemove(target, room, user) {
        ...
        const move = lib_1.Utils.deepClone(Dex.moves.get('tackle'));
    ...
    badnboosted(target, room, user) {
        const args = target.split(',');
        if (!toID(args[0]))
            return this.parse('/help badnboostedhelp');
        this.runBroadcast();
        let dex = Dex;
        if (args[1] && toID(args[1]) in Dex.dexes) {
            dex = Dex.dexes[toID(args[1])];
