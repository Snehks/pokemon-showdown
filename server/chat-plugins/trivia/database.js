...
exports.TriviaSQLiteDatabase = exports.LEADERBOARD_ENUM = void 0;
...
class TriviaSQLiteDatabase {
    ...
    async updateLeaderboardForUser(userid, additions) {
        ...
        for (const [lb, discrim] of Object.entries(exports.LEADERBOARD_ENUM)) {
            if (!additions[lb])
                continue;
    ...
    async mergeLeaderboardEntries(from, to) {
        ...
        for (const lbDiscrim of Object.values(exports.LEADERBOARD_ENUM)) {
            ...
            await Chat.database.run(this.leaderboardInsertion, [to, toScores.score, toScores.totalPoints, toScores.totalCorrectAnswers, lbDiscrim]);
    ...
    async acceptSubmissions(submissions) {
        ...
        await Chat.database.run(`UPDATE trivia_questions SET is_submission = 0 WHERE question IN (${(0, utils_1.formatSQLArray)(submissions)})`, submissions);
    }
    ...
    async getQuestions(categories, limit, options) {
        ...
        if (categories === 'all') {
            ...
            query = (`SELECT * FROM trivia_questions WHERE category IN (${(0, utils_1.formatSQLArray)(categories)}) AND is_submission = 0 ORDER BY ${options.order === 'random' ? 'RANDOM()' : `added_at ${(options.order === 'oldestfirst' ? 'ASC' : 'DESC')}`} LIMIT ?`);
    ...
    async getLeaderboardEntry(id, leaderboard) {
        ...
        const row = await this.leaderboardByUserQuery.get([id, exports.LEADERBOARD_ENUM[leaderboard]]);
        if (!row)
            return null;
        return {
            score: row.score,
    ...
    async deleteLeaderboardEntry(userid, leaderboard) {
        ...
        await this.leaderboardDeletionQuery.run([userid, exports.LEADERBOARD_ENUM[leaderboard]]);
    }
    async deleteSubmissions(submissions) {
        ...
        const query = await Chat.database.prepare(`DELETE FROM trivia_questions WHERE is_submission = 1 AND question IN (${(0, utils_1.formatSQLArray)(submissions)})`);
    ...
    async prepareStatements() {
        ...
        this.leaderboardInsertion = await Chat.database.prepare(`INSERT OR REPLACE INTO trivia_leaderboard (userid, score, total_points, total_correct_answers, leaderboard) VALUES (?, ?, ?, ?, ?) `);
        this.questionInsertion = await Chat.database.prepare(`INSERT OR IGNORE INTO trivia_questions (question, category, added_at, userid, is_submission) VALUES (?, ?, ?, ?, ?)`);
        this.answerInsertion = await Chat.database.prepare(`INSERT INTO trivia_answers (question_id, answer) VALUES (?, ?)`);
        this.gameHistoryInsertion = await Chat.database.prepare(`INSERT INTO trivia_game_history (mode, length, category, time, creator, gives_points) VALUES (?, ?, ?, ?, ?, ?)`);
        this.scoreHistoryInsertion = await Chat.database.prepare(`INSERT INTO trivia_game_scores (game_id, userid, score) VALUES (?, ?, ?)`);
        this.updateMoveEventQuestions = await Chat.database.prepare(`INSERT OR REPLACE INTO trivia_settings (key, value) VALUES ('moveEventQuestions', ?)`);
        this.categoryChangeQuery = await Chat.database.prepare(`UPDATE trivia_questions SET category = ? WHERE question = ?`);
        this.leaderboardChangeQuery = await Chat.database.prepare(`INSERT INTO trivia_leaderboard (userid, score, total_points, total_correct_answers, leaderboard) ` +
        ...
        this.migrateCategoryQuery = await Chat.database.prepare(`UPDATE OR REPLACE trivia_questions SET category = ? WHERE category = ?`);
        this.historyQuery = await Chat.database.prepare(`SELECT * FROM trivia_game_history ORDER BY time DESC LIMIT ?`);
        this.historyScoresQuery = await Chat.database.prepare(`SELECT userid, score FROM trivia_game_scores WHERE game_id = ?`);
        this.allQuestionsRandomOrderQuery = await Chat.database.prepare(`SELECT * FROM trivia_questions WHERE category IN ('ae', 'pokemon', 'sg', 'sh') AND is_submission = 0 ORDER BY RANDOM() LIMIT ?`);
        this.allQuestionsNewestFirstQuery = await Chat.database.prepare(`SELECT * FROM trivia_questions WHERE category IN ('ae', 'pokemon', 'sg', 'sh') AND is_submission = 0 ORDER BY added_at DESC LIMIT ?`);
        this.allQuestionsOldestFirstQuery = await Chat.database.prepare(`SELECT * FROM trivia_questions WHERE category IN ('ae', 'pokemon', 'sg', 'sh') AND is_submission = 0 ORDER BY added_at ASC LIMIT ?`);
        this.answersQuery = await Chat.database.prepare(`SELECT * FROM trivia_answers WHERE question_id = ?`);
        this.submissionsQuery = await Chat.database.prepare(`SELECT * FROM trivia_questions WHERE is_submission = 1 ORDER BY category ASC`);
        this.leaderboardQuery = await Chat.database.prepare(`SELECT * FROM trivia_leaderboard`);
        this.leaderboardByUserQuery = await Chat.database.prepare(`SELECT * FROM trivia_leaderboard WHERE userid = ? AND leaderboard = ?`);
        this.scoreAndPointsByUser = await Chat.database.prepare(`SELECT score, total_points as totalPoints, total_correct_answers as totalCorrectAnswers FROM trivia_leaderboard WHERE userid = ? AND leaderboard = ?`);
        this.eventQuestionQuery = await Chat.database.prepare(`SELECT * FROM trivia_settings WHERE key = 'moveEventQuestions'`);
        this.categoriesQuery = await Chat.database.prepare(`SELECT DISTINCT category FROM trivia_questions`);
        this.questionCountQuery = await Chat.database.prepare(`SELECT count(*) AS count FROM trivia_questions WHERE is_submission = 0`);
        this.categoryQuestionCountQuery = await Chat.database.prepare(`SELECT count(*) AS count FROM trivia_questions WHERE category = ? AND is_submission = 0`);
        this.questionSearchQuery = await Chat.database.prepare(`SELECT * FROM trivia_questions WHERE question LIKE ? AND is_submission = ? ORDER BY added_at DESC`);
        this.questionExistsQuery = await Chat.database.prepare(`SELECT * FROM trivia_questions WHERE question = ?`);
        this.leaderboardDeletionQuery = await Chat.database.prepare(`DELETE FROM trivia_leaderboard WHERE userid = ? AND leaderboard = ?`);
        this.clearAllSubmissionsQuery = await Chat.database.prepare(`DELETE FROM trivia_questions WHERE is_submission = 1`);
        this.clearCategoryQuery = await Chat.database.prepare(`DELETE FROM trivia_questions WHERE category = ? AND is_submission = 0`);
        ...
        this.clearCycleLeaderboardQuery = await Chat.database.prepare(`DELETE FROM trivia_leaderboard WHERE leaderboard = ${exports.LEADERBOARD_ENUM.cycle}`);
        this.deleteQuestionQuery = await Chat.database.prepare(`DELETE FROM trivia_questions WHERE question = ?`);
    ...
    async convertLegacyJSON() {
        ...
        let triviaData;
        try {
            triviaData = JSON.parse((0, lib_1.FS)(this.legacyJSONPath).readIfExistsSync() || "{}");
            if (!triviaData)
        ...
        if (typeof triviaData.leaderboard === 'object') {
            for (const userid in triviaData.leaderboard) {
                ...
                await Chat.database.run(this.leaderboardInsertion, [userid, score, totalGamePoints, totalCorrectAnswers, Number(true)]);
            }
        }
        if (typeof triviaData.altLeaderboard === 'object') {
            for (const userid in triviaData.altLeaderboard) {
                ...
                await Chat.database.run(this.leaderboardInsertion, [userid, score, totalGamePoints, totalCorrectAnswers, Number(false)]);
            }
        }
        // convert questions
        ...
        try {
            await (0, lib_1.FS)(this.legacyJSONPath).rename(this.legacyJSONPath + '.converted');
        }
        catch { }
    }
    rowToQuestion(row) {
