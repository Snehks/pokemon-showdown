...
exports.commands = exports.MastermindFinals = exports.MastermindRound = exports.Mastermind = exports.TriumvirateModeTrivia = exports.NumberModeTrivia = exports.TimerModeTrivia = exports.FirstModeTrivia = exports.Trivia = exports.cachedLadder = exports.pendingAltMerges = exports.database = void 0;
...
exports.database = new database_1.TriviaSQLiteDatabase('config/chat-plugins/triviadata.json');
/** from:to Map */
exports.pendingAltMerges = new Map();
...
async function getQuestions(categories, order, limit = 1000) {
    if (categories === 'random') {
        const history = await exports.database.getHistory(1);
        const lastCategoryID = toID(history?.[0].category).replace("random", "");
        const randCategory = lib_1.Utils.randomElement(Object.keys(MAIN_CATEGORIES)
            .filter(cat => toID(MAIN_CATEGORIES[cat]) !== lastCategoryID));
        return exports.database.getQuestions([randCategory], limit, { order });
    }
    else {
        const questions = [];
        for (const category of categories) {
            if (category === 'all') {
                questions.push(...(await exports.database.getQuestions('all', limit, { order })));
            }
            else if (!ALL_CATEGORIES[category]) {
                throw new Chat.ErrorMessage(`"${category}" is an invalid category.`);
            }
        }
        questions.push(...(await exports.database.getQuestions(categories.filter(c => c !== 'all'), limit, { order })));
        return questions;
    }
}
/**
...
async function requestAltMerge(from, to) {
    ...
    if (!(await exports.database.getLeaderboardEntry(from, 'alltime'))) {
        throw new Chat.ErrorMessage(`The user '${from}' does not have an entry in the Trivia leaderboard.`);
    }
    if (!(await exports.database.getLeaderboardEntry(to, 'alltime'))) {
...
async function mergeAlts(from, to) {
    ...
    await exports.database.mergeLeaderboardEntries(from, to);
...
class Ladder {
    ...
    async computeCachedLadder() {
        const leaderboards = await exports.database.getLeaderboards();
        for (const [lb, data] of Object.entries(leaderboards)) {
            ...
            for (const key of ['score', 'totalPoints', 'totalCorrectAnswers']) {
                lib_1.Utils.sortBy(leaders, ([, scores]) => -scores[key]);
                let max = Infinity;
                let rank = -1;
...
exports.cachedLadder = new Ladder();
...
class Trivia extends Rooms.RoomGame {
    ...
    addTriviaPlayer(user) {
        ...
        for (const id in this.playerTable) {
            ...
            if (targetUser) {
                const isSameUser = (targetUser.previousIDs.includes(user.id) ||
    ...
    init() {
        ...
        broadcast(this.room, this.room.tr(signupsMessage), this.room.tr `Mode: ${this.game.mode} | Category: ${this.game.category} | Cap: ${this.getDisplayableCap()}<br />` +
    ...
    formatPlayerList(settings) {
        return this.getTopPlayers(settings).map(player => {
            const buf = lib_1.Utils.html `${player.name} (${player.player.points || "0"})`;
    ...
    kick(user) {
        if (!this.playerTable[user.id]) {
            ...
            for (const kickedUserid of this.kickedUsers) {
                ...
                if (kickedUser) {
                    const isSameUser = (kickedUser.previousIDs.includes(user.id) ||
    ...
    async askQuestion() {
        ...
        if (!this.questions.length) {
            ...
            if (!cap.questions && !cap.points) {
                if (this.game.length === 'infinite') {
                    this.questions = await exports.database.getQuestions(this.categories, 1000, {
            ...
            if (this.phaseTimeout)
                clearTimeout(this.phaseTimeout);
            this.phaseTimeout = null;
            broadcast(this.room, this.room.tr `No questions are left!`, this.room.tr `The game has reached a stalemate`);
            if (this.room)
                this.destroy();
            return;
        }
        ...
        if (question.category === MOVE_QUESTIONS_AFTER_USE_FROM_CATEGORY && await exports.database.shouldMoveEventQuestions()) {
            await exports.database.moveQuestionToCategory(question.question, MOVE_QUESTIONS_AFTER_USE_TO_CATEGORY);
        }
    }
    setTallyTimeout() {
    ...
    sendQuestion(question) {
        broadcast(this.room, this.room.tr `Question ${this.questionNumber}: ${question.question}`, this.room.tr `Category: ${ALL_CATEGORIES[question.category]}`);
    }
    /**
    ...
    verifyAnswer(targetAnswer) {
        return this.curAnswers.some(answer => {
            ...
            return (answer === targetAnswer) || (lib_1.Utils.levenshtein(targetAnswer, answer, mla) <= mla);
        });
    }
    /**
    ...
    async win(buffer) {
        ...
        for (const i in this.playerTable) {
            ...
            if (!user)
                continue;
            user.sendTo(this.room.roomid, (this.game.givesPoints ? this.room.tr `You gained ${player.points} points and answered ` : this.room.tr `You answered `) +
        ...
        if (this.game.givesPoints) {
            ...
            for (let i = 0; i < winners.length; i++) {
                scores.set(winners[i].id, prizes[i]);
            }
            for (const userid in this.playerTable) {
                const player = this.playerTable[userid];
                if (!player.points)
                    continue;
                void exports.database.updateLeaderboardForUser(userid, {
                    alltime: {
        ...
        if (this.game.givesPoints) {
            await exports.database.addHistory([{
                    ...this.game,
    ...
    getTopPlayers(options = { max: null, requirePoints: true }) {
        ...
        lib_1.Utils.sortBy(ranks, ({ player }) => ([-player.points, player.lastQuestion, hrtimeToNanoseconds(player.answeredAt)]));
    ...
    getWinningMessage(winners) {
        ...
        let initialPart = this.room.tr `${lib_1.Utils.escapeHTML(p1.name)} won the game with a final score of <strong>${p1.player.points}</strong>`;
        if (!this.game.givesPoints) {
            return `${initialPart}.`;
        }
        else {
        ...
        switch (winners.length) {
            ...
            case 2:
                return this.room.tr `${initialPart}their leaderboard score has increased by <strong>${prizes[0]}</strong> points! ` +
                    this.room.tr `${lib_1.Utils.escapeHTML(p2.name)} was a runner-up and their leaderboard score has increased by <strong>${prizes[1]}</strong> points!`;
            case 3:
                return initialPart + lib_1.Utils.html `${this.room.tr `${p2.name} and ${p3.name} were runners-up. `}` +
    ...
    getStaffEndMessage(winners, mapper) {
        ...
        broadcast(this.room, lib_1.Utils.html `${this.room.tr `The game was forcibly ended by ${user.name}.`}`);
        this.destroy();
    }
}
...
class FirstModeTrivia extends Trivia {
    answerQuestion(answer, user) {
        ...
        const buffer = lib_1.Utils.html `${this.room.tr `Correct: ${players}`}<br />` +
    ...
    tallyAnswers() {
        ...
        broadcast(this.room, this.room.tr `The answering period has ended!`, this.room.tr `Correct: no one...` + `<br />` +
...
class TimerModeTrivia extends Trivia {
    ...
    tallyAnswers() {
        if (this.isPaused)
            return;
        this.phase = INTERMISSION_PHASE;
        let buffer = (this.room.tr `Answer(s): ${this.curAnswers.join(', ')}<br />` +
        ...
        for (const [pointValue, players] of innerBuffer) {
            if (!players.length)
                continue;
            rowAdded = true;
            const playerNames = lib_1.Utils.sortBy(players, ([name, answeredAt]) => answeredAt)
                .map(([name]) => name);
            buffer += ('<tr style="background-color: #6688AA">' +
                `<td style="text-align: center">${pointValue}</td>` +
                lib_1.Utils.html `<td>${playerNames.join(', ')}</td>` +
                '</tr>');
        }
        if (!rowAdded) {
            buffer += ('<tr style="background-color: #6688AA">' +
...
class NumberModeTrivia extends Trivia {
    ...
    tallyAnswers() {
        ...
        this.phase = INTERMISSION_PHASE;
        let buffer;
        const innerBuffer = lib_1.Utils.sortBy(Object.values(this.playerTable)
            .filter(player => !!player.isCorrect)
            .map(player => [player.name, hrtimeToNanoseconds(player.currentAnsweredAt)]), ([player, answeredAt]) => answeredAt);
        ...
        if (points) {
            ...
            const players = lib_1.Utils.escapeHTML(innerBuffer.map(([playerName]) => playerName).join(', '));
...
class TriumvirateModeTrivia extends Trivia {
    ...
    tallyAnswers() {
        ...
        lib_1.Utils.sortBy(correctPlayers, p => hrtimeToNanoseconds(p.currentAnsweredAt));
        const cap = this.getCap();
        let winner = cap.questions && this.questionNumber >= cap.questions;
        ...
        for (const player of correctPlayers) {
            ...
            player.incrementPoints(points, this.questionNumber);
            playersWithPoints.push(`${lib_1.Utils.escapeHTML(player.name)} (${points})`);
            if (cap.points && player.points >= cap.points) {
                winner = true;
            }
        }
...
class Mastermind extends Rooms.SimpleRoomGame {
    ...
    init() {
        broadcast(this.room, this.room.tr `Signups for a new Mastermind game have begun!`, this.room.tr `The top <strong>${this.numFinalists}</strong> players will advance to the finals!` + `<br />` +
    ...
    formatPlayerList() {
        return lib_1.Utils.sortBy(Object.values(this.playerTable), player => -(this.leaderboard.get(player.id)?.score || 0)).map(player => {
            const isFinalist = this.currentRound instanceof MastermindFinals && player.id in this.currentRound.playerTable;
            const name = isFinalist ? lib_1.Utils.html `<strong>${player.name}</strong>` : lib_1.Utils.escapeHTML(player.name);
    ...
    startRound(playerID, category, questions, timeout) {
        ...
        setTimeout(() => {
            ...
            broadcast(this.room, this.room.tr `The round of Mastermind has ended!`, points ? this.room.tr `${player} earned ${points} points!` : undefined);
        ...
        setTimeout(() => {
            void (async () => {
                if (this.currentRound) {
                    ...
                    if (winner) {
                        const winnerName = lib_1.Utils.escapeHTML(winner.name);
                        buf = this.room.tr `${winnerName} won the game of Mastermind with ${winner.player.points} points!`;
                    }
                    let smallBuf;
                    if (second && third) {
                        const secondPlace = lib_1.Utils.escapeHTML(second.name);
                        const thirdPlace = lib_1.Utils.escapeHTML(third.name);
    ...
    getTopPlayers(n) {
        if (n < 0)
            return [];
        const sortedPlayerIDs = lib_1.Utils.sortBy([...this.leaderboard].filter(([, info]) => !info.hasLeft), ([, info]) => -info.score).map(([userid]) => userid);
    ...
    kick(toKick, kicker) {
        ...
        if (this.numFinalists > (this.players.length - 1)) {
            throw new Chat.ErrorMessage(this.room.tr `Kicking ${toKick.name} would leave this game of Mastermind without enough players to reach ${this.numFinalists} finalists.`);
        }
...
class MastermindRound extends FirstModeTrivia {
    ...
    start() {
        ...
        const name = lib_1.Utils.escapeHTML(player.name);
        broadcast(this.room, this.room.tr `A Mastermind round in the ${this.game.category} category for ${name} is starting!`);
        player.sendRoom(`|tempnotify|mastermind|Your Mastermind round is starting|Your round of Mastermind is starting in the Trivia room.`);
        this.phase = INTERMISSION_PHASE;
...
const triviaCommands = {
    ...
    async new(target, room, user, connection, cmd) {
        ...
        if (isRandomMode) {
            ...
            mode = lib_1.Utils.shuffle(acceptableModes)[0];
        }
        if (!MODES[mode])
            throw new Chat.ErrorMessage(this.tr `"${mode}" is an invalid mode.`);
        let categories = targets[1]
            .split('+')
            .map(cat => {
            const id = toID(cat);
        ...
        if (questions.length < questionsNecessary) {
            if (categories === 'random') {
                throw new Chat.ErrorMessage(this.tr `There are not enough questions in the randomly chosen category to finish a trivia game.`);
            }
            if (categories.length === 1 && categories[0] === 'all') {
                throw new Chat.ErrorMessage(this.tr `There are not enough questions in the trivia database to finish a trivia game.`);
            }
            throw new Chat.ErrorMessage(this.tr `There are not enough questions under the specified categories to finish a trivia game.`);
        }
        let _Trivia;
        if (mode === 'first') {
        ...
        room.game = new _Trivia(room, mode, categories, givesPoints, length, questions, user.name, isRandomMode, false, isRandomCategory);
    },
    newhelp: [
    ...
    async win(target, room, user) {
        ...
        if (game.game.length !== 'infinite' && !user.can('editroom', null, room)) {
            throw new Chat.ErrorMessage(this.tr `Only Room Owners and higher can force a Trivia game to end with winners in a non-infinite length.`);
        }
    ...
    async add(target, room, user, connection, cmd) {
        ...
        for (const param of params) {
            ...
            const question = lib_1.Utils.escapeHTML(param[1].trim());
            if (!question) {
                this.errorReply(this.tr `'${param[1].trim()}' is not a valid question.`);
                continue;
            }
            if (question.length > MAX_QUESTION_LENGTH) {
                this.errorReply(this.tr `Question "${param[1].trim()}" is too long! It must remain under ${MAX_QUESTION_LENGTH} characters.`);
                continue;
            }
            await exports.database.ensureQuestionDoesNotExist(question);
            const cache = new Set();
            ...
            if (answers.some(answer => answer.length > MAX_ANSWER_LENGTH)) {
                this.errorReply(this.tr `Some of the answers entered for question '${param[1].trim()}' were too long!\n` +
        ...
        if (cmd === 'add') {
            await exports.database.addQuestions(questions);
            ...
            await exports.database.addQuestionSubmissions(questions);
    ...
    async review(target, room) {
        ...
        const submissions = await exports.database.getSubmissions();
    ...
    async accept(target, room, user, connection, cmd) {
        ...
        if (toID(target) === 'all') {
            if (isAccepting)
                await exports.database.addQuestions(submissions);
            await exports.database.clearSubmissions();
        ...
        if (/\d+(?:-\d+)?(?:, ?\d+(?:-\d+)?)*$/.test(target)) {
            ...
            if (!indicesLen) {
                throw new Chat.ErrorMessage(this.tr `'${target}' is not a valid set of submission index numbers.\n` +
                    this.tr `View /trivia review and /trivia help for more information.`);
            }
            if (isAccepting) {
                await exports.database.acceptSubmissions(questions);
            }
            else {
                await exports.database.deleteSubmissions(questions);
            }
    ...
    async delete(target, room, user) {
        ...
        const question = lib_1.Utils.escapeHTML(target).trim();
        if (!question) {
            throw new Chat.ErrorMessage(this.tr `'${target}' is not a valid argument. View /trivia help questions for more information.`);
        }
        const { category } = await exports.database.ensureQuestionExists(question);
        await exports.database.deleteQuestion(question);
    ...
    async move(target, room, user) {
        ...
        for (const param of params) {
            ...
            const { category: oldCat } = await exports.database.ensureQuestionExists(question);
            await exports.database.moveQuestionToCategory(question, category);
            this.modlog('TRIVIAQUESTION', null, `changed category for '${param[1].trim()}' from '${oldCat}' to '${param[0]}'`);
            return this.privateModAction(this.tr `${user.name} changed question category from '${oldCat}' to '${param[0]}' for '${param[1].trim()}' ` +
    ...
    async migrate(target, room, user) {
        ...
        user.lastCommand = '';
        const numQs = await exports.database.migrateCategory(sourceCategory, destinationCategory);
    ...
    async edit(target, room, user) {
        ...
        const { category } = await exports.database.ensureQuestionExists(lib_1.Utils.escapeHTML(oldQuestionText));
        let newQuestionText;
        if (!isAnswersOnly) {
        ...
        await exports.database.editQuestion(lib_1.Utils.escapeHTML(oldQuestionText), 
        // Cast is safe because if `newQuestionText` is undefined, `isAnswersOnly` is true.
        isAnswersOnly ? undefined : lib_1.Utils.escapeHTML(newQuestionText), isQuestionOnly ? undefined : answers);
    ...
    async qs(target, room, user) {
        ...
        if (!target) {
            if (!this.runBroadcast())
                return false;
            const counts = await exports.database.getQuestionCounts();
            if (!counts.total)
            ...
            for (const category of new Set([...Object.keys(ALL_CATEGORIES), ...Object.keys(counts)])) {
                ...
                if (category === 'random' || category === 'total')
                    continue;
        ...
        const list = await exports.database.getQuestions([category], Number.MAX_SAFE_INTEGER, { order: 'oldestfirst' });
        if (!list.length) {
    ...
    async search(target, room, user, connection, cmd) {
        ...
        if (/^q(?:uestion)?s?$/.test(type)) {
            ...
            return this.sendReplyBox(this.tr `No valid search category was entered. Valid categories: submissions, subs, questions, qs`);
        }
        ...
        const results = await exports.database.searchQuestions(queryString, options);
        if (!results.length)
        ...
        buffer += results.map((q, i) => this.tr `<tr><td><strong>${i + 1}</strong></td><td>${q.category}</td><td>${q.question}</td></tr>`).join('');
        buffer += "</table></div>";
        this.sendReply(buffer);
    },
    searchhelp: [
    ...
    async moveusedevent(target, room, user) {
        ...
        const currentSetting = await exports.database.shouldMoveEventQuestions();
        if (target) {
            this.checkCan('editroom', null, room);
            ...
            if (isEnabling) {
                ...
                await exports.database.setShouldMoveEventQuestions(true);
            }
            else if (this.meansNo(target)) {
                if (!currentSetting)
                    throw new Chat.ErrorMessage(`Moving used event questions is already disabled.`);
                await exports.database.setShouldMoveEventQuestions(false);
            }
            else {
                return this.parse(`/help trivia moveusedevent`);
            }
            this.modlog(`TRIVIA MOVE USED EVENT QUESTIONS`, null, isEnabling ? 'ON' : 'OFF');
            this.sendReply(`Trivia questions in the ${fromCatName} category will ${isEnabling ? 'now' : 'no longer'} be moved to the ${toCatName} category after they are used.`);
        }
        else {
            this.sendReply(currentSetting ?
    ...
    async rank(target, room, user) {
        ...
        let userid;
        if (!target) {
            name = lib_1.Utils.escapeHTML(user.name);
            userid = user.id;
        }
        else {
            name = lib_1.Utils.escapeHTML(target);
            userid = toID(target);
        }
        const allTimeScore = await exports.database.getLeaderboardEntry(userid, 'alltime');
        if (!allTimeScore)
            return this.sendReplyBox(this.tr `User '${name}' has not played any Trivia games yet.`);
        const score = (await exports.database.getLeaderboardEntry(userid, 'nonAlltime') ||
            { score: 0, totalPoints: 0, totalCorrectAnswers: 0 });
        const cycleScore = await exports.database.getLeaderboardEntry(userid, 'cycle');
        const ranks = (await exports.cachedLadder.get('nonAlltime'))?.ranks[userid];
        const allTimeRanks = (await exports.cachedLadder.get('alltime'))?.ranks[userid];
        const cycleRanks = (await exports.cachedLadder.get('cycle'))?.ranks[userid];
        function display(scores, ranking, key) {
            if (!scores?.[key])
                return `<td>N/A</td>`;
            return lib_1.Utils.html `<td>${scores[key]}${ranking?.[key] ? ` (rank ${ranking[key]})` : ``}</td>`;
        }
        this.sendReplyBox(`<div class="ladder"><table>` +
    ...
    async ladder(target, room, user, connection, cmd) {
        ...
        const ladder = (await exports.cachedLadder.get(leaderboard))?.ladder;
        if (!ladder?.length)
        ...
        for (let i = Math.max(0, num - 100); i < num; i++) {
            ...
            for (const leader of leaders) {
                const rank = await exports.database.getLeaderboardEntry(leader, leaderboard);
                if (!rank)
                    continue; // should never happen
                const leaderObj = Users.getExact(leader);
                const leaderid = leaderObj ? lib_1.Utils.escapeHTML(leaderObj.name) : leader;
    ...
    async resetcycleleaderboard(target, room, user) {
        ...
        user.lastCommand = '';
        await exports.database.clearCycleLeaderboard();
    ...
    async clearqs(target, room, user) {
        ...
        if (ALL_CATEGORIES[category]) {
            if (SPECIAL_CATEGORIES[category]) {
                await exports.database.clearCategory(category);
    ...
    async history(target, room, user) {
        ...
        const games = await exports.database.getHistory(lines);
        const buf = [];
        for (const [i, game] of games.entries()) {
            let gameInfo = lib_1.Utils.html `<b>${i + 1}.</b> ${this.tr `${game.mode} mode, ${game.length} length Trivia game in the ${game.category} category`}`;
            if (game.creator)
                gameInfo += lib_1.Utils.html ` ${this.tr `hosted by ${game.creator}`}`;
            gameInfo += '.';
            buf.push(gameInfo);
        }
    ...
    async lastofficialscore(target, room, user) {
        room = this.requireRoom('trivia');
        this.runBroadcast();
        const scores = Object.entries(await exports.database.getScoresForLastGame())
    ...
    async addpoints(target, room, user, connection, cmd) {
        ...
        await exports.database.updateLeaderboardForUser(userid, {
            alltime: change,
            nonAlltime: change,
            cycle: change,
        });
        ...
        this.privateModAction(isRemoval ?
    ...
    async removeleaderboardentry(target, room, user) {
        ...
        if (!(await exports.database.getLeaderboardEntry(userid, 'alltime'))) {
        ...
        user.lastCommand = '';
        await Promise.all(Object.keys(database_1.LEADERBOARD_ENUM)
            .map(lb => exports.database.deleteLeaderboardEntry(userid, lb)));
        exports.cachedLadder.invalidateCache();
    ...
    async mergescore(target, room, user) {
        ...
        try {
            ...
            return this.sendReply(`A Trivia leaderboard score merge with ${altid} is now pending! ` +
    ...
    triviahelp() {
        this.sendReply(`|html|<div class="infobox">` +
            ...
            lib_1.Utils.html `<li><code>${'/trivia edit <old question text> | <new question text> | <answer1, answer2, ...>'}</code>: Edit a question in the trivia database, replacing answers if specified. Requires: % @ # ~` +
            lib_1.Utils.html `<li><code>${'/trivia edit answers | <old question text> | <new answers>'}</code>: Replaces the answers of a question. Requires: % @ # ~</li>` +
            lib_1.Utils.html `<li><code>${'/trivia edit question | <old question text> | <new question text>'}</code>: Edits only the text of a question. Requires: % @ # ~</li>` +
...
const mastermindCommands = {
    ...
    mastermindhelp() {
        ...
        return this.sendReplyBox(`<strong>Mastermind</strong> is a game in which each player tries to score as many points as possible in a timed round where only they can answer, ` +
