...
exports.commands = exports.pages = exports.statusfilter = exports.nicknamefilter = exports.loginfilter = exports.namefilter = exports.chatfilter = exports.Filters = void 0;
exports.start = start;
...
exports.Filters = new class {
    ...
    generateRegex(word, isEvasion = false, isShortener = false, isReplacement = false) {
        try {
            ...
            throw new Chat.ErrorMessage(e.message.startsWith('Invalid regular expression: ') ? e.message : `Invalid regular expression: /${word}/: ${e.message}`);
        }
    }
    ...
    save(force = false) {
        (0, lib_1.FS)(MONITOR_FILE).writeUpdate(() => {
            const buf = {};
            for (const key in Chat.monitors) {
                buf[key] = [];
                for (const filterWord of filterWords[key]) {
                    const word = { ...filterWord };
                    delete word.regex; // no reason to save this. does not stringify.
                    buf[key].push(word);
                }
            }
    ...
    add(filterWord) {
        ...
        if (!filterWord.regex) {
            filterWord.regex = this.generateRegex(filterWord.word, punishment === 'EVASION', punishment === 'SHORTENER', !!filterWord.replacement);
        }
    ...
    load() {
        const legacy = (0, lib_1.FS)(LEGACY_MONITOR_FILE);
        ...
        const data = JSON.parse((0, lib_1.FS)(MONITOR_FILE).readIfExistsSync() || "{}");
        for (const k in data) {
            filterWords[k] = [];
            ...
            for (const entry of data[k]) {
                if (k === 'evasion') {
                    ...
                    entry.regex = new RegExp(k === 'shorteners' ? `\\b${entry.word}` : entry.word, entry.replacement ? 'igu' : 'iu');
                }
    ...
    loadLegacy() {
        let data;
        try {
            data = (0, lib_1.FS)(LEGACY_MONITOR_FILE).readSync();
        }
        catch (e) {
            if (e.code !== 'ENOENT')
                throw e;
            ...
            for (const key in Chat.monitors) {
                if (Chat.monitors[key].location === location && Chat.monitors[key].punishment === punishment) {
                    ...
                    if (punishment === 'EVASION') {
                        regex = exports.Filters.constructEvasionRegex(word);
                    }
                    else {
...
Chat.registerMonitor('autolock', {
    ...
    monitor(line, room, user, message, lcMessage, isStaff) {
        ...
        if (match) {
            ...
            if (room) {
                void Punishments.autolock(user, room, 'ChatMonitor', `Filtered phrase: ${word}`, `<<${room.roomid}>> ${user.name}: ||\`\`${message}\`\`${reason ? ` __(${reason})__` : ''}||`, true);
            }
            else {
...
Chat.registerMonitor('evasion', {
    ...
    monitor(line, room, user, message, lcMessage, isStaff) {
        ...
        if (match) {
            ...
            message = message.replace(/(https?):\/\//g, '$1__:__//');
            if (room) {
                void Punishments.autolock(user, room, 'FilterEvasionMonitor', `Evading filter: ${message} (${match[0]} => ${word})`, `<<${room.roomid}>> ${user.name}: ||\`\`${message}\`\` __(${match[0]} => ${word})__||`, true);
            }
            else {
...
Chat.registerMonitor('battlefilter', {
    ...
    monitor(line, room, user, message, lcMessage, isStaff) {
        ...
        if (match) {
            ...
            if (room) {
                room.mute(user);
                this.errorReply(`You have been muted for using a banned phrase. Please do not say '${match[0]}'${publicReason ? ` ${publicReason}` : ``}.`);
...
const chatfilter = function (message, user, room) {
    ...
    for (const list in Chat.monitors) {
        ...
        if (location === 'BATTLES' && !(room?.battle && room.battle.challengeType !== 'challenge'))
            continue;
...
const namefilter = (name, user) => {
    ...
    for (const list in filterWords) {
        ...
        for (const line of filterWords[list]) {
            const regex = (punishment === 'EVASION' ? exports.Filters.stripWordBoundaries(line.regex) : line.regex);
            if (regex.test(lcName)) {
                if (Chat.monitors[list].punishment === 'AUTOLOCK') {
                    void Punishments.autolock(user, 'staff', `NameMonitor`, `inappropriate name: ${name}`, `using an inappropriate name: ||${name} (from ${user.name})||`, false, name);
                }
                line.hits++;
                exports.Filters.save();
                return '';
...
const loginfilter = user => {
    ...
    if (user.trackRename) {
        ...
        Rooms.global.notifyRooms(['staff'], lib_1.Utils.html `|html|[NameMonitor] Username used: <span class="username">${user.name}</span> ${user.getAccountStatusString()} (${!manualForceRename ? 'automatically ' : ''}forcerenamed from <span class="username">${user.trackRename}</span>)`);
        user.trackRename = '';
    }
    ...
    for (const list in filterWords) {
        ...
        for (const line of filterWords[list]) {
            ...
            if (Chat.monitors[list].punishment === 'EVASION') {
                ...
                regex = exports.Filters.stripWordBoundaries(regex);
            }
            const match = regex.exec(lcName);
            if (match) {
                if (Chat.monitors[list].punishment === 'AUTOLOCK') {
                    void Punishments.autolock(user, 'staff', `NameMonitor`, `inappropriate Pokémon nickname: ${name}`, `${user.name} - using an inappropriate Pokémon nickname: ||${name}||`, true);
                }
                    ...
                    void Punishments.autolock(user, 'staff', 'FilterEvasionMonitor', `Evading filter in Pokémon nickname (${name} => ${word})`, `${user.name}: Pokémon nicknamed ||\`\`${name} => ${word}\`\`||`, true);
                }
                line.hits++;
                exports.Filters.save();
                return '';
        ...
        for (const line of filterWords[list]) {
            ...
            if (regex.test(lcStatus)) {
                if (punishment === 'AUTOLOCK') {
                    ...
                    void Punishments.autolock(user, 'staff', `NameMonitor`, `inappropriate status message: ${status}`, `${user.name} - using an inappropriate status: ||${status}||`, true);
                }
                line.hits++;
                exports.Filters.save();
                return '';
        ...
        for (const key in Chat.monitors) {
            ...
            if (filterWords[key].length) {
                content += filterWords[key].map(({ regex, word, reason, publicReason, replacement, hits }) => {
                    let entry = lib_1.Utils.html `<abbr title="${reason}"><code>${word}</code></abbr>`;
                    if (publicReason)
                        entry += lib_1.Utils.html ` <small>(public reason: ${publicReason})</small>`;
                    if (replacement)
                        entry += lib_1.Utils.html ` &rArr; ${replacement}`;
...
exports.commands = {
    ...
    filter: {
        add(target, room, user) {
            ...
            if (Chat.monitors[list].punishment === 'FILTERTO') {
                ...
                if (!filterWord.replacement) {
                    throw new Chat.ErrorMessage(`Syntax for word filters: /filter add ${list} ${separator} regex ${separator} reason [${separator} optional public reason]`);
                }
            }
            else {
            ...
            exports.Filters.add(filterWord);
        ...
        remove(target, room, user) {
            ...
            if (!list || !words.length)
                throw new Chat.ErrorMessage("Syntax: /filter remove list, words");
            if (!(list in filterWords)) {
                throw new Chat.ErrorMessage(`Invalid list: ${list}. Possible options: ${Object.keys(filterWords).join(', ')}`);
            }
        ...
        test(target, room, user) {
            ...
            if (buf.length) {
                ...
                throw new Chat.ErrorMessage(`"${target}" doesn't trigger any filters. Check spelling?`);
            }
        },
        testhelp: [
    ...
    allowname(target, room, user) {
        ...
        target = toID(target);
        if (!target)
            throw new Chat.ErrorMessage(`Syntax: /allowname username`);
