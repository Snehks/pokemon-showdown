...
exports.roomSettings = exports.commands = exports.pages = void 0;
exports.start = start;
...
let MafiaData = Object.create(null);
...
function readFile(path) {
    try {
        const json = (0, lib_1.FS)(path).readIfExistsSync();
        if (!json) {
            return false;
        }
...
function writeFile(path, data) {
    (0, lib_1.FS)(path).writeUpdate(() => (JSON.stringify(data)));
}
...
for (const section of tables) {
    ...
    if (Object.keys(logs[section]).length >= 3) {
        ...
        const keys = lib_1.Utils.sortBy(Object.keys(logs[section]), key => {
...
class MafiaPlayer extends Rooms.RoomGamePlayer {
    constructor(user, game) {
        super(user, game);
        this.safeName = lib_1.Utils.escapeHTML(this.name);
        this.role = null;
        this.voting = '';
...
class Mafia extends Rooms.RoomGame {
    constructor(room, host) {
        ...
        this.theme = null;
        this.hostid = host.id;
        this.host = lib_1.Utils.escapeHTML(host.name);
        this.cohostids = [];
        this.cohosts = [];
        this.subs = [];
        this.autoSub = true;
    ...
    setRoles(user, roleString, force = false, reset = false) {
        ...
        if (roles.length === 1) {
            ...
            if (themeName in MafiaData.themes) {
                ...
                if (!theme[this.playerCount]) {
                    return user.sendTo(this.room, `|error|The theme "${theme.name}" does not have a role list for ${this.playerCount} players.`);
                }
        ...
        if (force) {
            this.originalRoles = roles.map(r => ({
                name: r,
                safeName: lib_1.Utils.escapeHTML(r),
                id: toID(r),
                alignment: 'solo',
                image: '',
                memo: [`To learn more about your role, PM the host (${this.host}).`],
            }));
            lib_1.Utils.sortBy(this.originalRoles, role => role.name);
        ...
        lib_1.Utils.sortBy(this.originalRoles, role => [role.alignment, role.name]);
        this.roles = this.originalRoles.slice();
        this.originalRoleString = this.originalRoles.map(r => `<span style="font-weight:bold;color:${MafiaData.alignments[r.alignment].color || '#FFF'}">${r.safeName}</span>`).join(', ');
        this.roleString = this.originalRoleString;
        if (!reset)
    ...
    static parseRole(roleString) {
        ...
        const role = {
            name: roleName,
            safeName: lib_1.Utils.escapeHTML(roleName),
            id: toID(roleName),
            image: '',
    ...
    distributeRoles() {
        const roles = lib_1.Utils.shuffle(this.originalRoles.slice());
        if (roles.length) {
            for (const p of this.players) {
    ...
    vote(voter, targetId) {
        ...
        if (voter.lastVote + 2000 >= Date.now()) {
            return this.sendUser(voter, `|error|You must wait another ${Chat.toDurationString((voter.lastVote + 2000) - Date.now()) || '1 second'} before you can change your vote.`);
        }
        // -- VALID --
        ...
        if (previousVote) {
            ...
            this.sendTimestamp(name === 'No Vote' ?
        ...
        if (this.getHammerValue(targetId) <= vote.trueCount) {
            // HAMMER
            this.sendDeclare(`Hammer! ${targetId === 'novote' ? 'Nobody' : lib_1.Utils.escapeHTML(name)} was voted out!`);
            this.sendRoom(`|raw|<div class="infobox">${this.voteBox()}</div>`);
            if (targetId !== 'novote')
    ...
    unvote(voter, force = false) {
        ...
        if (!force) {
            this.sendTimestamp(voter.voting === 'novote' ?
    ...
    voteBox() {
        ...
        const list = lib_1.Utils.sortBy(Object.entries(this.votes), ([key, vote]) => [
            key === plur,
            -vote.count,
        ]);
    ...
    getPlurality() {
        ...
        topVotes = lib_1.Utils.sortBy(topVotes, ([key, vote]) => [
            vote.dir === 'down',
    ...
    revive(user, toRevive) {
        ...
        lib_1.Utils.sortBy(this.roles, r => [r.alignment, r.name]);
        this.updateRoleString();
        this.updatePlayers();
        return true;
    }
    ...
    nextSub(userid = null) {
        ...
        if (!sub?.connected || !sub.named || !this.room.users[sub.id])
            return; // should never happen, just to be safe
    ...
    customIdeaInit(user, choices, picks, rolesString) {
        ...
        this.roles = [];
        this.roleString = '';
        const roles = lib_1.Utils.stripHTML(rolesString);
    ...
    setSelfVote(user, setting) {
        ...
        if (from === setting) {
            return user.sendTo(this.room, `|error|Selfvoting is already ${setting ? `set to Self${setting === 'hammer' ? 'hammering' : 'voting'}` : 'disabled'}.`);
        }
        if (from) {
...
exports.pages = {
    mafia(query, user) {
        ...
        if (isHost) {
            ...
            for (const eliminated of game.getEliminatedPlayers()) {
                ...
                if (eliminated.isSpirit())
                    buf += ` (is a Restless Spirit)`;
                if (game.voteModifiers[eliminated.id] !== undefined)
    ...
    mafialadder(query, user) {
        ...
        const entries = lib_1.Utils.sortBy(Object.entries(logs[section][month]), ([key, value]) => (-value));
    ...
    mafia: {
        ...
        reveal(target, room, user) {
            ...
            if ((action === 'off' && game.noReveal) || (action === 'on' && !game.noReveal)) {
                return user.sendTo(room, `|error|Revealing of roles is already ${game.noReveal ? 'disabled' : 'enabled'}.`);
            }
        ...
        setroles(target, room, user, connection, cmd) {
            ...
            if (reset) {
                ...
                if (game.phase !== 'locked' && game.phase !== 'IDEAlocked') {
                    throw new Chat.ErrorMessage(game.phase === 'signups' ? `You need to close signups first.` : `The game has already started.`);
                }
            }
            if (!target)
        ...
        customidea(target, room, user) {
            ...
            const [options, roles] = lib_1.Utils.splitFirst(target, '\n');
            if (!options || !roles)
        ...
        rolelist(target, room, user, connection, cmd) {
            ...
            const roleString = lib_1.Utils.sortBy((showOrl ? game.originalRoles : game.roles), role => (role.alignment)).map(role => role.safeName).join(', ');
        ...
        playerroles(target, room, user) {
            ...
            this.sendReplyBox(game.players.map(p => `${p.safeName}: ${p.role ? (p.role.alignment === 'solo' ? 'Solo ' : '') + p.role.safeName : 'No role'}`).join('<br/>'));
        },
        spectate: 'view',
            ...
            switch (action) {
                ...
                case 'next':
                    ...
                    if (!game.subs.length) {
                        ...
                        user.sendTo(room, `|error|There are no subs to replace ${toSub}, they will be subbed if a sub is available before they speak next.`);
        ...
        subhost(target, room, user, connection, cmd) {
            ...
            if (cmd.includes('cohost')) {
                ...
                game.cohosts.push(lib_1.Utils.escapeHTML(targetUser.name));
                game.sendDeclare(lib_1.Utils.html `${targetUser.name} has been added as a cohost by ${user.name}`);
                for (const conn of targetUser.connections) {
                    void Chat.resolvePage(`view-mafia-${room.roomid}`, targetUser, conn);
                }
                ...
                if (queueIndex > -1)
                    hostQueue.splice(queueIndex, 1);
                game.host = lib_1.Utils.escapeHTML(targetUser.name);
                game.hostid = targetUser.id;
                game.played.push(targetUser.id);
                for (const conn of targetUser.connections) {
                    void Chat.resolvePage(`view-mafia-${room.roomid}`, targetUser, conn);
                }
                game.sendDeclare(lib_1.Utils.html `${targetUser.name} has been substituted as the new host, replacing ${oldHostid}.`);
        ...
        removecohost(target, room, user) {
            ...
            game.sendDeclare(lib_1.Utils.html `${target} was removed as a cohost by ${user.name}`);
        ...
        addtheme(target, room, user, connection, cmd) {
            ...
            for (const rolelist of rolelists) {
                const [players, roles] = lib_1.Utils.splitFirst(rolelist, ':', 2).map(e => e.trim());
        ...
        listdata(target, room, user, connection, cmd, message) {
            ...
            if (cmd === 'random' || cmd === 'randomdata' || cmd === 'randdata') {
                searchType =
                    [`themes`, `roles`, `alignments`, `IDEAs`, `terms`][Math.floor(Math.random() * 5)];
                foundSearchType = true;
            }
            if (!foundSearchType) {
            ...
            for (let i = 0; i < targets.length; i++) {
                ...
                if (searchType === `aliases`) {
                    ...
                    if (typeof (entries) === 'undefined')
                        return;
                    if (random)
                        entries = lib_1.Utils.shuffle(entries);
                    if (number > 0)
