...
exports.punishmentfilter = exports.pages = exports.commands = exports.lastLogTime = exports.pokemonRegex = exports.handlers = exports.loginfilter = exports.chatfilter = exports.classifier = exports.punishmentCache = exports.metadata = exports.reviews = exports.settings = exports.muted = exports.cache = exports.migrated = void 0;
exports.searchModlog = searchModlog;
...
exports.migrated = global.Chat?.oldPlugins['abuse-monitor']?.migrated || false;
exports.cache = (() => {
    ...
    exports.migrated = true;
    return plugin.cache;
})();
exports.muted = Chat.oldPlugins['abuse-monitor']?.muted || new WeakMap();
const defaults = {
    threshold: 4,
...
exports.settings = (() => {
    try {
        ...
        return { ...defaults, ...JSON.parse((0, lib_1.FS)('config/chat-plugins/nf.json').readSync()) };
    }
    catch (e) {
        if (e.code !== "ENOENT")
            throw e;
...
exports.reviews = (() => {
    try {
        return JSON.parse((0, lib_1.FS)(`config/chat-plugins/artemis-reviews.json`).readSync());
    }
    catch {
        return {};
    }
...
exports.metadata = (() => {
    try {
        return JSON.parse((0, lib_1.FS)('config/chat-plugins/artemis-metadata.json').readSync());
    }
    catch {
        return {};
    }
...
function isFlaggedUserid(name, room) {
    ...
    const entry = exports.cache[room]?.staffNotified;
    if (!entry)
        return false;
...
function visualizePunishmentKey(punishment, key) {
    if (key === 'secondaryTypes') {
        if (!punishment.secondaryTypes)
            return '';
        const keys = lib_1.Utils.sortBy(Object.keys(punishment.secondaryTypes));
...
function visualizePunishment(punishment) {
    return lib_1.Utils
        ...
        .map(k => `${k}: ${visualizePunishmentKey(punishment, k)}`)
        .join(', ');
}
function displayResolved(review, justSubmitted = false) {
    ...
    const idx = exports.reviews[review.staff].findIndex(r => r.room === review.room);
    if (idx > -1)
        exports.reviews[review.staff].splice(idx, 1);
    if (!exports.reviews[review.staff].length) {
        delete exports.reviews[review.staff];
    }
    saveReviews();
}
exports.punishmentCache = (Chat.oldPlugins['abuse-monitor']?.punishmentCache || new WeakMap());
async function searchModlog(query) {
    const userObj = Users.get(query.user);
    if (userObj) {
        const data = exports.punishmentCache.get(userObj);
        if (data) {
            let sum = 0;
    ...
    if (!modlog)
        return 0;
    const ignores = exports.metadata.modlogIgnores?.[query.user];
    if (userObj) {
        ...
        exports.punishmentCache.set(userObj, cacheEntry);
        let sum = 0;
...
exports.classifier = new Artemis.RemoteClassifier();
async function runActions(user, room, message, response) {
    const keys = lib_1.Utils.sortBy(Object.keys(response), k => -response[k]);
    const recommended = [];
    const roomRecord = exports.cache[room.roomid];
    const prevRecommend = roomRecord?.recommended?.[user.id];
    for (const punishment of exports.settings.punishments) {
        ...
        for (const type of keys) {
            ...
            if (punishment.count) {
                let hits = await Chat.database.all(`SELECT * FROM perspective_flags WHERE userid = ? AND type = ? AND certainty >= ?`, [user.id, type, num]);
    ...
    if (recommended.length) {
        lib_1.Utils.sortBy(recommended, ([punishment]) => -PUNISHMENTS.indexOf(punishment));
        ...
        if (user.trusted) {
            ...
            Rooms.get('staff')?.add(`|c|~|/log [Artemis] ${getViewLink(room.roomid)} ${punishment} recommended for trusted user ${user.id}` +
                `${user.trusted !== user.id ? ` [${user.trusted}]` : ''} `).update();
            return; // we want nothing else to be executed. staff want trusted users to be reviewed manually for now
        }
        const result = await punishmentHandlers[(0, dex_data_1.toID)(punishment)]?.(user, room, response, message);
        ...
        if (result !== false) {
            ...
            if (notified) {
                if (typeof notified === 'string') {
                    ...
                    if (!notified.length) {
                        delete exports.cache[room.roomid].staffNotified;
                        void Chat.database.run(`INSERT INTO perspective_stats (staff, roomid, result, timestamp) VALUES ($staff, $roomid, $result, $timestamp) ` +
            ...
            if (roomRecord && !Object.keys(roomRecord.users).length) {
                delete exports.cache[room.roomid];
            }
            notifyStaff();
        }
    }
...
function globalModlog(action, user, note, roomid) {
    ...
    void Rooms.Modlog.write(roomid || 'global', {
        ...
        userid: (0, dex_data_1.toID)(user) || undefined,
        loggedBy: 'artemis',
        note,
    });
}
...
const DISCLAIMER = ('<small>This action was done automatically.' +
...
async function lock(user, room, reason, isWeek) {
    if (exports.settings.recommendOnly) {
        Rooms.get('staff')?.add(`|c|~|/log [Artemis] ${getViewLink(room.roomid)} ${isWeek ? "WEEK" : ""}LOCK recommended for ${user.id}`).update();
        room.hideText([user.id], undefined, true);
        return false;
    }
    const affected = await Punishments.lock(user, isWeek ? Date.now() + 7 * 24 * 60 * 60 * 1000 : null, user.id, false, reason, false, ['#artemis']);
    ...
    if (affected.length > 1) {
        Rooms.get('staff')?.add(`|c|~|/log (${user.id}'s ` +
    ...
    if (roomauth.length) {
        Monitor.log(`[CrisisMonitor] Locked user ${user.name} ` +
...
const punishmentHandlers = {
    report(user, room) {
        for (const k in room.users) {
            ...
            if (room.auth.get(u) !== Users.PLAYER_SYMBOL)
                continue;
            u.sendTo(room.roomid, `|c|~|/uhtml report,` +
    ...
    mute(user, room) {
        const roomMutes = exports.muted.get(room) || new WeakMap();
        if (!user.trusted) {
            exports.muted.set(room, roomMutes);
    ...
    warn(user, room, response, message) {
        ...
        const punishments = exports.punishmentCache.get(user) || {};
        if (!punishments['WARN'])
            punishments['WARN'] = 0;
        punishments['WARN']++;
        exports.punishmentCache.set(user, punishments);
        room.add(`|c|~|/raw ${DISCLAIMER}`).update();
        room.hideText([user.id], undefined, true);
    },
...
function makeScore(roomid, result) {
    ...
    for (const type in result) {
        ...
        if (exports.settings.minScore && data < exports.settings.minScore)
            continue;
        const curScore = score;
        if (exports.settings.specials[type]) {
            for (const k in exports.settings.specials[type]) {
                if (data < Number(k))
                    continue;
                const num = exports.settings.specials[type][k];
                if (num === 'MAXIMUM') {
...
const chatfilter = function (message, user, room) {
    ...
    if (!room?.battle || !['rated', 'unrated'].includes(room.battle.challengeType))
        return;
    const mutes = exports.muted.get(room);
    const muteEntry = mutes?.get(user);
    if (muteEntry) {
        if (Date.now() > muteEntry) {
            mutes.delete(user);
            if (!mutes.size) {
                exports.muted.delete(room);
            }
        }
        else {
            this.sendReply(`|c|~|/raw <div class="message-error">` +
    ...
    void (async () => {
        ...
        message = message.replace(exports.pokemonRegex, '[[Pokemon]]');
        for (const k in exports.settings.replacements) {
            message = message.replace(new RegExp(k, 'gi'), exports.settings.replacements[k]);
        }
        const response = await exports.classifier.classify(message);
        const { score, flags, main } = makeScore(roomid, response || {});
        if (score) {
            if (!exports.cache[roomid])
                exports.cache[roomid] = { users: {} };
            if (!exports.cache[roomid].users[user.id])
                exports.cache[roomid].users[user.id] = 0;
            exports.cache[roomid].users[user.id] += score;
            let hitThreshold = 0;
            if (exports.cache[roomid].users[user.id] >= calcThreshold(roomid)) {
                let notified = exports.cache[roomid].staffNotified;
                if (notified) {
                    if (!Array.isArray(notified)) {
                        exports.cache[roomid].staffNotified = notified = [notified];
                    }
                    ...
                    exports.cache[roomid].staffNotified = [user.id];
                }
                notifyStaff();
                hitThreshold = 1;
                void room?.uploadReplay?.(user, this.connection, "forpunishment");
                await Chat.database.run(`INSERT INTO perspective_flags (userid, score, certainty, type, roomid, time) VALUES (?, ?, ?, ?, ?, ?)`, 
            ...
            await Chat.database.run('INSERT INTO perspective_logs (userid, message, score, flags, roomid, time, hit_threshold) VALUES (?, ?, ?, ?, ?, ?, ?)', [user.id, message, score, lib_1.Utils.sortBy(flags).join(','), roomid, Date.now(), hitThreshold]);
        }
    })();
};
exports.chatfilter = chatfilter;
// to avoid conflicts with other filters
exports.chatfilter.priority = -100;
const loginfilter = user => {
    if (exports.reviews[user.id]?.length) {
        for (const r of exports.reviews[user.id]) {
            displayResolved(r);
        }
    }
};
...
function calcThreshold(roomid) {
    const incr = exports.settings.thresholdIncrement;
    let num = exports.settings.threshold;
...
exports.handlers = {
    onRoomDestroy(roomid) {
        const entry = exports.cache[roomid];
        if (entry) {
            delete exports.cache[roomid];
    ...
    onRoomClose(roomid, user) {
        ...
        if (exports.cache[targetId]?.claimed === user.id) {
            delete exports.cache[targetId].claimed;
            notifyStaff();
        }
    },
    onRenameRoom(oldId, newId, room) {
        if (exports.cache[oldId]) {
            exports.cache[newId] = exports.cache[oldId];
            delete exports.cache[oldId];
            notifyStaff();
        }
    },
};
function getFlaggedRooms() {
    return Object.keys(exports.cache).filter(roomid => exports.cache[roomid].staffNotified);
}
...
function saveSettings(path) {
    if (!path)
        path = 'nf';
    (0, lib_1.FS)(`config/chat-plugins/${path}.json`).writeUpdate(() => JSON.stringify(exports.settings));
}
function saveReviews() {
    (0, lib_1.FS)(`config/chat-plugins/artemis-reviews.json`).writeUpdate(() => JSON.stringify(exports.reviews));
}
function saveMetadata() {
    (0, lib_1.FS)('config/chat-plugins/artemis-metadata.json').writeUpdate(() => JSON.stringify(exports.metadata));
}
exports.pokemonRegex = new RegExp(// we want only base formes and existent stuff
`\\b(${Dex.species.all().filter(s => !s.forme && s.num > 0).map(f => f.id).join('|')})\\b`, 'gi');
exports.lastLogTime = Chat.oldPlugins['abuse-monitor']?.lastLogTime || 0;
function notifyStaff() {
    ...
    if (staffRoom) {
        ...
        if ((exports.lastLogTime + STAFF_NOTIF_INTERVAL) < Date.now() || !flagged.length) {
            staffRoom.send(`|uhtml|abusemonitor|<div class="infobox">${buf}</div>`);
            // if there are none, don't update the time - no point
            if (flagged.length) {
                exports.lastLogTime = Date.now();
            }
            else {
                exports.lastLogTime = 0;
            }
        }
        // always update flags
...
exports.commands = {
    ...
    abusemonitor: {
        ...
        async test(target, room, user) {
            ...
            this.runBroadcast();
            let response = await exports.classifier.classify(text);
            if (!response)
                response = {};
            for (const k in exports.settings.replacements) {
                target = target.replace(new RegExp(k, 'gi'), exports.settings.replacements[k]);
            }
            ...
            const punishments = [];
            for (const [i, p] of exports.settings.punishments.entries()) {
                const matches = [];
        ...
        async compare(target) {
            checkAccess(this);
            const [base, against] = lib_1.Utils.splitFirst(target, ',').map(f => f.trim());
            if (!(base && against))
                return this.parse(`/help abusemonitor`);
            const colors = {
                '0': 'Purple',
            ...
            const baseResponse = await exports.classifier.classify(base) || {};
            const againstResponse = await new Promise(resolve => {
                // bit of a hack, but this has to be done so rate limits don't get hit
                setTimeout(() => {
                    resolve(exports.classifier.classify(against));
                }, 500);
            }) || {};
            let buf = lib_1.Utils.html `<strong>Compared scores for "${base}" `;
            buf += `(<strong style="color: ${colors['1']}">1</strong>) `;
            buf += lib_1.Utils.html `and "${against}" (<strong style="color: ${colors['2']}">2</strong>): </strong><br />`;
        ...
        async score(target) {
            ...
            if (!target)
                return this.parse(`/help abusemonitor`);
            const [text, scoreText] = lib_1.Utils.splitFirst(target, ',').map(f => f.trim());
            ...
            const response = await exports.classifier.suggestScore(text, scores);
            if (response.error)
        ...
        toggle(target) {
            ...
            this.privateGlobalModAction(`${this.user.name} ${!exports.settings.disabled ? 'enabled' : 'disabled'} the abuse monitor.`);
            this.globalModlog('ABUSEMONITOR', null, !exports.settings.disabled ? 'enable' : 'disable');
        },
        threshold(target) {
            checkAccess(this);
            if (!target) {
                return this.sendReply(`The current abuse monitor threshold is ${exports.settings.threshold}.`);
            }
            ...
            this.sendReply(`|html|Remember to use <code>/am respawn</code> to deploy the settings to the child process.`);
        },
        async resolve(target) {
            this.checkCan('lock');
            target = target.toLowerCase().trim().replace(/ +/g, '');
            let [roomid, rawResult] = lib_1.Utils.splitFirst(target, ',').map(f => f.trim());
            const tarRoom = Rooms.get(roomid);
            if (!tarRoom || !exports.cache[tarRoom.roomid] || !exports.cache[tarRoom.roomid]?.staffNotified) {
            ...
            let result = (0, dex_data_1.toID)(rawResult) === 'success' ? 1 : (0, dex_data_1.toID)(rawResult) === 'failure' ? 0 : null;
            if (result === null)
            ...
            delete exports.cache[tarRoom.roomid];
            notifyStaff();
            ...
            await Chat.database.run(`INSERT INTO perspective_stats (staff, roomid, result, timestamp) VALUES ($staff, $roomid, $result, $timestamp) ` +
                ...
                `ON CONFLICT (roomid) DO UPDATE SET result = $result, timestamp = $timestamp`, { staff: this.user.id, roomid, result, timestamp: Date.now() });
            return this.parse(`/j view-abusemonitor-flagged`);
        },
        unmute(target, room, user) {
            this.checkCan('lock');
            room = this.requireRoom();
            target = (0, dex_data_1.toID)(target);
            if (!target) {
                return this.parse(`/help am`);
            }
            const roomMutes = exports.muted.get(room);
            if (!roomMutes) {
        ...
        async nojoinpunish(target, room, user) {
            this.checkCan('lock');
            const [roomid, type, rest] = lib_1.Utils.splitFirst(target, ',', 2).map(f => f.trim());
            const tarRoom = Rooms.get(roomid);
            if (!tarRoom)
                return this.popupReply(`The room "${roomid}" does not exist.`);
            const cmd = NOJOIN_COMMAND_WHITELIST[(0, dex_data_1.toID)(type)];
            if (!cmd) {
                throw new Chat.ErrorMessage(`Invalid punishment given. ` +
            ...
            if (result) { // command succeeded - send followup
                this.add('|c|~|/raw If you have questions about this action, please contact staff ' +
        ...
        logs(target) {
            checkAccess(this);
            const [count, userid] = lib_1.Utils.splitFirst(target, ',').map(dex_data_1.toID);
            this.parse(`/join view-abusemonitor-logs-${count || '200'}${userid ? `-${userid}` : ""}`);
        },
        ul: 'userlogs',
        userlogs(target) {
            checkAccess(this, 'lock');
            return this.parse(`/join view-abusemonitor-userlogs-${(0, dex_data_1.toID)(target)}`);
        },
        stats(target) {
            checkAccess(this);
        ...
        async respawn(target, room, user) {
            ...
            const unspawned = await exports.classifier.respawn();
        ...
        async rescale(target, room, user) {
            ...
            for (const chunk of examples) {
                ...
                const data = await exports.classifier.classify(message);
                if (!data) {
                ...
                await lib_1.Utils.waitUntil(Date.now() + 1000);
            }
            ...
            for (const p of exports.settings.punishments) {
                if (p.type !== type)
                    continue;
                if (p.certainty)
            ...
            if (type in exports.settings.specials) {
                for (const n in exports.settings.specials[type]) {
                    const num = exports.settings.specials[type][n];
                    delete exports.settings.specials[type][n];
                    exports.settings.specials[type][round(parseFloat(n) * change)] = num;
                }
            }
            saveSettings();
            this.refreshPage('abusemonitor-settings');
            this.addGlobalModAction(`${user.name} used /abusemonitor rescale ${type.toLowerCase().replace('_', '')}`);
        ...
        async userclear(target, room, user) {
            ...
            const targetId = (0, dex_data_1.toID)(targetUsername);
            if (!targetId)
                return this.parse(`/help abusemonitor`);
            ...
            user.lastCommand = '';
            const results = await Chat.database.run('DELETE FROM perspective_logs WHERE userid = ?', [targetId]);
            if (!results.changes) {
        ...
        async deletelog(target, room, user) {
            ...
            const row = await Chat.database.get('SELECT * FROM perspective_logs WHERE rowid = ?', [num]);
            if (!row) {
            ...
            this.stafflog(`Message: "${row.message}", room: ${row.roomid}, time: ${Chat.toTimestamp(new Date(row.time))}`);
        ...
        editspecial(target, room, user) {
            ...
            const score = parseInt(rawScore) || (0, dex_data_1.toID)(rawScore).toUpperCase();
            switch (typeof score) {
                case 'string':
                    if (score !== 'MAXIMUM') {
        ...
        deletespecial(target, room, user) {
            ...
            if (!exports.settings.specials[type]?.[percent]) {
                throw new Chat.ErrorMessage(`That special case does not exist.`);
            }
            delete exports.settings.specials[type][percent];
            if (!Object.keys(exports.settings.specials[type]).length) {
                delete exports.settings.specials[type];
            }
            saveSettings();
            this.refreshPage('abusemonitor-settings');
        ...
        exportpunishment(target) {
            ...
            this.sendReply(`|html|Punishment ${num + 1}: <code>` +
                `${visualizePunishment(punishment).replace(/: /g, ' = ')}</code>`);
        },
        changeall(target, room, user) {
            checkAccess(this);
            const [to, from] = target.split(',').map(f => (0, dex_data_1.toID)(f));
            if (!(to && from)) {
                throw new Chat.ErrorMessage(`Specify a type to change and a type to change to.`);
            }
            if (![to, from].every(f => punishmentHandlers[f])) {
                throw new Chat.ErrorMessage(`Invalid types given. Valid types: ${Object.keys(punishmentHandlers).join(', ')}.`);
            }
            const changed = [];
            for (const [i, punishment] of exports.settings.punishments.entries()) {
                if ((0, dex_data_1.toID)(punishment.type) === to) {
        ...
        exportpunishments() {
            checkAccess(this);
            let buf = exports.settings.punishments.map(punishment => {
                const line = [];
        ...
        addpunishment(target, room, user) {
            ...
            for (const cur of targets) {
                let [key, value] = lib_1.Utils.splitFirst(cur, '=').map(f => f.trim());
                key = (0, dex_data_1.toID)(key);
                ...
                switch (key) {
                    ...
                    case 'p':
                        ...
                        value = (0, dex_data_1.toID)(value).toUpperCase();
                    ...
                    case 't':
                        ...
                        if (!Artemis.RemoteClassifier.ATTRIBUTES[value]) {
                            throw new Chat.ErrorMessage(`Invalid attribute: ${value}. ` +
                    ...
                    case 's':
                    case 'secondary':
                        let [sType, sValue] = lib_1.Utils.splitFirst(value, '|').map(f => f.trim());
                        if (!sType || !sValue) {
                        ...
                        if (!Artemis.RemoteClassifier.ATTRIBUTES[sType]) {
                            throw new Chat.ErrorMessage(`Invalid secondary attribute: ${sType}. ` +
                                `Valid attributes: ${Object.keys(Artemis.RemoteClassifier.ATTRIBUTES).join(', ')}.`);
                        }
        ...
        thresholdincrement(target, room, user) {
            ...
            const [rawTurns, rawIncrement, rawMin] = lib_1.Utils.splitFirst(target, ',', 2).map(dex_data_1.toID);
            ...
            exports.settings.thresholdIncrement = { amount: increment, turns };
            if (min) {
                exports.settings.thresholdIncrement.minTurns = min;
            }
            saveSettings();
            this.refreshPage('abusemonitor-settings');
            this.privateGlobalModAction(`${user.name} set the abuse-monitor threshold increment ${increment} every ${Chat.count(turns, 'turns')}` +
                `${min ? ` after ${Chat.count(min, 'turns')}` : ""}`);
            this.globalModlog(`ABUSEMONITOR INCREMENT`, null, `${increment} every ${turns} turn(s)${min ? ` after ${min} turn(s)` : ""}`);
        },
        di: 'deleteincrement',
        deleteincrement(target, room, user) {
            checkAccess(this);
            if (!exports.settings.thresholdIncrement)
                throw new Chat.ErrorMessage(`The threshold increment is already disabled.`);
            exports.settings.thresholdIncrement = null;
            saveSettings();
            this.refreshPage('abusemonitor-settings');
        ...
        async failures(target) {
            ...
            let logs = await Chat.database.all('SELECT * FROM perspective_stats WHERE result = 0 AND timestamp > ? AND timestamp < ?', [timeNum, timeNum + 24 * 60 * 60 * 1000]);
            ...
            this.sendReplyBox(`<strong>${Chat.count(logs, 'logs')}</strong> found on the date ${target}:<hr />` +
        ...
        async backupsettings(target, room, user) {
            ...
            if (target) {
                ...
                await (0, lib_1.FS)(`config/chat-plugins/artemis/`).mkdirIfNonexistent();
            }
            else {
                target = `/nf.backup`;
            }
        ...
        async loadbackup(target, room, user) {
            ...
            const backup = await (0, lib_1.FS)(`config/chat-plugins/${path}.json`).readIfExists();
            if (!backup)
                throw new Chat.ErrorMessage(`No backup settings saved.`);
            const backupSettings = JSON.parse(backup);
            Object.assign(exports.settings, backupSettings);
            saveSettings();
        ...
        async deletebackup(target, room, user) {
            ...
            const path = (0, lib_1.FS)(`config/chat-plugins/artemis/${target}.json`);
        ...
        async backups() {
            ...
            const files = await (0, lib_1.FS)(`config/chat-plugins/artemis`).readdirIfExists();
            if (!files.length) {
        ...
        togglepunishments(target, room, user) {
            ...
            if (this.meansYes(target)) {
                ...
                exports.settings.recommendOnly = false;
                ...
                exports.settings.recommendOnly = true;
                message = `${user.name} disabled automatic punishments for the Artemis battle monitor`;
            }
            else {
                return this.sendReply(`Automatic punishments are: ${!exports.settings.recommendOnly ? 'ON' : 'OFF'}.`);
            }
            this.privateGlobalModAction(message);
            this.globalModlog(`ABUSEMONITOR TOGGLE`, null, exports.settings.recommendOnly ? 'off' : 'on');
            saveSettings();
        },
        review() {
            this.checkCan('lock');
        ...
        async submitreview(target, room, user) {
            ...
            const [roomid, reason] = lib_1.Utils.splitFirst(target, ',').map(f => f.trim());
            const log = await (0, helptickets_1.getBattleLog)((0, helptickets_1.getBattleLinks)(roomid)[0] || "");
            if (!log) {
                return this.popupReply(`No logs found for that roomid.`);
            }
            if (exports.reviews[user.id]?.some(f => f.room === roomid)) {
            ...
            (exports.reviews[_a = user.id] || (exports.reviews[_a] = [])).push({
                room: roomid,
                details: reason,
                staff: user.id,
                time: Date.now(),
        ...
        resolvereview(target, room, user) {
            checkAccess(this);
            let [userid, roomid, accurate, result] = lib_1.Utils.splitFirst(target, ',', 3).map(f => f.trim());
            userid = (0, dex_data_1.toID)(userid);
            roomid = (0, helptickets_1.getBattleLinks)(roomid)[0] || "";
            if (!userid || !roomid || !accurate || !result) {
                return this.parse(`/help abusemonitor resolvereview`);
            }
            if (!exports.reviews[userid]) {
                throw new Chat.ErrorMessage(`No reviews found by that user.`);
            }
            const review = exports.reviews[userid].find(f => (0, helptickets_1.getBattleLinks)(f.room).includes(roomid));
            if (!review) {
        ...
        replace(target, room, user) {
            ...
            if ((0, dex_data_1.toID)(old) === (0, dex_data_1.toID)(newWord))
                throw new Chat.ErrorMessage(`The old word and the new word are the same.`);
            if (exports.settings.replacements[old]) {
                throw new Chat.ErrorMessage(`The old word '${old}' is already in use (for '${exports.settings.replacements[old]}').`);
            }
            Chat.validateRegex(target);
            exports.settings.replacements[old] = newWord;
            saveSettings();
        ...
        removereplace(target, room, user) {
            ...
            if (!target)
                return this.parse(`/help am`);
            const replaceTo = exports.settings.replacements[target];
            if (!replaceTo) {
                throw new Chat.ErrorMessage(`${target} is not a currently set replacement.`);
            }
            delete exports.settings.replacements[target];
            saveSettings();
        ...
        ignoremodlog: {
            add(target, room, user) {
                ...
                targetUser = (0, dex_data_1.toID)(targetUser);
                if (!targetUser || !target) {
                    return this.popupReply(`Must specify a user and a target date (or modlog entry number).`);
                }
                if (!exports.metadata.modlogIgnores)
                    exports.metadata.modlogIgnores = {};
                const num = Number(target);
                if (isNaN(num)) {
                    if (!/[0-9]{4}-[0-9]{2}-[0-9]{2}/.test(target)) {
                        throw new Chat.ErrorMessage(`Invalid date provided. Must be in YYYY-MM-DD format.`);
                    }
                    exports.metadata.modlogIgnores[targetUser] = target;
                    target = 'before and including ' + target;
                }
                else {
                    let ignores = exports.metadata.modlogIgnores[targetUser];
                    if (!Array.isArray(ignores)) {
                        exports.metadata.modlogIgnores[targetUser] = ignores = [];
                    }
            ...
            remove(target, room, user) {
                ...
                if (!targetUser || !rawNum) {
                    return this.popupReply(`Specify a target user and a target (either a modlog entry # or a date).`);
                }
                const entry = exports.metadata.modlogIgnores?.[targetUser];
                if (!entry) {
                    throw new Chat.ErrorMessage(`That user has no ignored modlog entries registered.`);
                }
                if (typeof entry === 'string') {
                    rawNum = entry;
                    delete exports.metadata.modlogIgnores[targetUser];
                }
                else {
                    if (isNaN(num)) {
...
exports.pages = {
    abusemonitor: {
        flagged(query, user) {
            ...
            for (const roomid of ids) {
                ...
                buf += lib_1.Utils.html `<td>${Rooms.get(roomid)?.title || roomid}</td>`;
        ...
        async view(query, user) {
            ...
            buf += lib_1.Utils.html ` - ${room.title}</h2>`;
            this.title = `[Abuse Monitor] ${titleParts.join('-')}`;
            buf += `<p>${Chat.formatText(`<<${room.roomid}>>`)}</p>`;
            buf += `<hr />`;
            if (!exports.cache[roomid].claimed) {
                exports.cache[roomid].claimed = user.id;
                notifyStaff();
            }
            else {
                buf += `<p><strong>Claimed:</strong> ${exports.cache[roomid].claimed}</p>`;
            }
            ...
            const users = new lib_1.Utils.Multiset();
            const logData = await (0, helptickets_1.getBattleLog)(room.roomid, true);
            ...
            for (const line of logData.log) {
                ...
                const id = (0, dex_data_1.toID)(data.user);
                if (!id)
                    continue;
                users.add(id);
                buf += `<div class="chat chatmessage">`;
                buf += `<strong style="color: ${helptickets_1.HelpTicket.colorName(id, logData)}">`;
                buf += lib_1.Utils.html `<span class="username">${data.user}:</span></strong> ${data.message}</div>`;
            }
            buf += `</div></details>`;
            const recs = exports.cache[roomid].recommended || {};
            ...
            const sortedUsers = lib_1.Utils.sortBy([...users], ([id, num]) => ([isFlaggedUserid(id, roomid), -num, id]));
            for (const [id] of sortedUsers) {
                const curUser = Users.getExact(id);
                buf += lib_1.Utils.html `<details class="readmore"><summary>${curUser?.name || id} `;
                ...
                for (const name of punishments) {
                    buf += `<form data-submitsend="/am nojoinpunish ${roomid},${(0, dex_data_1.toID)(name)},${id},{reason}">`;
        ...
        async userlogs(query, user) {
            ...
            const userid = (0, dex_data_1.toID)(query.shift());
        ...
        async logs(query, user) {
            ...
            lib_1.Utils.sortBy(logs, log => [-log.time, log.roomid, log.userid]);
            ...
            for (const log of logs) {
                ...
                buf += lib_1.Utils.html `<td>${log.message}</td>`;
        ...
        async stats(query, user) {
            ...
            buf += `<hr />`;
            const logs = await Chat.database.all(`SELECT * FROM perspective_stats WHERE timestamp > ? AND timestamp < ?`, [new Date(dateString + '-01').getTime(), new Date(nextMonth(dateString)).getTime()]);
            ...
            if (punishmentStats.total) {
                ...
                header = '';
                data = '';
                const sortedDayStats = lib_1.Utils.sortBy(Object.keys(punishmentStats.byDay), d => new Date(d).getTime());
                ...
                const sorted = lib_1.Utils.sortBy(Object.entries(punishmentStats.types), e => e[1]);
            ...
            for (const id of lib_1.Utils.sortBy(Object.keys(staffStats), k => -staffStats[k])) {
        ...
        async settings() {
            ...
            if (await (0, lib_1.FS)('config/chat-plugins/nf.backup.json').exists()) {
            ...
            buf += `<input name="num" value="${exports.settings.minScore}"/>`;
            ...
            buf += `<input name="num" value="${exports.settings.threshold}"/>`;
            ...
            const replacements = Object.keys(exports.settings.replacements);
            if (replacements.length) {
                buf += `<br />Replacements: `;
                buf += replacements.map(k => `${k}: ${exports.settings.replacements[k]}`).join(', ');
                buf += `<br />`;
            }
            ...
            if (exports.settings.punishments.length) {
                for (const [i, p] of exports.settings.punishments.entries()) {
                    buf += `&bull; ${i + 1}: `;
                    buf += Object.keys(p).map(f => `${f}: ${visualizePunishmentKey(p, f)}`).join(', ');
            ...
            const keys = lib_1.Utils.sortBy(Object.keys(Artemis.RemoteClassifier.ATTRIBUTES), k => [-Object.keys(exports.settings.specials[k] || {}).length, k]);
            for (const k of keys) {
                buf += `<strong>${k}</strong>:<br />`;
                if (exports.settings.specials[k]) {
                    for (const percent in exports.settings.specials[k]) {
                        buf += `&bull; ${percent}%: ${exports.settings.specials[k][percent]} `;
        ...
        reviews() {
            ...
            for (const userid in exports.reviews) {
                const curReviews = exports.reviews[userid].filter(f => !f.resolved);
                ...
                for (const review of curReviews) {
                    ...
                    buf += `Battle: <a href="//${Config.routes.client}/${(0, helptickets_1.getBattleLinks)(review.room)[0]}">${review.room}</a><br />`;
                    buf += lib_1.Utils.html `<details class="readmore"><summary>Review details:</summary>${review.details}</details>`;
        ...
        async edithistory(query, user) {
            this.checkCan('lock');
            const targetUser = (0, dex_data_1.toID)(query[0]);
            if (!targetUser) {
            ...
            buf += modlogEntries.results.map(result => {
                ...
                let innerBuf = lib_1.Utils.html `<td><small>#${result.entryID}</small> [${day}] `;
                innerBuf += `${result.action}${result.note ? ` (${result.note.trim()})` : ``}</td>`;
                const existingIgnore = exports.metadata.modlogIgnores?.[targetUser];
    ...
    async battlechat(query) {
        const [format, num, pw] = query.map(dex_data_1.toID);
        this.checkCan('lock');
        if (!format || !num) {
        ...
        const logData = await (0, helptickets_1.getBattleLog)(full);
        if (!logData) {
        ...
        buf += `Players: ${Object.values(logData.players).map(dex_data_1.toID).filter(Boolean).join(', ')}<hr />`;
        let atLeastOne = false;
        for (const line of log) {
            const [, , username, message] = lib_1.Utils.splitFirst(line, '|', 3);
            buf += lib_1.Utils.html `<div class="chat"><span class="username"><username>${username}:</username></span> ${message}</div>`;
            atLeastOne = true;
        }
        if (!atLeastOne)
            buf += `None found.`;
...
const punishmentfilter = (user, punishment) => {
    ...
    if (!Punishments.punishmentTypes.has(punishment.type))
        return;
    const cacheEntry = exports.punishmentCache.get(user) || {};
