...
exports.handlers = exports.pages = exports.commands = exports.chatfilter = exports.AutoResponder = exports.answererData = void 0;
const lib_1 = require("../../lib");
const chatlog_1 = require("./chatlog");
const room_faqs_1 = require("./room-faqs");
const DATA_PATH = 'config/chat-plugins/responder.json';
const LOG_PATH = Monitor.logPath('responder.jsonl').path;
exports.answererData = {};
try {
    exports.answererData = JSON.parse((0, lib_1.FS)(DATA_PATH).readSync());
}
catch { }
class AutoResponder {
    ...
    find(question, user) {
        ...
        const helpFaqs = room_faqs_1.roomFaqs[room.roomid];
        if (!helpFaqs)
            return null;
    ...
    visualize(question, hideButton, user) {
        ...
        if (response) {
            let buf = '';
            buf += lib_1.Utils.html `<strong>You said:</strong> ${question}<br />`;
            buf += `<strong>Our automated reply:</strong> ${Chat.collapseLineBreaksHTML((0, room_faqs_1.visualizeFaq)(response))}`;
            if (!hideButton) {
                buf += lib_1.Utils.html `<hr /><button class="button" name="send" value="A: ${question}">`;
    ...
    getFaqID(faq) {
        ...
        const room = this.room;
        const entry = room_faqs_1.roomFaqs[room.roomid][faq];
        if (!entry)
            throw new Chat.ErrorMessage(`FAQ ID "${faq}" not found.`);
        if (!entry.alias)
            return faq; // not an alias
        return entry.source;
    }
    async getStatsFor(date) {
        const stream = (0, lib_1.FS)(LOG_PATH).createReadStream();
        const buf = [];
    ...
    async listDays() {
        const stream = (0, lib_1.FS)(LOG_PATH).createReadStream();
        const buf = new lib_1.Utils.Multiset();
        for await (const raw of stream.byLine()) {
            try {
                const data = JSON.parse(raw);
    ...
    updateFaqData(faq) {
        ...
        if (!room)
            return;
        if (room_faqs_1.roomFaqs[room.roomid][faq])
            return true;
    ...
    stringRegex(str, raw) {
        [str] = lib_1.Utils.splitFirst(str, '=>');
    ...
    log(entry, faq, expression) {
        const [day] = lib_1.Utils.splitFirst(Chat.toTimestamp(new Date), ' ');
    ...
    writeState() {
        ...
        return (0, lib_1.FS)(DATA_PATH).writeUpdate(() => JSON.stringify(exports.answererData));
    }
...
AutoResponder.logStream = (0, lib_1.FS)(LOG_PATH).createAppendStream();
...
exports.commands = {
    ...
    autoresponder: {
        ...
        toggle(target, room, user) {
            room = this.requireRoom();
            if (!target) {
                return this.sendReply(`The Help auto-response filter is currently set to: ${room.responder ? 'ON' : "OFF"}`);
            }
        ...
        ignore(target, room, user) {
            ...
            this.privateModAction(`${user.name} added ${Chat.count(targets.length, "terms")} to the autoresponder ignore list.`);
        ...
        switch (args[0]) {
            case 'stats':
                ...
                if (date) {
                    ...
                    for (const entry of stats) {
                        buf += `<small>Message:</small>${chatlog_1.LogViewer.renderLine(entry.message)}`;
                        buf += `<small>FAQ: ${entry.faqName}</small><br />`;
                        buf += `<small>Regex: <code>${entry.regex}</code></small> <hr />`;
                    }
                    return chatlog_1.LogViewer.linkify(buf);
                }
            ...
            case 'keys':
                ...
                buf += Object.entries(roomData.pairs).map(([item, regexes]) => {
                    ...
                    for (const regex of regexes) {
                        ...
                        buffer += lib_1.Utils.html `<tr><td>${index}</td><td><code>${regex}</code></td>`;
                        if (canChange)
                            buffer += `<td>${button}</td>`;
                        buffer += `</tr>`;
                    }
            ...
            case 'ignore':
                ...
                for (const term of roomData.ignore) {
                    buf += `- ${lib_1.Utils.escapeHTML(term)} <button class="button" name="send"value="/msgroom ${room.roomid},/ar unignore ${term}">Remove</button><br />`;
                }
                buf += `</div>`;
                break;
            default:
...
exports.handlers = {
    onRenameRoom(oldID, newID) {
        if (exports.answererData[oldID]) {
            if (!exports.answererData[newID])
                exports.answererData[newID] = { pairs: {} };
            Object.assign(exports.answererData[newID], exports.answererData[oldID]);
            delete exports.answererData[oldID];
            (0, lib_1.FS)(DATA_PATH).writeUpdate(() => JSON.stringify(exports.answererData));
        }
    },
};
