...
exports.pages = exports.commands = exports.YouTube = exports.GroupWatch = exports.Twitch = exports.YoutubeInterface = exports.searchDataCache = exports.videoDataCache = void 0;
...
exports.videoDataCache = Chat.oldPlugins.youtube?.videoDataCache || new Map();
exports.searchDataCache = Chat.oldPlugins.youtube?.searchDataCache || new Map();
function loadData() {
    const raw = JSON.parse((0, lib_1.FS)(STORAGE_PATH).readIfExistsSync() || "{}");
    if (!(raw.channels && raw.categories)) { // hasn't been converted to new format
        const data = {};
        data.channels = raw;
        data.categories = [];
        // re-save into new format
        (0, lib_1.FS)(STORAGE_PATH).writeUpdate(() => JSON.stringify(data));
        return data;
    }
    return raw;
}
...
class YoutubeInterface {
    constructor(data) {
        this.linkRegex = /^(https?:\/\/)?(www\.)?(youtube\.com|youtu\.be)(\/|$)/i;
    ...
    async getChannelData(link, username) {
        ...
        const raw = await (0, lib_1.Net)(`${ROOT}channels`).get({
    ...
    randChannel(cat) {
        ...
        const id = lib_1.Utils.shuffle(channels)[0];
    ...
    async getVideoData(id) {
        ...
        let raw;
        try {
            raw = await (0, lib_1.Net)(`${ROOT}videos`).get({
                query: { part: 'snippet,statistics', id, key: Config.youtubeKey },
            });
        }
        catch (e) {
    ...
    save() {
        return (0, lib_1.FS)(STORAGE_PATH).writeUpdate(() => JSON.stringify(this.data));
    }
    async searchVideo(name, limit) {
        ...
        const raw = await (0, lib_1.Net)(`${ROOT}search`).get({
            query: {
                part: 'snippet', q: name,
    ...
    runInterval(time) {
        ...
        this.interval = setInterval(() => {
            void (async () => {
                ...
                if (!room)
                    return; // do nothing if the room doesn't exist anymore
    ...
    async getGroupwatchData(url) {
        ...
        if (['youtu.be', 'www.youtube.com'].includes(host)) {
            ...
            const data = await exports.Twitch.getChannel(urlData.pathname.slice(1));
            if (!data)
                throw new Chat.ErrorMessage(`Channel not found`);
...
exports.Twitch = new class {
    ...
    async getChannel(channel) {
        ...
        let res;
        try {
            res = await (0, lib_1.Net)(`https://api.twitch.tv/helix/search/channels`).get({
                headers: {
        ...
        lib_1.Utils.sortBy(data.channels, c => -c.followers);
...
class GroupWatch extends Rooms.SimpleRoomGame {
    ...
    display() {
        return (lib_1.Utils.html `<center><div class="pad"><strong>${this.room.title} Groupwatch - ${this.title}</strong><br /><br />` +
    ...
    async changeVideo(url) {
        const info = await exports.YouTube.getGroupwatchData(url);
        if (!info)
...
GroupWatch.groupwatches = new Map();
exports.YouTube = new YoutubeInterface(channelData);
function destroy() {
    if (exports.YouTube.interval)
        clearInterval(exports.YouTube.interval);
}
exports.commands = {
    async randchannel(target, room, user) {
        room = this.requireRoom('internetexplorers');
        if (Object.keys(exports.YouTube.data.channels).length < 1)
    ...
    youtube: {
        async addchannel(target, room, user) {
            ...
            return this.privateModAction(`${user.name} added channel with id ${id} ${name ? `and username (${name}) ` : ''} to the random channel pool.`);
        },
        ...
        removecategory(target, room, user) {
            ...
            for (const id in exports.YouTube.data.channels) {
                ...
                if (channel.category === target)
                    delete exports.YouTube.data.channels[id].category;
            }
            exports.YouTube.save();
        ...
        setcategory(target, room, user) {
            ...
            this.checkCan('mute', null, room);
            target = target.trim();
            const [category, id] = lib_1.Utils.splitFirst(target, ',').map(item => item.trim());
    ...
    groupwatch: {
        async create(target, room, user) {
            ...
            this.checkCan('mute', null, room);
            const [url, title] = lib_1.Utils.splitFirst(target, ',').map(p => p.trim());
            if (!url || !title)
                throw new Chat.ErrorMessage(`You must specify a video to watch and a title for the group watch.`);
            const game = await exports.YouTube.createGroupWatch(url, room, title);
            this.modlog(`YOUTUBE GROUPWATCH`, null, `${url} (${title})`);
            room.add(`|uhtml|${game.id}|` +
    ...
    async channels(args, user) {
        ...
        switch (toID(type)) {
            case 'categories':
                ...
                break;
            default:
                for (const id of lib_1.Utils.shuffle(Object.keys(exports.YouTube.data.channels))) {
