...
exports.pages = exports.commands = exports.destroy = exports.TeamsHandler = void 0;
exports.start = start;
...
function refresh(context) {
    return (`<button class="button" name="send" value="/j ${context.pageid}" style="float: right">` +
        ` <i class="fa fa-refresh"></i> ${context.tr('Refresh')}</button>`);
}
exports.TeamsHandler = new class {
    constructor() {
        this.readyPromise = teamsDB ? (async () => {
            try {
                await teamsDB.query() `SELECT * FROM teams LIMIT 1`;
            }
            catch {
                await teamsDB.query((0, database_1.SQL)((0, lib_1.FS)(`databases/schemas/teams.sql`).readSync()));
            }
        })() : null;
    }
    destroy() {
    ...
    async search(search, user, count = 10, includePrivate = false) {
        ...
        if (search.format) {
            where.push(where.length ? (0, database_1.SQL) ` AND ` : (0, database_1.SQL) `WHERE `);
            where.push((0, database_1.SQL) `format = ${toID(search.format)}`);
        }
        if (search.owner) {
            where.push(where.length ? (0, database_1.SQL) ` AND ` : (0, database_1.SQL) `WHERE `);
            where.push((0, database_1.SQL) `ownerid = ${toID(search.owner)}`);
        }
        if (search.gen) {
            where.push(where.length ? (0, database_1.SQL) ` AND ` : (0, database_1.SQL) `WHERE `);
            where.push((0, database_1.SQL) `format LIKE ${`gen${search.gen}%`}`);
        }
        if (!includePrivate) {
            where.push(where.length ? (0, database_1.SQL) ` AND ` : (0, database_1.SQL) `WHERE `);
            where.push((0, database_1.SQL) `private IS NULL`);
        }
        ...
        return result.filter(row => {
            ...
            if (search.pokemon?.length) {
                match = search.pokemon.some(pokemon => team.some(set => toID(set.species) === toID(pokemon)));
            }
            if (!match)
                return false;
            if (search.moves?.length) {
                match = search.moves.some(move => team.some(set => set.moves.some(m => toID(m) === toID(move))));
            }
            if (!match)
                return false;
            if (search.abilities?.length) {
                match = search.abilities.some(ability => team.some(set => toID(set.ability) === toID(ability)));
            }
            return match;
        });
    }
    ...
    async save(context, team, isUpdate) {
        ...
        if (unownWord) {
            ...
            if (!filtered || filtered !== unownWord) {
                connection.popup(`Your team was rejected for the following reason:\n\n` +
        ...
        if (team.privacy === true)
            team.privacy = existing?.private || exports.TeamsHandler.generatePassword();
        // the && existing doesn't really matter because we've verified it above, this is just for TS
        if (isUpdate && existing) {
            const differenceExists = (existing.team !== team.packedTeam ||
            ...
            await teamsTable.updateOne({ team: team.packedTeam, title: team.name, private: team.privacy, format: format.id }) `WHERE teamid = ${isUpdate}`;
    ...
    updateViews(teamid) {
        return teamsTable.updateOne((0, database_1.SQL) `views = views + 1`) `WHERE teamid = ${teamid}`;
    }
    list(userid, count, publicOnly = false) {
        const publicOnlyQuery = publicOnly ? (0, database_1.SQL) `AND private IS NULL ` : (0, database_1.SQL) ``;
    ...
    preview(teamData, user, isFull = false) {
        let buf = lib_1.Utils.html `<strong>${teamData.title || `Untitled ${teamData.teamid}`}`;
        if (teamData.private)
            buf += ` (Private)`;
        buf += `</strong><br />`;
    ...
    renderTeam(teamData, user) {
        ...
        buf += team.map(set => {
            ...
            if (set.name && set.name !== set.species) {
                teamBuf = teamBuf.replace(set.name, lib_1.Utils.html `<psicon pokemon="${set.species}" /> <br />${set.name}`);
            }
            else {
                teamBuf = teamBuf.replace(set.species, `<psicon pokemon="${set.species}" /> <br />${set.species}`);
            }
            if (set.item) {
                const tester = new RegExp(`${lib_1.Utils.escapeRegex(set.item)}\\b`);
    ...
    validateAccess(conn, popup = false) {
        ...
        if (!user.autoconfirmed)
            err(`To use the teams database, you must be autoconfirmed, which means being registered for at least ` +
    ...
    async count(user) {
        const id = toID(user);
        const result = await teamsTable.queryOne() `SELECT count(*) AS count FROM teams WHERE ownerid = ${id}`;
...
const destroy = () => exports.TeamsHandler.destroy();
...
exports.commands = {
    teams: {
        ...
        async save(target, room, user, connection, cmd) {
            exports.TeamsHandler.validateAccess(connection, true);
            const isEdit = cmd === 'update';
            const targets = lib_1.Utils.splitFirst(target, ',', isEdit ? 4 : 3).map(x => x.trim());
            ...
            const result = await exports.TeamsHandler.save(this, {
        ...
        async delete(target, room, user, connection) {
            ...
            const teamData = await exports.TeamsHandler.get(teamid);
            if (!teamData)
            ...
            await exports.TeamsHandler.delete(teamid);
        ...
        async setprivacy(target, room, user, connection) {
            ...
            if (this.meansYes(rawPrivacy)) {
                privacy = exports.TeamsHandler.generatePassword();
            }
            ...
            const team = await exports.TeamsHandler.get(teamId);
            if (!team)
                return this.popupReply(`Team not found.`);
...
exports.pages = {
    ...
    team(query, user, connection) {
        return exports.pages.teams.view.call(this, query, user, connection);
    },
    teams: {
        async all(query, user, connection) {
            if (!user.named)
                return Rooms.RETRY_AFTER_LOGIN;
            exports.TeamsHandler.validateAccess(connection);
            const targetUserid = toID(query.shift()) || user.id;
            let count = Number(query.shift()) || 10;
            if (count > MAX_TEAMS)
                count = MAX_TEAMS;
            this.title = `[Teams] ${targetUserid}`;
            const teams = await exports.TeamsHandler.list(targetUserid, count, user.id !== targetUserid);
            ...
            for (const team of teams) {
                buf += exports.TeamsHandler.preview(team, user);
                buf += `<hr />`;
            }
            const total = await exports.TeamsHandler.count(user.id);
            if (total > count) {
        ...
        async filtered(query, user, connection) {
            ...
            buf += `<hr />`;
            buf += teams.map(team => exports.TeamsHandler.preview(team, user)).join('<hr />');
            buf += `</div>`;
            return buf;
        },
        async view(query, user, connection) {
            ...
            if (user.id !== team.ownerid && team.views >= 0) {
                void exports.TeamsHandler.updateViews(team.teamid);
            }
            return `<div class="ladder pad">` + exports.TeamsHandler.renderTeam(team, user) + "</div>";
        },
        ...
        async edit(query, user, connection) {
            ...
            const data = await exports.TeamsHandler.get(teamID);
            if (!data) {
            ...
            const teamStr = Teams.export(Teams.import(data.team), {
                removeNicknames: name => exports.TeamsHandler.isOMNickname(name),
            }).replace(/\n/g, '&#13;');
        ...
        async searchpublic(query, user, connection) {
            ...
            const results = await exports.TeamsHandler.search(search, user, 50, isPersonal);
            ...
            buf += results.map(t => exports.TeamsHandler.preview(t, user)).join('<hr />');
            return buf;
        },
        async searchpersonal(query, user, connection) {
            ...
            return exports.pages.teams.searchpublic.call(this, `${user.id}${query.join('-')}`.split('-'), user, connection);
        },
        async browse(query, user, connection) {
            ...
            switch (sorter) {
                case 'views':
                    order = (0, database_1.SQL) ` ORDER BY views DESC `;
                    name = 'most viewed';
                    break;
                case 'latest':
                    order = (0, database_1.SQL) ` ORDER BY date DESC`;
                    break;
                default:
...
function start() {
    if (Config.usepostgres && Config.usepostgresteams) {
        teamsDB = new database_1.PGDatabase(Config.usepostgres);
        teamsTable = teamsDB.getTable('teams', 'teamid');
    }
