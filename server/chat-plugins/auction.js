...
exports.roomSettings = exports.commands = exports.Auction = void 0;
/**
...
class Team {
    ...
    isSuspended() {
        return this.suspended || (this.auction.type === 'snake' ?
...
class Auction extends Rooms.SimpleRoomGame {
    constructor(room, startingCredits = 100000) {
        super(room);
        this.gameid = 'auction';
        this.owners = new Set();
        this.teams = new Map();
    ...
    generateUsernameList(players, max = players.length, clickable = false) {
        ...
        buf += players.slice(0, max).map(p => {
            if (typeof p === 'object') {
                return `<username ${clickable ? ' class="username"' : ''}>${lib_1.Utils.escapeHTML(p.name)}</username>`;
            }
            return `<username${clickable ? ' class="username"' : ''}>${lib_1.Utils.escapeHTML(p)}</username>`;
        }).join(', ');
        if (players.length > max) {
            buf += ` <span title="${players.slice(max).map(p => lib_1.Utils.escapeHTML(typeof p === 'object' ? p.name : p)).join(', ')}">(+${players.length - max})</span>`;
        }
        buf += `</span>`;
        return buf;
    }
    generatePriceList() {
        const players = lib_1.Utils.sortBy(this.getDraftedPlayers(), p => -p.price);
        let buf = '';
        let smogonExport = '';
        for (const team of this.teams.values()) {
            ...
            for (const player of players.filter(p => p.team === team)) {
                table += lib_1.Utils.html `<tr><td>${player.name}</td><td>${player.price}</td></tr>`;
                smogonTable += `[TR][TD]${player.name}[/TD][TD]${player.price}[/TD][/TR]`;
            }
            table += `</table>`;
            smogonTable += `[/TABLE]`;
            buf += `<details><summary>${lib_1.Utils.escapeHTML(team.name)}</summary>${table}</details><br/>`;
            if (this.ended)
                smogonExport += `[SPOILER="${team.name}"]${smogonTable}[/SPOILER]`;
        }
        let table = `<table>`;
        let smogonTable = `[TABLE]`;
        for (const player of players) {
            table += lib_1.Utils.html `<tr><td>${player.name}</td><td>${player.price}</td><td>${player.team.name}</td></tr>`;
        ...
        if (this.ended) {
            ...
            buf += lib_1.Utils.html `<copytext value="${smogonExport}">Copy Smogon Export</copytext>`;
        }
        return buf;
    }
    generateAuctionTable() {
        ...
        for (const team of this.teams.values()) {
            ...
            buf += `<td style="white-space: nowrap"><strong>${lib_1.Utils.escapeHTML(team.name)}</strong><br/>${this.generateUsernameList(team.getManagers(), 2, true)}</td>`;
            if (this.type !== 'snake') {
            ...
            buf += `<td title="${team.players.map(p => lib_1.Utils.escapeHTML(p.name)).join(', ')}"><div style="min-height: 32px${!this.ended ? `; height: 32px; overflow: hidden; resize: vertical` : ''}"><span style="float: right">${team.players.length}</span>${this.generateUsernameList(team.players)}</div></td>`;
            buf += `</tr>`;
        }
        buf += `</table></div>`;
        const players = lib_1.Utils.sortBy(this.getUndraftedPlayers(), p => p.name);
        const tierArrays = new Map();
        for (const player of players) {
        ...
        if (sortedTiers.length) {
            ...
            for (const tier of sortedTiers) {
                ...
                buf += `<li><details><summary>${lib_1.Utils.escapeHTML(tier)} (${tierPlayers.length})</summary>${this.generateUsernameList(tierPlayers)}</details></li>`;
            }
    ...
    sendBidInfo() {
        ...
        buf += lib_1.Utils.html `Player: <username>${this.nominatedPlayer.name}</username> `;
        if (this.type === 'auction') {
            buf += `Top bid: <b>${this.highestBid}</b> `;
            buf += lib_1.Utils.html `Top bidder: <b>${this.highestBidder.name}</b> `;
            buf += `Managers: ${this.highestBidder.getManagers().map(m => `<username class="username">${lib_1.Utils.escapeHTML(m)}</username>`).join(' ')}<br/>`;
        }
        buf += lib_1.Utils.html `Tiers Played: <b>${this.nominatedPlayer.tiersPlayed.length ? `${this.nominatedPlayer.tiersPlayed.join(', ')}` : 'N/A'}</b><br/>`;
        buf += lib_1.Utils.html `Tiers Not Played: <b>${this.nominatedPlayer.tiersNotPlayed.length ? `${this.nominatedPlayer.tiersNotPlayed.join(', ')}` : 'N/A'}</b>`;
        buf += `</div>`;
    ...
    next() {
        ...
        this.sendHTMLBox(this.generateAuctionTable());
        this.sendMessage(`/html It is now <b>${lib_1.Utils.escapeHTML(this.nominatingTeam.name)}</b>'s turn to nominate a player. Managers: ${this.nominatingTeam.getManagers().map(m => `<username class="username">${lib_1.Utils.escapeHTML(m)}</username>`).join(' ')}`);
        this.startNomTimer();
    }
    nominate(user, target) {
        ...
        if (this.type === 'snake') {
            this.sendMessage(lib_1.Utils.html `/html <b>${this.nominatingTeam.name}</b> drafted <username>${this.nominatedPlayer.name}</username>!`);
            ...
            this.sendMessage(lib_1.Utils.html `/html <username class="username">${user.name}</username> from team <b>${this.nominatingTeam.name}</b> has nominated <username>${player.name}</username> for auction!`);
            const notifyMsg = lib_1.Utils.html `|notify|${this.room.title} Auction|${player.name} has been nominated!`;
            for (const currManager of this.managers.values()) {
                ...
                curUser?.sendTo(this.room, `|raw|Send a message with the amount you want to bid (e.g. <code>.5</code> or <code>5</code> will place a bid of 5000)!`);
            }
            this.sendBidInfo();
            this.startBidTimer();
        }
    ...
    bid(user, bid) {
        ...
        if (this.type === 'blind') {
            ...
            const msg = `|c:|${Math.floor(Date.now() / 1000)}|&|/html Your team placed a bid of <b>${bid}</b> on <username>${lib_1.Utils.escapeHTML(this.nominatedPlayer.name)}</username>.`;
        ...
        if (Number(message.replace(',', '.'))) {
            ...
            try {
                ...
                if (e.name?.endsWith('ErrorMessage')) {
                    user.sendTo(this.room, lib_1.Utils.html `|raw|<span class="message-error">${e.message}</span>`);
                }
                else {
    ...
    finishCurrentNom() {
        if (this.type === 'blind') {
            ...
            if (!this.bidsPlaced.has(this.nominatingTeam)) {
                buf += lib_1.Utils.html `<tr><td>${this.nominatingTeam.name}</td><td>${this.minBid}</td></tr>`;
            }
            for (const [team, bid] of this.bidsPlaced) {
                buf += lib_1.Utils.html `<tr><td>${team.name}</td><td>${bid}</td></tr>`;
            }
            buf += `</table></div>`;
            this.sendHTMLBox(buf);
            this.bidsPlaced.clear();
        }
        this.sendMessage(lib_1.Utils.html `/html <b>${this.highestBidder.name}</b> bought <username>${this.nominatedPlayer.name}</username> for <b>${this.highestBid}</b> credits!`);
...
exports.commands = {
    auction: {
        create(target, room, user) {
            ...
            if (room.settings.auctionDisabled)
                throw new Chat.ErrorMessage('Auctions are currently disabled in this room.');
            let startingCredits;
            if (target) {
        ...
        async importplayers(target, room, user) {
            ...
            let data = '';
            try {
                data = await (0, lib_1.Net)(`https://pastebin.com/raw/${target.split('/').pop()}`).get();
            }
            catch { }
            if (!data)
                throw new Chat.ErrorMessage('Error fetching data from pastebin.');
    ...
    auctionhelp() {
        if (!this.runBroadcast())
            return;
        this.sendReplyBox(`Auction commands<br/>` +
