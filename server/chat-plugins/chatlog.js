...
exports.commands = exports.pages = exports.LogSearcher = exports.DatabaseLogSearcher = exports.RipgrepLogSearcher = exports.FSLogSearcher = exports.Searcher = exports.LogViewer = exports.LogReader = exports.LogReaderRoom = void 0;
const lib_1 = require("../../lib");
const database_1 = require("../../lib/database");
const roomlogs_1 = require("../roomlogs");
...
class LogReaderRoom {
    ...
    async listMonths() {
        if (roomlogs_1.roomlogTable) {
            const dates = await roomlogs_1.roomlogTable.query() `SELECT DISTINCT month FROM roomlog_dates WHERE roomid = ${this.roomid}`;
    ...
    async listDays(month) {
        if (roomlogs_1.roomlogTable) {
            const dates = await (roomlogs_1.roomlogTable.query() `SELECT DISTINCT date FROM roomlog_dates WHERE roomid = ${this.roomid} AND month = ${month}`);
    ...
    async getLog(day) {
        if (roomlogs_1.roomlogTable) {
            const [dayStart, dayEnd] = exports.LogReader.dayToRange(day);
            const logs = await roomlogs_1.roomlogTable.selectAll(['log', 'time']) `WHERE roomid = ${this.roomid} AND time BETWEEN ${dayStart}::int::timestamp AND ${dayEnd}::int::timestamp`;
            return new lib_1.Streams.ObjectReadStream({
                read() {
...
exports.LogReader = new class {
    async get(roomid) {
        if (roomlogs_1.roomlogTable) {
            if (!(await roomlogs_1.roomlogTable.selectOne() `WHERE roomid = ${roomid}`))
                return null;
        }
        else {
    ...
    async list() {
        if (roomlogs_1.roomlogTable) {
            const roomids = await roomlogs_1.roomlogTable.query() `SELECT DISTINCT roomid FROM roomlog_dates`;
...
exports.LogViewer = new class {
    ...
    renderLine(fullLine, opts, data) {
        ...
        if (opts !== 'all' && (line.startsWith(`userstats|`) ||
        ...
        if (opts === 'txt')
            return lib_1.Utils.html `<div ${getClass('chat')}>${fullLine}</div>`;
        const cmd = line.slice(0, line.indexOf('|'));
        if (opts?.includes('onlychat')) {
            if (cmd !== 'c')
                return '';
            if (opts.includes('txt'))
                return `<div ${getClass('chat')}>${lib_1.Utils.escapeHTML(fullLine)}</div>`;
        }
        const timeLink = data ?
        ...
        switch (cmd) {
            case 'c': {
                const [, name, message] = lib_1.Utils.splitFirst(line, '|', 2);
                if (name.length <= 1) {
                ...
                return `<div ${getClass('chat')}>` +
                    `<small>[${timeLink}]` + lib_1.Utils.html ` ${group}</small><username>${name.slice(1)}:</username> ` +
                    `<q>${Chat.formatText(message)}</q>` +
                    `</div>`;
            }
            case 'html':
            case 'raw': {
                const [, html] = lib_1.Utils.splitFirst(line, '|', 1);
            ...
            case 'uhtmlchange': {
                if (cmd !== 'uhtml')
                    return ``;
                const [, , html] = lib_1.Utils.splitFirst(line, '|', 2);
                return `<div ${getClass('notice')}>${html}</div>`;
            }
            case '!NT':
                return `<div ${getClass('chat')}>${lib_1.Utils.escapeHTML(fullLine)}</div>`;
            case '':
                return `<div ${getClass('chat')}><small>[${timeLink}] </small>${lib_1.Utils.escapeHTML(line.slice(1))}</div>`;
            default:
                return `<div ${getClass('chat')}><small>[${timeLink}] </small><code>${'|' + lib_1.Utils.escapeHTML(line)}</code></div>`;
        }
    }
...
class Searcher {
    ...
    renderLinecountResults(results, roomid, month, user) {
        let buf = lib_1.Utils.html `<div class="pad"><h2>Linecounts on `;
        ...
        if (!results) {
            ...
            buf += '<hr /><ol>';
            const sortedDays = lib_1.Utils.sortBy(Object.keys(results));
            let total = 0;
            ...
            const totalResults = {};
            for (const date of lib_1.Utils.sortBy(Object.keys(results))) {
            ...
            const sortedResults = lib_1.Utils.sortBy(resultKeys, userid => (-totalResults[userid])).slice(0, MAX_TOPUSERS);
            let total = 0;
    ...
    async runLinecountSearch(context, roomid, month, user) {
        context.setHTML(`<div class="pad"><h2>Searching linecounts on room ${roomid}${user ? ` for the user ${user}` : ''}.</h2></div>`);
        context.setHTML(await exports.LogSearcher.searchLinecounts(roomid, month, user));
    }
    async runSearch(context, search, roomid, date, limit) {
        ...
        context.setHTML(`<div class="pad"><h2>Running a chatlog search for "${search}" on room ${roomid}` +
    ...
    async roomStats(room, month) {
        const stats = await exports.LogSearcher.activityStats(room, month);
...
class FSLogSearcher extends Searcher {
    ...
    async dayStats(room, day) {
        ...
        const path = Monitor.logPath(`chat/${room}/${exports.LogReader.getMonth(day)}/${day}.txt`);
        if (!path.existsSync())
            return false;
        ...
        for await (const line of stream.byLine()) {
            ...
            switch (type) {
                ...
                case 'c:':
                case 'c': {
                    const { time, username } = exports.LogViewer.parseChatLine(line, day);
...
class RipgrepLogSearcher extends FSLogSearcher {
    async ripgrepSearchMonth(opts) {
        ...
        try {
            ...
            const { stdout } = await lib_1.ProcessManager.exec(['rg', ...options], {
                maxBuffer: MAX_MEMORY,
                cwd: lib_1.FS.ROOT_PATH,
            });
...
class DatabaseLogSearcher extends Searcher {
    async searchLogs(roomid, rawSearch, limit, month) {
        ...
        if (limit > 5000)
            limit = 5000;
        const search = {};
        const [monthStart, monthEnd] = exports.LogReader.monthToRange(month);
        if (!Rooms.Roomlogs.table) {
        ...
        const results = await Rooms.Roomlogs.table.selectAll() `
			WHERE ${search.user ? (0, database_1.SQL) `userid ${search.user[1] ? (0, database_1.SQL) `!=` : (0, database_1.SQL) `=`} ${search.user[0]} AND ` : (0, database_1.SQL) ``} time BETWEEN ${monthStart}::int::timestamp AND ${monthEnd}::int::timestamp AND
			type = ${'c'} AND roomid = ${roomid}
			${parsedSearch.length ? (0, database_1.SQL) ` AND content @@ plainto_tsquery(${parsedSearch.join(',')})` : (0, database_1.SQL) ``} LIMIT ${limit}
		`;
        let curDate = '';
        ...
        let buf = lib_1.Utils.html `<div class="pad"><strong>Results on ${roomid} for ${parsedSearchStr} during the month ${month}:</strong>`;
        buf += limit ? ` ${results.length} (capped at ${limit})` : '';
        buf += `<hr />`;
        const searchStr = `search-${lib_1.Dashycode.encode(rawSearch)}--limit-${limit}`;
        const pref = `/join view-chatlog-${roomid}--`;
        buf += `<button class="button" name="send" value="${pref}${exports.LogReader.prevMonth(month)}--${searchStr}">Previous month</button> `;
        buf += `<button class="button" name="send" value="${pref}${exports.LogReader.nextMonth(month)}--${searchStr}">Next month</button>`;
        buf += `<br />`;
        buf += `<div class="pad"><blockquote>`;
        buf += lib_1.Utils.sortBy(results, line => -line.time.getTime()).map(resultRow => {
            let [lineDate, lineTime] = Chat.toTimestamp(resultRow.time).split(' ');
            let line = exports.LogViewer.renderLine(`${lineTime} ${resultRow.log}`, '', {
    ...
    async searchLinecounts(roomid, month, user) {
        ...
        const rows = await Rooms.Roomlogs.table.selectAll() `
			WHERE ${user ? (0, database_1.SQL) `userid = ${user} AND ` : (0, database_1.SQL) ``}roomid = ${roomid} AND
...
exports.DatabaseLogSearcher = DatabaseLogSearcher;
exports.LogSearcher = new (Rooms.Roomlogs.table ? DatabaseLogSearcher :
...
exports.pages = {
    async chatlog(args, user, connection) {
        ...
        let [roomid, date, opts] = lib_1.Utils.splitFirst(args.join('-'), '--', 2);
        if (!roomid || roomid.startsWith('-')) {
            this.title = '[Logs]';
            return exports.LogViewer.list(user, roomid?.slice(1));
        }
        ...
        if (opts?.startsWith('search-')) {
            ...
            input = input.slice(7);
            search = lib_1.Dashycode.decode(input);
            if (search.length < 3)
        ...
        if (search) {
            ...
            return exports.LogSearcher.runSearch(this, search, roomid, date, limit);
        }
        else {
            if (date === 'today') {
                this.setHTML(await exports.LogViewer.day(roomid, exports.LogReader.today(), opts));
                if (isTime)
                    this.send(`|scroll|div[data-server="${opts}"]`);
            }
            else if (date.split('-').length === 3) {
                this.setHTML(await exports.LogViewer.day(roomid, parsedDate.toISOString().slice(0, 10), opts));
                if (isTime)
                    this.send(`|scroll|div[data-server="${opts}"]`);
            }
            else {
    ...
    roomstats(args, user) {
        ...
        const [, date, target] = lib_1.Utils.splitFirst(args.join('-'), '--', 3).map(item => item.trim());
        if (isNaN(new Date(date).getTime())) {
            throw new Chat.ErrorMessage(`Invalid date.`);
        }
        if (!exports.LogReader.isMonth(date)) {
            throw new Chat.ErrorMessage(`You must specify an exact month - both a year and a month.`);
        }
        this.title = `[Log Stats] ${date}`;
        return exports.LogSearcher.runLinecountSearch(this, room ? room.roomid : args[0], date, toID(target));
    },
    async logsaccess(query) {
        ...
        for await (const line of accessStream.byLine()) {
            const [id, rest] = lib_1.Utils.splitFirst(line, ': ');
            if (userid && id !== userid)
                continue;
...
exports.commands = {
    ...
    searchlogs(target, room) {
        ...
        let date = exports.LogReader.getMonth(exports.LogReader.today());
        const searches = [];
        let limit = '500';
        ...
        return this.parse(`/join view-chatlog-${targetRoom}--${date}--search-` +
            `${lib_1.Dashycode.encode(searches.join(','))}--limit-${limit}`);
    },
    searchlogshelp() {
    ...
    linecount(target, room, user) {
        ...
        for (const [i, param] of params.entries()) {
            ...
            if (!toID(val))
                continue; // unset, continue and allow defaults to apply
            ...
            switch (key) {
                ...
                case 'month':
                case 'time':
                    if (!exports.LogReader.isMonth(val)) {
                        throw new Chat.ErrorMessage(`Invalid date.`);
                    }
                    search.date = val;
            }
        ...
        if (!search.date) {
            search.date = exports.LogReader.getMonth();
        }
        return this.parse(`/join view-roomstats-${search.roomid}--${search.date}${search.user ? `--${search.user}` : ''}`);
    },
    linecounthelp() {
        return this.sendReplyBox(`<code>/linecount OR /roomstats OR /topusers</code> [<code>key=value</code> formatted parameters] - ` +
    ...
    async getbattlechat(target, room, user) {
        this.checkCan('lock');
        let [roomName, userName] = lib_1.Utils.splitFirst(target, ',').map(f => f.trim());
        if (!roomName) {
            if (!room) {
        ...
        if (tarRoom) {
            ...
            try {
                const raw = await (0, lib_1.Net)(`https://${Config.routes.replays}/${roomid.slice('battle-'.length)}.json`).get();
        ...
        if (!atLeastOne)
            buf = `<br />None found.`;
        this.runBroadcast();
        return this.sendReplyBox(lib_1.Utils.html `<strong>Chat messages in the battle '${roomid}'` +
    ...
    async groupchatsearch(target, room, user) {
        ...
        lib_1.Utils.sortBy(buffer, roomid => !!Rooms.get(roomid));
        return this.sendReplyBox(`Groupchats with a roomid matching '${target}': ` +
