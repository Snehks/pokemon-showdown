...
exports.ScavMods = exports.ScavengerGameTemplate = void 0;
const scavengers_1 = require("./scavengers");
...
class Leaderboard {
    ...
    visualize(sortBy, userid) {
        ...
        return new Promise((resolve, reject) => {
            ...
            let lastPlacement = 1;
            const ladder = lib_1.Utils.sortBy(Object.entries(this.data).filter(([u, bit]) => sortBy in bit), ([u, bit]) => -bit[sortBy]).map(([u, bit], i) => {
...
const TWISTS = {
    perfectscore: {
        ...
        onComplete(player, time, blitz) {
            const isPerfect = !this.leftGame?.includes(player.id) &&
                Object.values(player.answers).every((attempts) => attempts.length <= 1);
        ...
        onAfterEnd(isReset) {
            ...
            if (perfect.length) {
                this.announce(lib_1.Utils.html `${Chat.toListString(perfect)} ${perfect.length > 1 ? 'have' : 'has'} completed the hunt without a single wrong answer!`);
            }
        },
    },
    bonusround: {
        ...
        onAfterEnd(isReset) {
            ...
            if (noSkip.length) {
                this.announce(lib_1.Utils.html `${Chat.toListString(noSkip)} ${noSkip.length > 1 ? 'have' : 'has'} completed the hunt without skipping the last question!`);
            }
        },
    },
    incognito: {
    ...
    spamfilter: {
        ...
        onConfirmCompletion(player, time, blitz, place, result) {
            ...
            const deductionMessage = player.incorrect?.length ?
                ...
                "Perfect!";
            return `<em>${lib_1.Utils.escapeHTML(player.name)}</em> has finished the hunt! (Final Time: ${time} - ${deductionMessage}${(blitz ? " - BLITZ" : "")})`;
        },
        onEnd() {
            lib_1.Utils.sortBy(this.completed, entry => entry.total);
        },
    },
    blindincognito: {
    ...
    timetrial: {
        ...
        onComplete(player, time, blitz) {
            ...
            this.announce(lib_1.Utils.html `<em>${result.name}</em> is the ${place} player to finish the hunt! (${takenTime}${(blitz ? " - BLITZ" : "")})`);
            lib_1.Utils.sortBy(this.completed, entry => entry.duration);
    ...
    scavengersfeud: {
        ...
        onSubmit(player, jumble, value) {
            ...
            if (currentQuestion + 1 === this.questions.length) {
                this.guesses[player.id] = value.split(',').map((part) => (0, scavengers_1.sanitizeAnswer)(part));
                this.onComplete(player);
                return true;
            }
        },
    ...
    pointless: {
        ...
        onAfterEnd(isReset) {
            ...
            for (const [idx, data] of this.correct.entries()) {
                ...
                const matchDisplay = minEntries.map(entry => this.questions[idx].answer.find(answer => toID(answer) === entry.value)).join(', ');
                const playerDisplay = minEntries.flatMap(entry => entry.players.map(foundPlayer => this.players.find(player => player.id === foundPlayer).name)).join(', ');
                buffer.push(`Q${idx + 1}: ${matchDisplay} - ${playerDisplay}`);
            }
    ...
    minesweeper: {
        ...
        onIncorrectAnswer(player, value) {
            ...
            if (!this.guesses[curr][player.id])
                this.guesses[curr][player.id] = new Set();
            this.guesses[curr][player.id].add((0, scavengers_1.sanitizeAnswer)(value));
        ...
        onShowEndBoard(endedBy) {
            ...
            this.announce(`The ${this.gameType ? `${this.gameType} ` : ""}scavenger hunt by ${hosts} was ended ${(endedBy ? "by " + lib_1.Utils.escapeHTML(endedBy.name) : "automatically")}.<br />` +
                ...
                `${this.questions.map((q, i) => (`${i + 1}) ${this.formatOutput(q.hint)} <span style="color: lightgreen">[<em>${lib_1.Utils.escapeHTML(q.answer.join(' / '))}</em>]</span><br/>` +
                    `<details style="cursor: pointer;"><summary>Mines: </summary>${mines[i].map(({ mine, users }) => lib_1.Utils.escapeHTML(`${mine}: ${users.join(' / ') || '-'}`)).join('<br />')}</details>`)).join("<br />")}` +
                `</details>`);
            return true;
        },
        onEnd(isReset) {
            ...
            for (const [q, guessObj] of this.guesses.entries()) {
                ...
                for (const [playerId, guesses] of Object.entries(guessObj)) {
                    ...
                    if (!player.mines)
                        player.mines = [];
                    player.mines.push(...mines
                        .filter(mine => guesses.has((0, scavengers_1.sanitizeAnswer)(mine)))
        ...
        onAfterEnd(isReset) {
            ...
            if (noMines.length) {
                this.announce(lib_1.Utils.html `${Chat.toListString(noMines)} ${noMines.length > 1 ? 'have' : 'has'} completed the hunt without hitting a single mine!`);
...
const MODES = {
                ...
                for (const [i, time] of game.jumpstart.entries()) {
                    ...
                    this.jumpstartTimers[i] = setTimeout(() => {
                        ...
                        if (targetUser) {
                            ...
                            targetUser.sendTo(this.room, `|raw|<strong>The first hint to the next hunt is:</strong> <div style="overflow:auto; max-height: 50vh"> ${this.formatOutput(this.questions[0].hint)}</div>`);
                            targetUser.sendTo(this.room, `|notify|Early Hint|The first hint to the next hunt is: <div style="overflow:auto; max-height: 50vh">  ${this.formatOutput(this.questions[0].hint)}</div>`);
                        }
            ...
            onCreateCallback() {
                if (this.answerLock) {
                    return `|raw|<div class="broadcast-blue"><strong>${['official', 'unrated'].includes(this.gameType) ? 'An' : 'A'} ${this.gameType} ` +
                        `Scavenger Hunt by <em>${lib_1.Utils.escapeHTML(Chat.toListString(this.hosts.map(h => h.name)))}</em> ` +
                        `has been started${(this.hosts.some(h => h.id === this.staffHostId) ? '' : ` by <em>${lib_1.Utils.escapeHTML(this.staffHostName)}</em>`)}.` +
    ...
    teamscavs: {
        ...
        advanceTeam(answerer, isFinished) {
            const hunt = this.room.getGame(scavengers_1.ScavengerHunt);
        ...
        mod: {
            ...
            onCorrectAnswer(player, value) {
                const game = this.room.scavgame;
                if (player.currentQuestion + 1 < this.questions.length) {
                    game.teamAnnounce(player, lib_1.Utils.html `<strong>${player.name}</strong> has gotten the correct answer (${value}) for question #${player.currentQuestion + 1}.`);
            ...
            onAnySubmit(player, value) {
                ...
                const team = game.getPlayerTeam(player);
                if (!team)
                    return true; // handle players who get kicked during the hunt.
                if (player.currentQuestion !== team.question - 1)
                    player.currentQuestion = team.question - 1;
                if (team.completed)
                    player.completed = true;
                ...
                if (team.answers.includes(value))
                    return;
                game.teamAnnounce(player, lib_1.Utils.html `${player.name} has guessed "${value}".`);
            ...
            onConfirmCompletion(player, time, blitz) {
                ...
                team.completed = true;
                team.question++;
                game.teamAnnounce(player, lib_1.Utils.html `<strong>${player.name}</strong> has gotten the correct answer for question #${player.currentQuestion}.  Your team has completed the hunt!`);
            },
            onEnd() {
                const game = this.room.scavgame;
                for (const teamID in game.teams) {
...
class ScavengerGameTemplate {
    ...
    destroy(force) {
        ...
        const game = this.room.getGame(scavengers_1.ScavengerHunt);
        if (force && game)
            game.onEnd(false);
...
exports.ScavengerGameTemplate = ScavengerGameTemplate;
const LoadGame = function (room, gameid) {
    ...
    const scavgame = Object.assign(base, lib_1.Utils.deepClone(game));
