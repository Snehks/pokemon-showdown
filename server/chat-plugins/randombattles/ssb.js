...
exports.commands = exports.disabledSets = void 0;
const random_teams_1 = require("../../../data/mods/gen9ssb/random-teams");
const _1 = require(".");
class SSBSetHTML extends Chat.JSX.Component {
    render() {
        const { set, dex, baseDex } = this.props;
        if (set.skip) {
            const baseSet = toID(Object.values(random_teams_1.ssbSets[set.skip]).join());
            const skipSet = toID(Object.values(set).join()).slice(0, -toID(set.skip).length);
            if (baseSet === skipSet)
                return null;
        }
        const sigMove = baseDex.moves.get(set.signatureMove);
        return Chat.h("details", null,
            Chat.h("summary", null, "Set"),
            Chat.h("ul", { style: { listStyleType: 'none' } },
                Chat.h("li", null,
                    set.species,
                    set.gender && Chat.h(Chat.Fragment, null,
                        " (",
                        set.gender,
                        ")"),
                    set.item && ' @ ',
                ...
                Chat.h("li", null,
                    ...
                    Array.isArray(set.ability) ?
                        set.ability.map(x => dex.abilities.get(x).name).join(' / ') : dex.abilities.get(set.ability).name),
                set.teraType &&
                    Chat.h("li", null,
                        "Tera Type: ",
                        Array.isArray(set.teraType) ?
                            set.teraType.map(x => dex.types.get(x).name).join(' / ') :
                            set.teraType === 'Any' ? 'Any' : dex.types.get(set.teraType).name),
                set.shiny &&
                    Chat.h("li", null,
                        "Shiny: ",
                        typeof set.shiny === 'number' ? `1 in ${set.shiny} chance` : `Yes`),
                set.evs && Chat.h("li", null,
                    "EVs: ",
                    Object.entries(set.evs).filter(v => !!v[1]).map(([statid, ev], idx, arr) => (Chat.h(Chat.Fragment, null,
                        ev,
                        " ",
                        _1.STAT_NAMES[statid],
                        idx !== arr.length - 1 && ' / ')))),
                set.nature && Chat.h("li", null,
                    Array.isArray(set.nature) ?
                        set.nature.map(_1.formatNature).join(" / ") : (0, _1.formatNature)(set.nature),
                    " Nature"),
                set.ivs && Chat.h("li", null,
                    "IVs: ",
                    Object.entries(set.ivs).filter(v => v[1] !== 31).map(([statid, iv], idx, arr) => (Chat.h(Chat.Fragment, null,
                        iv,
                        " ",
                        _1.STAT_NAMES[statid],
                        idx !== arr.length - 1 && ' / ')))),
                set.moves.map(moveid => (Chat.h("li", null,
                    "- ",
                    Array.isArray(moveid) ? moveid.map(x => dex.moves.get(x).name).join(" / ") : dex.moves.get(moveid).name))),
                Chat.h("li", null,
                    "- ",
                    !sigMove.exists ? Chat.h("i", null, sigMove.name) : sigMove.name)));
    }
}
    ...
    render() {
        ...
        if (sigMove.shortDesc || sigMove.desc) {
            return Chat.h(Chat.Fragment, null,
                Chat.h("hr", null),
                Chat.h("span", { dangerouslySetInnerHTML: { __html: Chat.getDataMoveHTML(sigMove) } }),
                Chat.h("font", { size: "1" }, Object.entries(details).map(([detail, value], idx, arr) => {
                    const lastEntry = idx === arr.length - 1;
                    if (!value)
                        return Chat.h(Chat.Fragment, null,
                            "\u2713 ",
                            detail,
                            !lastEntry && Chat.h(Chat.Fragment, null, "\u00A0|\u205F\u200A"));
                    if (value === 'x')
                        return Chat.h(Chat.Fragment, null,
                            "\u2717 ",
                            detail,
                ...
                (sigMove.desc && sigMove.desc !== sigMove.shortDesc) &&
                    Chat.h("details", null,
                        Chat.h("summary", null,
                            Chat.h("strong", null, "In-Depth Description")),
                        sigMove.desc));
        }
    }
}
    ...
    render() {
        ...
        if (!Array.isArray(set.item)) {
            ...
            if (!baseItem.exists || (baseItem.desc || baseItem.shortDesc) !== (sigItem.desc || sigItem.shortDesc)) {
                return Chat.h(Chat.Fragment, null,
                    Chat.h("hr", null),
                    Chat.h("span", { dangerouslySetInnerHTML: { __html: Chat.getDataItemHTML(sigItem) } }),
                    Chat.h("font", { size: "1" }, Object.entries(details).map(([detail, value], idx, arr) => {
                        const lastEntry = idx === arr.length - 1;
                        if (value === '')
                            return Chat.h(Chat.Fragment, null,
...
class SSBAbilityHTML extends Chat.JSX.Component {
    render() {
        ...
        if (!Array.isArray(set.ability) &&
            ...
            return Chat.h(Chat.Fragment, null,
                Chat.h("hr", null),
                Chat.h("span", { dangerouslySetInnerHTML: { __html: Chat.getDataAbilityHTML(sigAbil) } }),
                Chat.h("font", { size: "1" }, Object.entries(details).map(([detail, value], idx, arr) => {
                    const lastEntry = idx === arr.length - 1;
                    if (value === '')
                        return Chat.h(Chat.Fragment, null,
...
class SSBPokemonHTML extends Chat.JSX.Component {
    render() {
        ...
        if (newSpecies.types.join('/') !== origSpecies.types.join('/') ||
            ...
            return Chat.h(Chat.Fragment, null,
                Chat.h("hr", null),
                Chat.h("span", { dangerouslySetInnerHTML: { __html: Chat.getDataPokemonHTML(newSpecies, dex.gen, 'SSB') } }),
                Chat.h("font", { size: "1" }, Object.entries(details).map(([detail, value], idx, arr) => {
                    const lastEntry = idx === arr.length - 1;
                    if (detail.includes('<font')) {
                        return Chat.h(Chat.Fragment, null,
                            Chat.h("span", { dangerouslySetInnerHTML: { __html: detail } }),
                            !lastEntry && Chat.h(Chat.Fragment, null, "\u00A0|\u205F\u200A"));
                    }
                    return Chat.h(Chat.Fragment, null,
                        Chat.h("font", { color: "#686868" },
                            ...
                            ":"),
                        " ",
                        value.includes('<em>') ? Chat.h("span", { dangerouslySetInnerHTML: { __html: value } }) : value,
                        !lastEntry && Chat.h(Chat.Fragment, null, "\u00A0|\u205F\u200A"));
                })));
        }
    }
    ...
    render() {
        ...
        if (effect.innateName) {
            ...
            return Chat.h(Chat.Fragment, null,
                ...
                "Innate Ability:",
                Chat.h("br", null),
                Chat.h("span", { dangerouslySetInnerHTML: { __html: Chat.getDataAbilityHTML(baseAbility) } }),
                Chat.h("font", { size: "1" },
                    Chat.h("font", { color: "#686868" }, "Gen:"),
                    " 9"),
                longDesc && Chat.h("details", null,
                    Chat.h("summary", null,
                        Chat.h("strong", null, "In-Depth Description")),
                    longDesc));
        }
...
class SSBSetsHTML extends Chat.JSX.Component {
    render() {
        ...
        if (!Object.keys(random_teams_1.ssbSets).map(toID).includes(targetID)) {
            throw new Chat.ErrorMessage(`Error: ${target.trim()} doesn't have a [Gen 9] Super Staff Bros Ultimate set.`);
        }
        let name = '';
        for (const member in random_teams_1.ssbSets) {
            if (toID(member) === targetID)
                name = member;
        }
        const sets = [];
        for (const set in random_teams_1.ssbSets) {
            if (!set.startsWith(name))
                continue;
            if (!random_teams_1.ssbSets[set].skip && set !== name)
                continue;
            sets.push(set);
        }
        return sets.map(setName => {
            const set = random_teams_1.ssbSets[setName];
            ...
            return Chat.h(Chat.Fragment, null,
                !set.skip ? Chat.h("h1", null,
                    Chat.h("psicon", { pokemon: mutatedSpecies.id }),
                    setName) :
                    Chat.h("span", { dangerouslySetInnerHTML: { __html: `<details><summary><psicon pokemon="${set.species}" /><strong>` +
                                `${`${setName.split('-').slice(1).join('-')} forme`}</strong></summary>` } }),
                Chat.h(SSBSetHTML, { set: set, dex: dex, baseDex: baseDex }),
                (!set.skip || set.signatureMove !== random_teams_1.ssbSets[set.skip].signatureMove) && Chat.h(SSBMoveHTML, { sigMove: sigMove, dex: dex }),
                Chat.h(SSBItemHTML, { set: set, dex: dex, baseDex: baseDex }),
                Chat.h(SSBAbilityHTML, { set: set, dex: dex, baseDex: baseDex }),
                Chat.h(SSBInnateHTML, { name: setName, dex: dex, baseDex: baseDex }),
                Chat.h(SSBPokemonHTML, { species: set.species, dex: dex, baseDex: baseDex }),
                (!Array.isArray(set.item) && item.megaStone) && Chat.h(SSBPokemonHTML, { species: Object.values(item.megaStone)[0], dex: dex, baseDex: baseDex }),
                ['Rayquaza'].includes(set.species) && Chat.h(SSBPokemonHTML, { species: `${set.species}-Mega`, dex: dex, baseDex: baseDex }),
                ['Cinderace'].includes(set.species) && Chat.h(SSBPokemonHTML, { species: `${set.species}-Gmax`, dex: dex, baseDex: baseDex }),
                set.skip && Chat.h("span", { dangerouslySetInnerHTML: { __html: `</details>` } }));
        });
    }
}
exports.disabledSets = Chat.oldPlugins.ssb?.disabledSets || [];
function enforceDisabledSets() {
    for (const process of Rooms.PM.processes) {
        process.getProcess().send(`EVAL\n\nConfig.disabledssbsets = ${JSON.stringify(exports.disabledSets)}`);
    }
}
enforceDisabledSets();
exports.commands = {
    ssb(target, room, user) {
        ...
        return this.sendReplyBox(Chat.h(SSBSetsHTML, { target: target }));
    },
    ssbhelp: [
    ...
    disablessbset(target, room, user, connection, cmd) {
        ...
        target = toID(target);
        if (!Object.keys(random_teams_1.ssbSets).map(toID).includes(target)) {
            throw new Chat.ErrorMessage(`${target} has no SSB set.`);
        }
        const disableIdx = exports.disabledSets.indexOf(target);
        if (cmd.startsWith('enable')) {
            if (disableIdx < 0) {
                throw new Chat.ErrorMessage(`${target}'s set is not disabled.`);
            }
            exports.disabledSets.splice(disableIdx, 1);
            ...
            exports.disabledSets.push(target);
