...
exports.commands = exports.STAT_NAMES = void 0;
exports.formatNature = formatNature;
const lib_1 = require("../../../lib");
function getHTMLCriteriaDescription(criteria) {
    const format = (list) => list.map(m => lib_1.Utils.html `<strong>${m.name}</strong>`);
    const parts = [];
    ...
    if (ability.mustHave) {
        parts.push(lib_1.Utils.html `had the ability <strong>${ability.mustHave.name}</strong>`);
    }
    ...
    if (item.mustHave) {
        parts.push(lib_1.Utils.html `had the item <strong>${item.mustHave.name}</strong>`);
    }
    ...
    if (nature.mustHave) {
        parts.push(lib_1.Utils.html `had the nature <strong>${nature.mustHave.name}</strong>`);
    }
    ...
    if (teraType.mustHave) {
        parts.push(lib_1.Utils.html `had the Tera Type <strong>${teraType.mustHave.name}</strong>`);
    }
...
function setProbability(species, format, criteria, rounds = 700) {
    ...
    for (let i = 0; i < rounds; i++) {
        const set = generator.randomSet(species, {}, false, format.gameType !== 'singles', format.ruleTable?.has('dynamaxclause'));
...
function getSets(species, format = 'gen9randombattle') {
    ...
    if (format.team === 'randomFFA')
        folderName += 'ffa';
    const setsFile = JSON.parse((0, lib_1.FS)(`data/random-battles/${folderName}/${isDoubles ? 'doubles-' : ''}sets.json`)
        .readIfExistsSync() || '{}');
...
function getData(species, format) {
    ...
    format = Dex.formats.get(format);
    species = dex.species.get(species);
    const dataFile = JSON.parse((0, lib_1.FS)(`data/random-battles/${format.mod}/data.json`).readIfExistsSync() || '{}');
...
function getLetsGoMoves(species) {
    ...
    if (!data)
        return false;
    const isLetsGoLegal = ((species.num <= 151 || ['Meltan', 'Melmetal'].includes(species.name)) &&
...
function battleFactorySets(species, tier, gen = 'gen9', isBSS = false, is1v1 = false) {
    ...
    if (isNaN(genNum) || genNum < 6 || (isBSS && genNum < 7) || (is1v1 && genNum < 9))
        return null;
    const statsFile = JSON.parse((0, lib_1.FS)(`data/random-battles/gen${genNum}/${isBSS ? `bss-` : is1v1 ? `1v1-` : ``}factory-sets.json`).readIfExistsSync() ||
        "{}");
        ...
        for (const [i, set] of setObj.sets.entries()) {
            ...
            if (set.evs) {
                ...
                for (ev in set.evs) {
                    if (set.evs[ev] === 0)
                        continue;
                    evs.push(`${set.evs[ev]} ${exports.STAT_NAMES[ev]}`);
                }
                buf += `${evs.join(" / ")}</li>`;
            }
            ...
            if (set.ivs) {
                ...
                for (iv in set.ivs) {
                    if (set.ivs[iv] === 31)
                        continue;
                    ivs.push(`${set.ivs[iv]} ${exports.STAT_NAMES[iv]}`);
                }
                buf += `${ivs.join(" / ")}</li>`;
            }
...
function CAP1v1Sets(species) {
    species = Dex.species.get(species);
    const statsFile = JSON.parse((0, lib_1.FS)(`data/random-battles/gen8/cap-1v1-sets.json`).readIfExistsSync() ||
        "{}");
    if (!Object.keys(statsFile).length)
        return null;
    if (species.isNonstandard !== "CAP") {
...
exports.commands = {
        ...
        if (dex.gen === 1) {
            ...
            if ([2, 3, 4, 5, 6, 7, 9].includes(dex.gen)) {
                ...
                for (let pokemon of setsToCheck) {
                    ...
                    movesets.push(`<details class="details">` +
    ...
    randombattlesetprobabilities(target, room, user) {
        ...
        return this.sendReplyBox(lib_1.Utils.html `Generated ${results.rounds} sets for <strong>${species.name}</strong> in ${format.name}:<br />` +
    ...
    randombattlesetprobabilitieshelp() {
        return this.sendReplyBox(`<code>/randombattlesetprobabilities [optional format], [species], [conditions]</code>: Gives the probability of a set matching the conditions appearing for the given species.<br />` +
    ...
    generateteam(target, room, user) {
        ...
        const teamHTML = team
            ...
            return `<details class="details"><summary>${set.name}</summary>${lib_1.Utils.escapeHTML(Teams.exportSet(set))}<br /></details>`;
        })
            .join('');
