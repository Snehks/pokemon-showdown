...
exports.commands = exports.pages = void 0;
...
function visualizeRangeList(ranges) {
    ...
    for (const range of ranges) {
        ...
        html += lib_1.Utils.html `<td>${range.host}</td>`;
        html += `</tr>`;
    }
    return html;
}
...
exports.pages = {
    proxies(query, user) {
        ...
        lib_1.Utils.sortBy(openProxies, ip => {
    ...
    sharedipblacklist(args, user, connection) {
        ...
        if (!Punishments.sharedIpBlacklist.size) {
            ...
            lib_1.Utils.sortBy(sortedSharedIPBlacklist, ([ipOrRange]) => {
    ...
    sharedips(args, user, connection) {
        ...
        if (!Punishments.sharedIps.size) {
            ...
            lib_1.Utils.sortBy(sortedSharedIPs, ([ip]) => {
                const number = IPTools.ipToNumber(ip);
                if (number === null) {
...
exports.commands = {
    ...
    ipranges: {
        ...
        add(target, room, user, connection, cmd) {
            ...
            if (!range)
                throw new Chat.ErrorMessage(`Couldn't parse IP range '${stringRange}'.`);
            ...
            (Rooms.get('upperstaff') || Rooms.get('staff'))?.addByUser(user, `${user.name} added the IP range ${formatRange(range)} to the list of ${type} ranges.`).update();
        ...
        remove(target, room, user) {
            ...
            if (!range)
                throw new Chat.ErrorMessage(`Couldn't parse the IP range '${target}'.`);
            if (!IPTools.getRange(range.minIP, range.maxIP))
                throw new Chat.ErrorMessage(`No IP range found at '${target}'.`);
            void IPTools.removeRange(range.minIP, range.maxIP);
            (Rooms.get('upperstaff') || Rooms.get('staff'))?.addByUser(user, `${user.name} removed the IP range ${formatRange(range)}.`).update();
        ...
        rename(target, room, user) {
            ...
            if (!toRename)
                throw new Chat.ErrorMessage(`Couldn't parse IP range '${rangeString}'.`);
            const exists = IPTools.getRange(toRename.minIP, toRename.maxIP);
            if (!exists)
                throw new Chat.ErrorMessage(`No IP range found at '${rangeString}'.`);
            const range = {
            ...
            (Rooms.get('upperstaff') || Rooms.get('staff'))?.addByUser(user, `${user.name} renamed the IP range ${formatRange(toRename)} to ${range.host}.`).update();
    ...
    addhosts(target, room, user, connection, cmd) {
            ...
            case 'openproxy':
                for (const host of hosts) {
                    if (!IPTools.ipRegex.test(host))
                        throw new Chat.ErrorMessage(`'${host}' is not a valid IP address.`);
            ...
            case 'proxy':
                for (const host of hosts) {
                    if (!IPTools.hostRegex.test(host))
                        throw new Chat.ErrorMessage(`'${host}' is not a valid host.`);
        ...
        this.privateGlobalModAction(`${user.name} ${removing ? 'removed' : 'added'} ${hosts.length} hosts (${hosts.join(', ')}) ${removing ? 'from' : 'to'} the ${type} category`);
        ...
        if (!IPTools.ipRegex.test(ip)) {
            ...
            for (const range of Punishments.sharedRanges.keys()) {
                if (IPTools.rangeIntersects(range, pattern)) {
                    throw new Chat.ErrorMessage(`Range ${IPTools.rangeToString(pattern)} intersects with shared range ${IPTools.rangeToString(range)}`);
                }
            }
        }
        if (Punishments.isSharedIp(ip))
            throw new Chat.ErrorMessage("This IP is already marked as shared.");
    ...
    unmarkshared(target, room, user) {
        ...
        if (!shared)
            throw new Chat.ErrorMessage(`That IP/range isn't marked as shared.`);
    ...
    nomarkshared: {
        add(target, room, user) {
            ...
            if (!IPTools.ipRangeRegex.test(ip))
                throw new Chat.ErrorMessage(`Please enter a valid IP address or range.`);
            if (!reasonArr?.length) {
            ...
            if (!IPTools.ipRegex.test(ip)) {
                ...
                if (!range)
                    throw new Chat.ErrorMessage(`Invalid IP range.`);
