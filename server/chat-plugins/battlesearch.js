...
exports.PM = exports.commands = exports.pages = void 0;
...
async function runBattleSearch(userids, month, tierid, turnLimit) {
    ...
    if (useRipgrep) {
        ...
        let output;
        try {
            output = await lib_1.ProcessManager.exec(['rg', '-i', regexString, '--no-line-number', '-tjson', ...files]);
        }
        catch {
            return results;
        }
        for (const line of output.stdout.split('\n').reverse()) {
            const [file, raw] = lib_1.Utils.splitFirst(line, ':');
            if (!raw || !line)
                continue;
    ...
    for (const file of files) {
        const subFiles = (0, lib_1.FS)(`${file}`).readdirSync();
        const day = file.split('/')[3];
        for (const dayFile of subFiles) {
            const json = (0, lib_1.FS)(`${file}/${dayFile}`).readIfExistsSync();
...
async function rustBattleSearch(context, targetUser, daysString, format) {
    ...
    try {
        await lib_1.ProcessManager.exec(`psbattletools --version`, {
    ...
    for (let daysAgo = 0; daysAgo < days; daysAgo++) {
        ...
        directories.push(Monitor.logPath(path.join(`${year}-${month}`, format, `${year}-${month}-${day}`)).path);
    }
    // TODO: implement flag?
...
exports.pages = {
    async battlesearch(args, user, connection) {
        ...
        const [ids, rawLimit, month, formatid, confirmation] = lib_1.Utils.splitFirst(this.pageid.slice(18), '--', 5);
        ...
        buf += `</p>`;
        const months = lib_1.Utils.sortBy((await Monitor.logPath('/').readdir()).filter(f => f.length === 7 && f.includes('-')), name => ({ reverse: name }));
        if (!month) {
        ...
        const tiers = lib_1.Utils.sortBy(await Monitor.logPath(`${month}/`).readdir(), tier => [
        ...
        return (`<div class="pad ladder"><h2>Battle Search</h2><p>` +
...
exports.commands = {
    ...
    battlesearchhelp() {
        ...
        this.runBroadcast();
        return this.sendReply('/battlesearch [args] - Searches rated battle history for the provided [args] and returns information on battles between the userids given.\n' +
 ...
 * Process manager
 *********************************************************/
exports.PM = new lib_1.ProcessManager.QueryProcessManager('battlesearch', module, async (data) => {
...
function start(processCount) {
    exports.PM.spawn(processCount['battlesearch'] ?? 1);
}
function destroy() {
