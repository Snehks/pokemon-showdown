...
exports.commands = exports.suspectTests = void 0;
exports.saveSuspectTests = saveSuspectTests;
const lib_1 = require("../../lib");
const fs_1 = require("../../lib/fs");
const SUSPECTS_FILE = 'config/suspects.json';
exports.suspectTests = JSON.parse((0, fs_1.FS)(SUSPECTS_FILE).readIfExistsSync() || "{}");
function saveSuspectTests() {
    (0, fs_1.FS)(SUSPECTS_FILE).writeUpdate(() => JSON.stringify(exports.suspectTests));
}
const defaults = {
    whitelist: [],
    suspects: {},
};
if (!exports.suspectTests.whitelist && !exports.suspectTests.suspects) {
    const suspects = { ...exports.suspectTests };
    exports.suspectTests = { ...defaults, suspects };
    saveSuspectTests();
}
function checkPermissions(context) {
    const user = context.user;
    if (exports.suspectTests.whitelist?.includes(user.id))
        return true;
...
exports.commands = {
    ...
    suspects: {
        ''(target, room, user) {
            const suspects = exports.suspectTests.suspects;
            ...
            for (const test of Object.values(suspects)) {
                buffer += '<br />';
                buffer += `${lib_1.Utils.escapeHTML(test.tier)}: <a href="${test.url}">${lib_1.Utils.escapeHTML(test.suspect)}</a> (${test.date})`;
            }
        ...
        async add(target, room, user) {
            ...
            const [out, error] = await LoginServer.request(exports.suspectTests.suspects[format.id] ? "suspects/edit" : "suspects/add", {
                format: format.id,
            ...
            const prevSuspect = exports.suspectTests.suspects[format.id];
            ...
            exports.suspectTests.suspects[format.id] = {
                tier: format.name,
        ...
        async remove(target, room, user) {
            ...
            const test = exports.suspectTests.suspects[format];
            if (!test)
            ...
            delete exports.suspectTests.suspects[format];
            saveSuspectTests();
        ...
        whitelist(target, room, user) {
            ...
            if (!userid || userid.length > 18) {
                if (exports.suspectTests.whitelist.length) {
                    this.sendReplyBox(`Current users with /suspects access: <username>${exports.suspectTests.whitelist.join('</username>, <username>')}</username>`);
                }
                return this.parse(`/help suspects`);
            }
            if (exports.suspectTests.whitelist.includes(userid)) {
            ...
            exports.suspectTests.whitelist.push(userid);
            saveSuspectTests();
        },
        unwhitelist(target, room, user) {
            ...
            const index = exports.suspectTests.whitelist.indexOf(userid);
            if (index < 0) {
            ...
            exports.suspectTests.whitelist.splice(index, 1);
            saveSuspectTests();
        },
        async verify(target, room, user) {
            const formatid = toID(target);
            if (!exports.suspectTests.suspects[formatid]) {
        ...
        async setbvalue(target, room, user, connection, cmd) {
            ...
            this.addGlobalModAction(`${user.name} ${bVal ? `set B value for ${format.name} to ${bVal}` : `removed B value for ${format.name}`}.`);
            if (source) {
    ...
    suspectshelp() {
        this.sendReplyBox(`Commands to manage suspect tests:<br />` +
