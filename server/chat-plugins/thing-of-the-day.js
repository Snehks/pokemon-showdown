...
exports.destroy = exports.roomSettings = exports.punishmentfilter = exports.handlers = exports.commands = exports.pages = exports.otdCommands = exports.otds = exports.otdData = exports.prenoms = void 0;
...
exports.prenoms = JSON.parse((0, lib_1.FS)(PRENOMS_FILE).readIfExistsSync() || "{}");
exports.otdData = JSON.parse((0, lib_1.FS)(DATA_FILE).readIfExistsSync() || "{}");
exports.otds = new Map();
const FINISH_HANDLERS = {
    cotw: async (winner) => {
        const { channel, nominator } = winner;
        const searchResults = await youtube_1.YouTube.searchChannel(channel, 1);
        const result = searchResults?.[0];
        if (result) {
            if (youtube_1.YouTube.data.channels[result])
                return;
            void youtube_1.YouTube.getChannelData(`https://www.youtube.com/channel/${result}`);
            const yt = Rooms.search('youtube');
            if (!yt)
                return;
            yt.sendMods(`|c|~|/log The channel with ID ${result} was added to the YouTube channel database.`);
            yt.modlog({
                action: `ADDCHANNEL`,
...
function savePrenoms() {
    return (0, lib_1.FS)(PRENOMS_FILE).writeUpdate(() => JSON.stringify(exports.prenoms));
}
...
class OtdHandler {
    constructor(id, room, settings) {
        ...
        this.lastPrenom = 0;
        this.winners = exports.otdData[this.id]?.winners || [];
    }
    ...
    addNomination(user, nomination) {
        ...
        for (const value of this.removedNominations.values()) {
            if (value.userids.includes(toID(user)) || (!Config.noipchecks && value.ips.includes(user.latestIp))) {
                return user.sendTo(this.room, `Since your nomination has been removed by staff, you cannot submit another ${this.name.toLowerCase()} until the next round.`);
            }
        }
    ...
    generateNomWindow() {
        ...
        const entries = [];
        for (const value of this.nominations.values()) {
            entries.push(lib_1.Utils.html `<li><b>${value.nomination}</b> <i>(Submitted by ${value.name})</i></li>`);
        }
    ...
    rollWinner() {
        ...
        if (!winner)
            return false; // Should never happen but shuts typescript up.
        ...
        this.room.add(lib_1.Utils.html `|uhtml|otd|<div class="broadcast-blue"><p style="font-weight:bold;text-align:center;font-size:12pt;">` +
    ...
    save(destroy = false) {
        ...
        (0, lib_1.FS)(DATA_FILE).writeUpdate(() => JSON.stringify(exports.otdData));
    }
    destroy() {
        this.room = null;
    ...
    async generateWinnerDisplay() {
        ...
        let output = `<div class="broadcast-blue" style="text-align:center;">` +
            lib_1.Utils.html `<p><span style="font-weight:bold;font-size:11pt">The ${this.name} of the ${this.timeLabel} is ` +
            lib_1.Utils.html `${winner[this.keys[0]]}${winner.author ? ` by ${winner.author}` : ''}.</span>`;
        if (winner.quote)
            output += lib_1.Utils.html `<br /><span style="font-style:italic;">"${winner.quote}"</span>`;
        if (winner.tagline)
            output += lib_1.Utils.html `<br />${winner.tagline}`;
        output += `</p><table style="margin:auto;"><tr>`;
        if (winner.image) {
            try {
                const [width, height] = await Chat.fitImage(winner.image, 100, 100);
                output += lib_1.Utils.html `<td><img src="${winner.image}" width=${width} height=${height}></td>`;
            }
            catch { }
        }
        output += `<td style="text-align:right;margin:5px;">`;
        if (winner.event)
            output += lib_1.Utils.html `<b>Event:</b> ${winner.event}<br />`;
        if (winner.song) {
            output += `<b>Song:</b> `;
            if (winner.link) {
                output += lib_1.Utils.html `<a href="${winner.link}">${winner.song}</a>`;
            }
            else {
                output += lib_1.Utils.escapeHTML(winner.song);
            }
            output += `<br />`;
        }
        else if (winner.link) {
            output += lib_1.Utils.html `<b>Link:</b> <a href="${winner.link}">${winner.link}</a><br />`;
        }
        // Batch these together on 2 lines. Order intentional.
        const athleteDetails = [];
        if (winner.sport)
            athleteDetails.push(lib_1.Utils.html `<b>Sport:</b> ${winner.sport}`);
        if (winner.team)
            athleteDetails.push(lib_1.Utils.html `<b>Team:</b> ${winner.team}`);
        if (winner.age)
            athleteDetails.push(lib_1.Utils.html `<b>Age:</b> ${winner.age}`);
        if (winner.country)
            athleteDetails.push(lib_1.Utils.html `<b>Nationality:</b> ${winner.country}`);
        ...
        output += lib_1.Utils.html `Nominated by ${winner.nominator}.`;
            ...
            const entry = columns.map(col => {
                ...
                switch (col) {
                    case 'time':
                        ...
                        return lib_1.Utils.html `${pad(date.getMonth() + 1)}-${pad(date.getDate())}-${date.getFullYear()}`;
                    case 'song':
                    ...
                    // falls through
                    case columns[0]:
                        return `${lib_1.Utils.escapeHTML(val)}${this.winners[i].nominator ? lib_1.Utils.html `<br /><span style="font-style:italic;font-size:8pt;">nominated by ${this.winners[i].nominator}</span>` : ''}`;
                    default:
                        return lib_1.Utils.escapeHTML(val);
                }
            });
...
exports.commands = {
    otd: {
        ...
        updateroom(target, room, user) {
            ...
            otd.room = targetRoom;
            otd.save();
            this.privateGlobalModAction(`${user.name} updated the room for the ${otd.name} of the ${otd.timeLabel} from ${oldRoom} to ${targetRoom}`);
...
const getKeyByRoomId = (id) => {
    for (const [key, handler] of exports.otds.entries()) {
...
const punishmentfilter = (user, punishment) => {
    ...
    if (!['NAMELOCK', 'BAN'].includes(punishment.type))
        return;
    for (const handler of exports.otds.values()) {
