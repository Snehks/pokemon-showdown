...
exports.pages = exports.commands = exports.Recs = exports.LastFM = exports.LastFMInterface = void 0;
...
const lastfm = JSON.parse((0, lib_1.FS)(LASTFM_DB).readIfExistsSync() || "{}");
const recommendations = JSON.parse((0, lib_1.FS)(RECOMMENDATIONS).readIfExistsSync() || "{}");
...
updateRecTags();
function saveLastFM() {
    (0, lib_1.FS)(LASTFM_DB).writeUpdate(() => JSON.stringify(lastfm));
}
function saveRecommendations() {
    (0, lib_1.FS)(RECOMMENDATIONS).writeUpdate(() => JSON.stringify(recommendations));
}
class LastFMInterface {
    async getScrobbleData(username, displayName) {
        ...
        let raw;
        try {
            raw = await (0, lib_1.Net)(API_ROOT).get({
                query: {
        ...
        if (track.image?.length) {
            ...
            buf += `<td><strong><a href="https://www.last.fm/user/${accountName}">${lib_1.Utils.escapeHTML(displayName || accountName)}</a></strong>`;
            ...
            let videoIDs;
            try {
                videoIDs = await youtube_1.YouTube.searchVideo(trackName, 1);
            }
            catch (e) {
            ...
            if (recommendations.youtubeSearchDisabled) {
                buf += lib_1.Utils.escapeHTML(trackName);
            }
            else {
                buf += `<a href="https://youtu.be/${videoIDs[0]}">${lib_1.Utils.escapeHTML(trackName)}</a>`;
            }
    ...
    async tryGetTrackData(track, artist) {
        ...
        let raw;
        try {
            raw = await (0, lib_1.Net)(API_ROOT).get({ query });
        }
        catch {
        ...
        if (req.results?.trackmatches?.track?.length) {
            ...
            let videoIDs;
            try {
                videoIDs = await youtube_1.YouTube.searchVideo(searchName, 1);
            }
            catch (e) {
                if (!recommendations.youtubeSearchDisabled) {
                    throw new Chat.ErrorMessage(`Error while fetching video data: ${e.message}`);
...
class RecommendationsInterface {
    ...
    async add(artist, title, url, description, username, tags, avatar) {
        ...
        if (!/^https?:\/\//.test(url))
            url = `https://${url}`;
        if (!youtube_1.YouTube.linkRegex.test(url)) {
            throw new Chat.ErrorMessage(`Please provide a valid YouTube link.`);
        }
        url = url.split('~')[0];
        const videoInfo = await youtube_1.YouTube.getVideoData(url);
        this.checkTags(tags);
        // JUST in case
        if (!recommendations.saved)
            recommendations.saved = [];
    ...
    async render(rec, suggested = false) {
        ...
        if (!rec.videoInfo) {
            ...
            rec.videoInfo = await youtube_1.YouTube.getVideoData(youtube_1.YouTube.getId(rec.url));
            saveRecommendations();
        }
        if (rec.videoInfo) {
        ...
        buf += lib_1.Utils.html `<td style="max-width:300px"><a href="${rec.url}" style="color:#000;font-weight:bold;">${rec.artist} - ${rec.title}</a>`;
        const tags = rec.tags.map(x => lib_1.Utils.escapeHTML(x))
            .filter(x => toID(x) !== toID(rec.userData.name) && toID(x) !== toID(rec.artist));
        if (tags.length) {
            buf += `<br /><strong>Tags:</strong> <em>${tags.join(', ')}</em>`;
        }
        if (rec.description) {
            buf += `<br /><span style="display:inline-block;line-height:1.15em;"><strong>Description:</strong> ${lib_1.Utils.escapeHTML(rec.description)}</span>`;
        }
        ...
        buf += `<hr />`;
        if (suggested) {
            buf += lib_1.Utils.html `<button class="button" name="send" value="/msgroom thestudio,/approvesuggestion ${rec.userData.name}|${rec.artist}|${rec.title}">Approve</button> | `;
            buf += lib_1.Utils.html `<button class="button" name="send" value="/msgroom thestudio,/denysuggestion ${rec.userData.name}|${rec.artist}|${rec.title}">Deny</button>`;
        }
        else {
            buf += lib_1.Utils.html `<button class="button" name="send" value="/msgroom thestudio,/likerec ${rec.artist}|${rec.title}" style="float:right;display:inline;padding:3px 5px;font-size:8pt;">`;
    ...
    getIndex(artist, title, submitter = null, fromSuggestions = false) {
        ...
        if (fromSuggestions)
            recs = recommendations.suggested;
        return recs.findIndex(x => (toID(x.artist) === toID(artist) &&
            toID(x.title) === toID(title) &&
            (!submitter || toID(x.userData.name) === toID(submitter))));
    }
    checkTags(tags) {
...
exports.LastFM = new LastFMInterface();
exports.Recs = new RecommendationsInterface();
exports.commands = {
    ...
    async lastfm(target, room, user) {
        ...
        const username = exports.LastFM.getAccountName(targetUsername);
        this.sendReplyBox(await exports.LastFM.getScrobbleData(username, targetUsername));
    },
    lastfmhelp: [
    ...
    async recommendation(target, room, user) {
        ...
        const sample = lib_1.Utils.shuffle(matches)[0];
