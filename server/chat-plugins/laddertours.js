...
exports.handlers = exports.pages = exports.commands = exports.trackers = exports.LadderTracker = void 0;
/**
 * Many parts borrowed from POSHO-9000.
 * Refactored at length by @author mia-pi-git
 */
const lib_1 = require("../../lib");
const username_prefixes_1 = require("./username-prefixes");
const MINUTE = 60000;
...
class LadderTracker {
    ...
    async getLeaderboard(display) {
        ...
        let response;
        try {
            response = await (0, lib_1.Net)(url).get().then(JSON.parse);
        }
        catch (e) {
    ...
    static save() {
        (0, lib_1.FS)('config/chat-plugins/laddertrackers.json').writeUpdate(() => {
            const out = {};
            for (const roomid in exports.trackers) {
                exports.trackers[roomid].restoreConfig();
                out[roomid] = exports.trackers[roomid].config;
            }
        ...
        for (const [i, p] of leaderboard.entries()) {
            ...
            buf +=
                `<tr><td style="text-align: right"><a href='${link}' class="subtle">${rank}</a></td>` +
                    lib_1.Utils.html `<td><username class="username">${p.name}</username></td>` +
    ...
    togglePrefix(oldPrefix) {
        if (!this.room.settings.isPrivate) {
            try {
                if (oldPrefix)
                    username_prefixes_1.prefixManager.removePrefix(oldPrefix, 'privacy');
            }
            catch { } // suppress errorMessages in case it's the first start and it hasn't been made priv yet
            try {
                username_prefixes_1.prefixManager.addPrefix(this.prefix, 'privacy');
            }
            catch { } // same as above
        }
    }
...
exports.LadderTracker = LadderTracker;
exports.trackers = {};
try {
    const data = JSON.parse((0, lib_1.FS)("config/chat-plugins/laddertrackers.json").readIfExistsSync() || "{}");
    for (const roomid in data) {
        ...
        exports.trackers[roomid] = new LadderTracker(room, data[roomid]);
        exports.trackers[roomid].start();
    }
}
catch { }
exports.commands = {
    ...
    laddertrack: {
        ...
        ''(target, room, user, connection, cmd) {
            ...
            if (exports.trackers[room.roomid] && !cmd.includes('override')) {
                return this.errorReply(`There is already a tracker for this room. Use /laddertrack override if you want to set a new one.`);
            }
            const config = {};
            /* cutoff?: number; */
            ...
            exports.trackers[room.roomid] = new LadderTracker(room, config);
            exports.trackers[room.roomid].start();
            LadderTracker.save();
        ...
        endtrack(target, room, user, nike, cmd) {
            ...
            this.checkCan('mute', null, tracker.room);
            tracker.stop();
            delete exports.trackers[tracker.room.roomid];
            LadderTracker.save();
...
exports.handlers = {
    onBattleCreate(battle, players) {
        for (const tracker of Object.values(exports.trackers)) {
