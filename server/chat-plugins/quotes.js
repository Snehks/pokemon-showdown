...
exports.handlers = exports.pages = exports.commands = void 0;
const lib_1 = require("../../lib");
const STORAGE_PATH = 'config/chat-plugins/quotes.json';
const MAX_QUOTES = 300;
const quotes = JSON.parse((0, lib_1.FS)(STORAGE_PATH).readIfExistsSync() || "{}");
...
function saveQuotes() {
    (0, lib_1.FS)(STORAGE_PATH).writeUpdate(() => JSON.stringify(quotes));
}
convertOldQuotes();
exports.commands = {
    ...
    viewquote(target, room, user) {
        ...
        if (!roomQuotes?.length)
            throw new Chat.ErrorMessage(`This room has no quotes.`);
        const [num, showAuthor] = lib_1.Utils.splitFirst(target, ',');
...
exports.pages = {
    quotes(args, user) {
        ...
        buffer += lib_1.Utils.html `<h2>Quotes for ${room.title} (${roomQuotes.length}):</h2>`;
