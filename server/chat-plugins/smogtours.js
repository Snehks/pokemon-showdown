...
exports.pages = exports.commands = exports.tours = void 0;
exports.renderPageChooser = renderPageChooser;
exports.start = start;
/**
...
try {
    const data = JSON.parse((0, lib_1.FS)('config/chat-plugins/smogtours.json').readSync());
    // settings should prioritize hardcoded values for these keys
    const PRIO = ['title', 'icon'];
    for (const key in data) {
        const section = (exports.tours[key] || (exports.tours[key] = data[key]));
...
function saveTours() {
    (0, lib_1.FS)('config/chat-plugins/smogtours.json').writeUpdate(() => JSON.stringify(exports.tours));
}
function getTour(categoryID, id) {
    id = toID(id);
    if (!exports.tours[categoryID])
        return null;
    const idx = exports.tours[categoryID].tours.findIndex(f => f.id === id) ?? -1;
    const tour = exports.tours[categoryID].tours[idx];
    if (!tour) {
        return null;
    }
    if (tour.ends && Date.now() > tour.ends) {
        exports.tours[categoryID].tours.splice(idx, 1);
        return null;
    }
    return tour;
}
function checkWhitelisted(category, user) {
    return category ?
        exports.tours[category].whitelist?.includes(user.id) :
        Object.values(exports.tours).some(f => f.whitelist?.includes(user.id));
}
function checkCanEdit(user, context, category) {
...
exports.commands = {
    smogtours: {
        ...
        async add(target, room, user, connection, cmd) {
            ...
            const [title, rawSection, url, rawEnds, rawImg, rawCredit, rawArtistName, rawShort, rawDesc,] = lib_1.Utils.splitFirst(targets.join('|'), '|', 8).map(f => f.trim());
            const sectionID = toID(rawSection);
            if (!toID(title)) {
                return this.popupReply(`Invalid title. Must have at least one alphanumeric character.`);
            }
            const section = exports.tours[sectionID];
            if (!section) {
            ...
            const tour = {
                title: lib_1.Utils.escapeHTML(title),
                url,
                image,
                artistCredit,
        ...
        whitelist(target, room, user) {
            ...
            const section = exports.tours[sectionID];
            if (!section) {
                throw new Chat.ErrorMessage(`Invalid section ID: "${sectionID}". Valid IDs: ${Object.keys(exports.tours).join(', ')}`);
            }
            ...
            this.privateGlobalModAction(`${user.name} whitelisted ${targetID} to manage tours for the ${section.title} section`);
        ...
        unwhitelist(target, room, user) {
            ...
            this.privateGlobalModAction(`${user.name} removed ${targetID} from the tour management whitelist for the ${section.title} section`);
...
function renderTab(inner, isLink, isCur) {
    let buf = '';
    if (isLink && isCur) {
...
const refresh = (pageid) => (`<button class="button" name="send" value="/join ${pageid}" style="float: right">` +
    `<i class="fa fa-refresh"></i> Refresh</button>`);
const back = (section) => (`<a class="button" target="replace" href="/view-tournaments-${section ? `section-${section}` : 'all'}" style="float: left">` +
...
function renderPageChooser(curPage, buffer, user) {
    ...
    buf += renderTab(`<strong>Home</strong>`, `view-tournaments-all`, curPage === '');
    const keys = Object.keys(exports.tours);
    buf += keys.map(cat => {
        let innerBuf = '';
        const tourData = exports.tours[cat];
        innerBuf += renderTab(`<strong>${tourData.title}</strong>`, `view-tournaments-section-${cat}`, curPage === cat);
        buf += `<div class="foldersep"></div>`;
        if (tourData.tours.length) {
            lib_1.Utils.sortBy(tourData.tours, t => -t.date);
            innerBuf += tourData.tours.map(t => (renderTab(`<i class="fa fa-trophy"></i> ${t.title}`, `view-tournaments-view-${cat}-${t.id}`, curPage === `${cat}-${t.id}`))).join('');
        }
        else {
    ...
    if (user && (checkWhitelisted('', user) || user?.can('rangeban'))) {
        buf += `<div class="foldersep"></div>`;
        buf += renderTab(`<strong>Manage</strong>`, `view-tournaments-manage`, curPage === 'manage');
        buf += renderTab(`<i class="fa fa-pencil"></i>Start new`, `view-tournaments-start`, curPage === 'start');
        buf += renderTab(`<i class="fa fa-pencil"></i>Edit existing`, `view-tournaments-edit`, curPage === 'edit');
        if (user.can('rangeban')) {
            buf += renderTab(`<i class="fa fa-pencil"></i>Whitelist`, `view-tournaments-whitelists`, curPage === 'whitelist');
        }
    }
...
function instantTournaments() {
    ...
    const started = lib_1.Utils.sortBy(roomTours.filter(tour => tour.isTournamentStarted), tour => tour.room.roomid);
    const signups = lib_1.Utils.sortBy(roomTours.filter(tour => !tour.isTournamentStarted), tour => tour.room.roomid);
    function renderLink(tour) {
        ...
        return `<li><a href="/${tour.room.roomid}" class="blocklink">&laquo;<strong>${tour.room.roomid}</strong>&raquo;<small style="float:right">(${tour.players.length} player${plural})</small><br />${icon} <small>${lib_1.Utils.escapeHTML(name)} ${tour.generator.name}</small></a></li>`;
    }
    let buf = ``;
    if (signups.length) {
...
exports.pages = {
    tournaments: {
        all(query, user) {
            ...
            const icon = exports.tours.official.icon;
            if (icon)
            ...
            buf += Object.keys(exports.tours).map(catID => (`<li><a class="blocklink" target="replace" href="/view-tournaments-section-${catID}">` +
                `<i class="fa fa-play"></i> <strong>${exports.tours[catID].title}</strong></a></li>`)).join(' ');
            buf += `</ul>`;
            buf += `<hr />`;
        ...
        view(query, user) {
            ...
            buf += `<hr />`;
            buf += lib_1.Utils.escapeHTML(tour.desc).replace(/\n/ig, '<br />');
        ...
        section(query, user) {
            ...
            this.title = '[Tournaments] ' + categoryID;
            const category = exports.tours[categoryID];
            if (!category) {
                return error('', lib_1.Utils.html `Invalid section specified: '${categoryID}'`, user);
            }
            ...
            if (!category.tours.length) {
                ...
                buf += category.tours.map(tour => {
                    ...
                    innerBuf += lib_1.Utils.escapeHTML(tour.shortDesc);
                    innerBuf += `</div>`;
                    return innerBuf;
                }).join('<br />');
            }
        ...
        start(query, user) {
            ...
            let possibleCategory = Object.keys(exports.tours)[0];
            for (const k in exports.tours) {
                if (exports.tours[k].whitelist?.includes(user.id)) {
            ...
            const keys = lib_1.Utils.sortBy(Object.keys(exports.tours), k => [k === possibleCategory, k]).filter(cat => (checkWhitelisted(toID(cat), user) || user.can('rangeban')));
        ...
        edit(query, user) {
            ...
            const desc = lib_1.Utils.escapeHTML(tour.desc).replace(/<br \/>/g, '&#10;');
        ...
        manage(query, user) {
            ...
            buf += Object.keys(exports.tours).map(cat => {
                let innerBuf = '';
                try {
                ...
                const section = exports.tours[cat];
                ...
                innerBuf += section.tours.map(t => `&bull; <a href="/view-tournaments-edit-${cat}-${t.id}">${t.title}</a>`).join('<br />') || "None active.";
                return innerBuf;
        ...
        whitelists(query, user) {
            ...
            for (const k in exports.tours) {
                buf += `<strong>${exports.tours[k].title}</strong><br />`;
                const whitelist = exports.tours[k].whitelist || [];
                if (!whitelist.length) {
                    buf += `None.<br />`;
                    continue;
                }
                buf += lib_1.Utils.sortBy(whitelist).map(f => `<li>${f}</li>`).join('');
                buf += `<br />`;
            }
