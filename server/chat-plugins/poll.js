...
exports.pages = exports.commands = exports.Poll = void 0;
exports.start = start;
/*
 * Poll chat plugin
...
class Poll extends Rooms.MinorActivity {
    ...
    generateVotes(user) {
        ...
        if (this.multiPoll) {
            ...
            for (const [num, answer] of this.answers) {
                ...
                output += `<div style="margin-top: 5px"><button style="text-align: left; border: none; background: none; color: inherit;" value="/poll ${selected ? 'de' : ''}select ${num}" name="send" title="${selected ? "Deselect" : "Select"} ${num}. ${lib_1.Utils.escapeHTML(answer.name)}">${selected ? "<strong>" : ''}${selected ? chosen : empty} ${num}. `;
            ...
            for (const [num, answer] of this.answers) {
                output += `<div style="margin-top: 5px"><button class="button" style="text-align: left" value="/poll vote ${num}" name="send" title="${this.room.tr `Vote for ${num}`}. ${lib_1.Utils.escapeHTML(answer.name)}">${num}.`;
    ...
    update() {
        ...
        for (const id in this.room.users) {
            ...
            if (selection) {
                if (selection.length) {
                    user.sendTo(this.room, `|uhtmlchange|poll${this.activityNumber}|${Poll.generateResults(state, this.room, false, selection)}`);
                }
                else {
    ...
    updateTo(user, connection = null) {
        ...
        const selection = this.voters[user.id] || this.voterIps[user.latestIp];
        if (selection) {
            recipient.sendTo(this.room, `|uhtmlchange|poll${this.activityNumber}|${Poll.generateResults(state, this.room, false, selection)}`);
        }
        else {
    ...
    updateFor(user) {
        ...
        if (user.id in this.voters) {
            user.sendTo(this.room, `|uhtmlchange|poll${this.activityNumber}|${Poll.generateResults(state, this.room, false, this.voters[user.id])}`);
        }
        else {
    ...
    displayTo(user, connection = null) {
        ...
        const recipient = connection || user;
        if (user.id in this.voters) {
            recipient.sendTo(this.room, `|uhtml|poll${this.activityNumber}|${Poll.generateResults(state, this.room, false, this.voters[user.id])}`);
        }
        else if (user.latestIp in this.voterIps && !Config.noipchecks) {
            recipient.sendTo(this.room, `|uhtml|poll${this.activityNumber}|${Poll.generateResults(state, this.room, false, this.voterIps[user.latestIp])}`);
        }
        else {
...
exports.commands = {
    ...
    pollhelp() {
        this.sendReply(`|html|<details class="readmore"><summary>/poll allows rooms to run their own polls (limit 1 at a time).<br />` +
...
exports.pages = {
    pollqueue(args, user) {
        ...
        for (const [i, poll] of queue.entries()) {
            ...
            const button = (`<strong>${this.tr `#${number} in queue`} </strong>` +
