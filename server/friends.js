...
exports.PM = exports.FriendsDatabase = exports.FailureMessage = exports.DEFAULT_FILE = exports.MAX_REQUESTS = exports.MAX_FRIENDS = void 0;
...
/** Max friend requests. */
exports.MAX_REQUESTS = 6;
exports.DEFAULT_FILE = (0, lib_1.FS)('databases/friends.db').path;
...
class FriendsDatabase {
    ...
    static setupDatabase(fileName) {
        ...
        const exists = (0, lib_1.FS)(file).existsSync() || file === ':memory:';
        const database = new (require('better-sqlite3'))(file);
        if (!exists) {
            database.exec((0, lib_1.FS)('databases/schemas/friends.sql').readSync());
        }
        else {
            let val;
            try {
            ...
            const actualVersion = (0, lib_1.FS)(`databases/migrations/friends`).readdirIfExistsSync().length;
            if (val === undefined) {
        ...
        database.exec((0, lib_1.FS)(`databases/schemas/friends-startup.sql`).readSync());
    ...
    async request(user, receiverID) {
        ...
        let buf = lib_1.Utils.html `/uhtml sent-${user.id},<button class="button" name="send" value="/friends accept ${user.id}">Accept</button> | `;
        buf += lib_1.Utils.html `<button class="button" name="send" value="/friends reject ${user.id}">Deny</button><br /> `;
        buf += `<small>(You can also stop this user from sending you friend requests with <code>/ignore</code>)</small>`;
        const disclaimer = (`/raw <small>Note: If this request is accepted, your friend will be notified when you come online, ` +
        ...
        sendPM(`/nonotify You sent a friend request to ${receiver?.connected ? receiver.name : receiverID}!`, user.name);
        sendPM(`/uhtml undo-${receiverID},<button class="button" name="send" value="/friends undorequest ${lib_1.Utils.escapeHTML(receiverID)}">` +
...
const transactions = {};
const ACTIONS = {
    add: (`REPLACE INTO friends (user1, user2) VALUES ($user1, $user2) ON CONFLICT (user1, user2) ` +
    ...
    findRequest: (`SELECT count(*) as num FROM friend_requests WHERE ` +
    ...
    login: (`INSERT INTO friend_settings (userid, send_login_data, last_login, public_list) VALUES (?, 0, ?, 0) ` +
    ...
    expire: (`DELETE FROM friend_requests WHERE EXISTS` +
    ...
    toggleList: (`INSERT INTO friend_settings (userid, send_login_data, last_login, public_list) VALUES (?, 0, 0, ?) ` +
...
const TRANSACTIONS = {
    send: requests => {
        for (const request of requests) {
            ...
            if (totalRequests >= exports.MAX_REQUESTS) {
                throw new FailureMessage(`You already have ${exports.MAX_REQUESTS} pending friend requests. Use "/friends view sent" to see your outgoing requests and "/friends view receive" to see your incoming requests.`);
            }
    ...
    accept: requests => {
        for (const request of requests) {
            ...
            if (friends?.length >= exports.MAX_FRIENDS) {
                throw new FailureMessage(`You are at the maximum number of friends.`);
            }
...
exports.PM = new lib_1.ProcessManager.QueryProcessManager('friends', module, query => {
...
function start(processCount) {
    exports.PM.spawn(processCount['friends'] ?? 1);
}
function destroy() {
