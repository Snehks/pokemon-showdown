...
class Pokemon {
    constructor(set, side) {
        ...
        this.getHealth = () => {
            ...
            if (!this.hp)
                return { side: this.side.id, secret: '0 fnt', shared: '0 fnt', pboTag };
        ...
        // [PBO] PBO NPC pokemon can have extreme EVs for raid bosses (e.g. 12M HP EV for Dynamax)
    ...
    */
    ...
    getSwitchRequestData(forAlly) {
        ...
        if (this.databaseId)
            entry.databaseId = this.databaseId;
        return entry;
    }
    isLastActive() {
        if (!this.isActive)
