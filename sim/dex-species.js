...
const EMPTY_SPECIES = utils_1.Utils.deepFreeze(new Species({
...
class Learnset {
    constructor(data, species) {
        ...
        const eventData = utils_1.Utils.deepClone(this.eventData);
        let update = false;
        if (eventData) {
        ...
        if (update)
            this.eventData = utils_1.Utils.deepFreeze(eventData);
    }
}
    ...
    getByID(id) {
            ...
            if (this.dex.data.FormatsData.hasOwnProperty(id)) {
                ...
                if (species.cosmeticFormes) {
                    for (const forme of species.cosmeticFormes) {
                        if (this.dex.data.Pokedex.hasOwnProperty((0, dex_data_1.toID)(forme)))
                            continue;
        ...
        if (!this.dex.data.Pokedex.hasOwnProperty(id)) {
            ...
            for (const forme in formeNames) {
                ...
                pokeName = this.dex.getAlias(pokeName) || pokeName;
        ...
        if (id && this.dex.data.Pokedex.hasOwnProperty(id)) {
            ...
            if (!species.tier && !species.doublesTier && !species.natDexTier && species.baseSpecies !== species.name) {
                if (species.baseSpecies === 'Mimikyu') {
                    ...
                    species.doublesTier = this.dex.data.FormatsData[(0, dex_data_1.toID)(species.baseSpecies)].doublesTier || species.tier;
                    species.natDexTier = this.dex.data.FormatsData[(0, dex_data_1.toID)(species.baseSpecies)].natDexTier || species.tier;
                }
                else if (species.id.endsWith('totem')) {
                    species.tier = this.dex.data.FormatsData[species.id.slice(0, -5)].tier || 'Illegal';
                    species.doublesTier = this.dex.data.FormatsData[species.id.slice(0, -5)].doublesTier || species.tier;
                    ...
                    species.doublesTier = this.dex.data.FormatsData[(0, dex_data_1.toID)(species.battleOnly)]?.doublesTier || species.tier;
                    species.natDexTier = this.dex.data.FormatsData[(0, dex_data_1.toID)(species.battleOnly)]?.natDexTier || species.tier;
                }
                else {
            ...
            if (this.dex.currentMod === 'gen7letsgo' && !species.isNonstandard) {
                const isLetsGo = (species.gen <= 7 && (species.num <= 151 || ['Meltan', 'Melmetal'].includes(species.name)) &&
